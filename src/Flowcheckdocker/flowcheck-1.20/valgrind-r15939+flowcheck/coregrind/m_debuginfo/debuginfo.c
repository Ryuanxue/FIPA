...
// 递归初始化DiSym1结构体的指针字段
DiSym1* initDiSym1(DiSym *sym) {//add by xue
    if (sym == NULL) {
        VG_(printf)("Invalid input symbol");
        return NULL;
    }
    DiSym1* sym1 = (DiSym1*)VG_(malloc)(sizeof(DiSym1));
    if (sym1 == NULL) {
        VG_(printf)("Failed to allocate memory for DiSym1");
        return NULL;
    }
    // 其他字段可以根据需要初始化
    sym1->avmas = sym->avmas.main;
    sym1->size = sym->size;
    return sym1;
}

DiSym1** VG_(get_non_text_global_symbols)(DebugInfo* di) {//add by xue
      if (di == NULL) {
         VG_(printf)("Invalid input debug info");
         return NULL;
      }
    DiSym1** result = VG_(malloc)("_sym_stab_",sizeof(DiSym1*) * (di->symtab_used + 1));
    if (!result) {
        // 处理内存分配失败的情况
        return NULL;
    }
    ...
    int count = 0;
    for (int i = 1; i < di->symtab_used; i++) {
       
        DiSym* sym = &(di->symtab[i]);
      //   VG_(printf)("sym->avmas.main:%lx\n",sym->avmas.main);
      //   VG_(printf)("sym->size:%lx\n",sym->size);
      //   if (!sym->isText && sym->isGlobal) {
            DiSym1 *sym1=initDiSym1(sym);
            if (sym1 == NULL) {
   //              // 处理内存分配失败的情况
   //              // 内存分配失败，释放已经分配的 result 数组
