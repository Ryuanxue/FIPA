...
/*--------------------------------------------------------------------*/
/*--- The leak checker.                             fc_leakcheck.c ---*/
/*--------------------------------------------------------------------*/
...
#include "pub_tool_signals.h"       // Needed for fc_include.h
...
//
// There are 9 distinct cases.  See flowcheck/docs/mc-manual.xml for details.
...
// Compare the FC_Chunks by 'data' (i.e. the address of the block).
static Int compare_FC_Chunks(const void* n1, const void* n2)
{
...
find_active_chunks(Int* pn_chunks)
{
   ...
   mallocs = (FC_Chunk**) VG_(HT_to_array)( FC_(malloc_list), &n_mallocs );
   if (n_mallocs == 0) {
      tl_assert(mallocs == NULL);
      *pn_chunks = 0;
      return NULL;
   }
   VG_(ssort)(mallocs, n_mallocs, sizeof(VgHashNode*), compare_FC_Chunks);
...
SizeT FC_(bytes_leaked)     = 0;
SizeT FC_(bytes_indirect)   = 0;
SizeT FC_(bytes_dubious)    = 0;
SizeT FC_(bytes_reachable)  = 0;
SizeT FC_(bytes_suppressed) = 0;

SizeT FC_(blocks_leaked)     = 0;
SizeT FC_(blocks_indirect)   = 0;
SizeT FC_(blocks_dubious)    = 0;
SizeT FC_(blocks_reachable)  = 0;
SizeT FC_(blocks_suppressed) = 0;

// Subset of FC_(bytes_reachable) and FC_(blocks_reachable) which
// are considered reachable due to the corresponding heuristic.
static SizeT FC_(bytes_heuristically_reachable)[N_LEAK_CHECK_HEURISTICS]
                                               = {0,0,0,0};
static SizeT FC_(blocks_heuristically_reachable)[N_LEAK_CHECK_HEURISTICS]
                                                = {0,0,0,0};

// Determines if a pointer is to a chunk.  Returns the chunk number et al
// via call-by-reference.
static Bool
lc_is_a_chunk_ptr(Addr ptr, Int* pch_no, FC_Chunk** pch, LC_Extra** pex)
{
   Int ch_no;
   FC_Chunk* ch;
   LC_Extra* ex;
   ...
   if (!VG_(am_is_valid_for_client)(ptr, 1, VKI_PROT_READ)) {
      ...
      if (ch_no == -1) {
         ...
         // Ok, we've found a pointer to a chunk.  Get the FC_Chunk and its
         // LC_Extra.
         ch = lc_chunks[ch_no];
         ex = &(lc_extras[ch_no]);
...
static void lc_push(Int ch_no, FC_Chunk* ch)
{
...
{
   if (sizeof(Word) == 8)
      return FC_(is_valid_aligned_word)(a);

   return FC_(is_valid_aligned_word)(a)
      && FC_(is_valid_aligned_word)(a + 4);
}
...
/* The below leak_search_fault_catcher is used to catch memory access
   ...
   and defined VA bits (see fc_main.c function fc_new_mem_mprotect).
      ...
      if ( ptr == ch->data + 3 * sizeof(SizeT)
         ...
         if (3 * sizeof(SizeT) + capacity + 1 == ch->szB
            ...
            if (length <= capacity) {
               ...
               // ???
               // ??? could check that ch->allockind is FC_AllocNew ???
   ...
   if (HiS(LchNewArray, heur_set)) {
      ...
      if ( ptr == ch->data + sizeof(SizeT)
         ...
         if (nr_elts > 0 && (ch->szB - sizeof(SizeT)) % nr_elts == 0) {
            // ??? could check that ch->allockind is FC_AllocNewVec ???
            VG_(set_fault_catcher) (prev_catcher);
            return LchNewArray;
         }
      }
               ...
               Addr searched, SizeT szB)
{
   /* memory scan is based on the assumption that valid pointers are aligned
      ...
      These cannot be a valid pointer, and calls to FC_(is_valid_aligned_word)
   ...
   /* Optimisation: the loop below will check for each begin
      ...
      the loop. */
   if ( ! FC_(is_within_valid_secondary)(ptr) ) {
...
#     else
   ...
   while (ptr < end) {
      ...
      if ( FC_(is_valid_aligned_word)(ptr) ) {
...
static Word cmp_LossRecordKey_LossRecord(const void* key, const void* elem)
{
   ...
   if (VG_(eq_ExeContext)(
            FC_(clo_leak_resolution), a->allocated_at, b->allocated_at))
      return 0;
...
static void print_results(ThreadId tid, LeakCheckParams* lcp)
{
   ...
   SizeT        old_bytes_leaked      = FC_(bytes_leaked);
   SizeT        old_bytes_indirect    = FC_(bytes_indirect); 
   SizeT        old_bytes_dubious     = FC_(bytes_dubious); 
   SizeT        old_bytes_reachable   = FC_(bytes_reachable); 
   SizeT        old_bytes_suppressed  = FC_(bytes_suppressed); 
   SizeT        old_blocks_leaked     = FC_(blocks_leaked);
   SizeT        old_blocks_indirect   = FC_(blocks_indirect);
   SizeT        old_blocks_dubious    = FC_(blocks_dubious);
   SizeT        old_blocks_reachable  = FC_(blocks_reachable);
   SizeT        old_blocks_suppressed = FC_(blocks_suppressed);
   ...
   for (i = 0; i < N_LEAK_CHECK_HEURISTICS; i++) {
      old_bytes_heuristically_reachable[i]   
         =  FC_(bytes_heuristically_reachable)[i];
      FC_(bytes_heuristically_reachable)[i] = 0;
      old_blocks_heuristically_reachable[i]  
         =  FC_(blocks_heuristically_reachable)[i];
      FC_(blocks_heuristically_reachable)[i] = 0;
   }
   ...
   for (i = 0; i < lc_n_chunks; i++) {
      FC_Chunk*     ch = lc_chunks[i];
      LC_Extra*     ex = &(lc_extras)[i];
      LossRecord*   old_lr;
      LossRecordKey lrkey;
      lrkey.state        = ex->state;
      lrkey.allocated_at = FC_(allocated_at)(ch);

     if (ex->heuristic) {
        FC_(bytes_heuristically_reachable)[ex->heuristic] += ch->szB;
        FC_(blocks_heuristically_reachable)[ex->heuristic]++;
        if (VG_DEBUG_LEAKCHECK)
   ...
   // Zero totals.
   FC_(blocks_leaked)     = FC_(bytes_leaked)     = 0;
   FC_(blocks_indirect)   = FC_(bytes_indirect)   = 0;
   FC_(blocks_dubious)    = FC_(bytes_dubious)    = 0;
   FC_(blocks_reachable)  = FC_(bytes_reachable)  = 0;
   FC_(blocks_suppressed) = FC_(bytes_suppressed) = 0;
   ...
   if (lcp->mode == LC_Full && lcp->max_loss_records_output < n_lossrecords) {
      ...
      for (i = n_lossrecords - 1; i >= 0 && start_lr_output_scan == 0; i--) {
         ...
         is_suppressed = 
            FC_(record_leak_error) ( tid, i+1, n_lossrecords, lr, 
                                     False /* print_record */,
   ...
   for (i = start_lr_output_scan; i < n_lossrecords; i++) {
      ...
      get_printing_rules(lcp, lr, &count_as_error, &print_record);
      is_suppressed = 
         FC_(record_leak_error) ( tid, i+1, n_lossrecords, lr, print_record,
                                  count_as_error );

      if (is_suppressed) {
         FC_(blocks_suppressed) += lr->num_blocks;
         FC_(bytes_suppressed)  += lr->szB;

      } else if (Unreached == lr->key.state) {
         FC_(blocks_leaked)     += lr->num_blocks;
         FC_(bytes_leaked)      += lr->szB;

      } else if (IndirectLeak == lr->key.state) {
         FC_(blocks_indirect)   += lr->num_blocks;
         FC_(bytes_indirect)    += lr->szB;

      } else if (Possible == lr->key.state) {
         FC_(blocks_dubious)    += lr->num_blocks;
         FC_(bytes_dubious)     += lr->szB;

      } else if (Reachable == lr->key.state) {
         FC_(blocks_reachable)  += lr->num_blocks;
         FC_(bytes_reachable)   += lr->szB;
   ...
   if (VG_(clo_verbosity) > 0 && !VG_(clo_xml)) {
      ...
      HChar d_blocks[31];
#     define DBY(new,old) \
      FC_(snprintf_delta) (d_bytes, sizeof(d_bytes), (new), (old), \
                           lcp->deltamode)
#     define DBL(new,old) \
      FC_(snprintf_delta) (d_blocks, sizeof(d_blocks), (new), (old), \
                           lcp->deltamode)

      VG_(umsg)("LEAK SUMMARY:\n");
      VG_(umsg)("   definitely lost: %'lu%s bytes in %'lu%s blocks\n",
                FC_(bytes_leaked), 
                DBY (FC_(bytes_leaked), old_bytes_leaked),
                FC_(blocks_leaked),
                DBL (FC_(blocks_leaked), old_blocks_leaked));
      VG_(umsg)("   indirectly lost: %'lu%s bytes in %'lu%s blocks\n",
                FC_(bytes_indirect), 
                DBY (FC_(bytes_indirect), old_bytes_indirect),
                FC_(blocks_indirect),
                DBL (FC_(blocks_indirect), old_blocks_indirect));
      VG_(umsg)("     possibly lost: %'lu%s bytes in %'lu%s blocks\n",
                FC_(bytes_dubious), 
                DBY (FC_(bytes_dubious), old_bytes_dubious), 
                FC_(blocks_dubious),
                DBL (FC_(blocks_dubious), old_blocks_dubious));
      VG_(umsg)("   still reachable: %'lu%s bytes in %'lu%s blocks\n",
                FC_(bytes_reachable), 
                DBY (FC_(bytes_reachable), old_bytes_reachable), 
                FC_(blocks_reachable),
                DBL (FC_(blocks_reachable), old_blocks_reachable));
      for (i = 0; i < N_LEAK_CHECK_HEURISTICS; i++)
      ...
      for (i = 0; i < N_LEAK_CHECK_HEURISTICS; i++)
         if (old_blocks_heuristically_reachable[i] > 0 
            ...
            VG_(umsg)("                        %-19s: "
                      ...
                      pp_heuristic(i),
                      FC_(bytes_heuristically_reachable)[i], 
                      DBY (FC_(bytes_heuristically_reachable)[i],
                           old_bytes_heuristically_reachable[i]), 
                      FC_(blocks_heuristically_reachable)[i],
                      DBL (FC_(blocks_heuristically_reachable)[i],
                           old_blocks_heuristically_reachable[i]));
      VG_(umsg)("        suppressed: %'lu%s bytes in %'lu%s blocks\n",
                FC_(bytes_suppressed), 
                DBY (FC_(bytes_suppressed), old_bytes_suppressed), 
                FC_(blocks_suppressed),
                DBL (FC_(blocks_suppressed), old_blocks_suppressed));
      if (lcp->mode != LC_Full &&
          (FC_(blocks_leaked) + FC_(blocks_indirect) +
           FC_(blocks_dubious) + FC_(blocks_reachable)) > 0) {
      ...
      if (lcp->mode == LC_Full &&
          FC_(blocks_reachable) > 0 && !RiS(Reachable,lcp->show_leak_kinds)) {
...
static void print_clique (Int clique, UInt level, UInt *remaining)
{
   ...
   for (ind = 0; ind < lc_n_chunks && *remaining > 0; ind++) {
      ...
      if (ind_ex->state == IndirectLeak 
         ...
         FC_Chunk*    ind_ch = lc_chunks[ind];
         LossRecord*  ind_lr;
         LossRecordKey ind_lrkey;
         UInt lr_i;
         ind_lrkey.state = ind_ex->state;
         ind_lrkey.allocated_at = FC_(allocated_at)(ind_ch);
...
Bool FC_(print_block_list) ( UInt loss_record_nr_from,
                             ...
                             UInt heuristics)
{
   ...
   if (lc_chunks_n_frees_marker != FC_(get_cmalloc_n_frees)()) {
   ...
   for (loss_record_nr = loss_record_nr_from;
      ...
      if (loss_record_nr_from == loss_record_nr_to) {
         /* (+1 on loss_record_nr as user numbering for loss records
            starts at 1). */
         FC_(pp_LossRecord)(loss_record_nr+1, n_lossrecords, lr);
         lr_printed = True;
      }
...
void FC_(detect_memory_leaks) ( ThreadId tid, LeakCheckParams* lcp)
{
   ...
   FC_(leak_search_gen)++;
   FC_(detect_memory_leaks_last_delta_mode) = lcp->deltamode;
   ...
   lc_chunks_n_frees_marker = FC_(get_cmalloc_n_frees)();
   if (lc_n_chunks == 0) {
   ...
   VG_(ssort)(lc_chunks, lc_n_chunks, sizeof(VgHashNode*), compare_FC_Chunks);
   ...
   //
   for (i = 0; i < lc_n_chunks-1; i++) {
      FC_Chunk* ch1 = lc_chunks[i];
      FC_Chunk* ch2 = lc_chunks[i+1];

      Addr start1    = ch1->data;
      Addr start2    = ch2->data;
      Addr end1      = ch1->data + ch1->szB - 1;
      Addr end2      = ch2->data + ch2->szB - 1;
      Bool isCustom1 = ch1->allockind == FC_AllocCustom;
      Bool isCustom2 = ch2->allockind == FC_AllocCustom;
      ...
      //} else if (start1 == start2 && end1 == end2) {
         ...
         VG_(pp_ExeContext)( FC_(allocated_at)(ch1));
         VG_(umsg)("\n"),
         VG_(pp_ExeContext)(  FC_(allocated_at)(ch2));
   ...
   // calls FC_(print_block_list)). lr_table also used for delta leak reporting
