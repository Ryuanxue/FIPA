...
/* This file implements the Memcheck instrumentation, and in
   ...
   1. In the condition of a conditional branch.  Because it could cause
      ...
      behaviour.  [fc_translate.c:complainIfUndefined]
   ...
   2. As an argument to a system call, or as the value that specifies
      ...
      externally-visible side effect.  [fc_translate.c:fc_pre_reg_read]
   ...
   5. As an argument to setenv, unsetenv, or putenv.  Because it could put
      ...
      [fc_replace_strmem.c:VG_WRAP_FUNCTION_ZU(*, *env)]
...
/* Carries info about a particular tmp.  The tmp's number is not
   ...
   in FCEnv.  The tmp's type is also not recorded, as this is present
   in FCEnv.sb->tyenv.
...
/* SHADOW TMP MANAGEMENT.  Shadow tmps are allocated lazily (on
   ...
   Shadow IRTemps are therefore allocated on demand.  fce.tmpMap is a
...
/* Create a new IRTemp of type 'ty' and kind 'kind', and add it to
   both the table in fce->sb and to our auxiliary mapping.  Note that
   newTemp may cause fce->tmpMap to resize, hence previous results
   from VG_(indexXA)(fce->tmpMap) are invalidated. */
static IRTemp newTemp ( FCEnv* fce, IRType ty, TempKind kind )
{
   Word       newIx;
   TempMapEnt ent;
   IRTemp     tmp = newIRTemp(fce->sb->tyenv, ty);
   ent.kind    = kind;
...
IRTemp findShadowTmpV ( FCEnv* fce, IRTemp orig )
{
   ...
   if (ent->shadowV == IRTemp_INVALID) {
      IRTemp tmpV
        = newTemp( fce, shadowTypeV(fce->sb->tyenv->types[orig]), VSh );
      /* newTemp may cause fce->tmpMap to resize, hence previous results
...
/* Allocate a new shadow for the given original tmp.  This means any
   ...
   regardless. */
static void newShadowTmpV ( FCEnv* fce, IRTemp orig )
{
   TempMapEnt* ent;
   /* VG_(indexXA) range-checks 'orig', hence no need to check
      here. */
...
static Bool isOriginalAtom ( FCEnv* fce, IRAtom* a1 )
{
   if (a1->tag == Iex_Const)
      return True;
   if (a1->tag == Iex_RdTmp) {
...
static IRType shadowTypeV ( IRType ty )
{
   switch (ty) {
      ...
      default: ppIRType(ty); 
               VG_(tool_panic)("flowcheck:shadowTypeV");
   }
}
...
static IRExpr* definedOfType ( IRType ty ) {
   switch (ty) {
      ...
      default:       VG_(tool_panic)("flowcheck:definedOfType");
   }
}
...
/* add stmt to a bb */
static inline void stmt ( HChar cat, FCEnv* fce, IRStmt* st ) {
   if (fce->trace) {
      VG_(printf)("  %c: ", cat);
      ppIRStmt(st);
      VG_(printf)("\n");
   }
   addStmtToIRSB(fce->sb, st);
}

/* assign value to tmp */
static inline 
void assign ( HChar cat, FCEnv* fce, IRTemp tmp, IRExpr* expr ) {
   stmt(cat, fce, IRStmt_WrTmp(tmp,expr));
}
...
IRAtom* assignNew ( HChar cat, FCEnv* fce, IRType ty, IRExpr* e )
{
   TempKind k;
   IRTemp   t;
   IRType   tyE = typeOfIRExpr(fce->sb->tyenv, e);
...
static IRAtom* mkDifD8 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I8, binop(Iop_And8, a1, a2));
}

static IRAtom* mkDifD16 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I16, binop(Iop_And16, a1, a2));
}

static IRAtom* mkDifD32 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I32, binop(Iop_And32, a1, a2));
}

static IRAtom* mkDifD64 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I64, binop(Iop_And64, a1, a2));
}

static IRAtom* mkDifDV128 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_V128, binop(Iop_AndV128, a1, a2));
}

static IRAtom* mkDifDV256 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_V256, binop(Iop_AndV256, a1, a2));
}

/* --------- Undefined-if-either-undefined --------- */

static IRAtom* mkUifU8 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I8, binop(Iop_Or8, a1, a2));
}

static IRAtom* mkUifU16 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I16, binop(Iop_Or16, a1, a2));
}

static IRAtom* mkUifU32 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I32, binop(Iop_Or32, a1, a2));
}

static IRAtom* mkUifU64 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_I64, binop(Iop_Or64, a1, a2));
}

static IRAtom* mkUifU128 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   IRAtom *tmp1, *tmp2, *tmp3, *tmp4, *tmp5, *tmp6;
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   tmp1 = assignNew('V', fce, Ity_I64, unop(Iop_128to64, a1));
   tmp2 = assignNew('V', fce, Ity_I64, unop(Iop_128HIto64, a1));
   tmp3 = assignNew('V', fce, Ity_I64, unop(Iop_128to64, a2));
   tmp4 = assignNew('V', fce, Ity_I64, unop(Iop_128HIto64, a2));
   tmp5 = assignNew('V', fce, Ity_I64, binop(Iop_Or64, tmp1, tmp3));
   tmp6 = assignNew('V', fce, Ity_I64, binop(Iop_Or64, tmp2, tmp4));

   return assignNew('V', fce, Ity_I128, binop(Iop_64HLto128, tmp6, tmp5));
}

static IRAtom* mkUifUV128 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_V128, binop(Iop_OrV128, a1, a2));
}

static IRAtom* mkUifUV256 ( FCEnv* fce, IRAtom* a1, IRAtom* a2 ) {
   tl_assert(isShadowAtom(fce,a1));
   tl_assert(isShadowAtom(fce,a2));
   return assignNew('V', fce, Ity_V256, binop(Iop_OrV256, a1, a2));
}

static IRAtom* mkUifU ( FCEnv* fce, IRType vty, IRAtom* a1, IRAtom* a2 ) {
   switch (vty) {
      case Ity_I8:   return mkUifU8(fce, a1, a2);
      case Ity_I16:  return mkUifU16(fce, a1, a2);
      case Ity_I32:  return mkUifU32(fce, a1, a2);
      case Ity_I64:  return mkUifU64(fce, a1, a2);
      case Ity_I128: return mkUifU128(fce, a1, a2);
      case Ity_V128: return mkUifUV128(fce, a1, a2);
      case Ity_V256: return mkUifUV256(fce, a1, a2);
      default:
         VG_(printf)("\n"); ppIRType(vty); VG_(printf)("\n");
         VG_(tool_panic)("flowcheck:mkUifU");
   }
}

/* --------- The Left-family of operations. --------- */

static IRAtom* mkLeft8 ( FCEnv* fce, IRAtom* a1 ) {
   tl_assert(isShadowAtom(fce,a1));
   return assignNew('V', fce, Ity_I8, unop(Iop_Left8, a1));
}

static IRAtom* mkLeft16 ( FCEnv* fce, IRAtom* a1 ) {
   tl_assert(isShadowAtom(fce,a1));
   return assignNew('V', fce, Ity_I16, unop(Iop_Left16, a1));
}

static IRAtom* mkLeft32 ( FCEnv* fce, IRAtom* a1 ) {
   tl_assert(isShadowAtom(fce,a1));
   return assignNew('V', fce, Ity_I32, unop(Iop_Left32, a1));
}

static IRAtom* mkLeft64 ( FCEnv* fce, IRAtom* a1 ) {
   tl_assert(isShadowAtom(fce,a1));
   return assignNew('V', fce, Ity_I64, unop(Iop_Left64, a1));
}
...
*/
static IRAtom* mkImproveAND8 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
...
static IRAtom* mkImproveAND16 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
...
static IRAtom* mkImproveAND32 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
...
static IRAtom* mkImproveAND64 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
...
static IRAtom* mkImproveANDV128 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
...
static IRAtom* mkImproveANDV256 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
...
/* ImproveOR(data, vbits) = ~data OR vbits.  Defined (0) data 1s give
   defined (0); all other -> undefined (1).
*/
static IRAtom* mkImproveOR8 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_I8, 
             binop(Iop_Or8, 
                   assignNew('V', fce, Ity_I8, unop(Iop_Not8, data)), 
                   vbits) );
}

static IRAtom* mkImproveOR16 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_I16, 
             binop(Iop_Or16, 
                   assignNew('V', fce, Ity_I16, unop(Iop_Not16, data)), 
                   vbits) );
}

static IRAtom* mkImproveOR32 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_I32, 
             binop(Iop_Or32, 
                   assignNew('V', fce, Ity_I32, unop(Iop_Not32, data)), 
                   vbits) );
}

static IRAtom* mkImproveOR64 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_I64, 
             binop(Iop_Or64, 
                   assignNew('V', fce, Ity_I64, unop(Iop_Not64, data)), 
                   vbits) );
}

static IRAtom* mkImproveORV128 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_V128, 
             binop(Iop_OrV128, 
                   assignNew('V', fce, Ity_V128, unop(Iop_NotV128, data)), 
                   vbits) );
}

static IRAtom* mkImproveORV256 ( FCEnv* fce, IRAtom* data, IRAtom* vbits )
{
   tl_assert(isOriginalAtom(fce, data));
   tl_assert(isShadowAtom(fce, vbits));
  /* tl_assert(sameKindedAtoms(data, vbits));*/
   return assignNew(
             'V', fce, Ity_V256, 
             binop(Iop_OrV256, 
                   assignNew('V', fce, Ity_V256, unop(Iop_NotV256, data)), 
                   vbits) );
}
...
static IRAtom* mkPCastTo( FCEnv* fce, IRType dst_ty, IRAtom* vbits ) 
{
   IRType  src_ty;
   IRAtom* tmp1;
   ...
   tl_assert(isShadowAtom(fce,vbits));
   src_ty = typeOfIRExpr(fce->sb->tyenv, vbits);

   /* Fast-track some common cases */
   if (src_ty == Ity_I32 && dst_ty == Ity_I32)
      return assignNew('V', fce, Ity_I32, unop(Iop_CmpwNEZ32, vbits));

   if (src_ty == Ity_I64 && dst_ty == Ity_I64)
      return assignNew('V', fce, Ity_I64, unop(Iop_CmpwNEZ64, vbits));

   if (src_ty == Ity_I32 && dst_ty == Ity_I64) {
      /* PCast the arg, then clone it. */
      IRAtom* tmp = assignNew('V', fce, Ity_I32, unop(Iop_CmpwNEZ32, vbits));
      return assignNew('V', fce, Ity_I64, binop(Iop_32HLto64, tmp, tmp));
   }

   if (src_ty == Ity_I32 && dst_ty == Ity_V128) {
      /* PCast the arg, then clone it 4 times. */
      IRAtom* tmp = assignNew('V', fce, Ity_I32, unop(Iop_CmpwNEZ32, vbits));
      tmp = assignNew('V', fce, Ity_I64, binop(Iop_32HLto64, tmp, tmp));
      return assignNew('V', fce, Ity_V128, binop(Iop_64HLtoV128, tmp, tmp));
   }
   ...
   if (src_ty == Ity_I32 && dst_ty == Ity_V256) {
      ...
      IRAtom* tmp = assignNew('V', fce, Ity_I32, unop(Iop_CmpwNEZ32, vbits));
      tmp = assignNew('V', fce, Ity_I64, binop(Iop_32HLto64, tmp, tmp));
      tmp = assignNew('V', fce, Ity_V128, binop(Iop_64HLtoV128, tmp, tmp));
      return assignNew('V', fce, Ity_V256, binop(Iop_V128HLtoV256, tmp, tmp));
   }

   if (src_ty == Ity_I64 && dst_ty == Ity_I32) {
      /* PCast the arg.  This gives all 0s or all 1s.  Then throw away
         the top half. */
      IRAtom* tmp = assignNew('V', fce, Ity_I64, unop(Iop_CmpwNEZ64, vbits));
      return assignNew('V', fce, Ity_I32, unop(Iop_64to32, tmp));
   }
   ...
   if (src_ty == Ity_V128 && dst_ty == Ity_I64) {
      ...
      IRAtom* lohi64 
         = mkUifUV128(fce, hi64hi64, vbits);
      // Generates UifU(vbits[127:64],vbits[63:0])
      IRAtom* lo64
         = assignNew('V', fce, Ity_I64, unop(Iop_V128to64, lohi64));
      // Generates
      //   PCast-to-I64( UifU(vbits[127:64], vbits[63:0] )
      //   == PCast-to-I64( vbits[127:0] )
      IRAtom* res
         = assignNew('V', fce, Ity_I64, unop(Iop_CmpwNEZ64, lo64));
      return res;
   }
   ...
   switch (src_ty) {
      case Ity_I1:
         ...
         break;
      case Ity_I8: 
         tmp1 = assignNew('V', fce, Ity_I1, unop(Iop_CmpNEZ8, vbits));
         break;
      case Ity_I16: 
         tmp1 = assignNew('V', fce, Ity_I1, unop(Iop_CmpNEZ16, vbits));
         break;
      case Ity_I32: 
         tmp1 = assignNew('V', fce, Ity_I1, unop(Iop_CmpNEZ32, vbits));
         break;
      case Ity_I64: 
         tmp1 = assignNew('V', fce, Ity_I1, unop(Iop_CmpNEZ64, vbits));
         break;
      case Ity_I128: {
         /* Gah.  Chop it in half, OR the halves together, and compare
            that with zero. */
         IRAtom* tmp2 = assignNew('V', fce, Ity_I64, unop(Iop_128HIto64, vbits));
         IRAtom* tmp3 = assignNew('V', fce, Ity_I64, unop(Iop_128to64, vbits));
         IRAtom* tmp4 = assignNew('V', fce, Ity_I64, binop(Iop_Or64, tmp2, tmp3));
         tmp1         = assignNew('V', fce, Ity_I1, 
      ...
      case Ity_V128: {
          ...
          * with zero.
          */
         IRAtom* tmp2 = assignNew('V', fce, Ity_I64, unop(Iop_V128HIto64, vbits));
         IRAtom* tmp3 = assignNew('V', fce, Ity_I64, unop(Iop_V128to64, vbits));
         IRAtom* tmp4 = assignNew('V', fce, Ity_I64, binop(Iop_Or64, tmp2, tmp3));
         tmp1         = assignNew('V', fce, Ity_I1,
                                       unop(Iop_CmpNEZ64, tmp4));
         break;
   ...
   switch (dst_ty) {
      case Ity_I1:
         return tmp1;
      case Ity_I8: 
         return assignNew('V', fce, Ity_I8, unop(Iop_1Sto8, tmp1));
      case Ity_I16: 
         return assignNew('V', fce, Ity_I16, unop(Iop_1Sto16, tmp1));
      case Ity_I32: 
         return assignNew('V', fce, Ity_I32, unop(Iop_1Sto32, tmp1));
      case Ity_I64: 
         return assignNew('V', fce, Ity_I64, unop(Iop_1Sto64, tmp1));
      case Ity_V128:
         tmp1 = assignNew('V', fce, Ity_I64,  unop(Iop_1Sto64, tmp1));
         tmp1 = assignNew('V', fce, Ity_V128, binop(Iop_64HLtoV128, tmp1, tmp1));
         return tmp1;
      case Ity_I128:
         tmp1 = assignNew('V', fce, Ity_I64,  unop(Iop_1Sto64, tmp1));
         tmp1 = assignNew('V', fce, Ity_I128, binop(Iop_64HLto128, tmp1, tmp1));
         return tmp1;
      case Ity_V256:
         tmp1 = assignNew('V', fce, Ity_I64,  unop(Iop_1Sto64, tmp1));
         tmp1 = assignNew('V', fce, Ity_V128, binop(Iop_64HLtoV128,
                                                    tmp1, tmp1));
         tmp1 = assignNew('V', fce, Ity_V256, binop(Iop_V128HLtoV256,
                                                    tmp1, tmp1));
         return tmp1;
      default: 
         ppIRType(dst_ty);
...
static IRAtom* mkPCastXXtoXXlsb ( FCEnv* fce, IRAtom* varg, IRType ty )
{
   if (ty == Ity_V128) {
      /* --- Case for V128 --- */
      IRAtom* varg128 = varg;
      // generates: PCast-to-I64(varg128)
      IRAtom* pcdTo64 = mkPCastTo(fce, Ity_I64, varg128);
      // Now introduce zeros (defined bits) in the top 63 places
      // generates: Def--(63)--Def PCast-to-I1(varg128)
      IRAtom* d63pc 
         = assignNew('V', fce, Ity_I64, binop(Iop_And64, pcdTo64, mkU64(1)));
      // generates: Def--(64)--Def
      IRAtom* d64
         = definedOfType(Ity_I64);
      // generates: Def--(127)--Def PCast-to-I1(varg128)
      IRAtom* res
         = assignNew('V', fce, Ity_V128, binop(Iop_64HLtoV128, d64, d63pc));
      return res;
   }
   if (ty == Ity_I64) {
      /* --- Case for I64 --- */
      // PCast to 64
      IRAtom* pcd = mkPCastTo(fce, Ity_I64, varg);
      // Zero (Def) out the top 63 bits
      IRAtom* res 
         = assignNew('V', fce, Ity_I64, binop(Iop_And64, pcd, mkU64(1)));   
      return res;
   }
   /*NOTREACHED*/
   tl_assert(0);
...
*/
static IRAtom* expensiveCmpEQorNE ( FCEnv*  fce,
                                    IRType  ty,
                                    ...
                                    IRAtom* xx,  IRAtom* yy )
{
   ...
   tl_assert(isShadowAtom(fce,vxx));
 ...
 /*  tl_assert(sameKindedAtoms(vxx,xx));
   ...
   naive 
      = mkPCastTo(fce,ty,
                  assignNew('V', fce, ty, binop(opUIFU, vxx, vyy)));

   vec 
      = assignNew(
           'V', fce,ty, 
           binop( opOR,
                  assignNew('V', fce,ty, binop(opOR, vxx, vyy)),
                  assignNew(
                     'V', fce,ty, 
                     unop( opNOT,
                           assignNew('V', fce,ty, binop(opXOR, xx, yy))))));

   improvement_term
      = mkPCastTo( fce,ty,
                   assignNew('V', fce,Ity_I1, binop(opCMP, vec, top)));

   improved
      = assignNew( 'V', fce,ty, binop(opDIFD, naive, improvement_term) );

   final_cast
      = mkPCastTo( fce, Ity_I1, improved );
...
static IRAtom* doCmpORD ( FCEnv*  fce,
                          IROp    cmp_op,
                          ...
                          IRAtom* xx,     IRAtom* yy )
{
   ...
   tl_assert(isShadowAtom(fce,xxhash));
...
static IRAtom* schemeE ( FCEnv* fce, IRExpr* e ); /* fwds */
...
static void setHelperAnns ( FCEnv* fce, IRDirty* di ) {
   di->nFxState = 2;
   di->fxState[0].fx        = Ifx_Read;
   di->fxState[0].offset    = fce->layout->offset_SP;
   di->fxState[0].size      = fce->layout->sizeof_SP;
   di->fxState[0].nRepeats  = 0;
   di->fxState[0].repeatLen = 0;
   di->fxState[1].fx        = Ifx_Read;
   di->fxState[1].offset    = fce->layout->offset_IP;
   di->fxState[1].size      = fce->layout->sizeof_IP;
...
static void complainIfUndefined ( FCEnv* fce, IRAtom* atom, IRExpr *guard )
{
   ...
   if (FC_(clo_fc_level) == 1)
      return;

   if (guard)
      tl_assert(isOriginalAtom(fce, guard));
...
/*   tl_assert(sameKindedAtoms(atom, vatom)); */
   ...
   cond = mkPCastTo( fce, Ity_I1, vatom );
   ...
   if (FC_(clo_fc_level) == 3) {
      origin = schemeE( fce, atom );
      if (fce->hWordTy == Ity_I64) {
         origin = assignNew( 'B', fce, Ity_I64, unop(Iop_32Uto64, origin) );
      }
   } else {
      origin = NULL;
   }
...
//   tl_assert(nargs >= 0 && nargs <= 2);
   ...
   if (guard) {
      IRAtom *g1 = assignNew('V', fce, Ity_I32, unop(Iop_1Uto32, di->guard));
      IRAtom *g2 = assignNew('V', fce, Ity_I32, unop(Iop_1Uto32, guard));
      IRAtom *e  = assignNew('V', fce, Ity_I32, binop(Iop_And32, g1, g2));
      di->guard  = assignNew('V', fce, Ity_I1,  unop(Iop_32to1, e));
   }

   setHelperAnns( fce, di );
   ...
   if (atom->tag == Iex_RdTmp) {
      if (guard == NULL) {
         ...
         assign('V', fce, findShadowTmpV(fce, atom->Iex.RdTmp.tmp), 
                          definedOfType(ty));
      } else {
         // update the temp only conditionally.  Do this by copying
         // its old value when the guard is False.
         // The old value ..
         IRTemp old_tmpV = findShadowTmpV(fce, atom->Iex.RdTmp.tmp);
         newShadowTmpV(fce, atom->Iex.RdTmp.tmp);
         IRAtom* new_tmpV
            = assignNew('V', fce, shadowTypeV(ty),
                        IRExpr_ITE(guard, definedOfType(ty),
                                          mkexpr(old_tmpV)));
         assign('V', fce, findShadowTmpV(fce, atom->Iex.RdTmp.tmp), new_tmpV);
      }
   }
}
...
*/
static Bool isAlwaysDefd ( FCEnv* fce, Int offset, Int size )
{
   ...
   for (i = 0; i < fce->layout->n_alwaysDefd; i++) {
      minoffD = fce->layout->alwaysDefd[i].offset;
      maxoffD = minoffD + fce->layout->alwaysDefd[i].size - 1;
      ...
      VG_(tool_panic)("flowcheck:isAlwaysDefd:partial overlap");
   }
...
*/
static
void do_shadow_PUT ( FCEnv* fce,  Int offset, 
                     IRAtom* atom, IRAtom* vatom, IRExpr *guard )
{
   ...
   if (isAlwaysDefd(fce, offset, sizeofIRType(ty))) {
      ...
      if (guard) {
         ...
         cond    = assignNew('V', fce, Ity_I1, guard);
         iffalse = assignNew('V', fce, ty,
                             IRExpr_Get(offset + fce->layout->total_sizeB, ty));
         vatom   = assignNew('V', fce, ty, IRExpr_ITE(cond, vatom, iffalse));
      }
      stmt( 'V', fce, IRStmt_Put( offset + fce->layout->total_sizeB, vatom ));
   }
}
...
*/
static
void do_shadow_PUTI ( FCEnv* fce, IRPutI *puti)
{
   IRAtom* vatom;
   IRType  ty, tyS;
   Int     arrSize;;
   ...
   tl_assert(ty != Ity_I1);
   tl_assert(isOriginalAtom(fce,ix));
   complainIfUndefined(fce, ix, NULL);
   if (isAlwaysDefd(fce, descr->base, arrSize)) {
      /* later: no ... */
      /* emit code to emit a complaint if any of the vbits are 1. */
      /* complainIfUndefined(fce, atom); */
   } else {
      ...
      IRRegArray* new_descr 
         = mkIRRegArray( descr->base + fce->layout->total_sizeB, 
                         tyS, descr->nElems);
      stmt( 'V', fce, IRStmt_PutI( mkIRPutI(new_descr, ix, bias, vatom) ));
   }
}
...
*/
static 
IRExpr* shadow_GET ( FCEnv* fce, Int offset, IRType ty )
{
   ...
   if (isAlwaysDefd(fce, offset, sizeofIRType(ty))) {
      ...
      return IRExpr_Get( offset + fce->layout->total_sizeB, tyS );
   }
}
...
*/
static
IRAtom* mkLazy2 ( FCEnv* fce, IRType finalVty, IRAtom* va1, IRAtom* va2 )
{
   IRAtom* at;
   IRType t1 = typeOfIRExpr(fce->sb->tyenv, va1);
   IRType t2 = typeOfIRExpr(fce->sb->tyenv, va2);
   tl_assert(isShadowAtom(fce,va1));
   tl_assert(isShadowAtom(fce,va2));
   ...
   if (t1 == Ity_I64 && t2 == Ity_I64 && finalVty == Ity_I64) {
      ...
      at = mkUifU(fce, Ity_I64, va1, va2);
      at = mkPCastTo(fce, Ity_I64, at);
      return at;
   }
   ...
   at = mkPCastTo(fce, Ity_I32, va1);
   at = mkUifU(fce, Ity_I32, at, mkPCastTo(fce, Ity_I32, va2));
   at = mkPCastTo(fce, finalVty, at);
   return at;
}
...
static
IRAtom* mkLazy3 ( FCEnv* fce, IRType finalVty, 
                  IRAtom* va1, IRAtom* va2, IRAtom* va3 )
{
   IRAtom* at;
   IRType t1 = typeOfIRExpr(fce->sb->tyenv, va1);
   IRType t2 = typeOfIRExpr(fce->sb->tyenv, va2);
   IRType t3 = typeOfIRExpr(fce->sb->tyenv, va3);
   tl_assert(isShadowAtom(fce,va1));
   tl_assert(isShadowAtom(fce,va2));
   tl_assert(isShadowAtom(fce,va3));
   ...
   if (t1 == Ity_I32 && t2 == Ity_I64 && t3 == Ity_I64 
      ...
      /* Widen 1st arg to I64.  Since 1st arg is typically a rounding
         ...
         folded out later. */
      at = mkPCastTo(fce, Ity_I64, va1);
      /* Now fold in 2nd and 3rd args. */
      at = mkUifU(fce, Ity_I64, at, va2);
      at = mkUifU(fce, Ity_I64, at, va3);
   ...
   if (t1 == Ity_I32 && t2 == Ity_I8 && t3 == Ity_I64
      ...
      */
      IRAtom* at1 = mkPCastTo(fce, Ity_I64, va1);
      IRAtom* at2 = mkPCastTo(fce, Ity_I64, va2);
      at = mkUifU(fce, Ity_I64, at1, at2);  // UifU(PCast(va1), PCast(va2))
      at = mkUifU(fce, Ity_I64, at, va3);
      /* and PCast once again. */
      at = mkPCastTo(fce, Ity_I64, at);
      return at;
   ...
   if (t1 == Ity_I32 && t2 == Ity_I32 && t3 == Ity_I32 
      ...
      at = va1;
      at = mkUifU(fce, Ity_I32, at, va2);
      at = mkUifU(fce, Ity_I32, at, va3);
      at = mkPCastTo(fce, Ity_I32, at);
      return at;
   }
   ...
   if (t1 == Ity_I32 && t2 == Ity_I8 && t3 == Ity_I128
      ...
      IRAtom* at1 = mkPCastTo(fce, Ity_I64, va1);
      IRAtom* at2 = mkPCastTo(fce, Ity_I64, va2);
      IRAtom* at3 = mkPCastTo(fce, Ity_I64, va3);
      /* Now UifU all three together. */
      at = mkUifU(fce, Ity_I64, at1, at2);  // UifU(PCast(va1), PCast(va2))
      at = mkUifU(fce, Ity_I64, at, at3);   // ... `UifU` PCast(va3)
      /* and PCast once again. */
      at = mkPCastTo(fce, Ity_I128, at);
      return at;
   }
   ...
   at = mkPCastTo(fce, Ity_I32, va1);
   at = mkUifU(fce, Ity_I32, at, mkPCastTo(fce, Ity_I32, va2));
   at = mkUifU(fce, Ity_I32, at, mkPCastTo(fce, Ity_I32, va3));
   at = mkPCastTo(fce, finalVty, at);
   return at;
   */
}
...
static
IRAtom* mkLazy4 ( FCEnv* fce, IRType finalVty, 
                  IRAtom* va1, IRAtom* va2, IRAtom* va3, IRAtom* va4 )
{
   ...
   IRType t2 = typeOfIRExpr(fce->sb->tyenv, va2);
   IRType t3 = typeOfIRExpr(fce->sb->tyenv, va3);
   IRType t4 = typeOfIRExpr(fce->sb->tyenv, va4);
   tl_assert(isShadowAtom(fce,va1));
   tl_assert(isShadowAtom(fce,va2));
   tl_assert(isShadowAtom(fce,va3));
   tl_assert(isShadowAtom(fce,va4));
                  ...
                  IRAtom** exprvec, IRType finalVtype, IRCallee* cee )
{
   ...
   for (i = 0; exprvec[i]; i++) {
      tl_assert(i < 32);
      tl_assert(isOriginalAtom(fce, exprvec[i]));
      if (cee->mcx_mask & (1<<i))
         continue;
      if (typeOfIRExpr(fce->sb->tyenv, exprvec[i]) != Ity_I64)
         mergeTy64 = False;
   }
   ...
   for (i = 0; exprvec[i]; i++) {
      ...
      if (cee->mcx_mask & (1<<i)) {
         ...
         /* calculate the arg's definedness, and pessimistically merge
            it in. */
         here = mkPCastTo( fce, mergeTy, expr2vbits(fce, exprvec[i]) );
         curr = mergeTy64 
                   ? mkUifU64(fce, here, curr)
                   : mkUifU32(fce, here, curr);
      }
   }
   return mkPCastTo(fce, finalVtype, curr );
}
...
static
IRAtom* expensiveAddSub ( FCEnv*  fce,
                          Bool    add,
                          IRType  ty,
                          ...
                          IRAtom* aa,  IRAtom* bb )
{
   ...
   tl_assert(isShadowAtom(fce,qaa));
  ...
  /* tl_assert(sameKindedAtoms(qaa,aa));
   ...
   // a_min = aa & ~qaa
   a_min = assignNew('V', fce,ty, 
                     binop(opAND, aa,
                                  assignNew('V', fce,ty, unop(opNOT, qaa))));

   // b_min = bb & ~qbb
   b_min = assignNew('V', fce,ty, 
                     binop(opAND, bb,
                                  assignNew('V', fce,ty, unop(opNOT, qbb))));

   // a_max = aa | qaa
   a_max = assignNew('V', fce,ty, binop(opOR, aa, qaa));

   // b_max = bb | qbb
   b_max = assignNew('V', fce,ty, binop(opOR, bb, qbb));
   ...
   if (add) {
      ...
      assignNew('V', fce,ty,
         binop( opOR,
                assignNew('V', fce,ty, binop(opOR, qaa, qbb)),
                assignNew('V', fce,ty, 
                   binop( opXOR, 
                          assignNew('V', fce,ty, binop(opADD, a_min, b_min)),
                          assignNew('V', fce,ty, binop(opADD, a_max, b_max))
                   )
                )
         )
      );
      ...
      assignNew('V', fce,ty,
         binop( opOR,
                ...
                assignNew('V', fce,ty, 
                   binop( opXOR, 
                          assignNew('V', fce,ty, binop(opSUB, a_min, b_max)),
                          assignNew('V', fce,ty, binop(opSUB, a_max, b_min))
                   )
                )
         )
      );
...
static
IRAtom* expensiveCountTrailingZeroes ( FCEnv* fce, IROp czop,
                                       IRAtom* atom, IRAtom* vatom )
{
   ...
   switch (czop) {
      ...
      default:
         ppIROp(czop);
         VG_(tool_panic)("flowcheck:expensiveCountTrailingZeroes");
   }
...
/* Produce an interpretation for (aa << bb) (or >>s, >>u).  The basic
   ...
   optimisation.
*/
static IRAtom* scalarShift ( FCEnv*  fce,
                             IRType  ty,
                             IROp    original_op,
                             IRAtom* qaa, IRAtom* qbb, 
                             IRAtom* aa,  IRAtom* bb )
  ...
  /* tl_assert(sameKindedAtoms(qaa,aa));
   ...
   return 
      assignNew(
         'V', fce, ty,
         mkUifU( fce, ty,
                 assignNew('V', fce, ty, binop(original_op, qaa, bb)),
                 mkPCastTo(fce, ty, qbb)
         )
   );
}
...
static IRAtom* mkPCast8x16 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V128, unop(Iop_CmpNEZ8x16, at));
}

static IRAtom* mkPCast16x8 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V128, unop(Iop_CmpNEZ16x8, at));
}

static IRAtom* mkPCast32x4 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V128, unop(Iop_CmpNEZ32x4, at));
}

static IRAtom* mkPCast64x2 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V128, unop(Iop_CmpNEZ64x2, at));
}

static IRAtom* mkPCast64x4 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V256, unop(Iop_CmpNEZ64x4, at));
}

static IRAtom* mkPCast32x8 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V256, unop(Iop_CmpNEZ32x8, at));
}

static IRAtom* mkPCast32x2 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_I64, unop(Iop_CmpNEZ32x2, at));
}

static IRAtom* mkPCast16x16 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V256, unop(Iop_CmpNEZ16x16, at));
}

static IRAtom* mkPCast16x4 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_I64, unop(Iop_CmpNEZ16x4, at));
}

static IRAtom* mkPCast8x32 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_V256, unop(Iop_CmpNEZ8x32, at));
}

static IRAtom* mkPCast8x8 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_I64, unop(Iop_CmpNEZ8x8, at));
}

static IRAtom* mkPCast16x2 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_I32, unop(Iop_CmpNEZ16x2, at));
}

static IRAtom* mkPCast8x4 ( FCEnv* fce, IRAtom* at )
{
   return assignNew('V', fce, Ity_I32, unop(Iop_CmpNEZ8x4, at));
}
...
static
IRAtom* binary32Fx4 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV128(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_V128, mkPCast32x4(fce, at));
   return at;
}

static
IRAtom* unary32Fx4 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_V128, mkPCast32x4(fce, vatomX));
   return at;
}

static
IRAtom* binary32F0x4 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV128(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_I32, unop(Iop_V128to32, at));
   at = mkPCastTo(fce, Ity_I32, at);
   at = assignNew('V', fce, Ity_V128, binop(Iop_SetV128lo32, vatomX, at));
   return at;
}

static
IRAtom* unary32F0x4 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_I32, unop(Iop_V128to32, vatomX));
   at = mkPCastTo(fce, Ity_I32, at);
   at = assignNew('V', fce, Ity_V128, binop(Iop_SetV128lo32, vatomX, at));
   return at;
}
...
static
IRAtom* binary64Fx2 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV128(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_V128, mkPCast64x2(fce, at));
   return at;
}

static
IRAtom* unary64Fx2 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_V128, mkPCast64x2(fce, vatomX));
   return at;
}

static
IRAtom* binary64F0x2 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV128(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_I64, unop(Iop_V128to64, at));
   at = mkPCastTo(fce, Ity_I64, at);
   at = assignNew('V', fce, Ity_V128, binop(Iop_SetV128lo64, vatomX, at));
   return at;
}

static
IRAtom* unary64F0x2 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_I64, unop(Iop_V128to64, vatomX));
   at = mkPCastTo(fce, Ity_I64, at);
   at = assignNew('V', fce, Ity_V128, binop(Iop_SetV128lo64, vatomX, at));
   return at;
}
...
static
IRAtom* binary32Fx2 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifU64(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_I64, mkPCast32x2(fce, at));
   return at;
}

static
IRAtom* unary32Fx2 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_I64, mkPCast32x2(fce, vatomX));
   return at;
}
...
static
IRAtom* binary64Fx4 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV256(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_V256, mkPCast64x4(fce, at));
   return at;
}

static
IRAtom* unary64Fx4 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_V256, mkPCast64x4(fce, vatomX));
   return at;
}
...
static
IRAtom* binary32Fx8 ( FCEnv* fce, IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   tl_assert(isShadowAtom(fce, vatomY));
   at = mkUifUV256(fce, vatomX, vatomY);
   at = assignNew('V', fce, Ity_V256, mkPCast32x8(fce, at));
   return at;
}

static
IRAtom* unary32Fx8 ( FCEnv* fce, IRAtom* vatomX )
{
   IRAtom* at;
   tl_assert(isShadowAtom(fce, vatomX));
   at = assignNew('V', fce, Ity_V256, mkPCast32x8(fce, vatomX));
   return at;
}
...
static
IRAtom* binary64Fx2_w_rm ( FCEnv* fce, IRAtom* vRM,
                                       IRAtom* vatomX, IRAtom* vatomY )
{
   ...
   // "do" the vector args
   IRAtom* t1 = binary64Fx2(fce, vatomX, vatomY);
   // PCast the RM, and widen it to 128 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V128, vRM);
   // Roll it into the result
   t1 = mkUifUV128(fce, t1, t2);
   return t1;
}
...
static
IRAtom* binary32Fx4_w_rm ( FCEnv* fce, IRAtom* vRM,
                                       IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* t1 = binary32Fx4(fce, vatomX, vatomY);
   // PCast the RM, and widen it to 128 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V128, vRM);
   // Roll it into the result
   t1 = mkUifUV128(fce, t1, t2);
...
static
IRAtom* binary64Fx4_w_rm ( FCEnv* fce, IRAtom* vRM,
                                       IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* t1 = binary64Fx4(fce, vatomX, vatomY);
   // PCast the RM, and widen it to 256 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V256, vRM);
   // Roll it into the result
   t1 = mkUifUV256(fce, t1, t2);
   return t1;
}
...
static
IRAtom* binary32Fx8_w_rm ( FCEnv* fce, IRAtom* vRM,
                                       IRAtom* vatomX, IRAtom* vatomY )
{
   IRAtom* t1 = binary32Fx8(fce, vatomX, vatomY);
   // PCast the RM, and widen it to 256 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V256, vRM);
   // Roll it into the result
   t1 = mkUifUV256(fce, t1, t2);
...
static
IRAtom* unary64Fx2_w_rm ( FCEnv* fce, IRAtom* vRM, IRAtom* vatomX )
{
   /* Same scheme as binary64Fx2_w_rm. */
   // "do" the vector arg
   IRAtom* t1 = unary64Fx2(fce, vatomX);
   // PCast the RM, and widen it to 128 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V128, vRM);
   // Roll it into the result
   t1 = mkUifUV128(fce, t1, t2);
...
static
IRAtom* unary32Fx4_w_rm ( FCEnv* fce, IRAtom* vRM, IRAtom* vatomX )
{
   /* Same scheme as unary32Fx4_w_rm. */
   IRAtom* t1 = unary32Fx4(fce, vatomX);
   // PCast the RM, and widen it to 128 bits
   IRAtom* t2 = mkPCastTo(fce, Ity_V128, vRM);
   // Roll it into the result
   t1 = mkUifUV128(fce, t1, t2);
...
static
IRAtom* vectorNarrowBinV128 ( FCEnv* fce, IROp narrow_op, 
                              IRAtom* vatom1, IRAtom* vatom2)
{
   IRAtom *at1, *at2, *at3;
   IRAtom* (*pcast)( FCEnv*, IRAtom* );
   switch (narrow_op) {
   ...
   at1 = assignNew('V', fce, Ity_V128, pcast(fce, vatom1));
   at2 = assignNew('V', fce, Ity_V128, pcast(fce, vatom2));
   at3 = assignNew('V', fce, Ity_V128, binop(vanilla_narrow, at1, at2));
   return at3;
}

static
IRAtom* vectorNarrowBin64 ( FCEnv* fce, IROp narrow_op, 
                            IRAtom* vatom1, IRAtom* vatom2)
{
   IRAtom *at1, *at2, *at3;
   IRAtom* (*pcast)( FCEnv*, IRAtom* );
   ...
   tl_assert(isShadowAtom(fce,vatom1));
   tl_assert(isShadowAtom(fce,vatom2));
   at1 = assignNew('V', fce, Ity_I64, pcast(fce, vatom1));
   at2 = assignNew('V', fce, Ity_I64, pcast(fce, vatom2));
   at3 = assignNew('V', fce, Ity_I64, binop(vanilla_narrow, at1, at2));
   return at3;
}

static
IRAtom* vectorNarrowUnV128 ( FCEnv* fce, IROp narrow_op,
                             IRAtom* vatom1)
{
   IRAtom *at1, *at2;
   IRAtom* (*pcast)( FCEnv*, IRAtom* );
   ...
   IROp vanilla_narrow = vanillaNarrowingOpOfShape(narrow_op);
   at1 = assignNew('V', fce, Ity_V128, pcast(fce, vatom1));
   at2 = assignNew('V', fce, Ity_I64, unop(vanilla_narrow, at1));
   return at2;
}

static
IRAtom* vectorWidenI64 ( FCEnv* fce, IROp longen_op,
                         IRAtom* vatom1)
{
   IRAtom *at1, *at2;
   IRAtom* (*pcast)( FCEnv*, IRAtom* );
   ...
   tl_assert(isShadowAtom(fce,vatom1));
   at1 = assignNew('V', fce, Ity_V128, unop(longen_op, vatom1));
   at2 = assignNew('V', fce, Ity_V128, pcast(fce, at1));
   return at2;
}
...
static
IRAtom* binary8Ix32 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV256(fce, vatom1, vatom2);
   at = mkPCast8x32(fce, at);
   return at;   
}

static
IRAtom* binary16Ix16 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV256(fce, vatom1, vatom2);
   at = mkPCast16x16(fce, at);
   return at;   
}

static
IRAtom* binary32Ix8 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV256(fce, vatom1, vatom2);
   at = mkPCast32x8(fce, at);
   return at;   
}

static
IRAtom* binary64Ix4 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV256(fce, vatom1, vatom2);
   at = mkPCast64x4(fce, at);
   return at;   
}
...
static
IRAtom* binary8Ix16 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV128(fce, vatom1, vatom2);
   at = mkPCast8x16(fce, at);
   return at;   
}

static
IRAtom* binary16Ix8 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV128(fce, vatom1, vatom2);
   at = mkPCast16x8(fce, at);
   return at;   
}

static
IRAtom* binary32Ix4 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV128(fce, vatom1, vatom2);
   at = mkPCast32x4(fce, at);
   return at;   
}

static
IRAtom* binary64Ix2 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifUV128(fce, vatom1, vatom2);
   at = mkPCast64x2(fce, at);
   return at;   
}
...
static
IRAtom* binary8Ix8 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU64(fce, vatom1, vatom2);
   at = mkPCast8x8(fce, at);
   return at;   
}

static
IRAtom* binary16Ix4 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU64(fce, vatom1, vatom2);
   at = mkPCast16x4(fce, at);
   return at;   
}

static
IRAtom* binary32Ix2 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU64(fce, vatom1, vatom2);
   at = mkPCast32x2(fce, at);
   return at;   
}

static
IRAtom* binary64Ix1 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU64(fce, vatom1, vatom2);
   at = mkPCastTo(fce, Ity_I64, at);
...
static
IRAtom* binary8Ix4 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU32(fce, vatom1, vatom2);
   at = mkPCast8x4(fce, at);
   return at;   
}

static
IRAtom* binary16Ix2 ( FCEnv* fce, IRAtom* vatom1, IRAtom* vatom2 )
{
   IRAtom* at;
   at = mkUifU32(fce, vatom1, vatom2);
   at = mkPCast16x2(fce, at);
   return at;   
}
...
static 
IRAtom* expr2vbits_Qop ( FCEnv* fce,
                         IROp op,
                         ...
                         IRAtom* atom3, IRAtom* atom4 )
{
   ...
   tl_assert(isOriginalAtom(fce,atom1));
   tl_assert(isOriginalAtom(fce,atom2));
   tl_assert(isOriginalAtom(fce,atom3));
   tl_assert(isOriginalAtom(fce,atom4));
   tl_assert(isShadowAtom(fce,vatom1));
   tl_assert(isShadowAtom(fce,vatom2));
  ...
  /* tl_assert(sameKindedAtoms(atom1,vatom1)); 
   ...
   switch (op) {
      ...
      case Iop_MSubF64r32:
         ...
         return mkLazy4(fce, Ity_I64, vatom1, vatom2, vatom3, vatom4);
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flowcheck:expr2vbits_Qop");
   }
}


static 
IRAtom* expr2vbits_Triop ( FCEnv* fce,
                           IROp op,
                           IRAtom* atom1, IRAtom* atom2, IRAtom* atom3 )
{
   ...
   switch (op) {
      ...
      case Iop_QuantizeD128:
         ...
         return mkLazy3(fce, Ity_I128, vatom1, vatom2, vatom3);
      case Iop_AddF64:
      case Iop_AddD64:
      case Iop_AddF64r32:
      case Iop_SubF64:
      ...
      case Iop_QuantizeD64:
         ...
         return mkLazy3(fce, Ity_I64, vatom1, vatom2, vatom3);
      case Iop_PRemC3210F64:
      case Iop_PRem1C3210F64:
         /* I32(rm) x F64 x F64 -> I32 */
         return mkLazy3(fce, Ity_I32, vatom1, vatom2, vatom3);
      case Iop_AddF32:
      case Iop_SubF32:
      case Iop_MulF32:
      case Iop_DivF32:
      ...
      case Iop_SliceV128:
         /* (V128, V128, I8) -> V128 */
         complainIfUndefined(fce, atom3, NULL);
         return assignNew('V', fce, Ity_V128, triop(op, vatom1, vatom2, atom3));
      case Iop_Slice64:
         /* (I64, I64, I8) -> I64 */
         complainIfUndefined(fce, atom3, NULL);
         return assignNew('V', fce, Ity_I64, triop(op, vatom1, vatom2, atom3));
      case Iop_SetElem8x8:
      case Iop_SetElem16x4:
      case Iop_SetElem32x2:
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_I64, triop(op, vatom1, atom2, vatom3));
      ...
      case Iop_Mul64Fx2:
      case Iop_Div64Fx2:
         return binary64Fx2_w_rm(fce, vatom1, vatom2, vatom3);

      case Iop_Add32Fx4:
      case Iop_Sub32Fx4:
      case Iop_Mul32Fx4:
      case Iop_Div32Fx4:
        return binary32Fx4_w_rm(fce, vatom1, vatom2, vatom3);

      case Iop_Add64Fx4:
      case Iop_Sub64Fx4:
      case Iop_Mul64Fx4:
      case Iop_Div64Fx4:
         return binary64Fx4_w_rm(fce, vatom1, vatom2, vatom3);

      case Iop_Add32Fx8:
      case Iop_Sub32Fx8:
      case Iop_Mul32Fx8:
      case Iop_Div32Fx8:
         return binary32Fx8_w_rm(fce, vatom1, vatom2, vatom3);

      default:
         ppIROp(op);
         VG_(tool_panic)("flowcheck:expr2vbits_Triop");
   }
}


static 
IRAtom* expr2vbits_Binop ( FCEnv* fce,
                           IROp op,
                           IRAtom* atom1, IRAtom* atom2 )
{
   IRType  and_or_ty;
   IRAtom* (*uifu)    (FCEnv*, IRAtom*, IRAtom*);
   IRAtom* (*difd)    (FCEnv*, IRAtom*, IRAtom*);
   IRAtom* (*improve) (FCEnv*, IRAtom*, IRAtom*);
   ...
   switch (op) {
      ...
      case Iop_QSub16Ux2:
      case Iop_QAdd16Ux2:
         return binary16Ix2(fce, vatom1, vatom2);
      ...
      case Iop_QSub8Sx4:
      case Iop_QAdd8Sx4:
         return binary8Ix4(fce, vatom1, vatom2);
      ...
      case Iop_ShlN8x8:
         ...
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_I64, binop(op, vatom1, atom2));

      case Iop_QNarrowBin32Sto16Sx4:
      case Iop_QNarrowBin16Sto8Sx8:
      case Iop_QNarrowBin16Sto8Ux8:
         return vectorNarrowBin64(fce, op, vatom1, vatom2);
      ...
      case Iop_PolynomialMul8x8:
         return binary8Ix8(fce, vatom1, vatom2);
      ...
      case Iop_QRDMulHi16Sx4:
         return binary16Ix4(fce, vatom1, vatom2);
      ...
      case Iop_QRDMulHi32Sx2:
         return binary32Ix2(fce, vatom1, vatom2);
      ...
      case Iop_PwMax32Fx2:
      case Iop_PwMin32Fx2:
         return assignNew('V', fce, Ity_I64,
                          binop(Iop_PwMax32Ux2, 
                                mkPCast32x2(fce, vatom1),
                                mkPCast32x2(fce, vatom2)));
      ...
      case Iop_PwMin16Ux4:
         return assignNew('V', fce, Ity_I64,
                          binop(Iop_PwMax16Ux4,
                                mkPCast16x4(fce, vatom1),
                                mkPCast16x4(fce, vatom2)));
      ...
      case Iop_PwMin8Ux8:
         return assignNew('V', fce, Ity_I64,
                          binop(Iop_PwMax8Ux8,
                                mkPCast8x8(fce, vatom1),
                                mkPCast8x8(fce, vatom2)));

      case Iop_PwAdd32x2:
      case Iop_PwAdd32Fx2:
         return mkPCast32x2(fce,
               assignNew('V', fce, Ity_I64,
                         binop(Iop_PwAdd32x2,
                               mkPCast32x2(fce, vatom1),
                               mkPCast32x2(fce, vatom2))));

      case Iop_PwAdd16x4:
         return mkPCast16x4(fce,
               assignNew('V', fce, Ity_I64,
                         binop(op, mkPCast16x4(fce, vatom1),
                                   mkPCast16x4(fce, vatom2))));

      case Iop_PwAdd8x8:
         return mkPCast8x8(fce,
               assignNew('V', fce, Ity_I64,
                         binop(op, mkPCast8x8(fce, vatom1),
                                   mkPCast8x8(fce, vatom2))));
      ...
      case Iop_Sal8x8:
         return mkUifU64(fce,
                   assignNew('V', fce, Ity_I64, binop(op, vatom1, atom2)),
                   mkPCast8x8(fce,vatom2)
                );
      ...
      case Iop_InterleaveEvenLanes16x4:
         return assignNew('V', fce, Ity_I64, binop(op, vatom1, vatom2));

      case Iop_GetElem8x8:
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_I8, binop(op, vatom1, atom2));
      case Iop_GetElem16x4:
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_I16, binop(op, vatom1, atom2));
      case Iop_GetElem32x2:
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_I32, binop(op, vatom1, atom2));
      ...
      case Iop_Sqrt32Fx4:
         return unary32Fx4_w_rm(fce, vatom1, vatom2);
      case Iop_Sqrt64Fx2:
         return unary64Fx2_w_rm(fce, vatom1, vatom2);
      ...
      case Iop_ShlN64x2:
         ...
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_V128, binop(op, vatom1, atom2));
      ...
      case Iop_Sh8Sx16:
      case Iop_Sh8Ux16:
         return mkUifUV128(fce,
                   assignNew('V', fce, Ity_V128, binop(op, vatom1, atom2)),
                   mkPCast8x16(fce,vatom2)
                );
      ...
      case Iop_Rsh8Sx16:
      case Iop_Rsh8Ux16:
         return binary8Ix16(fce, vatom1, vatom2);
      case Iop_Rsh16Sx8:
      case Iop_Rsh16Ux8:
         return binary16Ix8(fce, vatom1, vatom2);
      case Iop_Rsh32Sx4:
      case Iop_Rsh32Ux4:
         return binary32Ix4(fce, vatom1, vatom2);
      case Iop_Rsh64Sx2:
      case Iop_Rsh64Ux2:
      ...
      case Iop_QNarrowBin16Uto8Ux16:
      case Iop_QNarrowBin16Sto8Ux16:
         return vectorNarrowBinV128(fce, op, vatom1, vatom2);
      ...
      case Iop_RSqrtStep64Fx2:
         return binary64Fx2(fce, vatom1, vatom2);      
      ...
      case Iop_Add64F0x2:
         return binary64F0x2(fce, vatom1, vatom2);      
      ...
      case Iop_RSqrtStep32Fx4:
         return binary32Fx4(fce, vatom1, vatom2);      
      ...
      case Iop_RSqrtStep32Fx2:
         return binary32Fx2(fce, vatom1, vatom2);      
      ...
      {
         IRAtom* (*fnPessim) (FCEnv*, IRAtom*) = NULL;
         ...
         IRAtom* shVnarrowed
            = assignNew('V', fce, Ity_I64, unop(opNarrow, shV));
         // Generates: Def--(63)--Def PCast-to-I1(narrowed)
         IRAtom* qV = mkPCastXXtoXXlsb(fce, shVnarrowed, Ity_I64);
         // and assemble the result
         return assignNew('V', fce, Ity_V128, 
      ...
      case Iop_QDMull32Sx2:
         return vectorWidenI64(fce, Iop_Widen32Sto64x2,
                                    mkUifU64(fce, vatom1, vatom2));

      case Iop_Mull16Sx4:
      case Iop_Mull16Ux4:
      case Iop_QDMull16Sx4:
         return vectorWidenI64(fce, Iop_Widen16Sto32x4,
                                    mkUifU64(fce, vatom1, vatom2));

      case Iop_Mull8Sx8:
      case Iop_Mull8Ux8:
      case Iop_PolynomialMull8x8:
         return vectorWidenI64(fce, Iop_Widen8Sto16x8,
                                    mkUifU64(fce, vatom1, vatom2));

      case Iop_PwAdd32x4:
         return mkPCast32x4(fce,
               assignNew('V', fce, Ity_V128, binop(op, mkPCast32x4(fce, vatom1),
                     mkPCast32x4(fce, vatom2))));

      case Iop_PwAdd16x8:
         return mkPCast16x8(fce,
               assignNew('V', fce, Ity_V128, binop(op, mkPCast16x8(fce, vatom1),
                     mkPCast16x8(fce, vatom2))));

      case Iop_PwAdd8x16:
         return mkPCast8x16(fce,
               assignNew('V', fce, Ity_V128, binop(op, mkPCast8x16(fce, vatom1),
                     mkPCast8x16(fce, vatom2))));
      ...
      case Iop_InterleaveEvenLanes32x4:
         return assignNew('V', fce, Ity_V128, binop(op, vatom1, vatom2));
     ...
     /* These two take the lower half of each 16-bit lane, sign/zero
      ...
      case Iop_MullEven16Sx8: {
         IRAtom* at;
         at = binary16Ix8(fce,vatom1,vatom2);
         at = assignNew('V', fce, Ity_V128, binop(Iop_ShlN32x4, at, mkU8(16)));
         at = assignNew('V', fce, Ity_V128, binop(Iop_SarN32x4, at, mkU8(16)));
	 return at;
      }
      ...
      case Iop_MullEven8Sx16: {
         IRAtom* at;
         at = binary8Ix16(fce,vatom1,vatom2);
         at = assignNew('V', fce, Ity_V128, binop(Iop_ShlN16x8, at, mkU8(8)));
         at = assignNew('V', fce, Ity_V128, binop(Iop_SarN16x8, at, mkU8(8)));
	 return at;
      }
      ...
      case Iop_MullEven32Sx4: {
         IRAtom* at;
         at = binary32Ix4(fce,vatom1,vatom2);
         at = assignNew('V', fce, Ity_V128, binop(Iop_ShlN64x2, at, mkU8(32)));
         at = assignNew('V', fce, Ity_V128, binop(Iop_SarN64x2, at, mkU8(32)));
         return at;
      }
      ...
      case Iop_BCDAdd:
      case Iop_BCDSub:
         return mkLazy2(fce, Ity_V128, vatom1, vatom2);
      ...
      case Iop_64HLto128:
         return assignNew('V', fce, Ity_I128, binop(op, vatom1, vatom2));

      /* V256-bit SIMD */

      case Iop_Max64Fx4:
      case Iop_Min64Fx4:
         return binary64Fx4(fce, vatom1, vatom2);

      case Iop_Max32Fx8:
      case Iop_Min32Fx8:
         return binary32Fx8(fce, vatom1, vatom2);

      /* V256-bit data-steering */
      case Iop_V128HLtoV256:
         return assignNew('V', fce, Ity_V256, binop(op, vatom1, vatom2));
      ...
      case Iop_F32toF16:
         ...
         return mkLazy2(fce, Ity_I16, vatom1, vatom2);
      ...
      case Iop_D64HLtoD128:
         return assignNew('V', fce, Ity_I128,
      ...
      case Iop_8HLto16:
         return assignNew('V', fce, Ity_I16, binop(op, vatom1, vatom2));
      case Iop_16HLto32:
         return assignNew('V', fce, Ity_I32, binop(op, vatom1, vatom2));
      case Iop_32HLto64:
         return assignNew('V', fce, Ity_I64, binop(op, vatom1, vatom2));
      ...
      case Iop_MullS64:
      case Iop_MullU64: {
         IRAtom* vLo64 = mkLeft64(fce, mkUifU64(fce, vatom1,vatom2));
         IRAtom* vHi64 = mkPCastTo(fce, Ity_I64, vLo64);
         return assignNew('V', fce, Ity_I128,
      ...
      case Iop_MullS32:
      case Iop_MullU32: {
         IRAtom* vLo32 = mkLeft32(fce, mkUifU32(fce, vatom1,vatom2));
         IRAtom* vHi32 = mkPCastTo(fce, Ity_I32, vLo32);
         return assignNew('V', fce, Ity_I64,
      ...
      case Iop_MullS16:
      case Iop_MullU16: {
         IRAtom* vLo16 = mkLeft16(fce, mkUifU16(fce, vatom1,vatom2));
         IRAtom* vHi16 = mkPCastTo(fce, Ity_I16, vLo16);
         return assignNew('V', fce, Ity_I32,
                          binop(Iop_16HLto32, vHi16, vLo16));
      }

      case Iop_MullS8:
      case Iop_MullU8: {
         IRAtom* vLo8 = mkLeft8(fce, mkUifU8(fce, vatom1,vatom2));
         IRAtom* vHi8 = mkPCastTo(fce, Ity_I8, vLo8);
         return assignNew('V', fce, Ity_I16, binop(Iop_8HLto16, vHi8, vLo8));
      }
      ...
      cheap_AddSub32:
      case Iop_Mul32:
         return mkLeft32(fce, mkUifU32(fce, vatom1,vatom2));

      case Iop_CmpORD32S:
      case Iop_CmpORD32U:
      case Iop_CmpORD64S:
      case Iop_CmpORD64U:
         return doCmpORD(fce, op, vatom1,vatom2, atom1,atom2);

      case Iop_Add64:
         if (fce->bogusLiterals || fce->useLLVMworkarounds)
            return expensiveAddSub(fce,True,Ity_I64, 
                                   vatom1,vatom2, atom1,atom2);
         else
            goto cheap_AddSub64;
      case Iop_Sub64:
         if (fce->bogusLiterals)
            return expensiveAddSub(fce,False,Ity_I64, 
                                   vatom1,vatom2, atom1,atom2);
         else
            goto cheap_AddSub64;
      ...
      cheap_AddSub64:
      case Iop_Mul64:
         return mkLeft64(fce, mkUifU64(fce, vatom1,vatom2));

      case Iop_Mul16:
      case Iop_Add16:
      case Iop_Sub16:
         return mkLeft16(fce, mkUifU16(fce, vatom1,vatom2));

      case Iop_Mul8:
      case Iop_Sub8:
      case Iop_Add8:
         return mkLeft8(fce, mkUifU8(fce, vatom1,vatom2));
      ...
      expensive_cmp64:
      case Iop_ExpCmpNE64:
         return expensiveCmpEQorNE(fce,Ity_I64, vatom1,vatom2, atom1,atom2 );

      cheap_cmp64:
      case Iop_CmpLE64S: case Iop_CmpLE64U: 
      case Iop_CmpLT64U: case Iop_CmpLT64S:
         return mkPCastTo(fce, Ity_I1, mkUifU64(fce, vatom1,vatom2));
      ...
      expensive_cmp32:
      case Iop_ExpCmpNE32:
         return expensiveCmpEQorNE(fce,Ity_I32, vatom1,vatom2, atom1,atom2 );

      cheap_cmp32:
      case Iop_CmpLE32S: case Iop_CmpLE32U: 
      case Iop_CmpLT32U: case Iop_CmpLT32S:
         return mkPCastTo(fce, Ity_I1, mkUifU32(fce, vatom1,vatom2));

      case Iop_CmpEQ16: case Iop_CmpNE16:
         return mkPCastTo(fce, Ity_I1, mkUifU16(fce, vatom1,vatom2));

      case Iop_ExpCmpNE16:
         return expensiveCmpEQorNE(fce,Ity_I16, vatom1,vatom2, atom1,atom2 );

      case Iop_CmpEQ8: case Iop_CmpNE8:
         return mkPCastTo(fce, Ity_I1, mkUifU8(fce, vatom1,vatom2));
      ...
      case Iop_CasCmpEQ64: case Iop_CasCmpNE64:
         ...
         return assignNew('V', fce, Ity_I1, definedOfType(Ity_I1));

      case Iop_Shl64: case Iop_Shr64: case Iop_Sar64:
         return scalarShift( fce, Ity_I64, op, vatom1,vatom2, atom1,atom2 );

      case Iop_Shl32: case Iop_Shr32: case Iop_Sar32:
         return scalarShift( fce, Ity_I32, op, vatom1,vatom2, atom1,atom2 );

      case Iop_Shl16: case Iop_Shr16: case Iop_Sar16:
         return scalarShift( fce, Ity_I16, op, vatom1,vatom2, atom1,atom2 );

      case Iop_Shl8: case Iop_Shr8: case Iop_Sar8:
         return scalarShift( fce, Ity_I8, op, vatom1,vatom2, atom1,atom2 );
      ...
      do_And_Or:
         ...
         assignNew(
            'V', fce, 
            and_or_ty,
            difd(fce, uifu(fce, vatom1, vatom2),
                      difd(fce, improve(fce, atom1, vatom1),
                                improve(fce, atom2, vatom2) ) ) );

      case Iop_Xor8:
         return mkUifU8(fce, vatom1, vatom2);
      case Iop_Xor16:
         return mkUifU16(fce, vatom1, vatom2);
      case Iop_Xor32:
         return mkUifU32(fce, vatom1, vatom2);
      case Iop_Xor64:
         return mkUifU64(fce, vatom1, vatom2);
      case Iop_XorV128:
         return mkUifUV128(fce, vatom1, vatom2);
      case Iop_XorV256:
         return mkUifUV256(fce, vatom1, vatom2);
      ...
      case Iop_ShlN64x4:
         /* Same scheme as with all other shifts.  Note: 22 Oct 05:
            ...
            Vector shifts should be fixed too. */
         complainIfUndefined(fce, atom2, NULL);
         return assignNew('V', fce, Ity_V256, binop(op, vatom1, atom2));
      ...
      case Iop_QAdd8Sx32:
      case Iop_Add8x32:
         return binary8Ix32(fce, vatom1, vatom2);
      ...
      case Iop_QAdd16Sx16:
      case Iop_Add16x16:
         return binary16Ix16(fce, vatom1, vatom2);
      ...
      case Iop_Min32Sx8:
      case Iop_Mul32x8:
         return binary32Ix8(fce, vatom1, vatom2);

      case Iop_Sub64x4:
      case Iop_Add64x4:
      case Iop_CmpEQ64x4:
      case Iop_CmpGT64Sx4:
         return binary64Ix4(fce, vatom1, vatom2);
     ...
     /* Perm32x8: rearrange values in left arg using steering values
      ...
      case Iop_Perm32x8:
         return mkUifUV256(
                   fce,
                   assignNew('V', fce, Ity_V256, binop(op, vatom1, atom2)),
                   mkPCast32x8(fce, vatom2)
                );
      ...
      case Iop_QandSQRsh8x16: case Iop_QandUQRsh8x16:
      {
         ...
         IRAtom* (*binaryNIxM)(FCEnv*,IRAtom*,IRAtom*) = NULL;
         switch (op) {
         ...
         IRAtom* shV = binaryNIxM(fce, vatom1, vatom2);
         // Generates: Def--(127)--Def PCast-to-I1(shV)
         IRAtom* qV = mkPCastXXtoXXlsb(fce, shV, Ity_V128);
         // and assemble the result
         return assignNew('V', fce, Ity_V256,
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flowcheck:expr2vbits_Binop");
   }
}


static 
IRExpr* expr2vbits_Unop ( FCEnv* fce, IROp op, IRAtom* atom )
{
   ...
   switch (op) {
      ...
      case Iop_RecipEst64Fx2:
         return unary64Fx2(fce, vatom);

      case Iop_Sqrt64F0x2:
         return unary64F0x2(fce, vatom);

      case Iop_Sqrt32Fx8:
      case Iop_RSqrtEst32Fx8:
      case Iop_RecipEst32Fx8:
         return unary32Fx8(fce, vatom);

      case Iop_Sqrt64Fx4:
         return unary64Fx4(fce, vatom);
      ...
      case Iop_RSqrtEst32Fx2:
         return unary32Fx2(fce, vatom);

      case Iop_Sqrt32F0x4:
      case Iop_RSqrtEst32F0x4:
      case Iop_RecipEst32F0x4:
         return unary32F0x4(fce, vatom);
      ...
      case Iop_DPBtoBCD:
      case Iop_BCDtoDPB:
         return mkPCastTo(fce, Ity_I64, vatom);
      ...
      case Iop_AbsF32:
      case Iop_F16toF32: 
         return mkPCastTo(fce, Ity_I32, vatom);

      case Iop_Ctz32:
      case Iop_Ctz64:
         return expensiveCountTrailingZeroes(fce, op, atom, vatom);
      ...
      case Iop_V256to64_2: case Iop_V256to64_3:
         return assignNew('V', fce, Ity_I64, unop(op, vatom));
      ...
      case Iop_8Sto32:
      case Iop_V128to32:
         return assignNew('V', fce, Ity_I32, unop(op, vatom));
      ...
      case Iop_GetMSBs8x16:
         return assignNew('V', fce, Ity_I16, unop(op, vatom));
      ...
      case Iop_64to8:
      case Iop_GetMSBs8x8:
         return assignNew('V', fce, Ity_I8, unop(op, vatom));

      case Iop_32to1:
         return assignNew('V', fce, Ity_I1, unop(Iop_32to1, vatom));

      case Iop_64to1:
         return assignNew('V', fce, Ity_I1, unop(Iop_64to1, vatom));
      ...
      case Iop_PwAddL32Sx2:
         return mkPCastTo(fce, Ity_I64,
               assignNew('V', fce, Ity_I64, unop(op, mkPCast32x2(fce, vatom))));

      case Iop_PwAddL16Ux4:
      case Iop_PwAddL16Sx4:
         return mkPCast32x2(fce,
               assignNew('V', fce, Ity_I64, unop(op, mkPCast16x4(fce, vatom))));

      case Iop_PwAddL8Ux8:
      case Iop_PwAddL8Sx8:
         return mkPCast16x4(fce,
               assignNew('V', fce, Ity_I64, unop(op, mkPCast8x8(fce, vatom))));

      case Iop_PwAddL32Ux4:
      case Iop_PwAddL32Sx4:
         return mkPCast64x2(fce,
               assignNew('V', fce, Ity_V128, unop(op, mkPCast32x4(fce, vatom))));

      case Iop_PwAddL16Ux8:
      case Iop_PwAddL16Sx8:
         return mkPCast32x4(fce,
               assignNew('V', fce, Ity_V128, unop(op, mkPCast16x8(fce, vatom))));

      case Iop_PwAddL8Ux16:
      case Iop_PwAddL8Sx16:
         return mkPCast16x8(fce,
               assignNew('V', fce, Ity_V128, unop(op, mkPCast8x16(fce, vatom))));

      case Iop_I64UtoF32:
      default:
         ppIROp(op);
         VG_(tool_panic)("flowcheck:expr2vbits_Unop");
   }
}
                              ...
                              IRAtom* addr, UInt bias, IRAtom* guard )
{
   ...
   if (end == Iend_LE) {
      switch (ty) {
         case Ity_V256: helper = &FC_(helperc_LOADV256le);
                        hname = "FC_(helperc_LOADV256le)";
                        ret_via_outparam = True;
                        break;
         case Ity_V128: helper = &FC_(helperc_LOADV128le);
                        hname = "FC_(helperc_LOADV128le)";
                        ret_via_outparam = True;
                        break;
         case Ity_I64:  helper = &FC_(helperc_LOADV64le);
                        hname = "FC_(helperc_LOADV64le)";
                        break;
         case Ity_I32:  helper = &FC_(helperc_LOADV32le);
                        hname = "FC_(helperc_LOADV32le)";
                        break;
         case Ity_I16:  helper = &FC_(helperc_LOADV16le);
                        hname = "FC_(helperc_LOADV16le)";
                        break;
         case Ity_I8:   helper = &FC_(helperc_LOADV8);
                        hname = "FC_(helperc_LOADV8)";
                        break;
         default:       ppIRType(ty);
                        VG_(tool_panic)("flowcheck:expr2vbits_Load_WRK(LE)");
      }
   } else {
      switch (ty) {
         case Ity_V256: helper = &FC_(helperc_LOADV256be);
                        hname = "FC_(helperc_LOADV256be)";
                        ret_via_outparam = True;
                        break;
         case Ity_V128: helper = &FC_(helperc_LOADV128be);
                        hname = "FC_(helperc_LOADV128be)";
                        ret_via_outparam = True;
                        break;
         case Ity_I64:  helper = &FC_(helperc_LOADV64be);
                        hname = "FC_(helperc_LOADV64be)";
                        break;
         case Ity_I32:  helper = &FC_(helperc_LOADV32be);
                        hname = "FC_(helperc_LOADV32be)";
                        break;
         case Ity_I16:  helper = &FC_(helperc_LOADV16be);
                        hname = "FC_(helperc_LOADV16be)";
                        break;
         case Ity_I8:   helper = &FC_(helperc_LOADV8);
                        hname = "FC_(helperc_LOADV8)";
                        break;
         default:       ppIRType(ty);
                        VG_(tool_panic)("flowcheck:expr2vbits_Load_WRK(BE)");
      }
   }
   ...
   if (bias == 0) {
      ...
      IROp    mkAdd;
      IRAtom* eBias;
      IRType  tyAddr  = fce->hWordTy;
      tl_assert( tyAddr == Ity_I32 || tyAddr == Ity_I64 );
      mkAdd   = tyAddr==Ity_I32 ? Iop_Add32 : Iop_Add64;
      eBias   = tyAddr==Ity_I32 ? mkU32(bias) : mkU64(bias);
      addrAct = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBias) );
   }

   /* We need to have a place to park the V bits we're just about to
      read. */
   IRTemp datavbits = newTemp(fce, ty, VSh);
...
*/
static
IRAtom* expr2vbits_Load ( FCEnv* fce,
                          IREndness end, IRType ty,
                          ...
                          IRAtom* guard )
{
   ...
   switch (shadowTypeV(ty)) {
      ...
      case Ity_V128:
      case Ity_V256:
         return expr2vbits_Load_WRK(fce, end, ty, addr, bias, guard);
      default:
...
*/
static
IRAtom* expr2vbits_Load_guarded_General ( FCEnv* fce,
                                          IREndness end, IRType ty,
                                          IRAtom* addr, UInt bias,
                                          IRAtom* guard,
                                          IROp vwiden, IRAtom* valt )
{
   ...
   switch (vwiden) {
      ...
      case Iop_16Uto32: case Iop_16Sto32: case Iop_8Uto32: case Iop_8Sto32:
         ...
         break;
      default:
         VG_(tool_panic)("flowcheck:expr2vbits_Load_guarded_General");
   }
   ...
   IRAtom* iftrue1
      = assignNew('V', fce, ty,
                  expr2vbits_Load(fce, end, ty, addr, bias, guard));
   ...
   IRAtom* iftrue2
      = vwiden == Iop_INVALID
           ? iftrue1
           : assignNew('V', fce, tyWide, unop(vwiden, iftrue1));
   ...
   return assignNew('V', fce, tyWide, IRExpr_ITE(cond, iftrue2, iffalse));
}
...
static
IRAtom* expr2vbits_Load_guarded_Simple ( FCEnv* fce, 
                                         IREndness end, IRType ty, 
                                         IRAtom* addr, UInt bias,
                                         IRAtom *guard )
{
   return expr2vbits_Load_guarded_General(
             fce, end, ty, addr, bias, guard, Iop_INVALID, definedOfType(ty)
          );
}


static
IRAtom* expr2vbits_ITE ( FCEnv* fce, 
                         IRAtom* cond, IRAtom* iftrue, IRAtom* iffalse )
{
   ...
   /* Given ITE(cond, iftrue,  iffalse),  generate
      ...
      lazy propagation. */
   tl_assert(isOriginalAtom(fce, cond));
   tl_assert(isOriginalAtom(fce, iftrue));
   tl_assert(isOriginalAtom(fce, iffalse));

   vbitsC = expr2vbits(fce, cond);
   vbits1 = expr2vbits(fce, iftrue);
   vbits0 = expr2vbits(fce, iffalse);
   ty = typeOfIRExpr(fce->sb->tyenv, vbits0);

   return
      mkUifU(fce, ty, assignNew('V', fce, ty, 
                                     IRExpr_ITE(cond, vbits1, vbits0)),
                      mkPCastTo(fce, ty, vbitsC) );
}      
...
IRExpr* expr2vbits ( FCEnv* fce, IRExpr* e )
{
   switch (e->tag) {
      ...
      case Iex_Get:
         return shadow_GET( fce, e->Iex.Get.offset, e->Iex.Get.ty );

      case Iex_GetI:
         return shadow_GETI( fce, e->Iex.GetI.descr, 
                                  e->Iex.GetI.ix, e->Iex.GetI.bias );

      case Iex_RdTmp:
         return IRExpr_RdTmp( findShadowTmpV(fce, e->Iex.RdTmp.tmp) );

      case Iex_Const:
         return definedOfType(shadowTypeV(typeOfIRExpr(fce->sb->tyenv, e)));
      ...
      case Iex_Unop:
         return expr2vbits_Unop( fce, e->Iex.Unop.op, e->Iex.Unop.arg );

      case Iex_Load:
         return expr2vbits_Load( fce, e->Iex.Load.end,
                                      e->Iex.Load.ty, 
                                      e->Iex.Load.addr, 0/*addr bias*/, 
                                      NULL/* guard == "always True"*/ );

      case Iex_CCall:
         return mkLazyN( fce, e->Iex.CCall.args, 
                              e->Iex.CCall.retty,
                              e->Iex.CCall.cee );

      case Iex_ITE:
         return expr2vbits_ITE( fce, e->Iex.ITE.cond, e->Iex.ITE.iftrue, 
                                     e->Iex.ITE.iffalse);
      ...
      default: 
         ...
         ppIRExpr(e);
         VG_(printf)("\n");
         VG_(tool_panic)("flowcheck: expr2vbits");
   }
}
...
static
IRExpr* zwidenToHostWord ( FCEnv* fce, IRAtom* vatom )
{
   IRType ty, tyH;
   ...
   if (tyH == Ity_I32) {
      switch (ty) {
         case Ity_I32:
            return vatom;
         case Ity_I16:
            return assignNew('V', fce, tyH, unop(Iop_16Uto32, vatom));
         case Ity_I8:
            return assignNew('V', fce, tyH, unop(Iop_8Uto32, vatom));
         default:
            goto unhandled;
      }
   } else
   if (tyH == Ity_I64) {
      switch (ty) {
         case Ity_I32:
            return assignNew('V', fce, tyH, unop(Iop_32Uto64, vatom));
         case Ity_I16:
            return assignNew('V', fce, tyH, unop(Iop_32Uto64, 
                   assignNew('V', fce, Ity_I32, unop(Iop_16Uto32, vatom))));
         case Ity_I8:
            return assignNew('V', fce, tyH, unop(Iop_32Uto64, 
                   assignNew('V', fce, Ity_I32, unop(Iop_8Uto32, vatom))));
         default:
            goto unhandled;
      }
   } else {
...
*/
static 
void do_shadow_Store ( FCEnv* fce, 
                       IREndness end,
                       IRAtom* addr, UInt bias,
                       ...
                       IRAtom* guard )
{
   ...
   if (data) {
      ...
      tl_assert(isOriginalAtom(fce, data));
      tl_assert(bias == 0);
      vdata = expr2vbits( fce, data );
   } else {
      tl_assert(vdata);
   }

   tl_assert(isOriginalAtom(fce,addr));
   tl_assert(isShadowAtom(fce,vdata));

   if (guard) {
      tl_assert(isOriginalAtom(fce, guard));
      tl_assert(typeOfIRExpr(fce->sb->tyenv, guard) == Ity_I1);
   }

   ty = typeOfIRExpr(fce->sb->tyenv, vdata);
   ...
   if (FC_(clo_fc_level) == 1) {
      switch (ty) {
         ...
         default:       VG_(tool_panic)("flowcheck:do_shadow_Store(LE)");
      }
   ...
   if (end == Iend_LE) {
      switch (ty) {
         ...
         case Ity_I64: helper = &FC_(helperc_STOREV64le);
                       hname = "FC_(helperc_STOREV64le)";
                       break;
         case Ity_I32: helper = &FC_(helperc_STOREV32le);
                       hname = "FC_(helperc_STOREV32le)";
                       break;
         case Ity_I16: helper = &FC_(helperc_STOREV16le);
                       hname = "FC_(helperc_STOREV16le)";
                       break;
         case Ity_I8:  helper = &FC_(helperc_STOREV8);
                       hname = "FC_(helperc_STOREV8)";
                       break;
         default:      VG_(tool_panic)("flowcheck:do_shadow_Store(LE)");
      }
   } else {
      switch (ty) {
         case Ity_V128: /* we'll use the helper twice */
         case Ity_I64: helper = &FC_(helperc_STOREV64be);
                       hname = "FC_(helperc_STOREV64be)";
                       break;
         case Ity_I32: helper = &FC_(helperc_STOREV32be);
                       hname = "FC_(helperc_STOREV32be)";
                       break;
         case Ity_I16: helper = &FC_(helperc_STOREV16be);
                       hname = "FC_(helperc_STOREV16be)";
                       break;
         case Ity_I8:  helper = &FC_(helperc_STOREV8);
                       hname = "FC_(helperc_STOREV8)";
                       break;
         ...
         default:      VG_(tool_panic)("flowcheck:do_shadow_Store(BE)");
      }
   }
   ...
   if (UNLIKELY(ty == Ity_V256)) {
      ...
      addrQ0  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasQ0) );
      vdataQ0 = assignNew('V', fce, Ity_I64, unop(Iop_V256to64_0, vdata));
      ...
      addrQ1  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasQ1) );
      vdataQ1 = assignNew('V', fce, Ity_I64, unop(Iop_V256to64_1, vdata));
      ...
      addrQ2  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasQ2) );
      vdataQ2 = assignNew('V', fce, Ity_I64, unop(Iop_V256to64_2, vdata));
      ...
      addrQ3  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasQ3) );
      vdataQ3 = assignNew('V', fce, Ity_I64, unop(Iop_V256to64_3, vdata));
      ...
      setHelperAnns( fce, diQ0 );
      setHelperAnns( fce, diQ1 );
      setHelperAnns( fce, diQ2 );
      setHelperAnns( fce, diQ3 );
      stmt( 'V', fce, IRStmt_Dirty(diQ0) );
      stmt( 'V', fce, IRStmt_Dirty(diQ1) );
      stmt( 'V', fce, IRStmt_Dirty(diQ2) );
      stmt( 'V', fce, IRStmt_Dirty(diQ3) );
      ...
      addrLo64  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasLo64) );
      vdataLo64 = assignNew('V', fce, Ity_I64, unop(Iop_V128to64, vdata));
      ...
      addrHi64  = assignNew('V', fce, tyAddr, binop(mkAdd, addr, eBiasHi64) );
      vdataHi64 = assignNew('V', fce, Ity_I64, unop(Iop_V128HIto64, vdata));
      ...
      setHelperAnns( fce, diLo64 );
      setHelperAnns( fce, diHi64 );
      stmt( 'V', fce, IRStmt_Dirty(diLo64) );
      stmt( 'V', fce, IRStmt_Dirty(diHi64) );
...
static IRType szToITy ( Int n )
{
   switch (n) {
      ...
      default: VG_(tool_panic)("szToITy(flowcheck)");
   }
}

static
void do_shadow_Dirty ( FCEnv* fce, IRDirty* d )
{
...
#  endif
   ...
   complainIfUndefined(fce, d->guard, NULL);
   ...
   for (i = 0; d->args[i]; i++) {
      ...
      if ( (d->cee->mcx_mask & (1<<i))
         ...
         here = mkPCastTo( fce, Ity_I32, expr2vbits(fce, arg) );
         curr = mkUifU32(fce, here, curr);
      }
   }
   ...
   for (i = 0; i < d->nFxState; i++) {
      ...
      for (k = 0; k < 1 + d->fxState[i].nRepeats; k++) {
         ...
         if (isAlwaysDefd(fce, gOff, gSz)) {
            if (0)
            VG_(printf)("flowcheck: Dirty gst: ignored off %d, sz %d\n",
                        gOff, gSz);
            continue;
         }
         ...
         while (True) {
            ...
            cond    = assignNew('V', fce, Ity_I1, d->guard);
            iftrue  = assignNew('V', fce, tySrc, shadow_GET(fce, gOff, tySrc));
            iffalse = assignNew('V', fce, tySrc, definedOfType(tySrc));
            src     = assignNew('V', fce, tySrc,
                                IRExpr_ITE(cond, iftrue, iffalse));

            here = mkPCastTo( fce, Ity_I32, src );
            curr = mkUifU32(fce, here, curr);
            gSz -= n;
            gOff += n;
         }
   ...
   if (d->mFx != Ifx_None) {
      ...
      IRType tyAddr;
      tl_assert(d->mAddr);
      complainIfUndefined(fce, d->mAddr, d->guard);

      tyAddr = typeOfIRExpr(fce->sb->tyenv, d->mAddr);
      tl_assert(tyAddr == Ity_I32 || tyAddr == Ity_I64);
      tl_assert(tyAddr == fce->hWordTy); /* not really right */
   }
   ...
   if (d->mFx == Ifx_Read || d->mFx == Ifx_Modify) {
      ...
      while (toDo >= 4) {
         here = mkPCastTo( 
                   ...
                   expr2vbits_Load_guarded_Simple(
                      fce, end, Ity_I32, d->mAddr, d->mSize - toDo, d->guard )
                );
         curr = mkUifU32(fce, here, curr);
         toDo -= 4;
      }
      ...
      while (toDo >= 2) {
         here = mkPCastTo( 
                   fce, Ity_I32,
                   expr2vbits_Load_guarded_Simple(
                      fce, end, Ity_I16, d->mAddr, d->mSize - toDo, d->guard )
                );
         curr = mkUifU32(fce, here, curr);
         toDo -= 2;
      }
      ...
      if (toDo == 1) {
         here = mkPCastTo( 
                   fce, Ity_I32,
                   expr2vbits_Load_guarded_Simple(
                      fce, end, Ity_I8, d->mAddr, d->mSize - toDo, d->guard )
                );
         curr = mkUifU32(fce, here, curr);
         toDo -= 1;
      }
   ...
   if (d->tmp != IRTemp_INVALID) {
      dst   = findShadowTmpV(fce, d->tmp);
      tyDst = typeOfIRTemp(fce->sb->tyenv, d->tmp);
      assign( 'V', fce, dst, mkPCastTo( fce, tyDst, curr) );
   }
      ...
      for (k = 0; k < 1 + d->fxState[i].nRepeats; k++) {
         ...
         while (True) {
            ...
            /* Write suitably-casted 'curr' to the state slice 
               gOff .. gOff+n-1 */
            tyDst = szToITy( n );
            do_shadow_PUT( fce, gOff,
                                NULL, /* original atom */
                                mkPCastTo( fce, tyDst, curr ), d->guard );
            gSz -= n;
            gOff += n;
         }
      }
   ...
   if (d->mFx == Ifx_Write || d->mFx == Ifx_Modify) {
      ...
      while (toDo >= 4) {
         do_shadow_Store( fce, end, d->mAddr, d->mSize - toDo,
                          NULL, /* original data */
                          mkPCastTo( fce, Ity_I32, curr ),
                          d->guard );
         toDo -= 4;
      }
      /* chew off 16-bit chunks */
...
/* We have an ABI hint telling us that [base .. base+len-1] is to
   ...
   We call 
   void FC_(helperc_MAKE_STACK_UNINIT) ( Addr base, UWord len,
                                                    Addr nia );
*/
static
void do_AbiHint ( FCEnv* fce, IRExpr* base, Int len, IRExpr* nia )
{
   IRDirty* di;
   ...
   if (FC_(clo_fc_level) == 3) {
      di = unsafeIRDirty_0_N(
              3/*regparms*/,
              "FC_(helperc_MAKE_STACK_UNINIT_w_o)",
              VG_(fnptr_to_fnentry)( &FC_(helperc_MAKE_STACK_UNINIT_w_o) ),
              mkIRExprVec_3( base, mkIRExpr_HWord( (UInt)len), nia )
           );
   } else {
      /* We ignore the supplied nia, since it is irrelevant. */
      tl_assert(FC_(clo_fc_level) == 2 || FC_(clo_fc_level) == 1);
      /* Special-case the len==128 case, since that is for amd64-ELF,
         which is a very common target. */
      if (len == 128) {
         di = unsafeIRDirty_0_N(
                 1/*regparms*/,
                 "FC_(helperc_MAKE_STACK_UNINIT_128_no_o)",
                 VG_(fnptr_to_fnentry)( &FC_(helperc_MAKE_STACK_UNINIT_128_no_o)),
                 mkIRExprVec_1( base )
              );
      } else {
         di = unsafeIRDirty_0_N(
                 2/*regparms*/,
                 "FC_(helperc_MAKE_STACK_UNINIT_no_o)",
                 VG_(fnptr_to_fnentry)( &FC_(helperc_MAKE_STACK_UNINIT_no_o) ),
...
/* FWDS */
static IRAtom* gen_load_b  ( FCEnv* fce, Int szB, 
                             IRAtom* baseaddr, Int offset );
static IRAtom* gen_maxU32  ( FCEnv* fce, IRAtom* b1, IRAtom* b2 );
static void    gen_store_b ( FCEnv* fce, Int szB,
                             IRAtom* baseaddr, Int offset, IRAtom* dataB,
                             IRAtom* guard );

static void do_shadow_CAS_single ( FCEnv* fce, IRCAS* cas );
static void do_shadow_CAS_double ( FCEnv* fce, IRCAS* cas );
...
{
   tl_assert(isOriginalAtom(fce, orig));
   tl_assert(isShadowAtom(fce, shadow));
   switch (orig->tag) {
      case Iex_Const:
      ...
      case Iex_RdTmp:
         ...
         if (how == 'V') {
            assign('V', fce, findShadowTmpV(fce,orig->Iex.RdTmp.tmp),
                   shadow);
         } else {
            tl_assert(how == 'B');
            assign('B', fce, findShadowTmpB(fce,orig->Iex.RdTmp.tmp),
                   shadow);
         }
         break;
      default:
...
static
void do_shadow_CAS ( FCEnv* fce, IRCAS* cas )
{
   ...
   */
   if (cas->oldHi == IRTemp_INVALID) {
      do_shadow_CAS_single( fce, cas );
   } else {
      do_shadow_CAS_double( fce, cas );
   }
}
...
static void do_shadow_CAS_single ( FCEnv* fce, IRCAS* cas )
{
   ...
   Int    elemSzB;
   IRType elemTy;
   Bool   otrak = FC_(clo_fc_level) >= 3; /* a shorthand */
   ...
   elemTy = typeOfIRExpr(fce->sb->tyenv, cas->expdLo);
   switch (elemTy) {
   ...
   tl_assert(isOriginalAtom(fce, cas->dataLo));
   vdataLo
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->dataLo));
   tl_assert(isShadowAtom(fce, vdataLo));
   if (otrak) {
      bdataLo
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->dataLo));
      tl_assert(isShadowAtom(fce, bdataLo));
   }

   /* 2. fetch expected# (what we expect to see at the address) */
   tl_assert(isOriginalAtom(fce, cas->expdLo));
   vexpdLo
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->expdLo));
   tl_assert(isShadowAtom(fce, vexpdLo));
   if (otrak) {
      bexpdLo
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->expdLo));
      tl_assert(isShadowAtom(fce, bexpdLo));
   }
   ...
   bind_shadow_tmp_to_orig('V', fce, mkexpr(cas->oldLo), voldLo);
   if (otrak) {
      boldLo
         = assignNew('B', fce, Ity_I32,
                     gen_load_b(fce, elemSzB, cas->addr, 0/*addr bias*/));
      bind_shadow_tmp_to_orig('B', fce, mkexpr(cas->oldLo), boldLo);
   }

   /* 5. the CAS itself */
   stmt( 'C', fce, IRStmt_CAS(cas) );
...
static void do_shadow_CAS_double ( FCEnv* fce, IRCAS* cas )
{
   ...
   /* 1. fetch data# (the proposed new value) */
   tl_assert(isOriginalAtom(fce, cas->dataHi));
   tl_assert(isOriginalAtom(fce, cas->dataLo));
   vdataHi
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->dataHi));
   vdataLo
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->dataLo));
   tl_assert(isShadowAtom(fce, vdataHi));
   tl_assert(isShadowAtom(fce, vdataLo));
   if (otrak) {
      bdataHi
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->dataHi));
      bdataLo
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->dataLo));
      tl_assert(isShadowAtom(fce, bdataHi));
      tl_assert(isShadowAtom(fce, bdataLo));
   }

   /* 2. fetch expected# (what we expect to see at the address) */
   tl_assert(isOriginalAtom(fce, cas->expdHi));
   tl_assert(isOriginalAtom(fce, cas->expdLo));
   vexpdHi
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->expdHi));
   vexpdLo
      = assignNew('V', fce, elemTy, expr2vbits(fce, cas->expdLo));
   tl_assert(isShadowAtom(fce, vexpdHi));
   tl_assert(isShadowAtom(fce, vexpdLo));
   if (otrak) {
      bexpdHi
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->expdHi));
      bexpdLo
         = assignNew('B', fce, Ity_I32, schemeE(fce, cas->expdLo));
      tl_assert(isShadowAtom(fce, bexpdHi));
      tl_assert(isShadowAtom(fce, bexpdLo));
   }
   ...
   bind_shadow_tmp_to_orig('V', fce, mkexpr(cas->oldHi), voldHi);
   bind_shadow_tmp_to_orig('V', fce, mkexpr(cas->oldLo), voldLo);
   if (otrak) {
      boldHi
         = assignNew('B', fce, Ity_I32,
                     gen_load_b(fce, elemSzB, cas->addr,
                                memOffsHi/*addr bias*/));
      boldLo
         = assignNew('B', fce, Ity_I32,
                     gen_load_b(fce, elemSzB, cas->addr,
                                memOffsLo/*addr bias*/));
      bind_shadow_tmp_to_orig('B', fce, mkexpr(cas->oldHi), boldHi);
      bind_shadow_tmp_to_orig('B', fce, mkexpr(cas->oldLo), boldLo);
   }
   ...
   /*
      ...
      expd_eq_old = xHL == 0;
   */
   xHi = assignNew('C', fce, elemTy,
                   binop(opXor, cas->expdHi, mkexpr(cas->oldHi))); 
   xLo = assignNew('C', fce, elemTy,
                   binop(opXor, cas->expdLo, mkexpr(cas->oldLo)));
   xHL = assignNew('C', fce, elemTy,
                   binop(opOr, xHi, xLo));
   expd_eq_old
      = assignNew('C', fce, Ity_I1,
   ...
   /* 7. if "expected == old"
            store data# to shadow memory */
   do_shadow_Store( fce, cas->end, cas->addr, memOffsHi/*bias*/,
                    NULL/*data*/, vdataHi/*vdata*/,
                    expd_eq_old/*guard for store*/ );
   do_shadow_Store( fce, cas->end, cas->addr, memOffsLo/*bias*/,
                    NULL/*data*/, vdataLo/*vdata*/,
                    expd_eq_old/*guard for store*/ );
   if (otrak) {
      gen_store_b( fce, elemSzB, cas->addr, memOffsHi/*offset*/,
                   bdataHi/*bdata*/,
                   expd_eq_old/*guard for store*/ );
      gen_store_b( fce, elemSzB, cas->addr, memOffsLo/*offset*/,
                   bdataLo/*bdata*/,
                   expd_eq_old/*guard for store*/ );
   }
}
...
static void do_shadow_LLSC ( FCEnv*    fce,
                             IREndness stEnd,
                             IRTemp    stResult,
                             IRExpr*   stAddr,
                             IRExpr*   stStoredata )
{
   ...
   IRType resTy  = typeOfIRTemp(fce->sb->tyenv, stResult);
   IRTemp resTmp = findShadowTmpV(fce, stResult);
   ...
   if (stStoredata == NULL) {
      ...
      assign( 'V', fce, resTmp,
                   expr2vbits_Load(
                      fce, stEnd, resTy, stAddr, 0/*addr bias*/,
                      NULL/*always happens*/) );
   } else {
      /* Store Conditional */
      /* Stay sane */
      IRType dataTy = typeOfIRExpr(fce->sb->tyenv,
                                   stStoredata);
      tl_assert(dataTy == Ity_I64 || dataTy == Ity_I32
                || dataTy == Ity_I16 || dataTy == Ity_I8);
      do_shadow_Store( fce, stEnd,
                            stAddr, 0/* addr bias */,
                            stStoredata,
                            NULL /* shadow data */,
                            NULL/*guard*/ );
      ...
      */
      ...
      assign( 'V', fce, resTmp, definedOfType(resTy) );
   }
}
...
static void do_shadow_StoreG ( FCEnv* fce, IRStoreG* sg )
{
   complainIfUndefined(fce, sg->guard, NULL);
...
static void do_shadow_LoadG ( FCEnv* fce, IRLoadG* lg )
{
   complainIfUndefined(fce, lg->guard, NULL);
   ...
   IRAtom* vbits_alt
      = expr2vbits( fce, lg->alt );
   IRAtom* vbits_final
      = expr2vbits_Load_guarded_General(fce, lg->end, loadedTy,
                                        lg->addr, 0/*addr bias*/,
                                        lg->guard, vwiden, vbits_alt );
   /* And finally, bind the V bits to the destination temporary. */
   assign( 'V', fce, findShadowTmpV(fce, lg->dst), vbits_final );
}
...
static void schemeS ( FCEnv* fce, IRStmt* st );
                        ...
                        IRType gWordTy, IRType hWordTy )
{
   ...
   tl_assert(FC_(clo_fc_level) >= 1 && FC_(clo_fc_level) <= 3);
   ...
   VG_(memset)(&fce, 0, sizeof(fce));
   fce.sb             = sb_out;
   fce.trace          = verboze;
   fce.layout         = layout;
   fce.hWordTy        = hWordTy;
   fce.bogusLiterals  = False;
   ...
   fce.useLLVMworkarounds = False;
#  if defined(VGO_darwin)
   fce.useLLVMworkarounds = True;
#  endif

   fce.tmpMap = VG_(newXA)( VG_(malloc), "mc.FC_(instrument).1", VG_(free),
                            sizeof(TempMapEnt));
   VG_(hintSizeXA) (fce.tmpMap, sb_in->tyenv->types_used);
   ...
   /* Nasty problem.  IR optimisation of the pre-instrumented IR may
      ...
      flowcheck to generate references to their shadows.  End effect is
   ...
   */
   for (j = 0; j < i; j++) {
      if (sb_in->stmts[j]->tag == Ist_WrTmp) {
         ...
         IRTemp tmp_v = findShadowTmpV(&fce, tmp_o);
         IRType ty_v  = typeOfIRTemp(sb_out->tyenv, tmp_v);
         assign( 'V', &fce, tmp_v, definedOfType( ty_v ) );
         if (FC_(clo_fc_level) == 3) {
            IRTemp tmp_b = findShadowTmpB(&fce, tmp_o);
            tl_assert(typeOfIRTemp(sb_out->tyenv, tmp_b) == Ity_I32);
            assign( 'B', &fce, tmp_b, mkU32(0)/* UNKNOWN ORIGIN */);
         }
         if (0) {
   ...
   for (/* use current i*/; i < sb_in->stmts_used; i++) {
      ...
      switch (st->tag) {
	    ...
	    break;
         ...
         case Ist_PutI:
            do_shadow_PUTI( &fce, st->Ist.PutI.details);
            break;
	    ...
	    do_rollback_Store(&fce, st->Ist.Store.end, st->Ist.Store.addr,
         ...
         case Ist_StoreG:
            do_shadow_StoreG( &fce, st->Ist.StoreG.details );
            break;

         case Ist_LoadG:
            do_shadow_LoadG( &fce, st->Ist.LoadG.details );
            break;

         case Ist_Exit:
            complainIfUndefined( &fce, st->Ist.Exit.guard, NULL );
            break;
         ...
         case Ist_Dirty:
            do_shadow_Dirty( &fce, st->Ist.Dirty.details );
            break;

         case Ist_AbiHint:
            do_AbiHint( &fce, st->Ist.AbiHint.base,
                              st->Ist.AbiHint.len,
                              st->Ist.AbiHint.nia );
            break;

         case Ist_CAS:
            do_shadow_CAS( &fce, st->Ist.CAS.details );
         ...
         default:
            ...
            ppIRStmt(st);
            VG_(printf)("\n");
            VG_(tool_panic)("flowcheck: unhandled IRStmt");
   ...
   tl_assert( VG_(sizeXA)( fce.tmpMap ) == fce.sb->tyenv->types_used );
   VG_(deleteXA)( fce.tmpMap );
...
/* This exploits the observation that Memcheck often produces
   ...
   Dirty G FC_(helperc_value_check0/1/4/8_fail)(UInt otag)
...
static Bool sameIRValue ( IRExpr* e1, IRExpr* e2 )
{
   ...
   switch (e1->tag) {
      ...
      case Iex_Binder:
         ...
         /* fallthrough */
      default:
         VG_(printf)("fc_translate.c: sameIRValue: unhandled: ");
         ppIRExpr(e1); 
         VG_(tool_panic)("flowcheck:sameIRValue");
         return False;
   }
}
...
static IRTemp findShadowTmpB ( FCEnv* fce, IRTemp orig )
{
   TempMapEnt* ent;
   /* VG_(indexXA) range-checks 'orig', hence no need to check
      here. */
...
static IRAtom* gen_maxU32 ( FCEnv* fce, IRAtom* b1, IRAtom* b2 )
{
   return assignNew( 'B', fce, Ity_I32, binop(Iop_Max32U, b1, b2) );
}
...
*/
static IRAtom* gen_guarded_load_b ( FCEnv* fce, Int szB, 
                                    IRAtom* baseaddr, 
                                    Int offset, IRExpr* guard )
{
   ...
   IRTemp   bTmp;
   IRDirty* di;
   IRType   aTy   = typeOfIRExpr( fce->sb->tyenv, baseaddr );
   IROp     opAdd = aTy == Ity_I32 ? Iop_Add32 : Iop_Add64;
   IRAtom*  ea    = baseaddr;
   if (offset != 0) {
      IRAtom* off = aTy == Ity_I32 ? mkU32( offset )
                                   : mkU64( (Long)(Int)offset );
      ea = assignNew( 'B', fce, aTy, binop(opAdd, ea, off));
   }
   bTmp = newTemp(fce, fce->hWordTy, BSh);

   switch (szB) {
      case 1: hFun  = (void*)&FC_(helperc_b_load1);
              hName = "FC_(helperc_b_load1)";
              break;
      case 2: hFun  = (void*)&FC_(helperc_b_load2);
              hName = "FC_(helperc_b_load2)";
              break;
      case 4: hFun  = (void*)&FC_(helperc_b_load4);
              hName = "FC_(helperc_b_load4)";
              break;
      case 8: hFun  = (void*)&FC_(helperc_b_load8);
              hName = "FC_(helperc_b_load8)";
              break;
      case 16: hFun  = (void*)&FC_(helperc_b_load16);
               hName = "FC_(helperc_b_load16)";
               break;
      case 32: hFun  = (void*)&FC_(helperc_b_load32);
               hName = "FC_(helperc_b_load32)";
               break;
      default:
         VG_(printf)("fc_translate.c: gen_load_b: unhandled szB == %d\n", szB);
         tl_assert(0);
   }
   ...
   stmt( 'B', fce, IRStmt_Dirty(di) );
   if (fce->hWordTy == Ity_I64) {
      /* 64-bit host */
      IRTemp bTmp32 = newTemp(fce, Ity_I32, BSh);
      assign( 'B', fce, bTmp32, unop(Iop_64to32, mkexpr(bTmp)) );
...
/* Generate IR to do a shadow origins load from BASEADDR+OFFSET.  The
   ...
   happens).
*/
static IRAtom* gen_load_b ( FCEnv* fce, Int szB, IRAtom* baseaddr,
                            Int offset )
{
   return gen_guarded_load_b(fce, szB, baseaddr, offset, NULL/*guard*/);
}
...
*/
static
IRAtom* expr2ori_Load_guarded_General ( FCEnv* fce,
                                        IRType ty,
                                        IRAtom* addr, UInt bias,
                                        IRAtom* guard, IRAtom* balt )
{
   ...
   IRAtom* iftrue
      = assignNew('B', fce, Ity_I32,
                  gen_guarded_load_b(fce, sizeofIRType(ty),
                                     addr, bias, guard));
   ...
   IRAtom* cond
      = guard == NULL  ? mkU1(1)  : guard;
   /* And assemble the final result. */
   return assignNew('B', fce, Ity_I32, IRExpr_ITE(cond, iftrue, iffalse));
}
                          ...
                          IRAtom* guard )
{
   ...
   if (fce->hWordTy == Ity_I64)
      dataB = assignNew( 'B', fce, Ity_I64, unop(Iop_32Uto64, dataB));

   switch (szB) {
      case 1: hFun  = (void*)&FC_(helperc_b_store1);
              hName = "FC_(helperc_b_store1)";
              break;
      case 2: hFun  = (void*)&FC_(helperc_b_store2);
              hName = "FC_(helperc_b_store2)";
              break;
      case 4: hFun  = (void*)&FC_(helperc_b_store4);
              hName = "FC_(helperc_b_store4)";
              break;
      case 8: hFun  = (void*)&FC_(helperc_b_store8);
              hName = "FC_(helperc_b_store8)";
              break;
      case 16: hFun  = (void*)&FC_(helperc_b_store16);
               hName = "FC_(helperc_b_store16)";
               break;
      case 32: hFun  = (void*)&FC_(helperc_b_store32);
               hName = "FC_(helperc_b_store32)";
               break;
      default:
         tl_assert(0);
   }
...
static IRAtom* narrowTo32 ( FCEnv* fce, IRAtom* e ) {
   IRType eTy = typeOfIRExpr(fce->sb->tyenv, e);
   if (eTy == Ity_I64)
      return assignNew( 'B', fce, Ity_I32, unop(Iop_64to32, e) );
   if (eTy == Ity_I32)
      return e;
   tl_assert(0);
}

static IRAtom* zWidenFrom32 ( FCEnv* fce, IRType dstTy, IRAtom* e ) {
   IRType eTy = typeOfIRExpr(fce->sb->tyenv, e);
   tl_assert(eTy == Ity_I32);
   if (dstTy == Ity_I64)
      return assignNew( 'B', fce, Ity_I64, unop(Iop_32Uto64, e) );
   tl_assert(0);
}


static IRAtom* schemeE ( FCEnv* fce, IRExpr* e )
{
   ...
   switch (e->tag) {
      ...
      case Iex_GetI: {
         ...
         IRType equivIntTy 
            = FC_(get_otrack_reg_array_equiv_int_type)(descr);
         ...
         descr_b = mkIRRegArray( descr->base + 2*fce->layout->total_sizeB,
         ...
         /* Do a shadow indexed get of the same size, giving t1.  Take
            ...
            value t4. */
         t1 = assignNew( 'B', fce, equivIntTy, 
                          IRExpr_GetI( descr_b, e->Iex.GetI.ix, 
                                                e->Iex.GetI.bias ));
         t2 = narrowTo32( fce, t1 );
         t3 = schemeE( fce, e->Iex.GetI.ix );
         t4 = gen_maxU32( fce, t2, t3 );
         return t4;
      }
      case Iex_CCall: {
         Int i;
         ...
         for (i = 0; args[i]; i++) {
            tl_assert(i < 32);
            tl_assert(isOriginalAtom(fce, args[i]));
            ...
            if (e->Iex.CCall.cee->mcx_mask & (1<<i)) {
               ...
               /* calculate the arg's definedness, and pessimistically
                  merge it in. */
               here = schemeE( fce, args[i] );
               curr = gen_maxU32( fce, curr, here );
            }
         }
         return curr;
      }
      case Iex_Load: {
         ...
         tl_assert(fce->hWordTy == Ity_I32 || fce->hWordTy == Ity_I64);
         return gen_load_b( fce, dszB, e->Iex.Load.addr, 0 );
      }
      case Iex_ITE: {
         IRAtom* b1 = schemeE( fce, e->Iex.ITE.cond );
         IRAtom* b3 = schemeE( fce, e->Iex.ITE.iftrue );
         IRAtom* b2 = schemeE( fce, e->Iex.ITE.iffalse );
         return gen_maxU32( fce, b1, gen_maxU32( fce, b2, b3 ));
      }
      case Iex_Qop: {
         IRAtom* b1 = schemeE( fce, e->Iex.Qop.details->arg1 );
         IRAtom* b2 = schemeE( fce, e->Iex.Qop.details->arg2 );
         IRAtom* b3 = schemeE( fce, e->Iex.Qop.details->arg3 );
         IRAtom* b4 = schemeE( fce, e->Iex.Qop.details->arg4 );
         return gen_maxU32( fce, gen_maxU32( fce, b1, b2 ),
                                 gen_maxU32( fce, b3, b4 ) );
      }
      case Iex_Triop: {
         IRAtom* b1 = schemeE( fce, e->Iex.Triop.details->arg1 );
         IRAtom* b2 = schemeE( fce, e->Iex.Triop.details->arg2 );
         IRAtom* b3 = schemeE( fce, e->Iex.Triop.details->arg3 );
         return gen_maxU32( fce, b1, gen_maxU32( fce, b2, b3 ) );
      }
      case Iex_Binop: {
         switch (e->Iex.Binop.op) {
            ...
            case Iop_CasCmpEQ64: case Iop_CasCmpNE64:
               ...
               return mkU32(0);
            default: {
               IRAtom* b1 = schemeE( fce, e->Iex.Binop.arg1 );
               IRAtom* b2 = schemeE( fce, e->Iex.Binop.arg2 );
               return gen_maxU32( fce, b1, b2 );
            }
         }
         tl_assert(0);
         /*NOTREACHED*/
      ...
      case Iex_Unop: {
         IRAtom* b1 = schemeE( fce, e->Iex.Unop.arg );
         return b1;
      }
      case Iex_Const:
         return mkU32(0);
      case Iex_RdTmp:
         return mkexpr( findShadowTmpB( fce, e->Iex.RdTmp.tmp ));
      case Iex_Get: {
         Int b_offset = FC_(get_otrack_shadow_offset)( 
                           e->Iex.Get.offset,
                           sizeofIRType(e->Iex.Get.ty) 
                        );
         tl_assert(b_offset >= -1
                   && b_offset <= fce->layout->total_sizeB -4);
         if (b_offset >= 0) {
            /* FIXME: this isn't an atom! */
            return IRExpr_Get( b_offset + 2*fce->layout->total_sizeB,
                               Ity_I32 );
         }
         return mkU32(0);
      }
      default:
         VG_(printf)("fc_translate.c: schemeE: unhandled: ");
         ppIRExpr(e); 
         VG_(tool_panic)("flowcheck:schemeE");
   }
}


static void do_origins_Dirty ( FCEnv* fce, IRDirty* d )
{
      ...
      for (k = 0; k < 1 + d->fxState[i].nRepeats; k++) {
         ...
         while (True) {
            ...
            /* update 'curr' with maxU32 of the state slice 
               gOff .. gOff+n-1 */
            b_offset = FC_(get_otrack_shadow_offset)(gOff, 4);
            if (b_offset != -1) {
               /* Observe the guard expression. If it is false use 0, i.e.
                  nothing is known about the origin */
               IRAtom *cond, *iffalse, *iftrue;

               cond = assignNew( 'B', fce, Ity_I1, d->guard);
               iffalse = mkU32(0);
               iftrue  = assignNew( 'B', fce, Ity_I32,
                                    IRExpr_Get(b_offset
                                                 + 2*fce->layout->total_sizeB,
                                               Ity_I32));
               here = assignNew( 'B', fce, Ity_I32,
                                 IRExpr_ITE(cond, iftrue, iffalse));
               curr = gen_maxU32( fce, curr, here );
            }
            gSz -= n;
   ...
   if (d->mFx == Ifx_Read || d->mFx == Ifx_Modify) {
      ...
      /* chew off 32-bit chunks.  We don't care about the endianness
         ...
         native to the platform. */
      while (toDo >= 4) {
         here = gen_guarded_load_b( fce, 4, d->mAddr, d->mSize - toDo,
                                    d->guard );
         curr = gen_maxU32( fce, curr, here );
         toDo -= 4;
      }
      ...
      while (toDo >= 2) {
         here = gen_guarded_load_b( fce, 2, d->mAddr, d->mSize - toDo,
                                    d->guard );
         curr = gen_maxU32( fce, curr, here );
         toDo -= 2;
      }
      /* chew off the remaining 8-bit chunk, if any */
      if (toDo == 1) {
         here = gen_guarded_load_b( fce, 1, d->mAddr, d->mSize - toDo,
                                    d->guard );
         curr = gen_maxU32( fce, curr, here );
         toDo -= 1;
      }
   ...
   /* Outputs: the destination temporary, if there is one. */
   if (d->tmp != IRTemp_INVALID) {
      dst   = findShadowTmpB(fce, d->tmp);
      assign( 'V', fce, dst, curr );
   }
      ...
      for (k = 0; k < 1 + d->fxState[i].nRepeats; k++) {
         ...
         while (True) {
            ...
            if (b_offset != -1) {
               ...
               cond    = assignNew('B', fce, Ity_I1,
                                   d->guard);
               iffalse = assignNew('B', fce, Ity_I32,
                                   IRExpr_Get(b_offset +
                                              2*fce->layout->total_sizeB,
                                              Ity_I32));
               curr = assignNew('V', fce, Ity_I32,
                                IRExpr_ITE(cond, curr, iffalse));

               stmt( 'B', fce, IRStmt_Put(b_offset
                                          + 2*fce->layout->total_sizeB,
                                          curr ));
            }
            gSz -= n;
   ...
   if (d->mFx == Ifx_Write || d->mFx == Ifx_Modify) {
      ...
      /* chew off 32-bit chunks */
      while (toDo >= 4) {
         gen_store_b( fce, 4, d->mAddr, d->mSize - toDo, curr,
                      d->guard );
         toDo -= 4;
      }
      /* handle possible 16-bit excess */
      while (toDo >= 2) {
         gen_store_b( fce, 2, d->mAddr, d->mSize - toDo, curr,
                      d->guard );
         toDo -= 2;
      }
      /* chew off the remaining 8-bit chunk, if any */
      if (toDo == 1) {
         gen_store_b( fce, 1, d->mAddr, d->mSize - toDo, curr,
                      d->guard );
         toDo -= 1;
      }
      tl_assert(toDo == 0);
...
static void do_origins_Store_guarded ( FCEnv* fce,
                                       IREndness stEnd,
                                       IRExpr* stAddr,
                                       IRExpr* stData,
                                       IRExpr* guard )
{
   ...
   dszB  = sizeofIRType( typeOfIRExpr(fce->sb->tyenv, stData ) );
   dataB = schemeE( fce, stData );
   gen_store_b( fce, dszB, stAddr, 0/*offset*/, dataB, guard );
}


/* Generate IR for origin shadowing for a plain store. */
static void do_origins_Store_plain ( FCEnv* fce,
                                     IREndness stEnd,
                                     IRExpr* stAddr,
                                     IRExpr* stData )
{
   do_origins_Store_guarded ( fce, stEnd, stAddr, stData,
                              NULL/*guard*/ );
}
...
static void do_origins_StoreG ( FCEnv* fce, IRStoreG* sg )
{
   do_origins_Store_guarded( fce, sg->end, sg->addr,
                             sg->data, sg->guard );
}

static void do_origins_LoadG ( FCEnv* fce, IRLoadG* lg )
{
   IRType loadedTy = Ity_INVALID;
   switch (lg->cvt) {
   ...
   IRAtom* ori_alt
      = schemeE( fce,lg->alt );
   IRAtom* ori_final
      = expr2ori_Load_guarded_General(fce, loadedTy,
                                      lg->addr, 0/*addr bias*/,
                                      lg->guard, ori_alt );
   /* And finally, bind the origin to the destination temporary. */
   assign( 'B', fce, findShadowTmpB(fce, lg->dst), ori_final );
}
   ...
   switch (st->tag) {
      ...
      case Ist_AbiHint:
         /* The value-check instrumenter handles this - by arranging
            ...
            FC_(helperc_MAKE_STACK_UNINIT).  This is all that needs to
      ...
      case Ist_PutI: {
         ...
         tl_assert(sizeofIRType(equivIntTy) == sizeofIRType(descr->elemTy));
         descr_b
            = mkIRRegArray( descr->base + 2*fce->layout->total_sizeB,
                            equivIntTy, descr->nElems );
         /* Compute a value to Put - the conjoinment of the origin for
            the data to be Put-ted (obviously) and of the index value
            (not so obviously). */
         t1 = schemeE( fce, puti->data );
         t2 = schemeE( fce, puti->ix );
         t3 = gen_maxU32( fce, t1, t2 );
         t4 = zWidenFrom32( fce, equivIntTy, t3 );
         stmt( 'B', fce, IRStmt_PutI( mkIRPutI(descr_b, puti->ix,
                                               puti->bias, t4) ));
         break;
      }

      case Ist_Dirty:
         do_origins_Dirty( fce, st->Ist.Dirty.details );
         break;

      case Ist_Store:
         do_origins_Store_plain( fce, st->Ist.Store.end,
                                      st->Ist.Store.addr,
                                      st->Ist.Store.data );
         break;

      case Ist_StoreG:
         do_origins_StoreG( fce, st->Ist.StoreG.details );
         break;

      case Ist_LoadG:
         do_origins_LoadG( fce, st->Ist.LoadG.details );
         break;
      ...
      case Ist_LLSC: {
         ...
         if (st->Ist.LLSC.storedata == NULL) {
            /* Load Linked */
            IRType resTy 
               = typeOfIRTemp(fce->sb->tyenv, st->Ist.LLSC.result);
            IRExpr* vanillaLoad
               = IRExpr_Load(st->Ist.LLSC.end, resTy, st->Ist.LLSC.addr);
            tl_assert(resTy == Ity_I64 || resTy == Ity_I32
                      || resTy == Ity_I16 || resTy == Ity_I8);
            assign( 'B', fce, findShadowTmpB(fce, st->Ist.LLSC.result),
                              schemeE(fce, vanillaLoad));
         } else {
            /* Store conditional */
            do_origins_Store_plain( fce, st->Ist.LLSC.end,
                                    st->Ist.LLSC.addr,
                                    st->Ist.LLSC.storedata );
      ...
      case Ist_Put: {
         Int b_offset
            = FC_(get_otrack_shadow_offset)(
                 st->Ist.Put.offset,
                 sizeofIRType(typeOfIRExpr(fce->sb->tyenv, st->Ist.Put.data))
              );
         if (b_offset >= 0) {
            /* FIXME: this isn't an atom! */
            stmt( 'B', fce, IRStmt_Put(b_offset + 2*fce->layout->total_sizeB, 
                                       schemeE( fce, st->Ist.Put.data )) );
         }
         break;
      }

      case Ist_WrTmp:
         assign( 'B', fce, findShadowTmpB(fce, st->Ist.WrTmp.tmp),
                           schemeE(fce, st->Ist.WrTmp.data) );
         break;
      ...
      default:
         VG_(printf)("fc_translate.c: schemeS: unhandled: ");
         ppIRStmt(st); 
         VG_(tool_panic)("flowcheck:schemeS");
   }
}
