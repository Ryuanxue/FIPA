...
/* See comment in fc_include.h */
Bool FC_(any_value_errors) = False;
...
typedef struct _FC_Error FC_Error;
...
const HChar* FC_(parse_leak_kinds_tokens) = 
...
UInt FC_(all_Reachedness)(void)
{
   ...
   if (all == 0) {
      ...
      Bool parseok = VG_(parse_enum_set)(FC_(parse_leak_kinds_tokens),
                                         True,/*allow_all*/
                                         "all",
                                         &all);
...
static void fc_pp_origin ( ExeContext* ec, UInt okind )
{
   const HChar* src = NULL;
   tl_assert(ec);

   switch (okind) {
      case FC_OKIND_STACK:   src = " by a stack allocation"; break;
      case FC_OKIND_HEAP:    src = " by a heap allocation"; break;
      case FC_OKIND_USER:    src = " by a client request"; break;
      case FC_OKIND_UNKNOWN: src = ""; break;
   }
...
HChar * FC_(snprintf_delta) (HChar * buf, Int size, 
                          ...
                          LossRecord* lr, Bool xml)
{
   ...
   FC_(snprintf_delta) (d_bytes, sizeof(d_bytes),
                        lr->szB + lr->indirect_szB, 
                        lr->old_szB + lr->old_indirect_szB,
                        FC_(detect_memory_leaks_last_delta_mode));
   FC_(snprintf_delta) (d_direct_bytes, sizeof(d_direct_bytes),
                        lr->szB,
                        lr->old_szB,
                        FC_(detect_memory_leaks_last_delta_mode));
   FC_(snprintf_delta) (d_indirect_bytes, sizeof(d_indirect_bytes),
                        lr->indirect_szB,
                        lr->old_indirect_szB,
                        FC_(detect_memory_leaks_last_delta_mode));
   FC_(snprintf_delta) (d_num_blocks, sizeof(d_num_blocks),
                        (SizeT) lr->num_blocks,
...
void FC_(pp_Error) ( const Error* err )
{
   ...
   FC_Error* extra = VG_(get_error_extra)(err);
   ...
   switch (VG_(get_error_kind)(err)) {
      ...
      case Err_Value:
         ...
         if (xml) {
            ...
            if (extra->Err.Value.origin_ec)
               fc_pp_origin( extra->Err.Value.origin_ec,
      ...
      case Err_Cond:
         ...
         if (xml) {
            ...
            if (extra->Err.Cond.origin_ec)
               fc_pp_origin( extra->Err.Cond.origin_ec,
      ...
      case Err_RegParam:
         ...
         if (xml) {
            ...
            if (extra->Err.RegParam.origin_ec)
               fc_pp_origin( extra->Err.RegParam.origin_ec,
      ...
      case Err_MemParam:
         ...
         if (xml) {
            ...
            if (extra->Err.MemParam.origin_ec 
                && !extra->Err.MemParam.isAddrErr)
               fc_pp_origin( extra->Err.MemParam.origin_ec,
      ...
      case Err_User:
         ...
         if (xml) { 
            ...
            if (extra->Err.User.origin_ec && !extra->Err.User.isAddrErr)
               fc_pp_origin( extra->Err.User.origin_ec,
      ...
      default: 
         ...
         VG_(tool_panic)("unknown error code in fc_pp_Error)");
   }
}
                                 ...
                                 Bool isWrite )
{
   ...
   if (FC_(clo_workaround_gcc296_bugs) && just_below_esp)
      return;
...
void FC_(record_freemismatch_error) ( ThreadId tid, FC_Chunk* mc )
{
   ...
   ai->Addr.Block.allocated_at = FC_(allocated_at) (mc);
   VG_(initThreadInfo) (&ai->Addr.Block.alloc_tinfo);
   ai->Addr.Block.freed_at = FC_(freed_at) (mc);
...
Bool FC_(record_fishy_value_error) ( ThreadId tid, const HChar *function_name,
...
{
   FC_Error* extra1 = VG_(get_error_extra)(e1);
   FC_Error* extra2 = VG_(get_error_extra)(e2);
   ...
   switch (VG_(get_error_kind)(e1)) {
      ...
      case Err_Leak:
         VG_(tool_panic)("Shouldn't get Err_Leak in fc_eq_Error,\n"
                         "since it's handled with VG_(unique_error)()!");

      default: 
         VG_(printf)("Error:\n  unknown error code %d\n",
                     VG_(get_error_kind)(e1));
         VG_(tool_panic)("unknown error code in fc_eq_Error");
   }
}

/* Functions used when searching FC_Chunk lists */
static
Bool addr_is_in_FC_Chunk_default_REDZONE_SZB(FC_Chunk* mc, Addr a)
{
   return VG_(addr_is_in_block)( a, mc->data, mc->szB,
                                 FC_(Malloc_Redzone_SzB) );
}
static
Bool addr_is_in_FC_Chunk_with_REDZONE_SZB(FC_Chunk* mc, Addr a, SizeT rzB)
{
   return VG_(addr_is_in_block)( a, mc->data, mc->szB,
                                 rzB );
}
...
static void describe_addr ( Addr a, /*OUT*/AddrInfo* ai )
{
   ...
   /* Blocks allocated by flowcheck malloc functions are either
   ...
   while ( (mc = VG_(HT_Next)(FC_(malloc_list))) ) {
      if (addr_is_in_FC_Chunk_default_REDZONE_SZB(mc, a)) {
         ai->tag = Addr_Block;
         ai->Addr.Block.block_kind = Block_Mallocd;
         if (FC_(get_freed_block_bracketting)( a ))
   ...
   mc = FC_(get_freed_block_bracketting)( a );
   if (mc) {
      ai->tag = Addr_Block;
...
UInt FC_(update_Error_extra)( const Error* err )
{
   ...
   // possible because we have the FC_Chunk on hand when the error is
   ...
   default: VG_(tool_panic)("fc_update_extra: bad errkind");
   }
}
                                         ...
                                         /*OUT*/AddrInfo* ai )
{
   ...
   FC_(get_ClientBlock_array)( &cgbs, &cgb_used );
   if (cgbs == NULL)
   ...
   while ( (mp = VG_(HT_Next)(FC_(mempool_list))) ) {
      if (mp->chunks != NULL) {
         ...
         while ( (mc = VG_(HT_Next)(mp->chunks)) ) {
            if (addr_is_in_FC_Chunk_with_REDZONE_SZB(mc, a, mp->rzB)) {
               ai->tag = Addr_Block;
...
typedef struct _FC_LeakSuppExtra FC_LeakSuppExtra;

struct _FC_LeakSuppExtra {
...
Bool FC_(read_extra_suppression_info) ( Int fd, HChar** bufpp,
                                        SizeT* nBufp, Int* lineno, Supp *su )
{
   ...
   if (VG_(get_supp_kind)(su) == ParamSupp) {
      ...
      FC_LeakSuppExtra* lse;
      lse = VG_(malloc)("mc.resi.2", sizeof(FC_LeakSuppExtra));
      lse->match_leak_kinds = FC_(all_Reachedness)();
      ...
      if (0 == VG_(strncmp)(*bufpp, "match-leak-kinds:", 17)) {
         ...
         while ((*bufpp)[i] && VG_(isspace)((*bufpp)[i]))
            i++;
         if (!VG_(parse_enum_set)(FC_(parse_leak_kinds_tokens),
                                  True/*allow_all*/,
...
Bool FC_(error_matches_suppression) ( const Error* err, const Supp* su )
{
   ...
   switch (VG_(get_supp_kind)(su)) {
      ...
      case LeakSupp:
         if (ekind == Err_Leak) {
            FC_LeakSuppExtra* lse = (FC_LeakSuppExtra*) VG_(get_supp_extra)(su);
            if (lse->leak_search_gen != FC_(leak_search_gen)) {
               // First time we see this suppression during this leak search.
               // => reset the counters to 0.
               lse->blocks_suppressed = 0;
               lse->bytes_suppressed = 0;
               lse->leak_search_gen = FC_(leak_search_gen);
            }
      ...
      case FishyValueSupp: {
         FC_FishyValueExtra *supp_extra = VG_(get_supp_extra)(su);
...
void FC_(update_extra_suppression_use) ( const Error* err, const Supp* su)
{
   if (VG_(get_supp_kind)(su) == LeakSupp) {
      ...
      tl_assert (lse->leak_search_gen == FC_(leak_search_gen));
