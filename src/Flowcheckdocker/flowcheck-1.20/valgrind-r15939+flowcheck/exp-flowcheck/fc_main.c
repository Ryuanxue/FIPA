...
static SecMap* copy_for_writing ( SecMap* dist_sm )
{
   ...
   if (new_sm == NULL)
      VG_(out_of_memory_NORETURN)( "flowcheck:allocate new SecMap", 
                                   sizeof(SecMap) );
...
// This must be a power of two;  this is checked in fc_pre_clo_init().
...
static void gcSecVBitTable(void)
{
   ...
   if (VG_(clo_verbosity) > 1 && n_nodes != 0) {
      VG_(message)(Vg_DebugMsg, "flowcheck GC: %d nodes, %d survivors (%.1f%%)\n",
   ...
   if ((Double)n_survivors 
      ...
      if (VG_(clo_verbosity) > 1)
         VG_(message)(Vg_DebugMsg,
                      "flowcheck GC: %d new table size (stepup)\n",
                      secVBitLimit);
   }
   else
   if (secVBitLimit < DRIFTUP_MAX_SIZE
      ...
      if (VG_(clo_verbosity) > 1)
         VG_(message)(Vg_DebugMsg,
                      "flowcheck GC: %d new table size (driftup)\n",
                      secVBitLimit);
   }
}
...
Bool FC_(in_ignored_range) ( Addr a )
{
   ...
   VG_(tool_panic)("FC_(in_ignore_range)");
   /*NOTREACHED*/
}
...
static Bool modify_ignore_ranges ( Bool addRange, Addr start, Addr len )
{
   ...
   if (addRange) {
      ...
      if (verbose)
         VG_(dmsg)("flowcheck: modify_ignore_ranges: add %p %p\n",
                   (void*)start, (void*)(start+len-1));
   } else {
      VG_(bindRangeMap)(gIgnoredAddressRanges,
                        start, start+len-1, IAR_NotIgnored);
      if (verbose)
         VG_(dmsg)("flowcheck: modify_ignore_ranges: del %p %p\n",
                   (void*)start, (void*)(start+len-1));
   }
   if (verbose) {
      VG_(dmsg)("flowcheck:   now have %u ranges:\n",
      ...
      for (i = 0; i < VG_(sizeRangeMap)(gIgnoredAddressRanges); i++) {
         ...
         VG_(dmsg)("flowcheck:      [%u]  %016lx-%016lx  %s\n",
                                ...
                                Addr a, SizeT nBits, Bool bigendian )
{
   ...
   if (!FC_(clo_partial_loads_ok)) {
      FC_(record_address_error)( VG_(get_running_tid)(), a, szB, False );
      return;
   }
...
static
__attribute__((noinline))
void fc_STOREVn_slow ( Addr a, SizeT nBits, ULong vbytes, Bool bigendian )
{
   SizeT szB = nBits / 8;
...
void FC_(make_mem_undefined_w_otag) ( Addr a, SizeT len, UInt otag )
{
   ...
   DEBUG("FC_(make_mem_undefined)(%p, %lu)\n", a, len);
   set_address_range_perms ( a, len, VA_BITS16_UNDEFINED, SM_DIST_UNDEFINED );
   if (UNLIKELY( FC_(clo_fc_level) == 3 ))
...
{
   FC_(make_mem_undefined_w_otag) ( a, len, FC_OKIND_UNKNOWN );
}

void FC_(make_mem_defined) ( Addr a, SizeT len )
{
   PROF_EVENT(MCPE_MAKE_MEM_DEFINED);
   DEBUG("FC_(make_mem_defined)(%p, %lu)\n", a, len);
...
/* For each byte in [a,a+len), if the byte is addressable, make it be
   ...
   words a version of FC_(make_mem_defined) that doesn't mess with
...
static void make_mem_defined_if_addressable ( Addr a, SizeT len )
{
   ...
   for (i = 0; i < len; i++) {
      ...
      if (LIKELY(VA_BITS2_NOACCESS != vabits2)) {
         set_vabits2(a+i, VA_BITS2_DEFINED);
         if (UNLIKELY(FC_(clo_fc_level) >= 3)) {
            FC_(helperc_b_store1)( a+i, 0 ); /* clear the origin tag */
         } 
      }
   }
}
...
void FC_(copy_address_range_state) ( Addr src, Addr dst, SizeT len )
{
   ...
   DEBUG("FC_(copy_address_range_state)\n");
...
/* AN OVERVIEW OF THE ORIGIN TRACKING IMPLEMENTATION
   ...
   * to convert from an ECU to an otag, OR in one of the FC_OKIND_
     constants defined in fc_include.h.
   ...
   * to convert an otag back to an ECU, AND it with ~3
   ...
   mapping is contained in fc_machine.c, which is quite lengthy but
   straightforward.
   ...
   functions schemeE and schemeS in fc_translate.c.  These generate
   ...
   FC_(helperc_MAKE_STACK_UNINIT), now takes a third argument, the
   ...
   denotes (to flowcheck) "unknown origin or defined value".  So these
...
static void init_OCache ( void )
{
   ...
   if (ocacheL1 == NULL) {
      VG_(out_of_memory_NORETURN)( "flowcheck:allocating ocacheL1", 
                                   sizeof(OCache) );
   }
   tl_assert(ocacheL1 != NULL);
...
{
   //// BEGIN inlined, specialised version of FC_(helperc_b_store8)
...
void make_aligned_word32_undefined_w_otag ( Addr a, UInt otag )
{
   ...
   //// BEGIN inlined, specialised version of FC_(helperc_b_store4)
   ...
   //// END inlined, specialised version of FC_(helperc_b_store4)
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_4_w_ECU(Addr new_SP, UInt ecu)
{
   ...
   if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 4, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_4(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_4);
   if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_4(Addr new_SP)
{
   PROF_EVENT(MCPE_DIE_MEM_STACK_4);
   if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word32_noaccess ( -VG_STACK_REDZONE_SZB + new_SP-4 );
   } else {
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-4, 4 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_8_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 8, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_8(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_8);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_8(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-8, 8 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_12_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 12, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_12(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_12);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_undefined ( -VG_STACK_REDZONE_SZB + new_SP );
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_12(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP-12 )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-12, 12 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_16_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 16, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_16(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_16);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      /* Have 8-alignment at +0, hence do 8 at +0 and 8 at +8. */
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_16(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-16, 16 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_32_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 32, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_32(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_32);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      /* Straightforward */
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_32(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-32, 32 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_112_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 112, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_112(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_112);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_undefined ( -VG_STACK_REDZONE_SZB + new_SP );
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_112(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-112, 112 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_128_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 128, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_128(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_128);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_undefined ( -VG_STACK_REDZONE_SZB + new_SP );
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_128(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-128, 128 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_144_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 144, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_144(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_144);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_undefined ( -VG_STACK_REDZONE_SZB + new_SP );
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_144(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-144, 144 );
   }
}
...
MAYBE_USED
static void VG_REGPARM(2) fc_new_mem_stack_160_w_ECU(Addr new_SP, UInt ecu)
{
   UInt otag = ecu | FC_OKIND_STACK;
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_undefined_w_otag) ( -VG_STACK_REDZONE_SZB + new_SP, 160, otag );
   }
}

MAYBE_USED
static void VG_REGPARM(1) fc_new_mem_stack_160(Addr new_SP)
{
   PROF_EVENT(MCPE_NEW_MEM_STACK_160);
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_undefined ( -VG_STACK_REDZONE_SZB + new_SP );
...
MAYBE_USED
static void VG_REGPARM(1) fc_die_mem_stack_160(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FC_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-160, 160 );
   }
}
...
#  if 0
   ...
   */
   if (LIKELY( VG_IS_8_ALIGNED(base) && len==128 )) {
      ...
      FC_(make_mem_undefined_w_otag)(base, len, otag);
   }
#  endif 
...
/* This is a version of FC_(helperc_MAKE_STACK_UNINIT_w_o) that is
...
static FC_ReadResult is_mem_defined ( Addr a, SizeT len,
                                      /*OUT*/Addr* bad_addr,
                                      /*OUT*/UInt* otag )
{
   SizeT i;
   ...
   for (i = 0; i < len; i++) {
      ...
      if (VA_BITS2_DEFINED != vabits2) {
         ...
         if (FC_(clo_fc_level) >= 2) {
            if (otag && FC_(clo_fc_level) == 3) {
               *otag = FC_(helperc_b_load1)( a );
            }
            return FC_ValueErr;
         }
      }
...
static Bool fc_is_defined_asciiz ( Addr a, Addr* bad_addr, UInt* otag )
{
   UWord vabits2;

   PROF_EVENT(MCPE_IS_DEFINED_ASCIIZ);
   DEBUG("fc_is_defined_asciiz\n");
                                ...
                                Addr base, SizeT size )
{
   ...
   if (!ok) {
      switch (part) {
      case Vg_CoreSysCall:
         FC_(record_memparam_error) ( tid, bad_addr, 
                                      /*isAddrErr*/True, s, 0/*otag*/ );
         break;

      case Vg_CoreSignal:
         FC_(record_core_mem_error)( tid, s );
         break;
...
{     
   UInt otag = 0;
   Addr bad_addr;
   FC_ReadResult res = is_mem_defined ( base, size, &bad_addr, &otag );

   if (FC_Ok != res) {
      Bool isAddrErr = ( FC_AddrErr == res ? True : False );

      switch (part) {
      case Vg_CoreSysCall:
         FC_(record_memparam_error) ( tid, bad_addr, isAddrErr, s,
                                      isAddrErr ? 0 : otag );
         break;
      
      case Vg_CoreSysCallArgInMem:
         FC_(record_regparam_error) ( tid, s, otag );
         break;

      /* If we're being asked to jump to a silly address, record an error 
         message before potentially crashing the entire system. */
      case Vg_CoreTranslate:
         FC_(record_jump_error)( tid, bad_addr );
         break;
                                   ...
                                   const HChar* s, Addr str )
{
   ...
   res = fc_is_defined_asciiz ( (Addr)str, &bad_addr, &otag );
   if (FC_Ok != res) {
      Bool isAddrErr = ( FC_AddrErr == res ? True : False );
      FC_(record_memparam_error) ( tid, bad_addr, isAddrErr, s,
                                   isAddrErr ? 0 : otag );
...
*/
static
void fc_new_mem_mmap ( Addr a, SizeT len, Bool rr, Bool ww, Bool xx,
                       ULong di_handle )
{
   if (rr || ww || xx) {
      ...
      FC_(make_mem_noaccess)(a, len);
   }
}

static
void fc_new_mem_mprotect ( Addr a, SizeT len, Bool rr, Bool ww, Bool xx )
{
   if (rr || ww || xx) {
      /* (4) mprotect other  ->  change any "noaccess" to "defined" */
...
static
void fc_new_mem_startup( Addr a, SizeT len,
                         Bool rr, Bool ww, Bool xx, ULong di_handle )
{
   ...
   DEBUG("fc_new_mem_startup(%#lx, %llu, rr=%u, ww=%u, xx=%u)\n",
         a, (ULong)len, rr, ww, xx);
   fc_new_mem_mmap(a, len, rr, ww, xx, di_handle);
}

static
void fc_post_mem_write(CorePart part, ThreadId tid, Addr a, SizeT len)
{
   FC_(make_mem_defined)(a, len);
}
                                             ...
                                             Int offset, SizeT size )
{
   ...
   UInt  area[3];
   UInt  otag;
   sh2off = FC_(get_otrack_shadow_offset)( offset, size );
   if (sh2off == -1)
...
*/
static void fc_post_reg_write ( CorePart part, ThreadId tid, 
...
static 
void fc_post_reg_write_clientcall ( ThreadId tid, 
                                    PtrdiffT offset, SizeT size, Addr f)
{
   fc_post_reg_write(/*dummy*/0, tid, offset, size);
}
...
/* Look at the definedness of the guest's shadow state for 
   ...
   a parameter error.
*/
static void fc_pre_reg_read ( CorePart part, ThreadId tid, const HChar* s, 
                              PtrdiffT offset, SizeT size)
{
   Int   i;
   Bool  bad;
...
static void fc_copy_mem_to_reg ( CorePart part, ThreadId tid, Addr a,
                                 PtrdiffT guest_state_offset, SizeT size )
{
   ...
   /* Track origins. */
   offset = FC_(get_otrack_shadow_offset)( guest_state_offset, size );
   if (offset == -1)
      return;

   switch (size) {
   case 1:
      d32 = FC_(helperc_b_load1)( a );
      break;
   case 2:
      d32 = FC_(helperc_b_load2)( a );
      break;
   case 4:
      d32 = FC_(helperc_b_load4)( a );
      break;
   case 8:
      d32 = FC_(helperc_b_load8)( a );
      break;
   case 16:
      d32 = FC_(helperc_b_load16)( a );
      break;
   case 32:
      d32 = FC_(helperc_b_load32)( a );
      break;
   default:
      tl_assert(0);
   }
...
static void fc_copy_reg_to_mem ( CorePart part, ThreadId tid,
                                 ...
                                 SizeT size )
{
   ...
   switch (size) {
   case 1:
      FC_(helperc_b_store1)( a, d32 );
      break;
   case 2:
      FC_(helperc_b_store2)( a, d32 );
      break;
   case 4:
      FC_(helperc_b_store4)( a, d32 );
      break;
   case 8:
      FC_(helperc_b_store8)( a, d32 );
      break;
   case 16:
      FC_(helperc_b_store16)( a, d32 );
      break;
   case 32:
      FC_(helperc_b_store32)( a, d32 );
      break;
   default:
      tl_assert(0);
   }
...
static INLINE
void fc_LOADV_128_or_256 ( /*OUT*/ULong* res,
                           Addr a, SizeT nBits, Bool isBigEndian )
{
   PROF_EVENT(MCPE_LOADV_128_OR_256);

#ifndef PERF_FAST_LOADV
   fc_LOADV_128_or_256_slow( res, a, nBits, isBigEndian );
   return;
#else
   {
...
VG_REGPARM(2) void FC_(helperc_LOADV256be) ( /*OUT*/V256* res, Addr a )
{
   fc_LOADV_128_or_256(&res->w64[0], a, 256, True);
}
VG_REGPARM(2) void FC_(helperc_LOADV256le) ( /*OUT*/V256* res, Addr a )
{
   fc_LOADV_128_or_256(&res->w64[0], a, 256, False);
}

VG_REGPARM(2) void FC_(helperc_LOADV128be) ( /*OUT*/V128* res, Addr a )
{
   fc_LOADV_128_or_256(&res->w64[0], a, 128, True);
}
VG_REGPARM(2) void FC_(helperc_LOADV128le) ( /*OUT*/V128* res, Addr a )
{
   fc_LOADV_128_or_256(&res->w64[0], a, 128, False);
}
...
static INLINE
ULong fc_LOADV64 ( Addr a, Bool isBigEndian )
{
   PROF_EVENT(MCPE_LOADV64);

#ifndef PERF_FAST_LOADV
   return fc_LOADVn_slow( a, 64, isBigEndian );
#else
   {
      ...
      if (UNLIKELY( UNALIGNED_OR_HIGH(a,64) )) {
         ...
         return (ULong)fc_LOADVn_slow( a, 64, isBigEndian );
      }
...
VG_REGPARM(1) ULong FC_(helperc_LOADV64be) ( Addr a )
{
   return fc_LOADV64(a, True);
}
...
".text                                  \n"
".align 2                               \n"
".global vgFlowCheck_helperc_LOADV64le   \n"
".type   vgFlowCheck_helperc_LOADV64le, %function \n"
"vgFlowCheck_helperc_LOADV64le:          \n"
"      tst    r0, #7                    \n"
...
"      bl     fc_LOADVn_slow            \n"
"      pop    {r4, pc}                  \n"
".size vgFlowCheck_helperc_LOADV64le, .-vgFlowCheck_helperc_LOADV64le \n"
".previous\n"
);
...
"      ret\n"
".LLV64LE2:\n"
"      xor    %ecx,  %ecx\n"  /* tail call to fc_LOADVn_slow(a, 64, 0) */
"      mov    $64,   %edx\n"
"      jmp    fc_LOADVn_slow\n"
".size vgFlowCheck_helperc_LOADV64le, .-vgFlowCheck_helperc_LOADV64le\n"
".previous\n"
...
VG_REGPARM(1) ULong FC_(helperc_LOADV64le) ( Addr a )
{
   return fc_LOADV64(a, False);
}
#endif
...
static INLINE
void fc_STOREV64 ( Addr a, ULong vbits64, Bool isBigEndian )
{
   ...
   // Investigate further.
   fc_STOREVn_slow( a, 64, vbits64, isBigEndian );
#else
   {
      UWord   sm_off16, vabits16;
      SecMap* sm;
...
VG_REGPARM(1) void FC_(helperc_STOREV64be) ( Addr a, ULong vbits64 )
{
   fc_STOREV64(a, vbits64, True);
}
VG_REGPARM(1) void FC_(helperc_STOREV64le) ( Addr a, ULong vbits64 )
{
   fc_STOREV64(a, vbits64, False);
}
...
static INLINE
UWord fc_LOADV32 ( Addr a, Bool isBigEndian )
{
   PROF_EVENT(MCPE_LOADV32);

#ifndef PERF_FAST_LOADV
   return (UWord)fc_LOADVn_slow( a, 32, isBigEndian );
#else
   {
...
// Generic for all platforms
VG_REGPARM(1) UWord FC_(helperc_LOADV32be) ( Addr a )
{
   return fc_LOADV32(a, True);
}
...
".text                                  \n"
".align 2                               \n"
".global vgFlowCheck_helperc_LOADV32le   \n"
".type   vgFlowCheck_helperc_LOADV32le, %function \n"
"vgFlowCheck_helperc_LOADV32le:          \n"
"      tst    r0, #3                    \n" // 1
"      movw   r3, #:lower16:primary_map \n" // 1
"      bne    .LLV32LEc4                \n" // 2  if misaligned
"      lsr    r2, r0, #16               \n" // 3
...
"      bl     fc_LOADVn_slow            \n"
"      pop    {r4, pc}                  \n"
".size vgFlowCheck_helperc_LOADV32le, .-vgFlowCheck_helperc_LOADV32le \n"
".previous\n"
);
...
"      ret\n"
".LLV32LE2:\n"
"      xor    %ecx,  %ecx\n"       /* tail call fc_LOADVn_slow(a, 32, 0) */
"      mov    $32,   %edx\n"
"      jmp    fc_LOADVn_slow\n"
".size vgFlowCheck_helperc_LOADV32le, .-vgFlowCheck_helperc_LOADV32le\n"
".previous\n"
...
#else
// Generic for all platforms except {arm32,x86}-linux and x86-solaris
VG_REGPARM(1) UWord FC_(helperc_LOADV32le) ( Addr a )
{
   return fc_LOADV32(a, False);
}
#endif
...
static INLINE
void fc_STOREV32 ( Addr a, UWord vbits32, Bool isBigEndian )
{
   PROF_EVENT(MCPE_STOREV32);

#ifndef PERF_FAST_STOREV
   fc_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
#else
   {
      UWord   sm_off, vabits8;
      SecMap* sm;
...
VG_REGPARM(2) void FC_(helperc_STOREV32be) ( Addr a, UWord vbits32 )
{
   fc_STOREV32(a, vbits32, True);
}
VG_REGPARM(2) void FC_(helperc_STOREV32le) ( Addr a, UWord vbits32 )
{
   fc_STOREV32(a, vbits32, False);
}
...
static INLINE
UWord fc_LOADV16 ( Addr a, Bool isBigEndian )
{
   PROF_EVENT(MCPE_LOADV16);

#ifndef PERF_FAST_LOADV
   return (UWord)fc_LOADVn_slow( a, 16, isBigEndian );
#else
   {
      UWord   sm_off, vabits8;
      SecMap* sm;
...
// Generic for all platforms
VG_REGPARM(1) UWord FC_(helperc_LOADV16be) ( Addr a )
{
   return fc_LOADV16(a, True);
}
...
".text                                  \n"
".align 2                               \n"
".global vgFlowCheck_helperc_LOADV16le   \n"
".type   vgFlowCheck_helperc_LOADV16le, %function \n"
"vgFlowCheck_helperc_LOADV16le:          \n" //
"      tst    r0, #1                    \n" // 
"      bne    .LLV16LEc12               \n" // if misaligned
"      lsr    r2, r0, #16               \n" // r2 = pri-map-ix
"      movw   r3, #:lower16:primary_map \n" //
...
"      bl     fc_LOADVn_slow            \n" //
"      pop    {r4, pc}                  \n" //
".size vgFlowCheck_helperc_LOADV16le, .-vgFlowCheck_helperc_LOADV16le \n"
".previous\n"
);
...
".LLV16LE5:\n"
"      xor    %ecx,  %ecx\n"        /* tail call fc_LOADVn_slow(a, 16, 0) */
"      mov    $16,   %edx\n"
"      jmp    fc_LOADVn_slow\n"
".size vgFlowCheck_helperc_LOADV16le, .-vgFlowCheck_helperc_LOADV16le \n"
".previous\n"
...
#else
// Generic for all platforms except {arm32,x86}-linux and x86-solaris
VG_REGPARM(1) UWord FC_(helperc_LOADV16le) ( Addr a )
{
   return fc_LOADV16(a, False);
}
#endif
...
static INLINE
void fc_STOREV16 ( Addr a, UWord vbits16, Bool isBigEndian )
{
   PROF_EVENT(MCPE_STOREV16);

#ifndef PERF_FAST_STOREV
   fc_STOREVn_slow( a, 16, (ULong)vbits16, isBigEndian );
#else
   {
      UWord   sm_off, vabits8;
      SecMap* sm;
...
VG_REGPARM(2) void FC_(helperc_STOREV16be) ( Addr a, UWord vbits16 )
{
   fc_STOREV16(a, vbits16, True);
}
VG_REGPARM(2) void FC_(helperc_STOREV16le) ( Addr a, UWord vbits16 )
{
   fc_STOREV16(a, vbits16, False);
}
...
".text                                  \n"
".align 2                               \n"
".global vgFlowCheck_helperc_LOADV8      \n"
".type   vgFlowCheck_helperc_LOADV8, %function \n"
"vgFlowCheck_helperc_LOADV8:             \n" //
"      lsr    r2, r0, #16               \n" // r2 = pri-map-ix
"      movw   r3, #:lower16:primary_map \n" //
"      uxth   r1, r0                    \n" // r1 = sec-map-offB
"      movt   r3, #:upper16:primary_map \n" //
...
"      bl     fc_LOADVn_slow            \n" //
"      pop    {r4, pc}                  \n" //
".size vgFlowCheck_helperc_LOADV8, .-vgFlowCheck_helperc_LOADV8 \n"
".previous\n"
);
...
"      xor    %ecx,  %ecx\n"        /* tail call to fc_LOADVn_slow(a, 8, 0) */
"      mov    $0x8,  %edx\n"
"      jmp    fc_LOADVn_slow\n"
".size vgFlowCheck_helperc_LOADV8, .-vgFlowCheck_helperc_LOADV8\n"
".previous\n"
...
// Generic for all platforms except {arm32,x86}-linux and x86-solaris
VG_REGPARM(1)
UWord FC_(helperc_LOADV8) ( Addr a )
{
   PROF_EVENT(MCPE_LOADV8);

#ifndef PERF_FAST_LOADV
   return (UWord)fc_LOADVn_slow( a, 8, False/*irrelevant*/ );
#else
   {
      UWord   sm_off, vabits8;
      SecMap* sm;
...
VG_REGPARM(2)
void FC_(helperc_STOREV8) ( Addr a, UWord vbits8 )
{
   PROF_EVENT(MCPE_STOREV8);

#ifndef PERF_FAST_STOREV
   fc_STOREVn_slow( a, 8, (ULong)vbits8, False/*irrelevant*/ );
#else
   {
      UWord   sm_off, vabits8;
      SecMap* sm;
      ...
      //
      // Cleverness 2:
      // We also avoid to call fc_STOREVn_slow if the V bits can directly
      ...
      //     we must call fc_STOREVn_slow in order to report accessibility
      //     errors).
...
static Int fc_get_or_set_vbits_for_client ( 
   Addr a, 
   Addr vbits, 
   SizeT szB, 
...
)
{
   ...
   if (setting) {
      ...
      if (is_client_request)
        ...
        FC_(make_mem_defined)(vbits, szB);
   }
...
Bool FC_(is_valid_aligned_word) ( Addr a )
{
   ...
   if (UNLIKELY(FC_(in_ignored_range)(a)))
      return False;
   else
      return True;
}
...
static Bool fc_cheap_sanity_check ( void )
{
   n_sanity_cheap++;
   PROF_EVENT(MCPE_CHEAP_SANITY_CHECK);
   /* Check for sane operating level */
   if (FC_(clo_fc_level) < 1 || FC_(clo_fc_level) > 3)
      return False;
   /* nothing else useful we can rapidly check */
   return True;
}

static Bool fc_expensive_sanity_check ( void )
{
   Int     i;
   ...
   if (bad) {
      VG_(printf)("flowcheck expensive sanity: "
   ...
   if (errmsg) {
      VG_(printf)("flowcheck expensive sanity, auxmaps:\n\t%s", errmsg);
      return False;
   }
...
Bool          FC_(clo_partial_loads_ok)       = True;
Long          FC_(clo_freelist_vol)           = 20*1000*1000LL;
Long          FC_(clo_freelist_big_blocks)    =  1*1000*1000LL;
LeakCheckMode FC_(clo_leak_check)             = LC_Summary;
VgRes         FC_(clo_leak_resolution)        = Vg_HighRes;
UInt          FC_(clo_show_leak_kinds)        = R2S(Possible) | R2S(Unreached);
UInt          FC_(clo_error_for_leak_kinds)   = R2S(Possible) | R2S(Unreached);
UInt          FC_(clo_leak_check_heuristics)  =   H2S(LchStdString)
                                                | H2S( LchLength64)
                                                | H2S( LchNewArray)
                                                | H2S( LchMultipleInheritance);
Bool          FC_(clo_workaround_gcc296_bugs) = False;
Int           FC_(clo_malloc_fill)            = -1;
Int           FC_(clo_free_fill)              = -1;
KeepStacktraces FC_(clo_keep_stacktraces)     = KS_alloc_and_free;
Int           FC_(clo_fc_level)               = 2;
...
const char *  FC_(project_name)             = 0; //用于识别次分析的项目名称，避免分析第三方库而耗时 //by xuerao
Bool          FC_(secret_start)             =False;
...
static Bool fc_process_cmd_line_options(const HChar* arg)
{
   const HChar* tmp_str;
   Int   tmp_show;
   ...
   */
   ...
   else if VG_BHEX_CLO(arg, "--malloc-fill", FC_(clo_malloc_fill), 0x00,0xFF) {}
   else if VG_BHEX_CLO(arg, "--free-fill",   FC_(clo_free_fill),   0x00,0xFF) {}

   else if VG_XACT_CLO(arg, "--keep-stacktraces=alloc",
                       FC_(clo_keep_stacktraces), KS_alloc) {}
   else if VG_XACT_CLO(arg, "--keep-stacktraces=free",
                       FC_(clo_keep_stacktraces), KS_free) {}
   else if VG_XACT_CLO(arg, "--keep-stacktraces=alloc-and-free",
                       FC_(clo_keep_stacktraces), KS_alloc_and_free) {}
   else if VG_XACT_CLO(arg, "--keep-stacktraces=alloc-then-free",
                       FC_(clo_keep_stacktraces), KS_alloc_then_free) {}
   else if VG_XACT_CLO(arg, "--keep-stacktraces=none",
                       FC_(clo_keep_stacktraces), KS_none) {}

   else if VG_BOOL_CLO(arg, "--show-mismatched-frees",
                       FC_(clo_show_mismatched_frees)) {}
   ...
   else if  VG_STR_CLO(arg, "--project-name", FC_(project_name)) {} // 用于识别次分析的项目名称，避免分析第三方库而耗时 //by xuerao
...
/* type CGenBlock is defined in fc_include.h */
...
static Bool handle_gdb_monitor_command (ThreadId tid, HChar *req)
{
   ...
   case  1: { /* get_vbits */
      ...
      if (VG_(strtok_get_address_and_size) (&address, &szB, &ssaveptr)) {
         ...
         Int unaddressable = 0;
         for (i = 0; i < szB; i++) {
            Int res = fc_get_or_set_vbits_for_client 
   ...
   case  2: { /* leak_check */
      ...
      for (kw = VG_(strtok_r) (NULL, " ", &ssaveptr); 
         ...
         case  2: { /* kinds */
            ...
            if (wcmd == NULL 
                || !VG_(parse_enum_set)(FC_(parse_leak_kinds_tokens),
                                        True/*allow_all*/,
                                        wcmd,
                                        &lcp.show_leak_kinds)) {
         ...
         case  3: /* reachable */
            lcp.show_leak_kinds = FC_(all_Reachedness)();
            break;
         case  4: /* possibleleak */
            lcp.show_leak_kinds 
         ...
         case  6: { /* heuristics */
            wcmd = VG_(strtok_r) (NULL, " ", &ssaveptr);
            if (wcmd == NULL 
                || !VG_(parse_enum_set)(FC_(parse_leak_heuristics_tokens),
                                        True,/*allow_all*/
                                        wcmd,
                                        &lcp.heuristics)) {
      ...
      if (!err)
         FC_(detect_memory_leaks)(tid, &lcp);
      return True;
   }
   ...
   case  3: { /* make_memory */
      ...
      case -2: break;
      case -1: break;
      case  0: FC_(make_mem_noaccess) (address, szB); break;
      case  1: make_mem_undefined_w_tid_and_okind ( address, szB, tid, 
                                                    FC_OKIND_USER ); break;
      case  2: FC_(make_mem_defined) ( address, szB ); break;
   ...
   case  5: { /* block_list */
      ...
      if (VG_(parse_slice) (NULL, &ssaveptr, &lr_nr_from, &lr_nr_to)) {
         ...
         if (lr_nr_from == 0 || ! FC_(print_block_list) (lr_nr_from-1,
                                                         lr_nr_to-1,
                                                         limit_blocks,
                                                         heuristics))
   ...
   case  7: { /* xb */
      ...
      if (VG_(strtok_get_address_and_size) (&address, &szB, &ssaveptr)) {
         ...
         for (i = 0; i < szB; i++) {
            Int bnr = i % 8;
            res[bnr] = fc_get_or_set_vbits_for_client 
...
static Bool fc_handle_client_request ( ThreadId tid, UWord* arg, UWord* ret )
{
   ...
   switch (arg[0]) {
      case VG_USERREQ__CHECK_MEM_IS_ADDRESSABLE: {
         ...
         if (!ok)
            FC_(record_user_error) ( tid, bad_addr, /*isAddrErr*/True, 0 );
      ...
      case VG_USERREQ__CHECK_MEM_IS_DEFINED: {
         ...
         if (errorV) {
            FC_(record_user_error) ( tid, bad_addrV,
                                     /*isAddrErr*/False, otagV );
         }
         if (errorA) {
            FC_(record_user_error) ( tid, bad_addrA,
                                     /*isAddrErr*/True, 0 );
         }
      ...
      case VG_USERREQ__DO_LEAK_CHECK: {
         ...
         if (arg[1] == 0)
            ...
            VG_(message)(Vg_UserMsg, 
                         "Warning: unknown flowcheck leak search mode\n");
            lcp.mode = LC_Full;
         }
          
         lcp.show_leak_kinds = FC_(clo_show_leak_kinds);
         lcp.errors_for_leak_kinds = FC_(clo_error_for_leak_kinds);
         lcp.heuristics = FC_(clo_leak_check_heuristics);
         ...
         if (arg[2] == 0)
            ...
            VG_(message)
               (Vg_UserMsg, 
                "Warning: unknown flowcheck leak search deltamode\n");
            lcp.deltamode = LCD_Any;
         }
         lcp.max_loss_records_output = 999999999;
      ...
      case VG_USERREQ__MAKE_MEM_NOACCESS:
         FC_(make_mem_noaccess) ( arg[1], arg[2] );
         *ret = -1;
         break;
      ...
      case VG_USERREQ__MAKE_MEM_DEFINED:
         FC_(make_mem_defined) ( arg[1], arg[2] );
         *ret = -1;
         break;
      ...
      case VG_USERREQ__GET_VBITS:
         *ret = fc_get_or_set_vbits_for_client
                   ( arg[1], arg[2], arg[3],
                     False /* get them */, 
      ...
      case VG_USERREQ__COUNT_LEAKS: { /* count leaked bytes */
         ...
         // FC_(bytes_leaked) et al were set by the last leak check (or zero
         // if no prior leak checks performed).
         *argp[1] = FC_(bytes_leaked) + FC_(bytes_indirect);
         *argp[2] = FC_(bytes_dubious);
         *argp[3] = FC_(bytes_reachable);
         *argp[4] = FC_(bytes_suppressed);
         // there is no argp[5]
         //*argp[5] = FC_(bytes_indirect);
      ...
      case VG_USERREQ__COUNT_LEAK_BLOCKS: { /* count leaked blocks */
         UWord** argp = (UWord**)arg;
         // FC_(blocks_leaked) et al were set by the last leak check (or zero
         // if no prior leak checks performed).
         *argp[1] = FC_(blocks_leaked) + FC_(blocks_indirect);
         *argp[2] = FC_(blocks_dubious);
         *argp[3] = FC_(blocks_reachable);
         *argp[4] = FC_(blocks_suppressed);
         // there is no argp[5]
         //*argp[5] = FC_(blocks_indirect);
         // XXX need to make *argp[1-4] defined;  currently done in the
      ...
      case VG_USERREQ__MALLOCLIKE_BLOCK: {
         ...
         FC_(new_block) ( tid, p, sizeB, /*ignored*/0, is_zeroed, 
                          FC_AllocCustom, FC_(malloc_list) );
         if (rzB > 0) {
            FC_(make_mem_noaccess) ( p - rzB, rzB);
            FC_(make_mem_noaccess) ( p + sizeB, rzB);
         }
         return True;
      }
      case VG_USERREQ__RESIZEINPLACE_BLOCK: {
         ...
         FC_(handle_resizeInPlace) ( tid, p, oldSizeB, newSizeB, rzB );
         return True;
      }
      case VG_USERREQ__FREELIKE_BLOCK: {
         ...
         FC_(handle_free) ( tid, p, rzB, FC_AllocCustom );
         return True;
      }

      case _VG_USERREQ__FLOWCHECK_RECORD_OVERLAP_ERROR: {
         HChar* s  = (HChar*)arg[1];
         Addr  dst = (Addr) arg[2];
         Addr  src = (Addr) arg[3];
         SizeT len = (SizeT)arg[4];
         FC_(record_overlap_error)(tid, s, src, dst, len);
         return True;
      }
      ...
      case VG_USERREQ__CREATE_MEMPOOL: {
         ...
         FC_(create_mempool) ( pool, rzB, is_zeroed );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_ALLOC: {
         ...
         FC_(mempool_alloc) ( tid, pool, addr, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_TRIM: {
         ...
         FC_(mempool_trim) ( pool, addr, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_CHANGE: {
         ...
         FC_(mempool_change) ( pool, addrA, addrB, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_EXISTS: {
         ...
         *ret = (UWord) FC_(mempool_exists) ( pool );
	 return True;
      }
	 ...
	 *ret = *((unsigned int *)arg[1]);
      ...
      case VG_USERREQ__NOTE_ITERATION:
	 {
      ...
      default:
         VG_(message)(
            Vg_UserMsg, 
            "Warning: unknown flowcheck client request code %llx\n",
            (ULong)arg[0]
         );
         return False;
   }
...
// if FC_PROFILE_MEMORY is defined.

#ifdef FC_PROFILE_MEMORY

ULong  FC_(event_ctr)[MCPE_LAST];

/* Event counter names. Use the name of the function that increases the
   event counter. Drop any FC_() and fc_ prefices. */
static const HChar* FC_(event_ctr_name)[MCPE_LAST] = {
...
static void done_prof_mem ( void )
{
   ...
   for (i = n = 0; i < MCPE_LAST; i++) {
      ...
      if (FC_(event_ctr)[i] > 0) {
         spaced = False;
         ++n;
         VG_(printf)( "prof mem event %3d: %11llu   %s\n", 
                      i, FC_(event_ctr)[i],
                      FC_(event_ctr_name)[i]);
      }
   }
}
...
UWord VG_REGPARM(1) FC_(helperc_b_load1)( Addr a ) {
   OCacheLine* line;
   UChar descr;
   UWord lineoff = oc_line_offset(a);
...
UWord VG_REGPARM(1) FC_(helperc_b_load2)( Addr a ) {
   OCacheLine* line;
   UChar descr;
   ...
   if (UNLIKELY(a & 1)) {
      ...
      UInt oLo   = (UInt)FC_(helperc_b_load1)( a + 0 );
      UInt oHi   = (UInt)FC_(helperc_b_load1)( a + 1 );
...
UWord VG_REGPARM(1) FC_(helperc_b_load4)( Addr a ) {
   OCacheLine* line;
   UChar descr;
   UWord lineoff;
   ...
   if (UNLIKELY(a & 3)) {
      /* Handle misaligned case, slowly. */
      UInt oLo   = (UInt)FC_(helperc_b_load2)( a + 0 );
      UInt oHi   = (UInt)FC_(helperc_b_load2)( a + 2 );
      return merge_origins(oLo, oHi);
   }
...
UWord VG_REGPARM(1) FC_(helperc_b_load8)( Addr a ) {
   OCacheLine* line;
   ...
   if (UNLIKELY(a & 7)) {
      /* Handle misaligned case, slowly. */
      UInt oLo   = (UInt)FC_(helperc_b_load4)( a + 0 );
      UInt oHi   = (UInt)FC_(helperc_b_load4)( a + 4 );
      return merge_origins(oLo, oHi);
   }
...
UWord VG_REGPARM(1) FC_(helperc_b_load16)( Addr a ) {
   UInt oLo   = (UInt)FC_(helperc_b_load8)( a + 0 );
   UInt oHi   = (UInt)FC_(helperc_b_load8)( a + 8 );
   UInt oBoth = merge_origins(oLo, oHi);
   return (UWord)oBoth;
}

UWord VG_REGPARM(1) FC_(helperc_b_load32)( Addr a ) {
   UInt oQ0   = (UInt)FC_(helperc_b_load8)( a + 0 );
   UInt oQ1   = (UInt)FC_(helperc_b_load8)( a + 8 );
   UInt oQ2   = (UInt)FC_(helperc_b_load8)( a + 16 );
   UInt oQ3   = (UInt)FC_(helperc_b_load8)( a + 24 );
...
void VG_REGPARM(2) FC_(helperc_b_store1)( Addr a, UWord d32 ) {
   OCacheLine* line;
   UWord lineoff = oc_line_offset(a);
   UWord byteoff = a & 3; /* 0, 1, 2 or 3 */
...
void VG_REGPARM(2) FC_(helperc_b_store2)( Addr a, UWord d32 ) {
   OCacheLine* line;
   UWord lineoff, byteoff;

   if (UNLIKELY(a & 1)) {
      /* Handle misaligned case, slowly. */
      FC_(helperc_b_store1)( a + 0, d32 );
      FC_(helperc_b_store1)( a + 1, d32 );
      return;
   }
...
void VG_REGPARM(2) FC_(helperc_b_store4)( Addr a, UWord d32 ) {
   OCacheLine* line;
   UWord lineoff;

   if (UNLIKELY(a & 3)) {
      /* Handle misaligned case, slowly. */
      FC_(helperc_b_store2)( a + 0, d32 );
      FC_(helperc_b_store2)( a + 2, d32 );
      return;
   }
...
void VG_REGPARM(2) FC_(helperc_b_store8)( Addr a, UWord d32 ) {
   OCacheLine* line;
   UWord lineoff;

   if (UNLIKELY(a & 7)) {
      /* Handle misaligned case, slowly. */
      FC_(helperc_b_store4)( a + 0, d32 );
      FC_(helperc_b_store4)( a + 4, d32 );
      return;
   }
...
void VG_REGPARM(2) FC_(helperc_b_store16)( Addr a, UWord d32 ) {
   FC_(helperc_b_store8)( a + 0, d32 );
   FC_(helperc_b_store8)( a + 8, d32 );
}

void VG_REGPARM(2) FC_(helperc_b_store32)( Addr a, UWord d32 ) {
   FC_(helperc_b_store8)( a +  0, d32 );
   FC_(helperc_b_store8)( a +  8, d32 );
   FC_(helperc_b_store8)( a + 16, d32 );
   FC_(helperc_b_store8)( a + 24, d32 );
}
...
static void ocache_sarp_Set_Origins ( Addr a, UWord len, UInt otag ) {
   if ((a & 1) && len >= 1) {
      FC_(helperc_b_store1)( a, otag );
      a++;
      len--;
   }
   if ((a & 2) && len >= 2) {
      FC_(helperc_b_store2)( a, otag );
      a += 2;
      len -= 2;
   }
   if (len >= 4) 
...
static void ocache_sarp_Clear_Origins ( Addr a, UWord len ) {
   if ((a & 1) && len >= 1) {
      FC_(helperc_b_store1)( a, 0 );
      a++;
      len--;
   }
   if ((a & 2) && len >= 2) {
      FC_(helperc_b_store2)( a, 0 );
      a += 2;
      len -= 2;
   }
   if (len >= 4) 
...
static void fc_post_clo_init ( void )
{
...
#     ifdef PERF_FAST_STACK
      VG_(track_new_mem_stack_4_w_ECU)   ( fc_new_mem_stack_4_w_ECU   );
      VG_(track_new_mem_stack_8_w_ECU)   ( fc_new_mem_stack_8_w_ECU   );
      VG_(track_new_mem_stack_12_w_ECU)  ( fc_new_mem_stack_12_w_ECU  );
      VG_(track_new_mem_stack_16_w_ECU)  ( fc_new_mem_stack_16_w_ECU  );
      VG_(track_new_mem_stack_32_w_ECU)  ( fc_new_mem_stack_32_w_ECU  );
      VG_(track_new_mem_stack_112_w_ECU) ( fc_new_mem_stack_112_w_ECU );
      VG_(track_new_mem_stack_128_w_ECU) ( fc_new_mem_stack_128_w_ECU );
      VG_(track_new_mem_stack_144_w_ECU) ( fc_new_mem_stack_144_w_ECU );
...
#     ifdef PERF_FAST_STACK
      VG_(track_new_mem_stack_4)   ( fc_new_mem_stack_4   );
      VG_(track_new_mem_stack_8)   ( fc_new_mem_stack_8   );
      VG_(track_new_mem_stack_12)  ( fc_new_mem_stack_12  );
      VG_(track_new_mem_stack_16)  ( fc_new_mem_stack_16  );
      VG_(track_new_mem_stack_32)  ( fc_new_mem_stack_32  );
      VG_(track_new_mem_stack_112) ( fc_new_mem_stack_112 );
      VG_(track_new_mem_stack_128) ( fc_new_mem_stack_128 );
      VG_(track_new_mem_stack_144) ( fc_new_mem_stack_144 );
...
#  endif
   ...
   FC_(chunk_poolalloc) = VG_(newPA)
      (sizeof(FC_Chunk) + FC_(n_where_pointers)() * sizeof(ExeContext*),
       1000,
       VG_(malloc),
       "mc.cMC.1 (FC_Chunk pools)",
       VG_(free));
...
static void fc_print_stats (void)
{
   SizeT max_secVBit_szB, max_SMs_szB, max_shmem_szB;

   VG_(message)(Vg_DebugMsg, " flowcheck: freelist: vol %lld length %lld\n",
                VG_(free_queue_volume), VG_(free_queue_length));
   VG_(message)(Vg_DebugMsg,
      " flowcheck: sanity checks: %d cheap, %d expensive\n",
      n_sanity_cheap, n_sanity_expensive );
   VG_(message)(Vg_DebugMsg,
      " flowcheck: auxmaps: %llu auxmap entries (%lluk, %lluM) in use\n",
      n_auxmap_L2_nodes, 
      n_auxmap_L2_nodes * 64, 
      n_auxmap_L2_nodes / 16 );
   VG_(message)(Vg_DebugMsg,
      " flowcheck: auxmaps_L1: %llu searches, %llu cmps, ratio %llu:10\n",
      n_auxmap_L1_searches, n_auxmap_L1_cmps,
      (10ULL * n_auxmap_L1_cmps) 
         / (n_auxmap_L1_searches ? n_auxmap_L1_searches : 1) 
   );   
   VG_(message)(Vg_DebugMsg,
      " flowcheck: auxmaps_L2: %llu searches, %llu nodes\n",
   ...
   VG_(message)(Vg_DebugMsg,
      " flowcheck: max sec V bit nodes:    %d (%luk, %luM)\n",
      max_secVBit_nodes, max_secVBit_szB / 1024,
                         max_secVBit_szB / (1024 * 1024));
   VG_(message)(Vg_DebugMsg,
      " flowcheck: set_sec_vbits8 calls: %llu (new: %llu, updates: %llu)\n",
      sec_vbits_new_nodes + sec_vbits_updates,
      sec_vbits_new_nodes, sec_vbits_updates );
   VG_(message)(Vg_DebugMsg,
      " flowcheck: max shadow mem size:   %luk, %luM\n",
...
static void fc_fini ( Int exitcode )
{
   FC_(print_malloc_stats)();

   if (FC_(clo_leak_check) != LC_Off) {
      LeakCheckParams lcp;
      lcp.mode = FC_(clo_leak_check);
      lcp.show_leak_kinds = FC_(clo_show_leak_kinds);
      lcp.heuristics = FC_(clo_leak_check_heuristics);
      lcp.errors_for_leak_kinds = FC_(clo_error_for_leak_kinds);
      lcp.deltamode = LCD_Any;
      lcp.max_loss_records_output = 999999999;
      lcp.requested_by_monitor_command = False;
      FC_(detect_memory_leaks)(1/*bogus ThreadId*/, &lcp);
   } else {
   ...
   if (FC_(any_value_errors) && !VG_(clo_xml) && VG_(clo_verbosity) >= 1
       && FC_(clo_fc_level) == 2) {
      VG_(message)(Vg_UserMsg,
...
static Bool fc_mark_unaddressable_for_watchpoint (PointKind kind, Bool insert,
                                                  Addr addr, SizeT len)
{
   ...
   if (insert)
      FC_(make_mem_noaccess) (addr, len);
   else
      FC_(make_mem_defined)  (addr, len);
   return True;
}
...
static void fc_pre_clo_init(void)
{
   ...
   VG_(needs_final_IR_tidy_pass)  ( FC_(final_tidy) );


   VG_(needs_core_errors)         ();
   VG_(needs_tool_errors)         (FC_(eq_Error),
                                   FC_(before_pp_Error),
                                   FC_(pp_Error),
                                   True,/*show TIDs for errors*/
                                   FC_(update_Error_extra),
                                   FC_(is_recognised_suppression),
                                   FC_(read_extra_suppression_info),
                                   FC_(error_matches_suppression),
                                   FC_(get_error_name),
                                   FC_(get_extra_suppression_info),
                                   FC_(print_extra_suppression_use),
                                   FC_(update_extra_suppression_use));
   VG_(needs_libc_freeres)        ();
   VG_(needs_cxx_freeres)         ();
   VG_(needs_command_line_options)(fc_process_cmd_line_options,
                                   fc_print_usage,
                                   fc_print_debug_usage);
   VG_(needs_client_requests)     (fc_handle_client_request);
   VG_(needs_sanity_checks)       (fc_cheap_sanity_check,
                                   fc_expensive_sanity_check);
   VG_(needs_print_stats)         (fc_print_stats);
   VG_(needs_info_location)       (FC_(pp_describe_addr));
   VG_(needs_malloc_replacement)  (FC_(malloc),
                                   FC_(__builtin_new),
                                   FC_(__builtin_vec_new),
                                   FC_(memalign),
                                   FC_(calloc),
                                   FC_(free),
                                   FC_(__builtin_delete),
                                   FC_(__builtin_vec_delete),
                                   FC_(realloc),
                                   FC_(malloc_usable_size), 
                                   FC_MALLOC_DEFAULT_REDZONE_SZB );
   FC_(Malloc_Redzone_SzB) = VG_(malloc_effective_client_redzone_size)();

   VG_(needs_xml_output)          ();

   VG_(track_new_mem_startup)     ( fc_new_mem_startup );

   // Handling of mmap and mprotect isn't simple (well, it is simple,
   // but the justification isn't.)  See comments above, just prior to
   // fc_new_mem_mmap.
   VG_(track_new_mem_mmap)        ( fc_new_mem_mmap );
   VG_(track_change_mem_mprotect) ( fc_new_mem_mprotect );
   
   VG_(track_copy_mem_remap)      ( FC_(copy_address_range_state) );

   VG_(track_die_mem_stack_signal)( FC_(make_mem_noaccess) ); 
   VG_(track_die_mem_brk)         ( FC_(make_mem_noaccess) );
   VG_(track_die_mem_munmap)      ( FC_(make_mem_noaccess) ); 
...
#  ifdef PERF_FAST_STACK
   VG_(track_die_mem_stack_4)     ( fc_die_mem_stack_4   );
   VG_(track_die_mem_stack_8)     ( fc_die_mem_stack_8   );
   VG_(track_die_mem_stack_12)    ( fc_die_mem_stack_12  );
   VG_(track_die_mem_stack_16)    ( fc_die_mem_stack_16  );
   VG_(track_die_mem_stack_32)    ( fc_die_mem_stack_32  );
   VG_(track_die_mem_stack_112)   ( fc_die_mem_stack_112 );
   VG_(track_die_mem_stack_128)   ( fc_die_mem_stack_128 );
   VG_(track_die_mem_stack_144)   ( fc_die_mem_stack_144 );
   VG_(track_die_mem_stack_160)   ( fc_die_mem_stack_160 );
#  endif
   VG_(track_die_mem_stack)       ( fc_die_mem_stack     );
   ...
   if (FC_(clo_fc_level) >= 2) {
      VG_(track_copy_mem_to_reg)  ( fc_copy_mem_to_reg );
      VG_(track_copy_reg_to_mem)  ( fc_copy_reg_to_mem );
   }
   ...
   tl_assert(FC_(chunk_poolalloc) == NULL);
   FC_(malloc_list)  = VG_(HT_construct)( "FC_(malloc_list)" );
   FC_(mempool_list) = VG_(HT_construct)( "FC_(mempool_list)" );
   init_prof_mem();

   tl_assert( fc_expensive_sanity_check() );
   ...
   /* We can't initialise ocacheL1/ocacheL2 yet, since we don't know
      ...
      processed yet.  Hence defer it to fc_post_clo_init. */
   tl_assert(ocacheL1 == NULL);
   tl_assert(ocacheL2 == NULL);
...
VG_DETERMINE_INTERFACE_VERSION(fc_pre_clo_init)
