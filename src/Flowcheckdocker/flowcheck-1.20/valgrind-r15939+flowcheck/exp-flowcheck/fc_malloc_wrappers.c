...
#include "pub_tool_tooliface.h"     // Needed for fc_include.h
...
SizeT FC_(Malloc_Redzone_SzB) = -10000000; // If used before set, should BOMB

/* Record malloc'd blocks. */
VgHashTable *FC_(malloc_list) = NULL;

/* Memory pools: a hash table of FC_Mempools.  Search key is
   FC_Mempool::pool. */
VgHashTable *FC_(mempool_list) = NULL;

/* Pool allocator for FC_Chunk. */   
PoolAlloc *FC_(chunk_poolalloc) = NULL;
static
FC_Chunk* create_FC_Chunk ( ThreadId tid, Addr p, SizeT szB,
                            FC_AllocKind kind);
static inline
void delete_FC_Chunk (FC_Chunk* mc);
...
/* Blocks freed by the client are queued in one of two lists of
   ...
   The blocks with a size >= FC_(clo_freelist_big_blocks)
...
static FC_Chunk* freed_list_start[2]  = {NULL, NULL};
static FC_Chunk* freed_list_end[2]    = {NULL, NULL};

/* Put a shadow chunk on the freed blocks queue, possibly freeing up
   some of the oldest blocks in the queue at the same time. */
static void add_to_freed_queue ( FC_Chunk* mc )
{
   const Bool show = False;
   const int l = (mc->szB >= FC_(clo_freelist_big_blocks) ? 0 : 1);
   ...
   if (freed_list_end[l] == NULL) {
      ...
      if (mc->szB >= FC_(clo_freelist_vol)) {
   ...
   if (show)
      VG_(printf)("fc_freelist: acquire: volume now %lld\n", 
...
/* Release enough of the oldest blocks to bring the free queue
   ...
   On entry, VG_(free_queue_volume) must be > FC_(clo_freelist_vol).
   On exit, VG_(free_queue_volume) will be <= FC_(clo_freelist_vol). */
static void release_oldest_block(void)
{
   const Bool show = False;
   int i;
   tl_assert (VG_(free_queue_volume) > FC_(clo_freelist_vol));
   tl_assert (freed_list_start[0] != NULL || freed_list_start[1] != NULL);

   for (i = 0; i < 2; i++) {
      while (VG_(free_queue_volume) > FC_(clo_freelist_vol)
         ...
         VG_(free_queue_length)--;
         if (show)
            VG_(printf)("fc_freelist: discard: volume now %lld\n", 
                        VG_(free_queue_volume));
         ...
         /* free FC_Chunk */
         if (FC_AllocCustom != mc1->allockind)
...
FC_Chunk* create_FC_Chunk ( ThreadId tid, Addr p, SizeT szB,
                            FC_AllocKind kind)
{
   FC_Chunk* mc  = VG_(allocEltPA)(FC_(chunk_poolalloc));
   mc->data      = p;
   mc->szB       = szB;
   mc->allockind = kind;
   switch ( FC_(n_where_pointers)() ) {
      case 2: mc->where[1] = 0; // fallback to 1
      case 1: mc->where[0] = 0; // fallback to 0
      case 0: break;
      default: tl_assert(0);
   }
   FC_(set_allocated_at) (tid, mc);

   /* Each time a new FC_Chunk is created, release oldest blocks
      if the free list volume is exceeded. */
   if (VG_(free_queue_volume) > FC_(clo_freelist_vol))
      release_oldest_block();

   /* Paranoia ... ensure the FC_Chunk is off-limits to the client, so
   ...
   if (!FC_(check_mem_is_noaccess)( (Addr)mc, sizeof(FC_Chunk), NULL )) {
      VG_(tool_panic)("create_FC_Chunk: shadow area is accessible");
   } 
   return mc;
}
...
void delete_FC_Chunk (FC_Chunk* mc)
{
   VG_(freeEltPA) (FC_(chunk_poolalloc), mc);
}

// True if mc is in the given block list.
static Bool in_block_list (const VgHashTable *block_list, FC_Chunk* mc)
{
   FC_Chunk* found_mc = VG_(HT_lookup) ( block_list, (UWord)mc->data );
   if (found_mc) {
      ...
      /* If a user builds a pool from a malloc-ed superblock
         ...
         twice in FC_(malloc_list) is a recipe for bugs.
...
static Bool live_block (FC_Chunk* mc)
{
   if (mc->allockind == FC_AllocCustom) {
      FC_Mempool* mp;
      VG_(HT_ResetIter)(FC_(mempool_list));
      while ( (mp = VG_(HT_Next)(FC_(mempool_list))) ) {
         if ( in_block_list (mp->chunks, mc) )
            return True;
      }
   }
   /* Note: we fallback here for a not found FC_AllocCustom
      as such a block can be inserted in FC_(malloc_list)
      by VALGRIND_MALLOCLIKE_BLOCK. */
   return in_block_list ( FC_(malloc_list), mc );
}

ExeContext* FC_(allocated_at) (FC_Chunk* mc)
{
   switch (FC_(clo_keep_stacktraces)) {
...
ExeContext* FC_(freed_at) (FC_Chunk* mc)
{
   switch (FC_(clo_keep_stacktraces)) {
      case KS_none:            return VG_(null_ExeContext) ();
...
void  FC_(set_allocated_at) (ThreadId tid, FC_Chunk* mc)
{
   switch (FC_(clo_keep_stacktraces)) {
      case KS_none:            return;
      case KS_alloc:           break;
...
void  FC_(set_freed_at) (ThreadId tid, FC_Chunk* mc)
{
   UInt pos;
   switch (FC_(clo_keep_stacktraces)) {
      case KS_none:            return;
...
void* FC_(new_block) ( ThreadId tid,
                       ...
                       Bool is_zeroed, FC_AllocKind kind, VgHashTable *table)
{
   FC_Chunk* mc;

   // Allocate and zero if necessary
   if (p) {
      tl_assert(FC_AllocCustom == kind);
   } else {
      tl_assert(FC_AllocCustom != kind);
      ...
      if (FC_(clo_malloc_fill) != -1) {
         tl_assert(FC_(clo_malloc_fill) >= 0x00 && FC_(clo_malloc_fill) <= 0xFF);
         VG_(memset)((void*)p, FC_(clo_malloc_fill), szB);
      }
   }
   ...
   mc = create_FC_Chunk (tid, p, szB, kind);
...
{
   if (FC_(record_fishy_value_error)(tid, "malloc", "size", n)) {
      return NULL;
   } else {
      return FC_(new_block) ( tid, 0, n, VG_(clo_alignment), 
         /*is_zeroed*/False, FC_AllocMalloc, FC_(malloc_list));
   }
}

void* FC_(__builtin_new) ( ThreadId tid, SizeT n )
{
   if (FC_(record_fishy_value_error)(tid, "__builtin_new", "size", n)) {
      return NULL;
   } else {
      return FC_(new_block) ( tid, 0, n, VG_(clo_alignment), 
         /*is_zeroed*/False, FC_AllocNew, FC_(malloc_list));
   }
}

void* FC_(__builtin_vec_new) ( ThreadId tid, SizeT n )
{
   if (FC_(record_fishy_value_error)(tid, "__builtin_vec_new", "size", n)) {
      return NULL;
   } else {
      return FC_(new_block) ( tid, 0, n, VG_(clo_alignment), 
         /*is_zeroed*/False, FC_AllocNewVec, FC_(malloc_list));
   }
}

void* FC_(memalign) ( ThreadId tid, SizeT alignB, SizeT n )
{
   if (FC_(record_fishy_value_error)(tid, "memalign", "size", n)) {
      return NULL;
   } else {
      return FC_(new_block) ( tid, 0, n, alignB, 
         /*is_zeroed*/False, FC_AllocMalloc, FC_(malloc_list));
   }
}
...
{
   if (FC_(record_fishy_value_error)(tid, "calloc", "nmemb", nmemb) ||
       FC_(record_fishy_value_error)(tid, "calloc", "size", size1)) {
      return NULL;
   } else {
      return FC_(new_block) ( tid, 0, nmemb*size1, VG_(clo_alignment),
         /*is_zeroed*/True, FC_AllocMalloc, FC_(malloc_list));
   }
}

static
void die_and_free_mem ( ThreadId tid, FC_Chunk* mc, SizeT rzB )
{
   /* Note: we do not free fill the custom allocs produced
      by MEMPOOL or by MALLOC/FREELIKE_BLOCK requests. */
   if (FC_(clo_free_fill) != -1 && FC_AllocCustom != mc->allockind ) {
      tl_assert(FC_(clo_free_fill) >= 0x00 && FC_(clo_free_fill) <= 0xFF);
      VG_(memset)((void*)mc->data, FC_(clo_free_fill), mc->szB);
   }

   /* Note: make redzones noaccess again -- just in case user made them
      accessible with a client request... */
   FC_(make_mem_noaccess)( mc->data-rzB, mc->szB + 2*rzB );

   /* Record where freed */
   FC_(set_freed_at) (tid, mc);
   /* Put it out of harm's way for a while */
   add_to_freed_queue ( mc );
   /* If the free list volume is bigger than FC_(clo_freelist_vol),
...
static
void record_freemismatch_error (ThreadId tid, FC_Chunk* mc)
{
   /* Only show such an error if the user hasn't disabled doing so. */
   if (!FC_(clo_show_mismatched_frees))
      return;

   /* FC_(record_freemismatch_error) reports errors for still
   ...
   VG_(HT_add_node)( FC_(malloc_list), mc );
   FC_(record_freemismatch_error) ( tid, mc );
   if ((mc != VG_(HT_remove) ( FC_(malloc_list), (UWord)mc->data )))
      tl_assert(0);
}

void FC_(handle_free) ( ThreadId tid, Addr p, UInt rzB, FC_AllocKind kind )
{
   FC_Chunk* mc;

   cmalloc_n_frees++;

   mc = VG_(HT_remove) ( FC_(malloc_list), (UWord)p );
   if (mc == NULL) {
      FC_(record_free_error) ( tid, p );
   } else {
...
{
   FC_(handle_free)( 
      tid, (Addr)p, FC_(Malloc_Redzone_SzB), FC_AllocMalloc );
}

void FC_(__builtin_delete) ( ThreadId tid, void* p )
{
   FC_(handle_free)(
      tid, (Addr)p, FC_(Malloc_Redzone_SzB), FC_AllocNew);
}

void FC_(__builtin_vec_delete) ( ThreadId tid, void* p )
{
   FC_(handle_free)(
      tid, (Addr)p, FC_(Malloc_Redzone_SzB), FC_AllocNewVec);
}

void* FC_(realloc) ( ThreadId tid, void* p_old, SizeT new_szB )
{
   FC_Chunk* old_mc;
   FC_Chunk* new_mc;
   Addr      a_new; 
   ...
   if (FC_(record_fishy_value_error)(tid, "realloc", "size", new_szB))
      return NULL;
   ...
   old_mc = VG_(HT_remove) ( FC_(malloc_list), (UWord)p_old );
   if (old_mc == NULL) {
      FC_(record_free_error) ( tid, (Addr)p_old );
      /* We return to the program regardless. */
      return NULL;
   }

   /* check if its a matching free() / delete / delete [] */
   if (FC_AllocMalloc != old_mc->allockind) {
   ...
   if (a_new) {
      ...
      new_mc = create_FC_Chunk( tid, a_new, new_szB, FC_AllocMalloc );

      // Now insert the new mc (with a new 'data' field) into malloc_list.
      VG_(HT_add_node)( FC_(malloc_list), new_mc );

      /* Retained part is copied, red zones set as normal */

      /* Redzone at the front */
      FC_(make_mem_noaccess)( a_new-FC_(Malloc_Redzone_SzB), 
                              FC_(Malloc_Redzone_SzB) );
      ...
      if (old_szB >= new_szB) {
         ...
         FC_(copy_address_range_state) ( (Addr)p_old, a_new, new_szB );
         ...
         if (FC_(clo_malloc_fill) != -1) {
            tl_assert(FC_(clo_malloc_fill) >= 0x00
                      && FC_(clo_malloc_fill) <= 0xFF);
            VG_(memset)((void*)(a_new+old_szB), FC_(clo_malloc_fill), 
                                                new_szB-old_szB);
         }
      }

      /* Redzone at the back. */
      FC_(make_mem_noaccess)        ( a_new+new_szB, FC_(Malloc_Redzone_SzB));

      /* Possibly fill freed area with specified junk. */
      if (FC_(clo_free_fill) != -1) {
         tl_assert(FC_(clo_free_fill) >= 0x00 && FC_(clo_free_fill) <= 0xFF);
         VG_(memset)((void*)p_old, FC_(clo_free_fill), old_szB);
      }

      /* Free old memory */
      /* Nb: we have to allocate a new FC_Chunk for the new memory rather
         than recycling the old one, so that any erroneous accesses to the
         old memory are reported. */
      die_and_free_mem ( tid, old_mc, FC_(Malloc_Redzone_SzB) );
      ...
      VG_(HT_add_node)( FC_(malloc_list), old_mc );
   }
...
{
   FC_Chunk* mc = VG_(HT_lookup) ( FC_(malloc_list), (UWord)p );
                               ...
                               SizeT oldSizeB, SizeT newSizeB, SizeT rzB)
{
   ...
   if (newSizeB < oldSizeB) {
      FC_(make_mem_noaccess)( p + newSizeB, oldSizeB - newSizeB + rzB );
   } else {
      ExeContext* ec  = VG_(record_ExeContext)(tid, 0/*first_ip_delta*/);
      UInt        ecu = VG_(get_ECU_from_ExeContext)(ec);
      FC_(make_mem_undefined_w_otag)( p + oldSizeB, newSizeB - oldSizeB,
                                      ecu | FC_OKIND_HEAP );
      if (rzB > 0)
         FC_(make_mem_noaccess)( p + newSizeB, rzB );
   }
}
...
static void check_mempool_sane(FC_Mempool* mp); /*forward*/
...
void FC_(create_mempool)(Addr pool, UInt rzB, Bool is_zeroed)
{
   ...
   mp = VG_(HT_lookup)(FC_(mempool_list), (UWord)pool);
   if (mp != NULL) {
     VG_(tool_panic)("FC_(create_mempool): duplicate pool creation");
   }
   
   mp = VG_(malloc)("mc.cm.1", sizeof(FC_Mempool));
   mp->pool       = pool;
   mp->rzB        = rzB;
   mp->is_zeroed  = is_zeroed;
   mp->chunks     = VG_(HT_construct)( "FC_(create_mempool)" );
   ...
   /* Paranoia ... ensure this area is off-limits to the client, so
      ...
      concerned. */
   if (!FC_(check_mem_is_noaccess)( (Addr)mp, sizeof(FC_Mempool), NULL )) {
      VG_(tool_panic)("FC_(create_mempool): shadow area is accessible");
   } 

   VG_(HT_add_node)( FC_(mempool_list), mp );
}
...
void FC_(destroy_mempool)(Addr pool)
{
   ...
   mp = VG_(HT_remove) ( FC_(mempool_list), (UWord)pool );

   if (mp == NULL) {
      ThreadId tid = VG_(get_running_tid)();
      FC_(record_illegal_mempool_error) ( tid, pool );
      return;
   }
   check_mempool_sane(mp);
   ...
   while ( (mc = VG_(HT_Next)(mp->chunks)) ) {
      /* Note: make redzones noaccess again -- just in case user made them
         accessible with a client request... */
      FC_(make_mem_noaccess)(mc->data-mp->rzB, mc->szB + 2*mp->rzB );
   }
   // Destroy the chunk table
   VG_(HT_destruct)(mp->chunks, (void (*)(void *))delete_FC_Chunk);
...
{
   const FC_Chunk* mc1 = *(const FC_Chunk *const *)n1;
   const FC_Chunk* mc2 = *(const FC_Chunk *const *)n2;
...
static void 
check_mempool_sane(FC_Mempool* mp)
{
   UInt n_chunks, i, bad = 0;   
   static UInt tick = 0;

   FC_Chunk **chunks = (FC_Chunk**) VG_(HT_to_array)( mp->chunks, &n_chunks );
   if (!chunks)
      return;
   ...
   if (VG_(clo_verbosity) > 1) {
	 ...
	 VG_(HT_ResetIter)(FC_(mempool_list));
	 while ( (mp2 = VG_(HT_Next)(FC_(mempool_list))) ) {
	   total_pools++;
   ...
   if (bad) {
         ...
         for (i = 0; i < n_chunks; ++i) {
            ...
            VG_(pp_ExeContext)(FC_(allocated_at)(chunks[i]));
         }
   }
   VG_(free)(chunks);
}

void FC_(mempool_alloc)(ThreadId tid, Addr pool, Addr addr, SizeT szB)
{
   FC_Mempool* mp;
   ...
   if (mp == NULL) {
      ...
      FC_(new_block)(tid, addr, szB, /*ignored*/0, mp->is_zeroed,
                     FC_AllocCustom, mp->chunks);
      if (mp->rzB > 0) {
         ...
         FC_(make_mem_noaccess) ( addr - mp->rzB, mp->rzB);
         FC_(make_mem_noaccess) ( addr + szB, mp->rzB);
      }
      if (MP_DETAILED_SANITY_CHECKS) check_mempool_sane(mp);
   }
}
...
void FC_(mempool_free)(Addr pool, Addr addr)
{
   ...
   if (mc == NULL) {
      FC_(record_free_error)(tid, (Addr)addr);
      return;
   }
...
void FC_(mempool_trim)(Addr pool, Addr addr, SizeT szB)
{
   FC_Mempool*  mp;
   FC_Chunk*    mc;
   ThreadId     tid = VG_(get_running_tid)();
...
#define EXTENT_CONTAINS(x) ((addr <= (x)) && ((x) < addr + szB))
      ...
      if (EXTENT_CONTAINS(lo) && EXTENT_CONTAINS(hi)) {
         ...
         if (VG_(HT_remove)(mp->chunks, (UWord)mc->data) == NULL) {
            FC_(record_free_error)(tid, (Addr)mc->data);
            VG_(free)(chunks);
            if (MP_DETAILED_SANITY_CHECKS) check_mempool_sane(mp);
            return;
         }
         ...
         if (min < lo && !EXTENT_CONTAINS(min)) {
           FC_(make_mem_noaccess)( min, lo - min);
         }

         if (hi < max && !EXTENT_CONTAINS(max)) {
           FC_(make_mem_noaccess)( hi, max - hi );
         }
...
void FC_(move_mempool)(Addr poolA, Addr poolB)
{
   ...
   mp = VG_(HT_remove) ( FC_(mempool_list), (UWord)poolA );

   if (mp == NULL) {
      ThreadId tid = VG_(get_running_tid)();
      FC_(record_illegal_mempool_error) ( tid, poolA );
      return;
   }
...
void FC_(mempool_change)(Addr pool, Addr addrA, Addr addrB, SizeT szB)
{
   FC_Mempool*  mp;
   FC_Chunk*    mc;
   ThreadId     tid = VG_(get_running_tid)();
   ...
   if (mc == NULL) {
      FC_(record_free_error)(tid, (Addr)addrA);
      return;
   }
...
void FC_(print_malloc_stats) ( void )
{
   ...
   while ( (mc = VG_(HT_Next)(FC_(malloc_list))) ) {
      nblocks++;
      nbytes += (ULong)mc->szB;
   }
