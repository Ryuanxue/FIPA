      ...
      if (CallInst *ci = dyn_cast<CallInst>(&*inst_i))
      {
        ...
        if (called_func != nullptr)
        {
          ...
          if (callee_node != nullptr)
          {
            ...
            //======================================================= add by xue
            /**
           * 获得ci的绝对路径，行号，所在函数信息，构造key=filepath +" + " +funname+" + "+linenumber
           * 获得called_fuunc的函数名calleefunname
           * 添加file_line_funmap[key]=calleefunname
           */

          // 获得ci的绝对路径，行号，所在函数信息
          const DebugLoc &debugInfo = ci->getDebugLoc();
          std::string filepath = "";
          ...
          std::string key = fullPath + " + " + funname + " + " + std::to_string(lineNumber);
          std::string calleefunname = called_func->getName().str();

          // 添加file_line_funmap[key]=calleefunname
          file_line_funMap[key] = calleefunname;
          //===================================================================
        ...
        else
        {
          ...
          for (auto ind_call_can : ind_call_candidates)
          {
            ...
            if (callee_node != nullptr)
            {
              ...
              /**
               * 获得ci的绝对路径，行号，所在函数信息，构造key=filepath +" + " +funname+" + "+linenumber
               * 获得ind_call_can的函数名calleefunname
               * 添加file_line_funmap[key]=calleefunname
               */
              ...
              std::string fullPath = directory + "/" + filepath;
              // 构造key=filepath +" + " + funname + " + " + linenumber
              std::string key = fullPath + " + " + funname + " + " + std::to_string(lineNumber);
              std::string calleefunname = ind_call_can->getName().str();
              if (calleefunname == "main")
              {
                  continue;
                  // errs() << "Indirect call found: " << key << " -> " << calleefunname << "\n";
              }
