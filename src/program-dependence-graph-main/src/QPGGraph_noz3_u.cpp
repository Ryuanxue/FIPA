    ...
    for (const auto& fileFunPair : file_line_funMap) {
        ...
        if (callSiteNode) {
            ...
            if (calleeIt != _st_node_map.end()) {
                ...
                if (lastnode==NULL)
                {
                  errs()<<"lastnode is null for function: " << calleeFuncName << "\n";
                  continue; // 跳过此边的添加
                }
...
// 迭代处理函数调用传播：如果敏感域节点调用非敏感域函数，则将整个被调用函数提升到敏感域
bool changed = true;
int iteration = 0;
while (changed && iteration < 10) { // 最多迭代10次防止无限循环
    changed = false;
    iteration++;
    
    std::cerr << "Function call propagation iteration " << iteration << std::endl;
    
    for (int i = 0; i < N; ++i) {
        if (solution[i] == 1) { // 如果节点在敏感域中
            pdg::Node* node = nodeMap[i];
            
            // 检查该节点的所有出边
            for (auto& neighbor : node->getOutEdgeSet()) {
                if (neighbor->getEdgeType() == pdg::EdgeType::CONTROLDEP_CALLINV) {
                    // 这是一个函数调用边
                    pdg::Node* entrynode = neighbor->getDstNode();
                    std::string calleeFuncName = entrynode->getStatement();
                    ...
                    if (firstnode) {
                        int j = findNodeIndex(firstnode);
                        if (j != -1 && solution[j] == 0) {
                            // 被调用的函数第一个节点在非敏感域中，需要将整个函数提升到敏感域
                            std::cerr << "Promoting function " << calleeFuncName << " to sensitive domain due to CALLINV edge" << std::endl;
                            
                            // 将该函数的所有节点都设置为敏感域
                            std::string targetFuncName = node_func_names[j];
                            for (int k = 0; k < N; ++k) {
                                if (node_func_names[k] == targetFuncName && solution[k] == 0) {
                                    solution[k] = 1;
                                    changed = true;
                                    std::cerr << "  Promoting node " << k << " (" << nodeMap[k]->getStatement() << ")" << std::endl;
                                }
                            }
                        }
                    }
                }
                else if (neighbor->getEdgeType() == pdg::EdgeType::TYPE_SSE) {
                    // 这是一个SSE边（顺序执行边）
                    pdg::Node* targetNode = neighbor->getDstNode();
                    int j = findNodeIndex(targetNode);
                    if (j != -1 && solution[j] == 0) {
                        // 目标节点在非敏感域中，需要将其提升到敏感域
                        solution[j] = 1;
                        changed = true;
                        std::cerr << "Promoting node " << j << " to sensitive domain due to SSE edge from node " << i << std::endl;
                        std::cerr << "  Promoted: " << nodeMap[j]->getStatement() << std::endl;
                    }
                }
            }
        }
...
std::cerr << "Function call propagation completed after " << iteration << " iterations" << std::endl;
...
double sz_ratio = (total_sz > 0) ? static_cast<double>(S_sz_total) / total_sz : 0.0;
...
std::cout << "解 1 (sz_ratio = " << sz_ratio << "): ";
out << "解 1 (sz_ratio = " << sz_ratio << "): ";
...
out << "  Sensitive Nodes Total sz Ratio: " << sz_ratio << std::endl << std::endl;


}
char qg::QUANProgramGraph::ID = 0;
static RegisterPass<qg::QUANProgramGraph>
    PDG("nz3qg", "Program Dependency Graph Construction", false, true);
