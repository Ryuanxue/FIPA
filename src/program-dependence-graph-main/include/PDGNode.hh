...
namespace pdg
{
  ...
  public:
    ...
    Node(std::string st,GraphNodeType node_type)//add by xue
    {
      statement = st;
      _node_type = node_type;
      _is_visited = false;
    ...
    void set_wscode(int wscode)//add by xue
    {
      w_scode=wscode;
    }

   void set_wquan(int wquan)//add by xue
   {
    w_quan=wquan;
   }
    ...
    std::string getStatement(){return statement;}//add by xue
    int get_wquan(){return w_quan;}//add by xue
    int get_wscode(){return w_scode;}//add by xue
  ...
  protected:
    ...
    EdgeSet _out_edge_set= EdgeSet(); //初始为一个空的边集合
    GraphNodeType _node_type; 
    llvm::DIType *_node_di_type;
    std::string statement;//add by xue
    int w_quan;//add by xue 信息量权重
    int w_scode; //add by xue 代码量权重
  };
