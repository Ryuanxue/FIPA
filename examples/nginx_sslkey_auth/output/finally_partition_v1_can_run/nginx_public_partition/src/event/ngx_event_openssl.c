...
extern CLIENT *sslkey_clnt;
void
rpcsslkey_1()
{
	int  *result_1;
	char_ptr verify_sslcert_1_value;

	// 初始化参数结构
	verify_sslcert_1_value.charptr.charptr_len = 0;
	verify_sslcert_1_value.charptr.charptr_val = NULL;


	result_1 = verify_sslcert_1(verify_sslcert_1_value, sslkey_clnt);
	if (result_1 == (int *) NULL) {
		clnt_perror (sslkey_clnt, "call failed");
	} else {
		printf("RPC call succeeded, result: %d\n", *result_1);
	}
#ifndef	DEBUG
	// clnt_destroy (sslkey_clnt);
#endif	 /* DEBUG */
}
...
/**这个函数是封装参数封装函数 */
static ngx_int_t 
ngx_ssl_certificate_rpc(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert,
    ngx_str_t *key, ngx_array_t *passwords)
{
    // ngx_ssl_certificate_request ngx_ssl_certificate_rpc_1_1_arg1;
    ...
    // 初始化 ngx_ssl_certificate_rpc_1 的参数
	// memset(&ngx_ssl_certificate_rpc_1_1_arg1, 0, sizeof(ngx_ssl_certificate_rpc_1_1_arg1));
	
	// // 设置证书路径
	// const char *cert_path = "../ssl/cert.pem";
	// ngx_ssl_certificate_rpc_1_1_arg1.cert_path.len = strlen(cert_path);
	// ngx_ssl_certificate_rpc_1_1_arg1.cert_path.data.charptr.charptr_len = strlen(cert_path);
	// ngx_ssl_certificate_rpc_1_1_arg1.cert_path.data.charptr.charptr_val = (char *)cert_path;
	
	// // 设置私钥路径
	// const char *key_path = "../ssl/cert.key";
	// ngx_ssl_certificate_rpc_1_1_arg1.key_path.len = strlen(key_path);
	// ngx_ssl_certificate_rpc_1_1_arg1.key_path.data.charptr.charptr_len = strlen(key_path);
	// ngx_ssl_certificate_rpc_1_1_arg1.key_path.data.charptr.charptr_val = (char *)key_path;
	
	// // 密码字段置空
	// ngx_ssl_certificate_rpc_1_1_arg1.passwords.charptr.charptr_len = 0;
	// ngx_ssl_certificate_rpc_1_1_arg1.passwords.charptr.charptr_val = NULL;
	// ngx_ssl_certificate_rpc_1_1_arg1.passwords_count = 0;
    
    // 初始化请求结构
    memset(&request, 0, sizeof(request));
    
    // 封装证书路径
    request.cert_path.len = cert->len;
    request.cert_path.data.charptr.charptr_len = cert->len;
    request.cert_path.data.charptr.charptr_val = (char *)malloc(cert->len + 1);
    if (request.cert_path.data.charptr.charptr_val == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "malloc failed for cert path");
        return NGX_ERROR;
    }
    memcpy(request.cert_path.data.charptr.charptr_val, cert->data, cert->len);
    request.cert_path.data.charptr.charptr_val[cert->len] = '\0';
    
    // 封装私钥路径
    request.key_path.len = key->len;
    request.key_path.data.charptr.charptr_len = key->len;
    request.key_path.data.charptr.charptr_val = (char *)malloc(key->len + 1);
    if (request.key_path.data.charptr.charptr_val == NULL) {
        free(request.cert_path.data.charptr.charptr_val);
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "malloc failed for key path");
        return NGX_ERROR;
    }
    memcpy(request.key_path.data.charptr.charptr_val, key->data, key->len);
    request.key_path.data.charptr.charptr_val[key->len] = '\0';
    
    // 封装密码
    if (passwords && passwords->nelts > 0) {
        ngx_str_t *pwd = passwords->elts;
        size_t total_len = 0;
        ngx_uint_t i;
        
        // 计算总长度
        for (i = 0; i < passwords->nelts; i++) {
            total_len += pwd[i].len + 1; // +1 for separator
        }
        
        request.passwords.charptr.charptr_val = (char *)malloc(total_len);
        if (request.passwords.charptr.charptr_val == NULL) {
            free(request.cert_path.data.charptr.charptr_val);
            free(request.key_path.data.charptr.charptr_val);
            return NGX_ERROR;
        }
        
        // 拷贝所有密码，用 '\0' 分隔
        char *p = request.passwords.charptr.charptr_val;
        for (i = 0; i < passwords->nelts; i++) {
            memcpy(p, pwd[i].data, pwd[i].len);
            p += pwd[i].len;
            *p++ = '\0';
        }
        request.passwords.charptr.charptr_len = total_len;
        request.passwords_count = passwords->nelts;
    } else {
    ...
    // 调用 RPC
    response = ngx_ssl_certificate_rpc_1_1(request, sslkey_clnt);

    if (response == (ngx_ssl_certificate_response *) NULL) {
		clnt_perror (sslkey_clnt, "call failed");
	} else {
		printf("ngx_ssl_certificate_rpc_1 call succeeded, status: %d\n", response->status);
		if (response->status == 0) {
			printf("Certificate loaded successfully\n");
			printf("Cert data length: %d\n", response->cert_data.charptr.charptr_len);
...
//     free(request.cert_path.data.charptr.charptr_val);
//     free(request.key_path.data.charptr.charptr_val);
//     if (request.passwords.charptr.charptr_val) {
//         free(request.passwords.charptr.charptr_val);
    // }
    ...
    ngx_array_t *keys, ngx_array_t *passwords)
{
    ...
    for (i = 0; i < certs->nelts; i++) {
        ...
        if (ngx_ssl_certificate_rpc(cf, ssl, &cert[i], &key[i], passwords)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
