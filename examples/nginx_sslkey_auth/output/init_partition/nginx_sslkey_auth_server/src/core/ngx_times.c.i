...
# 137 "src/core/ngx_times.c"
    ngx_localtime(sec, &tm);
    cached_gmtoff = (ngx_int_t) (tm.tm_gmtoff / 60);
    tp->gmtoff = cached_gmtoff;
# 150 "src/core/ngx_times.c"
    ...
    (void) ngx_sprintf(p2, "%02d/%s/%d:%02d:%02d:%02d %c%02i%02i",
                       ...
                       tm.tm_min, tm.tm_sec,
                       tp->gmtoff < 0 ? '-' : '+',
                       (((tp->gmtoff / 60) >= 0) ? (tp->gmtoff / 60) : - (tp->gmtoff / 60)), (((tp->gmtoff % 60) >= 0) ? (tp->gmtoff % 60) : - (tp->gmtoff % 60)));
