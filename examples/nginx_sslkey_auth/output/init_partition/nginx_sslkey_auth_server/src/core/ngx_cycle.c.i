...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    (void) memset(cycle->paths.elts, 0, n * sizeof(ngx_path_t *));
    ...
    ((&cycle->config_dump_sentinel)->color = 0); (&cycle->config_dump_rbtree)->root = &cycle->config_dump_sentinel; (&cycle->config_dump_rbtree)->sentinel = &cycle->config_dump_sentinel; (&cycle->config_dump_rbtree)->insert = ngx_str_rbtree_insert_value
                                                ;
    ...
    (void) memset(cycle->listening.elts, 0, n * sizeof(ngx_listening_t));


    (&cycle->reusable_connections_queue)->prev = &cycle->reusable_connections_queue; (&cycle->reusable_connections_queue)->next = &cycle->reusable_connections_queue;
    ...
    if (gethostname(hostname, 256) == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "gethostname() failed");
        ngx_destroy_pool(pool);
        return 0;
    }
    ...
    hostname[256 - 1] = '\0';
    cycle->hostname.len = strlen((const char *) hostname);

    cycle->hostname.data = ngx_pnalloc(pool, cycle->hostname.len);
    if (cycle->hostname.data == 0) {
        ngx_destroy_pool(pool);
        return 0;
    }
    ...
    for (i = 0; cycle->modules[i]; i++) {
        if (cycle->modules[i]->type != 0x45524F43) {
            continue;
        }
    ...
    (void) memset(&conf, 0, sizeof(ngx_conf_t));
    ...
    conf.temp_pool = ngx_create_pool((16 * 1024), log);
    if (conf.temp_pool == 0) {
        ngx_destroy_pool(pool);
        return 0;
    }
    ...
    conf.pool = pool;
    conf.log = log;
    conf.module_type = 0x45524F43;
    conf.cmd_type = 0x01000000;





    if (ngx_conf_param(&conf) != 0) {
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return 0;
    }

    if (ngx_conf_parse(&conf, &cycle->conf_file) != 0) {
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return 0;
    }
    ...
    if (ngx_test_config) {
        ...
        if (ngx_create_pidfile(&ccf->pid, log) != 0) {
            goto failed;
        }

    } else if (!(old_cycle->conf_ctx == 0)) {
        ...
        if (ccf->pid.len != old_ccf->pid.len
            || strcmp((const char *) ccf->pid.data, (const char *) old_ccf->pid.data) != 0)
        {
    ...
    if (ngx_test_lockfile(cycle->lock_file.data, log) != 0) {
        goto failed;
    }


    if (ngx_create_paths(cycle, ccf->user) != 0) {
        goto failed;
    }


    if (ngx_log_open_default(cycle) != 0) {
        goto failed;
    }
    ...
    for (i = 0; ; i++) {
        ...
        file[i].fd = open((const char *) file[i].name.data, (O_WRONLY|O_APPEND)|O_CREAT, 0644)
        ...
        if (file[i].fd == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "open()" " \"%s\" failed", file[i].name.data)
        ...
        if (fcntl(file[i].fd, F_SETFD, FD_CLOEXEC) == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "fcntl(FD_CLOEXEC) \"%s\" failed", file[i].name.data)
    ...
    for (i = 0; ; i++) {
        ...
        if (shm_zone[i].shm.size == 0) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, 0, "zero size shared memory zone \"%V\"", &shm_zone[i].shm.name)
        ...
        for (n = 0; ; n++) {
            ...
            if (strncmp((const char *) shm_zone[i].shm.name.data, (const char *) oshm_zone[n].shm.name.data, shm_zone[i].shm.name.len)
        ...
        if (ngx_shm_alloc(&shm_zone[i].shm) != 0) {
            goto failed;
        }

        if (ngx_init_zone_pool(cycle, &shm_zone[i]) != 0) {
            goto failed;
        }

        if (shm_zone[i].init(&shm_zone[i], 0) != 0) {
            goto failed;
        }
...
# 588 "src/core/ngx_cycle.c"
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ls[i].open = 1;
# 605 "src/core/ngx_cycle.c"
        }
    }

    if (ngx_open_listening_sockets(cycle) != 0) {
        goto failed;
    }
    ...
    for (i = 0; ; i++) {
        ...
        for (n = 0; ; n++) {
            ...
            if (strncmp((const char *) oshm_zone[i].shm.name.data, (const char *) shm_zone[n].shm.name.data, oshm_zone[i].shm.name.len)
...
old_shm_zone_done:
    ...
    for (i = 0; i < old_cycle->listening.nelts; i++) {
        ...
        if (close(ls[i].fd) == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "close() socket" " listening socket on %V failed", &ls[i].addr_text)
        ...
        if (ls[i].sockaddr->sa_family == AF_UNIX) {
            ...
            if ((cycle->log)->log_level >= 5) ngx_log_error_core(5, cycle->log, 0, "deleting socket %s", name)
                                                     ;
    ...
    for (i = 0; ; i++) {
        ...
        if (file[i].fd == -1 || file[i].fd == STDERR_FILENO) {
            continue;
        }

        if (close(file[i].fd) == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "close()" " \"%s\" failed", file[i].name.data)
    ...
    if (ngx_process == 1 || (old_cycle->conf_ctx == 0)) {
    ...
    if (ngx_temp_pool == 0) {
        ...
        if (ngx_temp_pool == 0) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "could not create ngx_temp_pool")
                                                           ;
            exit(1);
        }
        ...
        (void) memset(ngx_old_cycles.elts, 0, n * sizeof(ngx_cycle_t *));
...
# 908 "src/core/ngx_cycle.c"
        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "shared zone \"%V\" has no equal addresses: %p vs %p", &zn->shm.name, sp->addr, sp)
    ...
    file = 0;
# 934 "src/core/ngx_cycle.c"
    if (ngx_shmtx_create(&sp->mutex, &sp->lock, file) != 0) {
        return -1;
    }
...
ngx_create_pidfile(ngx_str_t *name, ngx_log_t *log)
{
    ...
    ngx_uint_t create;
    ngx_file_t file;
    u_char pid[(sizeof("-9223372036854775808") - 1) + 2];
    ...
    create = ngx_test_config ? O_CREAT : (O_CREAT|O_TRUNC);

    file.fd = open((const char *) file.name.data, O_RDWR|create, 0644)
                                                            ;

    if (file.fd == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "open()" " \"%s\" failed", file.name.data)
                                                                       ;
        return -1;
    }

    if (!ngx_test_config) {
        len = ngx_snprintf(pid, (sizeof("-9223372036854775808") - 1) + 2, "%P%N", ngx_pid) - pid;

        if (ngx_write_file(&file, pid, len, 0) == -1) {
            return -1;
        }
    }

    if (close(file.fd) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close()" " \"%s\" failed", file.name.data)
                                                                        ;
    }
...
ngx_delete_pidfile(ngx_cycle_t *cycle)
{
    ...
    if (unlink((const char *) name) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "unlink()" " \"%s\" failed", name)
                                                               ;
    }
}
...
ngx_signal_process(ngx_cycle_t *cycle, char *sig)
{
    ...
    ngx_file_t file;
    ngx_core_conf_t *ccf;
    u_char buf[(sizeof("-9223372036854775808") - 1) + 2];

    if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "signal process started");
    ...
    file.fd = open((const char *) file.name.data, O_RDONLY|0, 0644)
                                                                   ;

    if (file.fd == -1) {
        if ((cycle->log)->log_level >= 4) ngx_log_error_core(4, cycle->log, errno, "open()" " \"%s\" failed", file.name.data)
                                                                       ;
        return 1;
    }

    n = ngx_read_file(&file, buf, (sizeof("-9223372036854775808") - 1) + 2, 0);

    if (close(file.fd) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "close()" " \"%s\" failed", file.name.data)
                                                                        ;
    }
    ...
    while (n-- && (buf[n] == (u_char) '\r' || buf[n] == (u_char) '\n')) { }

    pid = ngx_atoi(buf, ++n);

    if (pid == (ngx_pid_t) -1) {
        if ((cycle->log)->log_level >= 4) ngx_log_error_core(4, cycle->log, 0, "invalid PID number \"%*s\" in \"%s\"", n, buf, file.name.data)
...
ngx_reopen_files(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    for (i = 0; ; i++) {
        ...
        fd = open((const char *) file[i].name.data, (O_WRONLY|O_APPEND)|O_CREAT, 0644)
                                                                            ;
        ...
        if (fd == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "open()" " \"%s\" failed", file[i].name.data)
                                                                              ;
            continue;
        }
        ...
        if (user != (ngx_uid_t) (ngx_uint_t) -1) {
            ...
            if (stat((const char *) file[i].name.data, &fi) == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "stat()" " \"%s\" failed", file[i].name.data)

                                                ;

                if (close(fd) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "close()" " \"%s\" failed", file[i].name.data)
            ...
            if (fi.st_uid != user) {
                if (chown((const char *) file[i].name.data, user, -1) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "chown(\"%s\", %d) failed", file[i].name.data, user)
            ...
            if ((fi.st_mode & (S_IRUSR|S_IWUSR)) != (S_IRUSR|S_IWUSR)) {
                ...
                if (chmod((const char *) file[i].name.data, fi.st_mode) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "chmod() \"%s\" failed", file[i].name.data)
                                                                             ;
        ...
        if (fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "fcntl(FD_CLOEXEC) \"%s\" failed", file[i].name.data)
...
ngx_shared_memory_add(ngx_conf_t *cf, ngx_str_t *name, size_t size, void *tag)
{
    ...
    for (i = 0; ; i++) {
        ...
        if (strncmp((const char *) name->data, (const char *) shm_zone[i].shm.name.data, name->len)
            != 0)
        {
            continue;
        }
