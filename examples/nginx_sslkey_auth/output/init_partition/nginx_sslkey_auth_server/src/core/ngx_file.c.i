...
# 43 "src/core/ngx_file.c"
    ...
    p = (((u_char *) memcpy(n, prefix->data, len)) + (len));
...
ngx_write_chain_to_temp_file(ngx_temp_file_t *tf, ngx_chain_t *chain)
{
    ...
    if (tf->file.fd == -1) {
        ...
        if (tf->log_level) {
            if ((tf->file.log)->log_level >= tf->log_level) ngx_log_error_core(tf->log_level, tf->file.log, 0, "%s %V", tf->warn, &tf->file.name)
                                                   ;
        }
    }
# 136 "src/core/ngx_file.c"
    ...
    ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
    ...
    p = (((u_char *) memcpy(file->name.data, name.data, name.len)) + (name.len));
    ...
    for ( ;; ) {
        ...
        if ((path->level[0] == 0) || (err != ENOENT)) {
            if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, err, "open()" " \"%s\" failed", file->name.data)

                                          ;
            return -1;
        }

        if (ngx_create_path(file, path) == -1) {
            return -1;
        }
    }
}
...
ngx_create_hashed_filename(ngx_path_t *path, u_char *file, size_t len)
{
    ...
    for (n = 0; n < 3; n++) {
        ...
        len -= level;
        file[i - 1] = '/';
        (void) memcpy(&file[i], &file[len], level);
        i += level + 1;
    }
}
...
ngx_create_path(ngx_file_t *file, ngx_path_t *path)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (mkdir((const char *) file->name.data, 0700) == -1) {
            err = errno;
            if (err != EEXIST) {
                if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, err, "mkdir()" " \"%s\" failed", file->name.data)
...
ngx_next_temp_number(ngx_uint_t collision)
{
    ...
    n = __sync_fetch_and_add(ngx_temp_number, add);
...
ngx_conf_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_conf_full_name(cf->cycle, &path->name, 0) != 0) {
        return (void *) -1;
    }
...
ngx_conf_set_access_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *access = 0;
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (strncmp((const char *) p, (const char *) "user:", sizeof("user:") - 1) == 0) {
            shift = 6;
            p += sizeof("user:") - 1;
            user = 0;

        } else if (strncmp((const char *) p, (const char *) "group:", sizeof("group:") - 1) == 0) {
            shift = 3;
            p += sizeof("group:") - 1;

        } else if (strncmp((const char *) p, (const char *) "all:", sizeof("all:") - 1) == 0) {
            shift = 0;
            p += sizeof("all:") - 1;
        ...
        if (strcmp((const char *) p, (const char *) "rw") == 0) {
            right = 6;

        } else if (strcmp((const char *) p, (const char *) "r") == 0) {
            right = 4;
...
invalid:
    ...
    ngx_conf_log_error(1, cf, 0, "invalid value \"%V\"", &value[i]);
...
ngx_add_path(ngx_conf_t *cf, ngx_path_t **slot)
{
    ...
    for (i = 0; i < cf->cycle->paths.nelts; i++) {
        if (p[i]->name.len == path->name.len
            && strcmp((const char *) p[i]->name.data, (const char *) path->name.data) == 0)
        {
            ...
            for (n = 0; n < 3; n++) {
                if (p[i]->level[n] != path->level[n]) {
                    if (path->conf_file == 0) {
                        if (p[i]->conf_file == 0) {
                            if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "the default path name \"%V\" has " "the same name as another default path, " "but the different levels, you need to " "redefine one of them in http section", &p[i]->name)
                        ...
                        if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "the path name \"%V\" in %s:%ui has " "the same name as default path, but " "the different levels, you need to " "define default path in http section", &p[i]->name, p[i]->conf_file, p[i]->line)
...
ngx_create_paths(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    for (i = 0; i < cycle->paths.nelts; i++) {
        ...
        if (mkdir((const char *) path[i]->name.data, 0700) == -1) {
            err = errno;
            if (err != EEXIST) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, err, "mkdir()" " \"%s\" failed", path[i]->name.data)
        ...
        if (user == (ngx_uid_t) (ngx_uint_t) -1) {
            continue;
        }
        ...
        if (stat((const char *) path[i]->name.data, &fi) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "stat()" " \"%s\" failed", path[i]->name.data)
                                                                               ;
            return -1;
        }
        ...
        if (fi.st_uid != user) {
            if (chown((const char *) path[i]->name.data, user, -1) == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "chown(\"%s\", %d) failed", path[i]->name.data, user)
                                                  ...
                                                  != (S_IRUSR|S_IWUSR|S_IXUSR))
        {
            ...
            if (chmod((const char *) path[i]->name.data, fi.st_mode) == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "chmod() \"%s\" failed", path[i]->name.data)
                                                                          ;
                return -1;
            }
        }
...
ngx_ext_rename_file(ngx_str_t *src, ngx_str_t *to, ngx_ext_rename_file_t *ext)
{
    ...
    if (ext->access) {
        if (chmod((const char *) src->data, ext->access) == -1) {
            if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, errno, "chmod()" " \"%s\" failed", src->data)
                                                                               ;
            err = 0;
            goto failed;
        }
    ...
    if (ext->time != -1) {
        if (ngx_set_file_time(src->data, ext->fd, ext->time) != 0) {
            if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, errno, "utimes()" " \"%s\" failed", src->data)
                                                                          ;
            err = 0;
            goto failed;
        }
    ...
    if (rename((const char *) src->data, (const char *) to->data) != -1) {
        return 0;
    }
    ...
    if (err == ENOENT) {
        ...
        err = ngx_create_full_path(to->data, (ext->path_access | (ext->path_access & 0444) >> 2));

        if (err) {
            if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, err, "mkdir()" " \"%s\" failed", to->data)
                                                                      ;
            err = 0;
            goto failed;
        }
...
# 728 "src/core/ngx_file.c"
    if (err == EXDEV) {
        ...
        if (ngx_copy_file(src->data, name, &cf) == 0) {

            if (rename((const char *) name, (const char *) to->data) != -1) {
                free(name);

                if (unlink((const char *) src->data) == -1) {
                    if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, errno, "unlink()" " \"%s\" failed", src->data)
            ...
            if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, errno, "rename()" " \"%s\" to \"%s\" failed", name, to->data)

                                         ;

            if (unlink((const char *) name) == -1) {
                if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, errno, "unlink()" " \"%s\" failed", name)
                                                                       ;
...
failed:
    ...
    if (err) {
        if ((ext->log)->log_level >= 3) ngx_log_error_core(3, ext->log, err, "rename()" " \"%s\" to \"%s\" failed", src->data, to->data)
...
ngx_copy_file(u_char *from, u_char *to, ngx_copy_file_t *cf)
{
    ...
    fd = open((const char *) from, O_RDONLY|0, 0);

    if (fd == -1) {
        if ((cf->log)->log_level >= 3) ngx_log_error_core(3, cf->log, errno, "open()" " \"%s\" failed", from)
                                                             ;
        goto failed;
    }
    ...
    if (cf->size != -1 && cf->access != 0 && cf->time != -1) {
        ...
        if (fstat(fd, &fi) == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "fstat()" " \"%s\" failed", from)
                                                               ;

            goto failed;
        }

        size = (cf->size != -1) ? cf->size : (&fi)->st_size;
        access = cf->access ? cf->access : ((&fi)->st_mode & 0777);
        time = (cf->time != -1) ? cf->time : (&fi)->st_mtime;
    }
    ...
    nfd = open((const char *) to, O_WRONLY|(O_CREAT|O_TRUNC), access);

    if (nfd == -1) {
        if ((cf->log)->log_level >= 3) ngx_log_error_core(3, cf->log, errno, "open()" " \"%s\" failed", to)
                                                           ;
        goto failed;
    }
    ...
    while (size > 0) {
        ...
        if (n == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "read()" " \"%s\" failed", from)
                                                               ;
            goto failed;
        }

        if ((size_t) n != len) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, 0, "read()" " has read only %z of %O from %s", n, size, from)
        ...
        if (n == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "write()" " \"%s\" failed", to)
                                                              ;
            goto failed;
        }

        if ((size_t) n != len) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, 0, "write()" " has written only %z of %O to %s", n, size, to)
    ...
    if (ngx_set_file_time(to, nfd, time) != 0) {
        if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "utimes()" " \"%s\" failed", to)
                                                               ;
        goto failed;
    }
...
failed:
    ...
    if (nfd != -1) {
        if (close(nfd) == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "close()" " \"%s\" failed", to)
                                                                ;
        }
    }
    ...
    if (fd != -1) {
        if (close(fd) == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "close()" " \"%s\" failed", from)
                                                                  ;
        }
    }
...
ngx_walk_tree(ngx_tree_ctx_t *ctx, ngx_str_t *tree)
{
    ...
    (&buf)->len = 0; (&buf)->data = 0;

   
                                            ;

    if (ngx_open_dir(tree, &dir) == -1) {
        if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, errno, "opendir()" " \"%s\" failed", tree->data)
                                                                  ;
        return -1;
    }
    ...
    if (ctx->alloc) {
        ...
        if (ctx->init_handler(data, prev) == -6) {
            goto failed;
        }
    ...
    for ( ;; ) {
        ...
        if (ngx_read_dir(&dir) == -1) {
            ...
            if (err == 0) {
                ...
                if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, err, "readdir()" " \"%s\" failed", tree->data)
                                                                          ;
                rc = -1;
            }
        ...
        if (file.len + 0 > buf.len) {

            if (buf.len) {
                free(buf.data);
            }

            buf.len = tree->len + 1 + len + 0;
        ...
        p = (((u_char *) memcpy(buf.data, tree->data, tree->len)) + (tree->len));
        *p++ = '/';
        ...
        if (!dir.valid_info) {
            if (ngx_de_info(file.data, &dir) == -1) {
                if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, errno, "stat()" " \"%s\" failed", file.data)
                                                                        ;
                continue;
            }
        }

        if ((((&dir)->type) ? ((&dir)->type == DT_REG) : (S_ISREG((&dir)->info.st_mode)))) {

           
                                                         ;

            ctx->size = (&dir)->info.st_size;
            ctx->fs_size = (((&dir)->info.st_size < (&dir)->info.st_blocks * 512) ? ((&dir)->info.st_blocks * 512) : ((&dir)->info.st_size));
            ctx->access = (((&dir)->info.st_mode) & 0777);
            ctx->mtime = (&dir)->info.st_mtime;

            if (ctx->file_handler(ctx, &file) == -6) {
                goto failed;
            }

        } else if ((((&dir)->type) ? ((&dir)->type == DT_DIR) : (S_ISDIR((&dir)->info.st_mode)))) {
            ...
            if (ctx->post_tree_handler(ctx, &file) == -6) {
                goto failed;
            }
            ...
            if (ctx->spec_handler(ctx, &file) == -6) {
                goto failed;
            }
        }
    }
...
failed:
    ...
    if (closedir((&dir)->dir) == -1) {
        if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, errno, "closedir()" " \"%s\" failed", tree->data)
                                                                   ;
    }
