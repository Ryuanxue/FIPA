...
# 9 "src/core/ngx_output_chain.c" 2
...
# 10 "src/core/ngx_output_chain.c" 2
...
# 11 "src/core/ngx_output_chain.c" 2
...
ngx_output_chain(ngx_output_chain_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    if (in) {
        if (ngx_output_chain_add_copy(ctx->pool, &ctx->in, in) == -1) {
            return -1;
        }
    }
    ...
    for ( ;; ) {
        ...
        while (ctx->in) {
            ...
            bsize = ((ctx->in->buf->temporary || ctx->in->buf->memory || ctx->in->buf->mmap) ? (off_t) (ctx->in->buf->last - ctx->in->buf->pos): (ctx->in->buf->file_last - ctx->in->buf->file_pos));

            if (bsize == 0 && !((ctx->in->buf->flush || ctx->in->buf->last_buf || ctx->in->buf->sync) && !(ctx->in->buf->temporary || ctx->in->buf->memory || ctx->in->buf->mmap) && !ctx->in->buf->in_file)) {

                if ((ctx->pool->log)->log_level >= 2) ngx_log_error_core(2, ctx->pool->log, 0, "zero size buf in output " "t:%d r:%d f:%d %p %p-%p %p %O-%O", ctx->in->buf->temporary, ctx->in->buf->recycled, ctx->in->buf->in_file, ctx->in->buf->start, ctx->in->buf->pos, ctx->in->buf->last, ctx->in->buf->file, ctx->in->buf->file_pos, ctx->in->buf->file_last)
# 120 "src/core/ngx_output_chain.c"
                                                      ;
            ...
            if (ctx->buf == 0) {
                ...
                if (rc != 0) {
                    ...
                    if (ctx->free) {
                        ...
                        cl->next = ctx->pool->chain; ctx->pool->chain = cl;

                    } else if (out || ctx->allocated == ctx->bufs.num) {

                        break;

                    } else if (ngx_output_chain_get_buf(ctx, bsize) != 0) {
                        return -1;
                    }
                }
            }
            ...
            if (((ctx->in->buf->temporary || ctx->in->buf->memory || ctx->in->buf->mmap) ? (off_t) (ctx->in->buf->last - ctx->in->buf->pos): (ctx->in->buf->file_last - ctx->in->buf->file_pos)) == 0) {
                ctx->in = ctx->in->next;
            }
...
ngx_output_chain_as_is(ngx_output_chain_ctx_t *ctx, ngx_buf_t *buf)
{
    ...
    if (((buf->flush || buf->last_buf || buf->sync) && !(buf->temporary || buf->memory || buf->mmap) && !buf->in_file)) {
        return 1;
    }
# 243 "src/core/ngx_output_chain.c"
    if (buf->in_file && buf->file->directio) {
        return 0;
    }

    sendfile = ctx->sendfile;
# 257 "src/core/ngx_output_chain.c"
    if (!sendfile) {
    ...
    if (ctx->need_in_memory && !(buf->temporary || buf->memory || buf->mmap)) {
        return 0;
    }
...
# 306 "src/core/ngx_output_chain.c"
static ngx_int_t
...
# 361 "src/core/ngx_output_chain.c"
        cl->buf = in->buf;
        in = in->next;
...
ngx_output_chain_get_buf(ngx_output_chain_ctx_t *ctx, off_t bsize)
{
    ...
    b = ngx_pcalloc(ctx->pool, sizeof(ngx_buf_t));
    if (b == 0) {
        return -1;
    }
...
ngx_output_chain_copy_buf(ngx_output_chain_ctx_t *ctx)
{
    ...
    size = ((src->temporary || src->memory || src->mmap) ? (off_t) (src->last - src->pos): (src->file_last - src->file_pos));
    size = ((size > dst->end - dst->pos) ? (dst->end - dst->pos) : (size));

    sendfile = ctx->sendfile && !ctx->directio;
# 525 "src/core/ngx_output_chain.c"
    if ((src->temporary || src->memory || src->mmap)) {
        (void) memcpy(dst->pos, src->pos, (size_t) size);
        src->pos += (size_t) size;
        ...
        if (ctx->unaligned) {
            if (ngx_directio_off(src->file->fd) == -1) {
                if ((ctx->pool->log)->log_level >= 2) ngx_log_error_core(2, ctx->pool->log, errno, "fcntl(!O_DIRECT)" " \"%s\" failed", src->file->name.data)

                                                   ;
            }
        }
# 594 "src/core/ngx_output_chain.c"
        {
        ...
        if (n != size) {
            if ((ctx->pool->log)->log_level >= 2) ngx_log_error_core(2, ctx->pool->log, 0, "pread()" " read only %z of %O from \"%s\"", n, size, src->file->name.data)
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
    ...
    for (size = 0; in; in = in->next) {
        ...
        if (((in->buf->temporary || in->buf->memory || in->buf->mmap) ? (off_t) (in->buf->last - in->buf->pos): (in->buf->file_last - in->buf->file_pos)) == 0 && !((in->buf->flush || in->buf->last_buf || in->buf->sync) && !(in->buf->temporary || in->buf->memory || in->buf->mmap) && !in->buf->in_file)) {

            if ((ctx->pool->log)->log_level >= 2) ngx_log_error_core(2, ctx->pool->log, 0, "zero size buf in chain writer " "t:%d r:%d f:%d %p %p-%p %p %O-%O", in->buf->temporary, in->buf->recycled, in->buf->in_file, in->buf->start, in->buf->pos, in->buf->last, in->buf->file, in->buf->file_pos, in->buf->file_last)
# 682 "src/core/ngx_output_chain.c"
                                             ;
        ...
        size += ((in->buf->temporary || in->buf->memory || in->buf->mmap) ? (off_t) (in->buf->last - in->buf->pos): (in->buf->file_last - in->buf->file_pos));
    ...
    for (cl = ctx->out; cl; cl = cl->next) {
        ...
        if (((cl->buf->temporary || cl->buf->memory || cl->buf->mmap) ? (off_t) (cl->buf->last - cl->buf->pos): (cl->buf->file_last - cl->buf->file_pos)) == 0 && !((cl->buf->flush || cl->buf->last_buf || cl->buf->sync) && !(cl->buf->temporary || cl->buf->memory || cl->buf->mmap) && !cl->buf->in_file)) {
            ...
            if ((ctx->pool->log)->log_level >= 2) ngx_log_error_core(2, ctx->pool->log, 0, "zero size buf in chain writer " "t:%d r:%d f:%d %p %p-%p %p %O-%O", cl->buf->temporary, cl->buf->recycled, cl->buf->in_file, cl->buf->start, cl->buf->pos, cl->buf->last, cl->buf->file, cl->buf->file_pos, cl->buf->file_last)
# 726 "src/core/ngx_output_chain.c"
                                             ;
    ...
    for (cl = ctx->out; cl && cl != chain; ) {
        ln = cl;
        cl = cl->next;
        ln->next = ctx->pool->chain; ctx->pool->chain = ln;
    }
