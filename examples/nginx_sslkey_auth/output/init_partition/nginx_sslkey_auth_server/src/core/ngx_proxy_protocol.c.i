# 1 "src/core/ngx_proxy_protocol.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/core/ngx_proxy_protocol.c" 2
...
# 10 "src/core/ngx_proxy_protocol.c" 2
# 19 "src/core/ngx_proxy_protocol.c"
typedef struct {
    u_char signature[12];
...
ngx_proxy_protocol_read(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    if (len < 8 || strncmp((const char *) p, (const char *) "PROXY ", 6) != 0) {
        goto invalid;
    }
    ...
    if (len >= 7 && strncmp((const char *) p, (const char *) "UNKNOWN", 7) == 0) {
    ...
    if (len < 5 || strncmp((const char *) p, (const char *) "TCP", 3) != 0
        || (p[3] != '4' && p[3] != '6') || p[4] != ' ')
    {
        goto invalid;
    }
    ...
    if (c->proxy_protocol_addr.data == 0) {
        return 0;
    }

    (void) memcpy(c->proxy_protocol_addr.data, addr, len);
...
invalid:
    ...
    if ((c->log)->log_level >= 4) ngx_log_error_core(4, c->log, 0, "broken header: \"%*s\"", (size_t) (last - buf), buf)
                                                                       ;
...
ngx_proxy_protocol_write(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    case AF_INET:
        buf = (((u_char *) memcpy(buf, "PROXY TCP4 ", sizeof("PROXY TCP4 ") - 1)) + (sizeof("PROXY TCP4 ") - 1));
        break;


    case AF_INET6:
        buf = (((u_char *) memcpy(buf, "PROXY TCP6 ", sizeof("PROXY TCP6 ") - 1)) + (sizeof("PROXY TCP6 ") - 1));
        break;


    default:
        return (((u_char *) memcpy(buf, "PROXY UNKNOWN" "\r\n", sizeof("PROXY UNKNOWN" "\r\n") - 1)) + (sizeof("PROXY UNKNOWN" "\r\n") - 1))
                                                           ;
    }
...
ngx_proxy_protocol_v2_read(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    if (version != 2) {
        if ((c->log)->log_level >= 4) ngx_log_error_core(4, c->log, 0, "unknown PROXY protocol version: %ui", version)
                                                                     ;
        return 0;
    }
    ...
    if ((size_t) (last - buf) < len) {
        if ((c->log)->log_level >= 4) ngx_log_error_core(4, c->log, 0, "header is too large");
        return 0;
    }
    ...
    case 1:
        ...
        c->proxy_protocol_port = ((in->src_port)[0] << 8 | (in->src_port)[1]);
    ...
    case 2:
        ...
        c->proxy_protocol_port = ((in6->src_port)[0] << 8 | (in6->src_port)[1]);
    ...
    c->proxy_protocol_addr.data = ngx_pnalloc(c->pool, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 324 "src/core/ngx_proxy_protocol.c" 3 4
                                                      0
# 324 "src/core/ngx_proxy_protocol.c"
                                                      ));
    if (c->proxy_protocol_addr.data == 0) {
        return 0;
    }
...
# 331 "src/core/ngx_proxy_protocol.c" 3 4
                                              0
# 331 "src/core/ngx_proxy_protocol.c"
                                              ), 0);
