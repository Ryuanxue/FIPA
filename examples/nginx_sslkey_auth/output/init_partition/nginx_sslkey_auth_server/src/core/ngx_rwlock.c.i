...
# 10 "src/core/ngx_rwlock.c" 2
# 19 "src/core/ngx_rwlock.c"
void
ngx_rwlock_wlock(ngx_atomic_t *lock)
{
    ...
    for ( ;; ) {
        ...
        if (*lock == 0 && __sync_bool_compare_and_swap(lock, 0, ((ngx_atomic_uint_t) -1))) {
            return;
        }
        ...
        if (ngx_ncpu > 1) {
            ...
            for (n = 1; n < 2048; n <<= 1) {
                ...
                if (*lock == 0
                    && __sync_bool_compare_and_swap(lock, 0, ((ngx_atomic_uint_t) -1)))
                {
                    return;
                }
            }
...
ngx_rwlock_rlock(ngx_atomic_t *lock)
{
    ...
    for ( ;; ) {
        ...
        if (readers != ((ngx_atomic_uint_t) -1)
...
ngx_rwlock_unlock(ngx_atomic_t *lock)
{
    ...
    if (readers == ((ngx_atomic_uint_t) -1)) {
        (void) __sync_bool_compare_and_swap(lock, ((ngx_atomic_uint_t) -1), 0);
        return;
    }
