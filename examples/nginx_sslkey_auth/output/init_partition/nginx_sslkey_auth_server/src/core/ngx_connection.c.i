...
# 26 "src/core/ngx_connection.c" 3 4
                          0
# 26 "src/core/ngx_connection.c"
                          )];
    ...
    len = ngx_sock_ntop(sa, socklen, text, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 45 "src/core/ngx_connection.c" 3 4
                                          0
# 45 "src/core/ngx_connection.c"
                                          ), 1);
    ...
    case AF_INET6:
        ls->addr_text_max_len = (sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255") - 1);
        break;


    case AF_UNIX:
        ls->addr_text_max_len = (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 56 "src/core/ngx_connection.c" 3 4
                               0
# 56 "src/core/ngx_connection.c"
    ...
    default:
        ls->addr_text_max_len = (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 64 "src/core/ngx_connection.c" 3 4
                               0
# 64 "src/core/ngx_connection.c"
                               );
        break;
    }

    ls->addr_text.data = ngx_pnalloc(cf->pool, len);
    if (ls->addr_text.data == 0) {
        return 0;
    }
    ...
    ((&ls->sentinel)->color = 0); (&ls->rbtree)->root = &ls->sentinel; (&ls->rbtree)->sentinel = &ls->sentinel; (&ls->rbtree)->insert = ngx_udp_rbtree_insert_value;
...
# 151 "src/core/ngx_connection.c"
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (getsockname(ls[i].fd, ls[i].sockaddr, &ls[i].socklen) == -1) {
            if ((cycle->log)->log_level >= 3) ngx_log_error_core(3, cycle->log, errno, "getsockname() of the inherited " "socket #%d failed", ls[i].fd)
        ...
        case AF_INET6:
            ls[i].addr_text_max_len = (sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255") - 1);
            len = (sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255") - 1) + sizeof("[]:65535") - 1;
            break;



        case AF_UNIX:
            ls[i].addr_text_max_len = (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 186 "src/core/ngx_connection.c" 3 4
                                     0
# 186 "src/core/ngx_connection.c"
                                     );
            len = (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 187 "src/core/ngx_connection.c" 3 4
                 0
# 187 "src/core/ngx_connection.c"
                 );
            break;
        ...
        default:
            if ((cycle->log)->log_level >= 3) ngx_log_error_core(3, cycle->log, errno, "the inherited socket #%d has " "an unsupported protocol family", ls[i].fd)
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_TYPE, (void *) &ls[i].type,
            ...
            == -1)
        {
            if ((cycle->log)->log_level >= 3) ngx_log_error_core(3, cycle->log, errno, "getsockopt(SO_TYPE) %V failed", &ls[i].addr_text)
                                                                            ;
            ls[i].ignore = 1;
            continue;
        }
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_RCVBUF, (void *) &ls[i].rcvbuf,
            ...
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "getsockopt(SO_RCVBUF) %V failed, ignored", &ls[i].addr_text)
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_SNDBUF, (void *) &ls[i].sndbuf,
            ...
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "getsockopt(SO_SNDBUF) %V failed, ignored", &ls[i].addr_text)
...
# 280 "src/core/ngx_connection.c"
        reuseport = 0;
        olen = sizeof(int);
# 299 "src/core/ngx_connection.c"
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_REUSEPORT,
                       (void *) &reuseport, &olen)
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "getsockopt(SO_REUSEPORT) %V failed, ignored", &ls[i].addr_text)
            ...
            == -1)
        {
            ...
            if (err != EOPNOTSUPP && err != ENOPROTOOPT
                && err != EINVAL)
            {
                if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, err, "getsockopt(TCP_FASTOPEN) %V failed, ignored", &ls[i].addr_text)
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (ls[i].add_reuseport) {
                ...
                int reuseport = 1;
# 463 "src/core/ngx_connection.c"
                if (setsockopt(ls[i].fd, SOL_SOCKET, SO_REUSEPORT,
                               (const void *) &reuseport, sizeof(int))
                    == -1)
                {
                    if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(SO_REUSEPORT) %V failed, ignored", &ls[i].addr_text)
            ...
            if (s == (ngx_socket_t) -1) {
                if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "socket()" " %V failed", &ls[i].addr_text)
                                                                          ;
                return -1;
            }
            ...
            if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
                ...
                == -1)
            {
                if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "setsockopt(SO_REUSEADDR) %V failed", &ls[i].addr_text)

                                               ;

                if (close(s) == -1) {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "close() socket" " %V failed", &ls[i].addr_text)
            ...
            if (ls[i].reuseport && !ngx_test_config) {
                ...
                reuseport = 1;
# 543 "src/core/ngx_connection.c"
                if (setsockopt(s, SOL_SOCKET, SO_REUSEPORT,
                               (const void *) &reuseport, sizeof(int))
                    == -1)
                {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "setsockopt(SO_REUSEPORT) %V failed", &ls[i].addr_text)
...
# 582 "src/core/ngx_connection.c"
            if (!(ngx_event_flags & 0x00000200)) {
                if (ngx_nonblocking(s) == -1) {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "ioctl(FIONBIO)" " %V failed", &ls[i].addr_text)
            ...
            if (bind(s, ls[i].sockaddr, ls[i].socklen) == -1) {
                ...
                if (err != EADDRINUSE || !ngx_test_config) {
            ...
            if (ls[i].sockaddr->sa_family == AF_UNIX) {
                ...
                if (chmod((char *) name, mode) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "chmod() \"%s\" failed", name)
                                                                ;
                }

                if (ngx_test_config) {
                    if (unlink((const char *) name) == -1) {
                        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "unlink()" " %s failed", name)
                                                                           ;
                    }
                }
            }
            ...
            if (listen(s, ls[i].backlog) == -1) {
                ...
                if (err != EADDRINUSE || !ngx_test_config) {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, err, "listen() to %V, backlog %d failed", &ls[i].addr_text, ls[i].backlog)
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ls[i].rcvbuf != -1) {
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_RCVBUF,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(SO_RCVBUF, %d) %V failed, ignored", ls[i].rcvbuf, &ls[i].addr_text)
        ...
        if (ls[i].sndbuf != -1) {
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_SNDBUF,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(SO_SNDBUF, %d) %V failed, ignored", ls[i].sndbuf, &ls[i].addr_text)
        ...
        if (ls[i].keepalive) {
            ...
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_KEEPALIVE,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(SO_KEEPALIVE, %d) %V failed, ignored", value, &ls[i].addr_text)
        ...
        if (ls[i].keepidle) {
            ...
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_KEEPIDLE,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(TCP_KEEPIDLE, %d) %V failed, ignored", value, &ls[i].addr_text)
        ...
        if (ls[i].keepintvl) {
            ...
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_KEEPINTVL,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(TCP_KEEPINTVL, %d) %V failed, ignored", value, &ls[i].addr_text)
        ...
        if (ls[i].keepcnt) {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_KEEPCNT,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(TCP_KEEPCNT, %d) %V failed, ignored", ls[i].keepcnt, &ls[i].addr_text)

                                                              ;
            }
        }
# 826 "src/core/ngx_connection.c"
        if (ls[i].fastopen != -1) {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_FASTOPEN,
                           (const void *) &ls[i].fastopen, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(TCP_FASTOPEN, %d) %V failed, ignored", ls[i].fastopen, &ls[i].addr_text)

                                                               ;
            }
        }
# 853 "src/core/ngx_connection.c"
        if (ls[i].listen) {



            if (listen(ls[i].fd, ls[i].backlog) == -1) {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "listen() to %V, backlog %d failed, ignored", &ls[i].addr_text, ls[i].backlog)

                                                              ;
            }
        }
# 975 "src/core/ngx_connection.c"
        if (ls[i].wildcard
            ...
            && ls[i].sockaddr->sa_family == AF_INET)
        {
            ...
            if (setsockopt(ls[i].fd, IPPROTO_IP, IP_PKTINFO,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(IP_PKTINFO) " "for %V failed, ignored", &ls[i].addr_text)
            ...
            && ls[i].sockaddr->sa_family == AF_INET6)
        {
            ...
            if (setsockopt(ls[i].fd, IPPROTO_IPV6, IPV6_RECVPKTINFO,
                ...
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setsockopt(IPV6_RECVPKTINFO) " "for %V failed, ignored", &ls[i].addr_text)
...
ngx_close_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (c) {
            if (c->read->active) {
                if (ngx_event_flags & 0x00000040) {
                    ...
                    ngx_event_actions.del(c->read, (EPOLLIN|EPOLLRDHUP), 0);

                } else {
                    ngx_event_actions.del(c->read, (EPOLLIN|EPOLLRDHUP), 1);
                }
            }
        ...
        if (close(ls[i].fd) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "close() socket" " %V failed", &ls[i].addr_text)
                                                                            ;
        }
...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    if (ngx_cycle->files && ngx_cycle->files[s] == 0) {
        ngx_cycle->files[s] = c;
    }
...
ngx_close_connection(ngx_connection_t *c)
{
    ...
    if (c->read->posted) {
        (c->read)->posted = 0; (&(c->read)->queue)->next->prev = (&(c->read)->queue)->prev; (&(c->read)->queue)->prev->next = (&(c->read)->queue)->next; ;;
    }

    if (c->write->posted) {
        (c->write)->posted = 0; (&(c->write)->queue)->next->prev = (&(c->write)->queue)->prev; (&(c->write)->queue)->prev->next = (&(c->write)->queue)->next; ;;
    }
    ...
    if (close(fd) == -1) {
        ...
        if ((c->log)->log_level >= level) ngx_log_error_core(level, c->log, err, "close() socket" " %d failed", fd);
    }
}
...
ngx_drain_connections(ngx_cycle_t *cycle)
{
    ...
    n = ((((32 > cycle->reusable_connections_n / 8) ? (cycle->reusable_connections_n / 8) : (32)) < 1) ? (1) : (((32 > cycle->reusable_connections_n / 8) ? (cycle->reusable_connections_n / 8) : (32))));

    for (i = 0; i < n; i++) {
        if ((&cycle->reusable_connections_queue == (&cycle->reusable_connections_queue)->prev)) {
            break;
        }

        q = (&cycle->reusable_connections_queue)->prev;
        c = (ngx_connection_t *) ((u_char *) q - 
# 1308 "src/core/ngx_connection.c" 3 4
           0
# 1308 "src/core/ngx_connection.c"
           );
    ...
    ngx_uint_t port)
{
    ...
    if (addr == 0) {
        ...
        if (getsockname(c->fd, &sa.sockaddr, &len) == -1) {
            ngx_connection_error(c, errno, "getsockname() failed");
            return -1;
        }

        c->local_sockaddr = ngx_palloc(c->pool, len);
        if (c->local_sockaddr == 0) {
            return -1;
        }
...
ngx_tcp_nodelay(ngx_connection_t *c)
{
    ...
    if (setsockopt(c->fd, IPPROTO_TCP, TCP_NODELAY,
        ...
        == -1)
    {
# 1443 "src/core/ngx_connection.c"
        ngx_connection_error(c, errno,
