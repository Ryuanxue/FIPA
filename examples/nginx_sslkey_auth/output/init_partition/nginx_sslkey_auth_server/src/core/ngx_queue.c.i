    ...
    ngx_int_t (*cmp)(const ngx_queue_t *, const ngx_queue_t *))
{
    ...
    for (q = (q)->next; q != (queue); q = next) {
        ...
        (q)->next = (prev)->next; (q)->next->prev = q; (q)->prev = prev; (prev)->next = q;
    }
}
