...
{
    if (strncmp((const char *) salt, (const char *) "$apr1$", sizeof("$apr1$") - 1) == 0) {
        return ngx_crypt_apr1(pool, key, salt, encrypted);

    } else if (strncmp((const char *) salt, (const char *) "{PLAIN}", sizeof("{PLAIN}") - 1) == 0) {
        return ngx_crypt_plain(pool, key, salt, encrypted);

    } else if (strncmp((const char *) salt, (const char *) "{SSHA}", sizeof("{SSHA}") - 1) == 0) {
        return ngx_crypt_ssha(pool, key, salt, encrypted);

    } else if (strncmp((const char *) salt, (const char *) "{SHA}", sizeof("{SHA}") - 1) == 0) {
...
ngx_crypt_apr1(ngx_pool_t *pool, u_char *key, u_char *salt, u_char **encrypted)
{
    ...
    *encrypted = ngx_pnalloc(pool, sizeof("$apr1$") - 1 + saltlen + 1 + 22 + 1);
    ...
    p = (((u_char *) memcpy(*encrypted, "$apr1$", sizeof("$apr1$") - 1)) + (sizeof("$apr1$") - 1));
    p = (((u_char *) memcpy(p, salt, saltlen)) + (saltlen));
    *p++ = '$';
...
ngx_crypt_plain(ngx_pool_t *pool, u_char *key, u_char *salt, u_char **encrypted)
{
    ...
    *encrypted = ngx_pnalloc(pool, sizeof("{PLAIN}") - 1 + len + 1);
    ...
    p = (((u_char *) memcpy(*encrypted, "{PLAIN}", sizeof("{PLAIN}") - 1)) + (sizeof("{PLAIN}") - 1));
    (void) memcpy(p, key, len + 1);
...
ngx_crypt_ssha(ngx_pool_t *pool, u_char *key, u_char *salt, u_char **encrypted)
{
    ...
    encoded.len = strlen((const char *) encoded.data);

    len = (((((encoded.len + 3) / 4) * 3) < 20) ? (20) : ((((encoded.len + 3) / 4) * 3)));
    ...
    ngx_sha1_init(&sha1);
    ngx_sha1_update(&sha1, key, strlen((const char *) key));
    ngx_sha1_update(&sha1, decoded.data + 20, decoded.len - 20);
    ngx_sha1_final(decoded.data, &sha1);



    len = sizeof("{SSHA}") - 1 + (((decoded.len + 2) / 3) * 4) + 1;
    ...
    *encrypted = ngx_pnalloc(pool, len);
    ...
    encoded.data = (((u_char *) memcpy(*encrypted, "{SSHA}", sizeof("{SSHA}") - 1)) + (sizeof("{SSHA}") - 1));
...
ngx_crypt_sha(ngx_pool_t *pool, u_char *key, u_char *salt, u_char **encrypted)
{
    ...
    len = sizeof("{SHA}") - 1 + (((decoded.len + 2) / 3) * 4) + 1;
    ...
    *encrypted = ngx_pnalloc(pool, len);
    ...
    encoded.data = (((u_char *) memcpy(*encrypted, "{SHA}", sizeof("{SHA}") - 1)) + (sizeof("{SHA}") - 1));
    ngx_encode_base64(&encoded, &decoded);
    encoded.data[encoded.len] = '\0';
