...
static ngx_command_t ngx_conf_commands[] = {
    ...
    { { sizeof("include") - 1, (u_char *) "include" },
      0x1F000000|0x00000002,
      ngx_conf_include,
      0,
      0,
      0 },
...
ngx_conf_param(ngx_conf_t *cf)
{
    ...
    (void) memset(&conf_file, 0, sizeof(ngx_conf_file_t));
...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        fd = open((const char *) filename->data, O_RDONLY|0, 0);
        ...
        if (fstat(fd, &cf->conf_file->file.info) == -1) {
            if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, errno, "fstat()" " \"%s\" failed", filename->data)
                                                                         ;
        }
        ...
        {
            if (ngx_conf_add_dump(cf, filename) != 0) {
                goto failed;
            }
    ...
    } else if (cf->conf_file->file.fd != -1) {
...
# 255 "src/core/ngx_conf_file.c"
        ...
        if (rc == 2) {
            ...
            if (type != parse_block) {
                ngx_conf_log_error(1, cf, 0, "unexpected \"}\"");
                goto failed;
            }
        ...
        if (cf->handler) {
            ...
            rv = (*cf->handler)(cf, 0, cf->handler_conf);
            if (rv == 0) {
                continue;
            }
...
failed:
    ...
    if (filename) {
        ...
        if (close(fd) == -1) {
            if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "close()" " %s failed", filename->data)
...
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    for (i = 0; cf->cycle->modules[i]; i++) {
        ...
        for ( ; cmd->name.len; cmd++) {
            ...
            if (strcmp((const char *) name->data, (const char *) cmd->name.data) != 0) {
                continue;
            }

            found = 1;

            if (cf->cycle->modules[i]->type != 0x464E4F43
            ...
            if (!(cmd->type & 0x00000400)) {

                if (cmd->type & 0x00000200) {

                    if (cf->args->nelts != 2) {
                        goto invalid;
                    }

                } else if (cmd->type & 0x00000800) {

                    if (cf->args->nelts < 2) {
                        goto invalid;
                    }

                } else if (cmd->type & 0x00001000) {

                    if (cf->args->nelts < 3) {
                        goto invalid;
                    }

                } else if (cf->args->nelts > 8) {
            ...
            if (cmd->type & 0x00010000) {
                conf = ((void **) cf->ctx)[cf->cycle->modules[i]->index];

            } else if (cmd->type & 0x01000000) {
                conf = &(((void **) cf->ctx)[cf->cycle->modules[i]->index]);
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    for ( ;; ) {
        ...
        if (b->pos >= b->last) {
            ...
            if (len) {
                (void) memmove(b->start, start, len);
            }
            ...
            if (dump) {
                dump->last = (((u_char *) memcpy(dump->last, b->pos, size)) + (size));
            }
        }
        ...
        if (need_space) {
            if (ch == ' ' || ch == '\t' || ch == (u_char) '\r' || ch == (u_char) '\n') {
                last_space = 1;
                need_space = 0;
                continue;
            }
        ...
        if (last_space) {
            ...
            } else if (ch == ' ' || ch == '\t' || ch == (u_char) '\r' || ch == (u_char) '\n'
                       || ch == ';' || ch == '{')
            {
                last_space = 1;
                found = 1;
...
ngx_conf_include(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    (void) memset(&gl, 0, sizeof(ngx_glob_t));

    gl.pattern = file.data;
    gl.log = cf->log;
    gl.test = 1;

    if (ngx_open_glob(&gl) != 0) {
        ngx_conf_log_error(1, cf, errno,
...
ngx_conf_open_file(ngx_cycle_t *cycle, ngx_str_t *name)
{
    ...
    (&full)->len = 0; (&full)->data = 0;


    if (name->len) {
        full = *name;

        if (ngx_conf_full_name(cycle, &full, 0) != 0) {
            return 0;
        }
        ...
        for (i = 0; ; i++) {
            ...
            if (strcmp((const char *) full.data, (const char *) file[i].name.data) == 0) {
                return &file[i];
            }
        }
    }
    ...
    const char *fmt, ...)
{
    ...
    if (cf->conf_file == 0) {
        if ((cf->log)->log_level >= level) ngx_log_error_core(level, cf->log, 0, "%*s", p - errstr, errstr);
        return;
    }

    if (cf->conf_file->file.fd == -1) {
        if ((cf->log)->log_level >= level) ngx_log_error_core(level, cf->log, 0, "%*s in command line", p - errstr, errstr)
                                         ;
        return;
    }

    if ((cf->log)->log_level >= level) ngx_log_error_core(level, cf->log, 0, "%*s in %s:%ui", p - errstr, errstr, cf->conf_file->file.name.data, cf->conf_file->line)
...
ngx_conf_set_bufs_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (bufs->num == -1 || bufs->num == 0) {
        return "invalid value";
    }

    bufs->size = ngx_parse_size(&value[2]);
    if (bufs->size == (size_t) -1 || bufs->size == 0) {
        return "invalid value";
    }
