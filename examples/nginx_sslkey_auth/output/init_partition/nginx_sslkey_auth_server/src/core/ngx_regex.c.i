...
static ngx_command_t ngx_regex_commands[] = {
    ...
    { { sizeof("pcre_jit") - 1, (u_char *) "pcre_jit" },
      0x01000000|0x00010000|0x00000200,
      ngx_conf_set_flag_slot,
      0,
      
# 38 "src/core/ngx_regex.c" 3 4
     0
# 38 "src/core/ngx_regex.c"
                                         ,
...
ngx_regex_compile(ngx_regex_compile_t *rc)
{
    ...
    n = pcre_fullinfo(re, 0, 
# 147 "src/core/ngx_regex.c" 3 4
                               2
# 147 "src/core/ngx_regex.c"
                                                     , &rc->captures);
    if (n < 0) {
    ...
    n = pcre_fullinfo(re, 0, 
# 157 "src/core/ngx_regex.c" 3 4
                               8
# 157 "src/core/ngx_regex.c"
                                                  , &rc->named_captures);
    if (n < 0) {
    ...
    n = pcre_fullinfo(re, 0, 
# 167 "src/core/ngx_regex.c" 3 4
                               7
# 167 "src/core/ngx_regex.c"
                                                      , &rc->name_size);
    if (n < 0) {
    ...
    n = pcre_fullinfo(re, 0, 
# 173 "src/core/ngx_regex.c" 3 4
                               9
# 173 "src/core/ngx_regex.c"
                                                  , &rc->names);
    if (n < 0) {
...
ngx_regex_exec_array(ngx_array_t *a, ngx_str_t *s, ngx_log_t *log)
{
    ...
    for (i = 0; i < a->nelts; i++) {
        ...
        n = pcre_exec(re[i].regex->code, re[i].regex->extra, (const char *) (s)->data, (s)->len, 0, 0, 0, 0);

        if (n == 
# 210 "src/core/ngx_regex.c" 3 4
                (-1)
# 210 "src/core/ngx_regex.c"
                                    ) {
            continue;
        }

        if (n < 0) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "pcre_exec()" " failed: %i on \"%V\" using \"%s\"", n, s, re[i].name)
...
# 324 "src/core/ngx_regex.c"
    ...
    for (i = 0; ; i++) {
        ...
        if (errstr != 0) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "pcre_study() failed: %s in \"%s\"", errstr, elts[i].name)

                                               ;
        }
# 364 "src/core/ngx_regex.c"
    }
...
ngx_regex_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    if (rcf->pcre_jit == -1) { rcf->pcre_jit = 0; };
