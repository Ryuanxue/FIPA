...
ngx_shmtx_create(ngx_shmtx_t *mtx, ngx_shmtx_sh_t *addr, u_char *name)
{
    ...
    if (sem_init(&mtx->sem, 1, 0) == -1) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "sem_init() failed")
                                          ;
    } else {
        mtx->semaphore = 1;
    }
...
ngx_shmtx_destroy(ngx_shmtx_t *mtx)
{
    ...
    if (mtx->semaphore) {
        if (sem_destroy(&mtx->sem) == -1) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "sem_destroy() failed")
                                                 ;
        }
    }
...
ngx_shmtx_lock(ngx_shmtx_t *mtx)
{
    ...
    for ( ;; ) {
        ...
        if (*mtx->lock == 0 && __sync_bool_compare_and_swap(mtx->lock, 0, ngx_pid)) {
            return;
        }
        ...
        if (mtx->semaphore) {
            ...
            while (sem_wait(&mtx->sem) == -1) {
                ...
                if (err != EINTR) {
                    if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, err, "sem_wait() failed while waiting on shmtx")
                                                                             ;
                    break;
                }
            }
...
ngx_shmtx_unlock(ngx_shmtx_t *mtx)
{
    ...
    if (__sync_bool_compare_and_swap(mtx->lock, ngx_pid, 0)) {
...
ngx_shmtx_force_unlock(ngx_shmtx_t *mtx, ngx_pid_t pid)
{
    ...
    if (__sync_bool_compare_and_swap(mtx->lock, pid, 0)) {
        ngx_shmtx_wakeup(mtx);
        return 1;
    }
...
ngx_shmtx_wakeup(ngx_shmtx_t *mtx)
{
    ...
    if (sem_post(&mtx->sem) == -1) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "sem_post() failed while wake shmtx")
                                                           ;
    }
