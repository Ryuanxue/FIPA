...
# 117 "src/core/nginx.c" 3 4
     0
# 117 "src/core/nginx.c"
                                             ,
      0 },
...
# 124 "src/core/nginx.c" 3 4
     0
# 124 "src/core/nginx.c"
                                           ,
      0 },

    { { sizeof("worker_shutdown_timeout") - 1, (u_char *) "worker_shutdown_timeout" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_msec_slot,
      0,
      
# 131 "src/core/nginx.c" 3 4
     0
# 131 "src/core/nginx.c"
                                                ,
      0 },
...
# 138 "src/core/nginx.c" 3 4
     0
# 138 "src/core/nginx.c"
                                                 ,
      0 },
    ...
    { { sizeof("load_module") - 1, (u_char *) "load_module" },
      0x01000000|0x00010000|0x00000002,
      ngx_load_module,
      0,
      0,
...
# 248 "src/core/nginx.c"
    ...
    if (ngx_test_config) {
        ...
        if (ngx_dump_config) {
            ...
            for (i = 0; i < cycle->config_dump.nelts; i++) {
                ...
                (void) ngx_write_fd(STDOUT_FILENO, cd[i].name.data,
                                    cd[i].name.len);
    ...
    if (log->file->fd != STDERR_FILENO) {
        if (close(log->file->fd) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "close()" " built-in log failed")
                                                                  ;
        }
    }
...
{
    ngx_write_stderr("nginx version: " "nginx/" "1.15.5" "\x0a");
    ...
    if (ngx_show_help) {
        ngx_write_stderr(
            ...
            "  -p prefix     : set prefix path (default: " "/home/raoxue/Desktop/IF-driver-partition/FIPA/examples/nginx/output/temp/nginx_install_64/" ")"
                               "\x0a"
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    if (v != p) {
        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "invalid socket number \"%s\" in " "NGINX" " environment variable, ignoring", v)
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    *p = '\0';
    ...
    env[n] = 0;
# 705 "src/core/nginx.c"
    ctx.envp = (char *const *) env;

    ccf = (ngx_core_conf_t *) cycle->conf_ctx[ngx_core_module.index];

    if (rename((const char *) ccf->pid.data, (const char *) ccf->oldpid.data) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "rename()" " %s to %s failed " "before executing new binary process \"%s\"", ccf->pid.data, ccf->oldpid.data, argv[0])
    ...
    if (pid == -1) {
        if (rename((const char *) ccf->oldpid.data, (const char *) ccf->pid.data)
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "rename()" " %s back to %s failed after " "an attempt to execute new binary process \"%s\"", ccf->oldpid.data, ccf->pid.data, argv[0])
...
# 962 "src/core/nginx.c"
        (&cycle->conf_prefix)->len = sizeof("conf/") - 1; (&cycle->conf_prefix)->data = (u_char *) "conf/";



        (&cycle->prefix)->len = sizeof("/home/raoxue/Desktop/IF-driver-partition/FIPA/examples/nginx/output/temp/nginx_install_64/") - 1; (&cycle->prefix)->data = (u_char *) "/home/raoxue/Desktop/IF-driver-partition/FIPA/examples/nginx/output/temp/nginx_install_64/";
...
ngx_core_module_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    if (ccf->shutdown_timeout == (ngx_msec_t) -1) { ccf->shutdown_timeout = 0; };
    ...
    if (ccf->user == (uid_t) (ngx_uint_t) -1 && geteuid() == 0) {
        ...
        pwd = getpwnam("nobody");
        if (pwd == 0) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "getpwnam(\"" "nobody" "\") failed")
                                                              ;
            return (void *) -1;
        }
        ...
        if (grp == 0) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "getgrnam(\"" "nogroup" "\") failed")
                                                               ;
            return (void *) -1;
        }
...
ngx_set_cpu_affinity(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (strcmp((const char *) value[1].data, (const char *) "auto") == 0) {
        ...
        CPU_ZERO(&mask[0]);
        for (i = 0; i < (ngx_uint_t) ((ngx_ncpu > CPU_SETSIZE) ? (CPU_SETSIZE) : (ngx_ncpu)); i++) {
            CPU_SET(i, &mask[0]);
        }
...
ngx_load_module(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_conf_full_name(cf->cycle, &file, 0) != 0) {
        return (void *) -1;
    }
    ...
    handle = dlopen((char *) file.data, RTLD_NOW | RTLD_GLOBAL);
    if (handle == 0) {
        ngx_conf_log_error(1, cf, 0,
                           "dlopen()" " \"%s\" failed (%s)",
    ...
    modules = dlsym(handle, "ngx_modules");
    if (modules == 0) {
        ngx_conf_log_error(1, cf, 0,
                           "dlsym()" " \"%V\", \"%s\" failed (%s)",
    ...
    names = dlsym(handle, "ngx_module_names");
    if (names == 0) {
        ngx_conf_log_error(1, cf, 0,
                           "dlsym()" " \"%V\", \"%s\" failed (%s)",
    ...
    order = dlsym(handle, "ngx_module_order");

    for (i = 0; modules[i]; i++) {
        module = modules[i];
        module->name = names[i];

        if (ngx_add_module(cf, &file, module, order) != 0) {
            return (void *) -1;
        }
...
ngx_unload_module(void *data)
{
    ...
    if (dlclose(handle) != 0) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "dlclose()" " failed (%s)", ngx_dlerror())
                                                                  ;
    }
}
