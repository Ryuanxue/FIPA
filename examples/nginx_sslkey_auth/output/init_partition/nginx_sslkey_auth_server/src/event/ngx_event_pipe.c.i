...
# 9 "src/event/ngx_event_pipe.c" 2
...
# 10 "src/event/ngx_event_pipe.c" 2
...
# 11 "src/event/ngx_event_pipe.c" 2
...
# 12 "src/event/ngx_event_pipe.c" 2
...
static inline void ngx_event_pipe_remove_shadow_links(ngx_buf_t *buf);
...
ngx_event_pipe(ngx_event_pipe_t *p, ngx_int_t do_write)
{
    ...
    for ( ;; ) {
        ...
        if (ngx_event_pipe_read_upstream(p) == -6) {
            return -6;
        }
    ...
    if (p->upstream->fd != (ngx_socket_t) -1) {
        ...
        flags = (rev->eof || rev->error) ? 1 : 0;

        if (ngx_handle_read_event(rev, flags) != 0) {
            return -6;
        }
...
# 136 "src/event/ngx_event_pipe.c"
    ...
    for ( ;; ) {
        ...
        if (p->preread_bufs == 0 && !p->upstream->read->ready) {
            break;
        }
...
# 197 "src/event/ngx_event_pipe.c"
            if (p->limit_rate) {
                if (p->upstream->read->delayed) {
                    break;
                }

                limit = (off_t) p->limit_rate * (ngx_cached_time->sec - p->start_sec + 1)
                        - p->read_length;
        ...
        while (cl && n > 0) {
            ...
            if (n >= size) {
                ...
                ln = cl;
                cl = cl->next;
                ln->next = p->pool->chain; p->pool->chain = ln;
...
# 440 "src/event/ngx_event_pipe.c"
    if (p->free_raw_bufs && p->length != -1) {
        ...
        if (cl->buf->last - cl->buf->pos >= p->length) {
            ...
            cl->next = p->pool->chain; p->pool->chain = cl;
        }
    }
    ...
    if ((p->upstream_eof || p->upstream_error) && p->free_raw_bufs) {
        ...
        if (p->input_filter(p, p->free_raw_bufs->buf) == -1) {
            return -6;
        }

        p->free_raw_bufs = p->free_raw_bufs->next;

        if (p->free_bufs && p->buf_to_file == 0) {
...
ngx_event_pipe_write_to_downstream(ngx_event_pipe_t *p)
{
    ...
    downstream = p->downstream;
                                                                         ...
                                                                         ;
# 524 "src/event/ngx_event_pipe.c"
    flushed = 0;
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            if (p->out) {
                ...
                if (cl->buf->recycled) {
                    if ((p->log)->log_level >= 2) ngx_log_error_core(2, p->log, 0, "recycled buffer in pipe out chain")
                                                                      ;
                }
    ...
    flush:
        ...
        for (cl = p->free; cl; cl = cl->next) {
            ...
            if (cl->buf->last_shadow) {
                if (ngx_event_pipe_add_free_buf(p, cl->buf->shadow) != 0) {
                    return -6;
                }
...
# 763 "src/event/ngx_event_pipe.c"
    if (p->buf_to_file) {
...
# 839 "src/event/ngx_event_pipe.c"
    n = ngx_write_chain_to_temp_file(p->temp_file, out);

    if (n == -1) {
        return -6;
    }
# 857 "src/event/ngx_event_pipe.c"
    if (p->buf_to_file) {
