...
# 9 "src/event/modules/ngx_epoll_module.c" 2
...
# 10 "src/event/modules/ngx_epoll_module.c" 2
...
# 11 "src/event/modules/ngx_epoll_module.c" 2
# 98 "src/event/modules/ngx_epoll_module.c"
typedef struct {
    ngx_uint_t events;
...
# 154 "src/event/modules/ngx_epoll_module.c"
ngx_uint_t ngx_use_epoll_rdhup;


static ngx_str_t epoll_name = { sizeof("epoll") - 1, (u_char *) "epoll" };

static ngx_command_t ngx_epoll_commands[] = {

    { { sizeof("epoll_events") - 1, (u_char *) "epoll_events" },
      0x02000000|0x00000002,
      ngx_conf_set_num_slot,
      0,
      
# 165 "src/event/modules/ngx_epoll_module.c" 3 4
     0
# 165 "src/event/modules/ngx_epoll_module.c"
                                       ,
      0 },

    { { sizeof("worker_aio_requests") - 1, (u_char *) "worker_aio_requests" },
      0x02000000|0x00000002,
      ngx_conf_set_num_slot,
      0,
      
# 172 "src/event/modules/ngx_epoll_module.c" 3 4
     0
# 172 "src/event/modules/ngx_epoll_module.c"
                                             ,
      0 },
...
# 322 "src/event/modules/ngx_epoll_module.c"
static ngx_int_t
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ngx_epoll_conf_t *epcf;

    epcf = (*(cycle->conf_ctx[ngx_events_module.index])) [ngx_epoll_module.ctx_index];;

    if (ep == -1) {
        ep = epoll_create(cycle->connection_n / 2);

        if (ep == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "epoll_create() failed")
                                                  ;
            return -1;
        }


        if (ngx_epoll_notify_init(cycle->log) != 0) {
            ngx_epoll_module_ctx.actions.notify = 0;
        }
...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    if (notify_fd == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "eventfd() failed");
        return -1;
    }
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, notify_fd, &ee) == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "epoll_ctl(EPOLL_CTL_ADD, eventfd) failed")
                                                                 ;

        if (close(notify_fd) == -1) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "eventfd close() failed")
                                                     ;
        }
...
ngx_epoll_notify_handler(ngx_event_t *ev)
{
    ...
    if (++ev->index == (uint32_t) 0xffffffff) {
        ev->index = 0;
        ...
        if ((size_t) n != sizeof(uint64_t)) {
            if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, err, "read() eventfd %d failed", notify_fd)
                                                                ;
        }
    }
...
ngx_epoll_test_rdhup(ngx_cycle_t *cycle)
{
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, s) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "socketpair() failed")
                                            ;
        return;
    }
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, s[0], &ee) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "epoll_ctl() failed")
                                           ;
        goto failed;
    }

    if (close(s[1]) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "close() failed")
                                       ;
        s[1] = -1;
        goto failed;
    }
    ...
    if (events == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "epoll_wait() failed")
                                            ;
        goto failed;
    }
    ...
    if (events) {
        ...
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, ETIMEDOUT, "epoll_wait() timed out")
                                               ;
    }
...
{
    if (close(ep) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "epoll close() failed")
                                             ;
    }
    ...
    if (close(notify_fd) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "eventfd close() failed")
                                               ;
    }

    notify_fd = -1;
# 571 "src/event/modules/ngx_epoll_module.c"
    free(event_list);
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (epoll_ctl(ep, op, c->fd, &ee) == -1) {
        if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, errno, "epoll_ctl(%d, %d) failed", op, c->fd)
                                                            ;
        return -1;
    }
...
ngx_epoll_add_connection(ngx_connection_t *c)
{
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, c->fd, &ee) == -1) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, errno, "epoll_ctl(EPOLL_CTL_ADD, %d) failed", c->fd)
                                                                   ;
        return -1;
    }
...
ngx_epoll_del_connection(ngx_connection_t *c, ngx_uint_t flags)
{
    ...
    if (epoll_ctl(ep, op, c->fd, &ee) == -1) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, errno, "epoll_ctl(%d, %d) failed", op, c->fd)
                                                            ;
        return -1;
    }
...
ngx_epoll_notify(ngx_event_handler_pt handler)
{
    ...
    if ((size_t) write(notify_fd, &inc, sizeof(uint64_t)) != sizeof(uint64_t)) {
        if ((notify_event.log)->log_level >= 2) ngx_log_error_core(2, notify_event.log, errno, "write() to eventfd %d failed", notify_fd)
                                                                ;
        return -1;
    }
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    if (flags & 1 || ngx_event_timer_alarm) {
        ngx_time_update();
    }
    ...
    if (err) {
        ...
        if ((cycle->log)->log_level >= level) ngx_log_error_core(level, cycle->log, err, "epoll_wait() failed");
        return -1;
    }
    ...
    if (events == 0) {
        ...
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "epoll_wait() returned no events without timeout")
                                                                        ;
        return -1;
    }
...
# 883 "src/event/modules/ngx_epoll_module.c"
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & 2) {
                ...
                if (!(rev)->posted) { (rev)->posted = 1; (&(rev)->queue)->prev = (queue)->prev; (&(rev)->queue)->prev->next = &(rev)->queue; (&(rev)->queue)->next = queue; (queue)->prev = &(rev)->queue; ; } else { ; };
        ...
        if ((revents & EPOLLOUT) && wev->active) {
            ...
            if (flags & 2) {
                if (!(wev)->posted) { (wev)->posted = 1; (&(wev)->queue)->prev = (&ngx_posted_events)->prev; (&(wev)->queue)->prev->next = &(wev)->queue; (&(wev)->queue)->next = &ngx_posted_events; (&ngx_posted_events)->prev = &(wev)->queue; ; } else { ; };
...
# 1026 "src/event/modules/ngx_epoll_module.c"
static void *
...
ngx_epoll_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    if (epcf->events == (ngx_uint_t) -1) { epcf->events = 512; };
    if (epcf->aio_requests == (ngx_uint_t) -1) { epcf->aio_requests = 32; };
