...
ngx_libc_crypt(ngx_pool_t *pool, u_char *key, u_char *salt, u_char **encrypted)
{
    ...
    if (value) {
        ...
        *encrypted = ngx_pnalloc(pool, len);
        ...
        (void) memcpy(*encrypted, value, len);
        return 0;
    }

    if ((pool->log)->log_level >= 3) ngx_log_error_core(3, pool->log, errno, "crypt_r() failed");
