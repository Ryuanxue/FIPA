# 1 "src/os/unix/ngx_posix_init.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/os/unix/ngx_posix_init.c" 2
...
# 10 "src/os/unix/ngx_posix_init.c" 2
...
ngx_os_init(ngx_log_t *log)
{
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "getrlimit(RLIMIT_NOFILE) failed")
                                                        ;
        return -1;
    }
...
{
    if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "nginx/" "1.15.5");


    if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "built by " "gcc 9.4.0 (Ubuntu 9.4.0-1ubuntu1~20.04.2) ");
    ...
    if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "getrlimit(RLIMIT_NOFILE): %r:%r", rlmt.rlim_cur, rlmt.rlim_max)
