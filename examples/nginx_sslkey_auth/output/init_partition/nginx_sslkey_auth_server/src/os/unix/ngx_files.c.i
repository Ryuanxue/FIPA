...
# 10 "src/os/unix/ngx_files.c" 2
# 18 "src/os/unix/ngx_files.c"
...
ngx_read_file(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
    ...
    if (n == -1) {
        if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, errno, "pread() \"%s\" failed", file->name.data)
                                                               ;
        return -1;
    }
# 72 "src/os/unix/ngx_files.c"
    file->offset += n;

    return n;
}
# 192 "src/os/unix/ngx_files.c"
ssize_t
ngx_write_file(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
    ...
    for ( ;; ) {
        ...
        if (n == -1) {
            ...
            if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, err, "pwrite() \"%s\" failed", file->name.data)
                                                                    ;
            return -1;
        }
...
ngx_chain_to_iovec(ngx_iovec_t *vec, ngx_chain_t *cl)
{
    ...
    for ( ; cl; cl = cl->next) {
        ...
        if (((cl->buf->flush || cl->buf->last_buf || cl->buf->sync) && !(cl->buf->temporary || cl->buf->memory || cl->buf->mmap) && !cl->buf->in_file)) {
            continue;
        }
...
eintr:
    ...
    if (n == -1) {
        ...
        if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, err, "pwritev() \"%s\" failed", file->name.data)
                                                                 ;
        return -1;
    }

    if ((size_t) n != vec->size) {
        if ((file->log)->log_level >= 3) ngx_log_error_core(3, file->log, 0, "pwritev() \"%s\" has written only %z of %uz", file->name.data, n, vec->size)

                                                    ;
        return -1;
    }
# 468 "src/os/unix/ngx_files.c"
    file->offset += n;

    return n;
}
# 600 "src/os/unix/ngx_files.c"
ngx_int_t
ngx_set_file_time(u_char *name, ngx_fd_t fd, time_t s)
{
    struct timeval tv[2];

    tv[0].tv_sec = ngx_cached_time->sec;
    tv[0].tv_usec = 0;
    tv[1].tv_sec = s;
    tv[1].tv_usec = 0;
...
{
    fm->fd = open((const char *) fm->name, O_RDWR|(O_CREAT|O_TRUNC), 0644)
                                                   ;

    if (fm->fd == -1) {
        if ((fm->log)->log_level >= 3) ngx_log_error_core(3, fm->log, errno, "open()" " \"%s\" failed", fm->name)
                                                                 ;
        return -1;
    }

    if (ftruncate(fm->fd, fm->size) == -1) {
        if ((fm->log)->log_level >= 3) ngx_log_error_core(3, fm->log, errno, "ftruncate() \"%s\" failed", fm->name)
                                                            ;
        goto failed;
    }

    fm->addr = mmap(0, fm->size, PROT_READ|PROT_WRITE, MAP_SHARED,
                    fm->fd, 0);
    if (fm->addr != MAP_FAILED) {
        return 0;
    }

    if ((fm->log)->log_level >= 3) ngx_log_error_core(3, fm->log, errno, "mmap(%uz) \"%s\" failed", fm->size, fm->name)
                                                                ;

failed:

    if (close(fm->fd) == -1) {
        if ((fm->log)->log_level >= 2) ngx_log_error_core(2, fm->log, errno, "close()" " \"%s\" failed", fm->name)
                                                                  ;
    }
...
ngx_close_file_mapping(ngx_file_mapping_t *fm)
{
    if (munmap(fm->addr, fm->size) == -1) {
        if ((fm->log)->log_level >= 3) ngx_log_error_core(3, fm->log, errno, "munmap(%uz) \"%s\" failed", fm->size, fm->name)
                                                                      ;
    }
...
ngx_open_glob(ngx_glob_t *gl)
{
    ...
    n = glob((char *) gl->pattern, 0, 0, &gl->pglob);

    if (n == 0) {
        return 0;
    }
# 723 "src/os/unix/ngx_files.c"
    return -1;
}
...
ngx_read_glob(ngx_glob_t *gl, ngx_str_t *name)
{
    ...
    if (gl->n < count) {
        ...
        name->len = (size_t) strlen((const char *) gl->pglob.gl_pathv[gl->n]);
