    ...
    char *name, ngx_int_t respawn)
{
    ...
    if (respawn != -5) {
        ...
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "socketpair() failed while spawning \"%s\"", name)
                                                                            ;
            return -1;
        }
        ...
        if (ngx_nonblocking(ngx_processes[s].channel[0]) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "ioctl(FIONBIO)" " failed while spawning \"%s\"", name)
        ...
        if (ioctl(ngx_processes[s].channel[0], FIOASYNC, &on) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "ioctl(FIOASYNC) failed while spawning \"%s\"", name)
                                                                               ;
            ngx_close_channel(ngx_processes[s].channel, cycle->log);
            return -1;
        }
        ...
        if (fcntl(ngx_processes[s].channel[0], F_SETOWN, ngx_pid) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "fcntl(F_SETOWN) failed while spawning \"%s\"", name)
                                                                               ;
            ngx_close_channel(ngx_processes[s].channel, cycle->log);
            return -1;
        }
        ...
        if (fcntl(ngx_processes[s].channel[0], F_SETFD, FD_CLOEXEC) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "fcntl(FD_CLOEXEC) failed while spawning \"%s\"", name)
    ...
    case -1:
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "fork() failed while spawning \"%s\"", name)
                                                                  ;
        ngx_close_channel(ngx_processes[s].channel, cycle->log);
        return -1;
...
ngx_execute_proc(ngx_cycle_t *cycle, void *data)
{
    ...
    if (execve(ctx->path, ctx->argv, ctx->envp) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "execve() failed while executing %s \"%s\"", ctx->name, ctx->path)
...
ngx_init_signals(ngx_log_t *log)
{
    ...
    for (sig = signals; sig->signo != 0; sig++) {
        ...
        if (sigaction(sig->signo, &sa, 0) == -1) {




            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "sigaction(%s) failed", sig->signame)
                                                               ;
            return -1;
...
# 384 "src/os/unix/ngx_process.c"
    ...
    if (siginfo && siginfo->si_pid) {
        if ((ngx_cycle->log)->log_level >= 6) ngx_log_error_core(6, ngx_cycle->log, 0, "signal %d (%s) received from %P%s", signo, sig->signame, siginfo->si_pid, action)
...
# 509 "src/os/unix/ngx_process.c"
            if (err == ECHILD) {
                if ((ngx_cycle->log)->log_level >= 7) ngx_log_error_core(7, ngx_cycle->log, err, "waitpid() failed")
                                                 ;
                return;
            }
        ...
        if (WTERMSIG(status)) {
            ...
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "%s %P exited on signal %d", process, pid, WTERMSIG(status))

                                                         ;


        } else {
            if ((ngx_cycle->log)->log_level >= 6) ngx_log_error_core(6, ngx_cycle->log, 0, "%s %P exited with code %d", process, pid, WEXITSTATUS(status))

                                                            ;
        }

        if (WEXITSTATUS(status) == 2 && ngx_processes[i].respawn) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "%s %P exited with fatal code %d " "and cannot be respawned", process, pid, WEXITSTATUS(status))
...
ngx_os_signal_process(ngx_cycle_t *cycle, char *name, ngx_pid_t pid)
{
    ...
    for (sig = signals; sig->signo != 0; sig++) {
        if (strcmp((const char *) name, (const char *) sig->name) == 0) {
            ...
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "kill(%P, %d) failed", pid, sig->signo)
                                                                 ;
        }
    }
