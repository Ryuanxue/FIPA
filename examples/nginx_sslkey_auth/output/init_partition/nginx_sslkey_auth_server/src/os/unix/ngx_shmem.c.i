...
{
    shm->addr = (u_char *) mmap(0, shm->size,
                                PROT_READ|PROT_WRITE,
                                MAP_ANON|MAP_SHARED, -1, 0);

    if (shm->addr == MAP_FAILED) {
        if ((shm->log)->log_level >= 2) ngx_log_error_core(2, shm->log, errno, "mmap(MAP_ANON|MAP_SHARED, %uz) failed", shm->size)
                                                                         ;
        return -1;
    }
...
ngx_shm_free(ngx_shm_t *shm)
{
    if (munmap((void *) shm->addr, shm->size) == -1) {
        if ((shm->log)->log_level >= 2) ngx_log_error_core(2, shm->log, errno, "munmap(%p, %uz) failed", shm->addr, shm->size)
                                                                     ;
    }
}
