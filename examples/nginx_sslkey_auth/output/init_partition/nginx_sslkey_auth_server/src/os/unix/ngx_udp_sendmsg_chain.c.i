# 1 "src/os/unix/ngx_udp_sendmsg_chain.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/os/unix/ngx_udp_sendmsg_chain.c" 2
...
# 10 "src/os/unix/ngx_udp_sendmsg_chain.c" 2
...
# 11 "src/os/unix/ngx_udp_sendmsg_chain.c" 2
...
# 47 "src/os/unix/ngx_udp_sendmsg_chain.c"
    if (limit == 0 || limit > (off_t) (9223372036854775807LL - ngx_pagesize)) {
        limit = 9223372036854775807LL - ngx_pagesize;
    }
    ...
    for ( ;; ) {
        ...
        if (cl && cl->buf->in_file) {
            if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "file buf in sendmsg " "t:%d r:%d f:%d %p %p-%p %p %O-%O", cl->buf->temporary, cl->buf->recycled, cl->buf->in_file, cl->buf->start, cl->buf->pos, cl->buf->last, cl->buf->file, cl->buf->file_pos, cl->buf->file_last)
# 78 "src/os/unix/ngx_udp_sendmsg_chain.c"
                                             ;
...
ngx_udp_output_chain_to_iovec(ngx_iovec_t *vec, ngx_chain_t *in, ngx_log_t *log)
{
    ...
    for ( ; in && !flush; in = in->next) {
        ...
        if (!(in->buf->temporary || in->buf->memory || in->buf->mmap)) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "bad buf in output chain " "t:%d r:%d f:%d %p %p-%p %p %O-%O", in->buf->temporary, in->buf->recycled, in->buf->in_file, in->buf->start, in->buf->pos, in->buf->last, in->buf->file, in->buf->file_pos, in->buf->file_last)
# 155 "src/os/unix/ngx_udp_sendmsg_chain.c"
                                             ;
        ...
        if (prev == in->buf->pos) {
            ...
            if (n == vec->nalloc) {
                if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "too many parts in a datagram")
                                                             ;
                return (ngx_chain_t *) -1;
            }
...
# 257 "src/os/unix/ngx_udp_sendmsg_chain.c"
        if (c->local_sockaddr->sa_family == AF_INET) {
            ...
            (void) memset(pkt, 0, sizeof(struct in_pktinfo));
        ...
        if (c->local_sockaddr->sa_family == AF_INET6) {
            ...
            (void) memset(pkt6, 0, sizeof(struct in6_pktinfo));
