# 1 "src/os/unix/ngx_readv_chain.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/os/unix/ngx_readv_chain.c" 2
...
# 10 "src/os/unix/ngx_readv_chain.c" 2
...
# 11 "src/os/unix/ngx_readv_chain.c" 2
...
ngx_readv_chain(ngx_connection_t *c, ngx_chain_t *chain, off_t limit)
{
    ...
    ngx_array_t vec;
    ngx_event_t *rev;
    struct iovec *iov, iovs[IOV_MAX];

    rev = c->read;
# 58 "src/os/unix/ngx_readv_chain.c"
    ...
    do {
        ...
        if (n == 0) {
            rev->ready = 0;
            rev->eof = 1;
# 140 "src/os/unix/ngx_readv_chain.c"
            return 0;
        }

        if (n > 0) {
# 170 "src/os/unix/ngx_readv_chain.c"
            if ((ngx_event_flags & 0x00000040)
                && ngx_use_epoll_rdhup)
            {
                if (n < size) {
            ...
            if (n < size && !(ngx_event_flags & 0x00000020)) {
                rev->ready = 0;
            }
