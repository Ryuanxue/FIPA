...
# 10 "src/os/unix/ngx_channel.c" 2
# 1 "src/os/unix/ngx_channel.h" 1
# 14 "src/os/unix/ngx_channel.h"
...
# 15 "src/os/unix/ngx_channel.h" 2
...
# 11 "src/os/unix/ngx_channel.c" 2
...
# 53 "src/os/unix/ngx_channel.c"
        (void) memcpy(CMSG_DATA(&cmsg.cm), &ch->fd, sizeof(int));
    }

    msg.msg_flags = 0;
# 71 "src/os/unix/ngx_channel.c"
    ...
    if (n == -1) {
        ...
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, err, "sendmsg() failed");
        return -1;
    }
...
ngx_read_channel(ngx_socket_t s, ngx_channel_t *ch, size_t size, ngx_log_t *log)
{
    ...
    if (n == -1) {
        ...
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, err, "recvmsg() failed");
        return -1;
    }
    ...
    if ((size_t) n < sizeof(ngx_channel_t)) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "recvmsg() returned not enough data: %z", n)
                                                                  ;
        return -1;
    }
    ...
    if (ch->command == 1) {
        ...
        if (cmsg.cm.cmsg_len < (socklen_t) CMSG_LEN(sizeof(int))) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "recvmsg() returned too small ancillary data")
                                                                        ;
            return -1;
        }
        ...
        {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "recvmsg() returned invalid ancillary data " "level %d or type %d", cmsg.cm.cmsg_level, cmsg.cm.cmsg_type)
        ...
        (void) memcpy(&ch->fd, CMSG_DATA(&cmsg.cm), sizeof(int));
    }

    if (msg.msg_flags & (MSG_TRUNC|MSG_CTRUNC)) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "recvmsg() truncated data")
                                                 ;
    }
# 194 "src/os/unix/ngx_channel.c"
    return n;
}
    ...
    ngx_event_handler_pt handler)
{
    ...
    if (ngx_event_actions.add_conn && (ngx_event_flags & 0x00000040) == 0) {
...
{
    if (close(fd[0]) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close() channel failed");
    }
