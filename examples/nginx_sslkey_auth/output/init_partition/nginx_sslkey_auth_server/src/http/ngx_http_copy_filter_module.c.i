# 1 "src/http/ngx_http_copy_filter_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/ngx_http_copy_filter_module.c" 2
...
# 10 "src/http/ngx_http_copy_filter_module.c" 2
...
# 11 "src/http/ngx_http_copy_filter_module.c" 2


typedef struct {
    ngx_bufs_t bufs;
} ngx_http_copy_filter_conf_t;
# 33 "src/http/ngx_http_copy_filter_module.c"
...
static ngx_command_t ngx_http_copy_filter_commands[] = {
    ...
    { { sizeof("output_buffers") - 1, (u_char *) "output_buffers" },
      0x02000000|0x04000000|0x08000000|0x00000004,
      ngx_conf_set_bufs_slot,
      
# 44 "src/http/ngx_http_copy_filter_module.c" 3 4
     0
# 44 "src/http/ngx_http_copy_filter_module.c"
                             ,
      
# 45 "src/http/ngx_http_copy_filter_module.c" 3 4
     0
# 45 "src/http/ngx_http_copy_filter_module.c"
                                                ,
      0 },
...
ngx_http_copy_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx = (r)->ctx[ngx_http_copy_filter_module.ctx_index];
    ...
    if (ctx == 0) {
        ...
        conf = (r)->loc_conf[ngx_http_copy_filter_module.ctx_index];
        ...
        ctx->filter_ctx = r;
# 143 "src/http/ngx_http_copy_filter_module.c"
        if (in && in->buf && ((in->buf->temporary || in->buf->memory || in->buf->mmap) ? (off_t) (in->buf->last - in->buf->pos): (in->buf->file_last - in->buf->file_pos))) {
            r->request_output = 1;
        }
    }
...
# 344 "src/http/ngx_http_copy_filter_module.c"
static void *
...
ngx_http_copy_filter_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->bufs.num == 0) { if (prev->bufs.num) { conf->bufs.num = prev->bufs.num; conf->bufs.size = prev->bufs.size; } else { conf->bufs.num = 2; conf->bufs.size = 32768; } };
