...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    ngx_http_max_module = ngx_count_modules(cf->cycle, 0x50545448);
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        ...
        if (module->preconfiguration) {
            if (module->preconfiguration(cf) != 0) {
                return (void *) -1;
            }
        }
    }
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != 0) {
            return (void *) -1;
        }

        if (ngx_http_init_static_location_trees(cf, clcf) != 0) {
            return (void *) -1;
        }
    }


    if (ngx_http_init_phases(cf, cmcf) != 0) {
        return (void *) -1;
    }

    if (ngx_http_init_headers_in_hash(cf, cmcf) != 0) {
        return (void *) -1;
    }
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        ...
        if (module->postconfiguration) {
            if (module->postconfiguration(cf) != 0) {
                return (void *) -1;
            }
        }
    }

    if (ngx_http_variables_init_vars(cf) != 0) {
        return (void *) -1;
    }
    ...
    *cf = pcf;
    ...
    if (ngx_http_init_phase_handlers(cf, cmcf) != 0) {
        return (void *) -1;
    }
    ...
    if (ngx_http_optimize_servers(cf, cmcf, cmcf->ports) != 0) {
        return (void *) -1;
    }
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
         ...
         q = (q)->next)
    {
        ...
        if (ngx_http_init_locations(cf, 0, clcf) != 0) {
            return -1;
        }
# 722 "src/http/ngx_http.c"
        if (clcf->named) {
            n++;
    ...
    if (q != (locations)) {
        (&tail)->prev = (locations)->prev; (&tail)->prev->next = &tail; (&tail)->next = q; (locations)->prev = (q)->prev; (locations)->prev->next = locations; (q)->prev = &tail;;
    }
    ...
    if (named) {
        ...
        *clcfp = 0;
        ...
        (&tail)->prev = (locations)->prev; (&tail)->prev->next = &tail; (&tail)->next = named; (locations)->prev = (named)->prev; (locations)->prev->next = locations; (named)->prev = &tail;;
    }
# 792 "src/http/ngx_http.c"
    return 0;
}
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
    ...
    if (ngx_http_join_exact_locations(cf, locations) != 0) {
        return -1;
    }
    ...
    ngx_http_core_loc_conf_t *clcf)
{
    ...
    if (*locations == 0) {
        *locations = ngx_palloc(cf->temp_pool,
        ...
        (*locations)->prev = *locations; (*locations)->next = *locations;
    }
    ...
    (&lq->list)->prev = &lq->list; (&lq->list)->next = &lq->list;

    (&lq->queue)->prev = (*locations)->prev; (&lq->queue)->prev->next = &lq->queue; (&lq->queue)->next = *locations; (*locations)->prev = &lq->queue;
...
ngx_http_cmp_locations(const ngx_queue_t *one, const ngx_queue_t *two)
{
    ...
    if (first->named && second->named) {
        return strcmp((const char *) first->name.data, (const char *) second->name.data);
    }
# 950 "src/http/ngx_http.c"
    rc = ngx_filename_cmp(first->name.data, second->name.data,
                          ((first->name.len > second->name.len) ? (second->name.len) : (first->name.len)) + 1);
    ...
    while (q != (locations)->prev) {
               ...
               == 0)
        {
            if ((lq->exact && lx->exact) || (lq->inclusive && lx->inclusive)) {
                if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "duplicate location \"%V\" in %s:%ui", lx->name, lx->file_name, lx->line)
...
ngx_http_create_locations_list(ngx_queue_t *locations, ngx_queue_t *q)
{
    ...
    (&tail)->prev = (locations)->prev; (&tail)->prev->next = &tail; (&tail)->next = q; (locations)->prev = (q)->prev; (locations)->prev->next = locations; (q)->prev = &tail;;
    (&lq->list)->prev->next = (&tail)->next; (&tail)->next->prev = (&lq->list)->prev; (&lq->list)->prev = (&tail)->prev; (&lq->list)->prev->next = &lq->list;;
    ...
    (&tail)->prev = (&lq->list)->prev; (&tail)->prev->next = &tail; (&tail)->next = x; (&lq->list)->prev = (x)->prev; (&lq->list)->prev->next = &lq->list; (x)->prev = &tail;;
    (locations)->prev->next = (&tail)->next; (&tail)->next->prev = (locations)->prev; (locations)->prev = (&tail)->prev; (locations)->prev->next = locations;;
...
# 1082 "src/http/ngx_http.c" 3 4
                     0 
# 1082 "src/http/ngx_http.c"
                                                                   + len);
    if (node == 0) {
        return 0;
    }
    ...
    node->len = (u_char) len;
    (void) memcpy(node->name, &lq->name->data[prefix], len);
...
# 1208 "src/http/ngx_http.c"
    ...
    for (i = 0; i < port->addrs.nelts; i++) {
        ...
        if (ngx_http_add_server(cf, cscf, &addr[i]) != 0) {
            return -1;
        }
    ...
    ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        for (a = 0; a < port[p].addrs.nelts; a++) {
            ...
            {
                if (ngx_http_server_names(cf, cmcf, &addr[a]) != 0) {
                    return -1;
                }
            }
        }

        if (ngx_http_init_listening(cf, &port[p]) != 0) {
            return -1;
        }
    }
    ...
    ngx_http_conf_addr_t *addr)
{
    ...
    (void) memset(&ha, 0, sizeof(ngx_hash_keys_arrays_t));

    ha.temp_pool = ngx_create_pool((16 * 1024), cf->log);
    if (ha.temp_pool == 0) {
        return -1;
    }
    ...
    if (ngx_hash_keys_array_init(&ha, 2) != 0) {
        goto failed;
    }
...
# 1465 "src/http/ngx_http.c"
            ...
            if (rc == -5) {
                if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "invalid server name or wildcard \"%V\" on %s", &name[n].name, addr->opt.addr)
            ...
            if (rc == -3) {
                if ((cf->log)->log_level >= 5) ngx_log_error_core(5, cf->log, 0, "conflicting server name \"%V\" on %s, ignored", &name[n].name, addr->opt.addr)
    ...
    if (ha.keys.nelts) {
        ...
        if (ngx_hash_init(&hash, ha.keys.elts, ha.keys.nelts) != 0) {
            goto failed;
        }
    }
    ...
    if (ha.dns_wc_head.nelts) {
        ...
        qsort(ha.dns_wc_head.elts, (size_t) ha.dns_wc_head.nelts,
    ...
    if (ha.dns_wc_tail.nelts) {
        ...
        qsort(ha.dns_wc_tail.elts, (size_t) ha.dns_wc_tail.nelts,
                  sizeof(ngx_hash_key_t), ngx_http_cmp_dns_wildcards);
...
# 1641 "src/http/ngx_http.c"
    ...
    while (i < last) {
        ...
        case AF_INET6:
            if (ngx_http_add_addrs6(cf, hport, addr) != 0) {
                return -1;
            }
            break;

        default:
            if (ngx_http_add_addrs(cf, hport, addr) != 0) {
                return -1;
            }
            break;
        }
    ...
    ngx_http_conf_addr_t *addr)
{
    ...
    for (i = 0; i < hport->naddrs; i++) {
        ...
        if (addr[i].hash.buckets == 0
            && (addr[i].wc_head == 0
                || addr[i].wc_head->hash.buckets == 0)
            && (addr[i].wc_tail == 0
                || addr[i].wc_tail->hash.buckets == 0)
    ...
    ngx_str_t *default_types)
{
    ...
    if (*keys) {
        ...
        if (ngx_hash_init(&hash, (*keys)->elts, (*keys)->nelts) != 0) {
            return (void *) -1;
        }
