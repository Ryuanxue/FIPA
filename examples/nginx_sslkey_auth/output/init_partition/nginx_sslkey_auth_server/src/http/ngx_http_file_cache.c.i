# 1 "src/http/ngx_http_file_cache.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/ngx_http_file_cache.c" 2
...
# 10 "src/http/ngx_http_file_cache.c" 2
...
# 11 "src/http/ngx_http_file_cache.c" 2
# 1 "src/core/ngx_md5.h" 1
# 16 "src/core/ngx_md5.h"
typedef struct {
    uint64_t bytes;
    uint32_t a, b, c, d;
    u_char buffer[64];
...
# 12 "src/http/ngx_http_file_cache.c" 2
...
static ssize_t ngx_http_file_cache_aio_read(ngx_http_request_t *r,
    ngx_http_cache_t *c);
# 31 "src/http/ngx_http_file_cache.c"
...
ngx_str_t ngx_http_cache_status[] = {
    { sizeof("MISS") - 1, (u_char *) "MISS" },
    { sizeof("BYPASS") - 1, (u_char *) "BYPASS" },
    { sizeof("EXPIRED") - 1, (u_char *) "EXPIRED" },
    { sizeof("STALE") - 1, (u_char *) "STALE" },
    { sizeof("UPDATING") - 1, (u_char *) "UPDATING" },
    { sizeof("REVALIDATED") - 1, (u_char *) "REVALIDATED" },
    { sizeof("HIT") - 1, (u_char *) "HIT" }
};


static u_char ngx_http_file_cache_key[] = { (u_char) '\n', 'K', 'E', 'Y', ':', ' ' };
...
ngx_http_file_cache_init(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (ocache) {
        if (strcmp((const char *) cache->path->name.data, (const char *) ocache->path->name.data) != 0) {
            if ((shm_zone->shm.log)->log_level >= 1) ngx_log_error_core(1, shm_zone->shm.log, 0, "cache \"%V\" uses the \"%V\" cache path " "while previously it used the \"%V\" cache path", &shm_zone->shm.name, &cache->path->name, &ocache->path->name)
        ...
        for (n = 0; n < 3; n++) {
            if (cache->path->level[n] != ocache->path->level[n]) {
                if ((shm_zone->shm.log)->log_level >= 1) ngx_log_error_core(1, shm_zone->shm.log, 0, "cache \"%V\" had previously different levels", &shm_zone->shm.name)
    ...
    ((&cache->sh->sentinel)->color = 0); (&cache->sh->rbtree)->root = &cache->sh->sentinel; (&cache->sh->rbtree)->sentinel = &cache->sh->sentinel; (&cache->sh->rbtree)->insert = ngx_http_file_cache_rbtree_insert_value
                                                            ;

    (&cache->sh->queue)->prev = &cache->sh->queue; (&cache->sh->queue)->next = &cache->sh->queue;
...
ngx_http_file_cache_new(ngx_http_request_t *r)
{
    ...
    if (ngx_array_init(&c->keys, r->pool, 4, sizeof(ngx_str_t)) != 0) {
        return -1;
    }
...
ngx_http_file_cache_create(ngx_http_request_t *r)
{
    ...
    if (ngx_http_file_cache_exists(cache, c) == -1) {
        return -1;
    }

    if (ngx_http_file_cache_name(r, cache->path) != 0) {
        return -1;
    }
...
ngx_http_file_cache_open(ngx_http_request_t *r)
{
    ...
    of.directio = 9223372036854775807LL;
        ...
        != 0)
    {
        ...
        default:
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, of.err, "open()" " \"%s\" failed", c->file.name.data)
                                                                              ;
            return -1;
        }
    }
...
ngx_http_file_cache_lock_wait(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if ((ngx_msec_int_t) timer <= 0) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "cache lock timeout")
                                           ;
        c->lock_timeout = 0;
        goto wakeup;
    }
...
ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if ((size_t) n < c->header_start) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" is too small", c->file.name.data)
                                                                          ;
        return -5;
    }
    ...
    if (h->version != 5) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "cache file \"%s\" version mismatch", c->file.name.data)
                                                                              ;
        return -5;
    }

    if (h->crc32 != c->crc32 || (size_t) h->header_start != c->header_start) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has md5 collision", c->file.name.data)
                                                                               ;
        return -5;
    }
    ...
    key = c->keys.elts;
    for (i = 0; i < c->keys.nelts; i++) {
        if (memcmp((const char *) p, (const char *) key[i].data, key[i].len) != 0) {
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has md5 collision", c->file.name.data)
    ...
    if ((size_t) h->body_start > c->body_start) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has too long header", c->file.name.data)
    ...
    if (h->vary_len > 128) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has incorrect vary length", c->file.name.data)
    ...
    if (h->vary_len) {
        ...
        if (memcmp((const char *) c->variant, (const char *) h->variant, 16) != 0) {
...
{
# 716 "src/http/ngx_http_file_cache.c"
    return ngx_read_file(&c->file, c->buf->pos, c->body_start, 0);
}
# 819 "src/http/ngx_http_file_cache.c"
static ngx_int_t
ngx_http_file_cache_exists(ngx_http_file_cache_t *cache, ngx_http_cache_t *c)
{
    ...
    if (fcn) {
        (&fcn->queue)->next->prev = (&fcn->queue)->prev; (&fcn->queue)->prev->next = (&fcn->queue)->next;
        ...
        if (fcn->error) {
            ...
            if (fcn->valid_sec < ngx_cached_time->sec) {
                goto renew;
            }
    ...
    if (fcn == 0) {
        ...
        fcn = ngx_slab_calloc_locked(cache->shpool,
                                     sizeof(ngx_http_file_cache_node_t));
        if (fcn == 0) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "could not allocate node%s", cache->shpool->log_ctx)
                                                                              ;
            rc = -1;
            goto failed;
        }
    ...
    (void) memcpy((u_char *) &fcn->node.key, c->key, sizeof(ngx_rbtree_key_t));

    (void) memcpy(fcn->key, &c->key[sizeof(ngx_rbtree_key_t)], 16 - sizeof(ngx_rbtree_key_t))
                                                                 ;
...
renew:
    ...
    fcn->expire = ngx_cached_time->sec + cache->inactive;

    (&fcn->queue)->next = (&cache->sh->queue)->next; (&fcn->queue)->next->prev = &fcn->queue; (&fcn->queue)->prev = &cache->sh->queue; (&cache->sh->queue)->next = &fcn->queue;
...
ngx_http_file_cache_name(ngx_http_request_t *r, ngx_path_t *path)
{
    ...
    if (c->file.name.data == 0) {
        return -1;
    }

    (void) memcpy(c->file.name.data, path->name.data, path->name.len);

    p = c->file.name.data + path->name.len + 1 + path->len;
    p = ngx_hex_dump(p, c->key, 16);
    *p = '\0';
...
ngx_http_file_cache_lookup(ngx_http_file_cache_t *cache, u_char *key)
{
    ...
    (void) memcpy((u_char *) &node_key, key, sizeof(ngx_rbtree_key_t));
    ...
    while (node != sentinel) {
        ...
        rc = memcmp((const char *) &key[sizeof(ngx_rbtree_key_t)], (const char *) fcn->key, 16 - sizeof(ngx_rbtree_key_t))
                                                                          ;
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            p = (memcmp((const char *) cn->key, (const char *) cnt->key, 16 - sizeof(ngx_rbtree_key_t))
    ...
    u_char *hash)
{
    ...
    ngx_md5_init(&md5);
    ngx_md5_update(&md5, r->cache->main, 16);
...
ngx_http_file_cache_reopen(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if (c->secondary) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has incorrect vary hash", c->file.name.data)
    ...
    (void) memcpy(c->key, c->variant, 16);
...
ngx_http_file_cache_set_header(ngx_http_request_t *r, u_char *buf)
{
    ...
    (void) memset(h, 0, sizeof(ngx_http_file_cache_header_t));
    ...
    if (c->etag.len <= 128) {
        ...
        (void) memcpy(h->etag, c->etag.data, c->etag.len);
    }
    ...
    if (c->vary.len) {
        ...
        (void) memcpy(h->vary, c->vary.data, c->vary.len);

        ngx_http_file_cache_vary(r, c->vary.data, c->vary.len, c->variant);
        (void) memcpy(h->variant, c->variant, 16);
    }

    if (ngx_http_file_cache_update_variant(r, c) != 0) {
        return -1;
    }

    p = buf + sizeof(ngx_http_file_cache_header_t);

    p = (((u_char *) memcpy(p, ngx_http_file_cache_key, sizeof(ngx_http_file_cache_key))) + (sizeof(ngx_http_file_cache_key)));

    key = c->keys.elts;
    for (i = 0; i < c->keys.nelts; i++) {
        p = (((u_char *) memcpy(p, key[i].data, key[i].len)) + (key[i].len));
    }
...
ngx_http_file_cache_update_variant(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if (c->vary.len
        && memcmp((const char *) c->variant, (const char *) c->key, 16) == 0)
    {
        return 0;
    }
...
ngx_http_file_cache_update(ngx_http_request_t *r, ngx_temp_file_t *tf)
{
    ...
    if (rc == 0) {
        ...
        if (fstat(tf->file.fd, &fi) == -1) {
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, errno, "fstat()" " \"%s\" failed", tf->file.name.data)
                                                                             ;
            ...
            uniq = (&fi)->st_ino;
            fs_size = ((((&fi)->st_size < (&fi)->st_blocks * 512) ? ((&fi)->st_blocks * 512) : ((&fi)->st_size)) + cache->bsize - 1) / cache->bsize;
        }
    }
...
ngx_http_file_cache_update_header(ngx_http_request_t *r)
{
    ...
    file.fd = open((const char *) file.name.data, O_RDWR|0, 0);
    ...
    if (file.fd == -1) {
        ...
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, err, "open()" " \"%s\" failed", file.name.data)
                                                                       ;
        return;
    }
    ...
    if (fstat(file.fd, &fi) == -1) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, errno, "fstat()" " \"%s\" failed", file.name.data)
                                                                     ;
        goto done;
    }
    ...
    if ((size_t) n != sizeof(ngx_http_file_cache_header_t)) {
        if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, 0, "pread()" " read only %z of %z from \"%s\"", n, sizeof(ngx_http_file_cache_header_t), file.name.data)
    ...
    if (close(file.fd) == -1) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, errno, "close()" " \"%s\" failed", file.name.data)
                                                                        ;
    }
}
...
ngx_http_file_cache_free(ngx_http_cache_t *c, ngx_temp_file_t *tf)
{
    ...
    if (c->temp_file) {
        if (tf && tf->file.fd != -1) {
            ...
            if (unlink((const char *) tf->file.name.data) == -1) {
                if ((c->file.log)->log_level >= 3) ngx_log_error_core(3, c->file.log, errno, "unlink()" " \"%s\" failed", tf->file.name.data)
...
ngx_http_file_cache_cleanup(void *data)
{
    ...
    if (c->updating && !c->background) {
        if ((c->file.log)->log_level >= 2) ngx_log_error_core(2, c->file.log, 0, "stalled cache updating, error:%ui", c->error)
                                                                    ;
    }

    ngx_http_file_cache_free(c, 0);
}
...
ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache)
{
    ...
    path = cache->path;
    len = path->name.len + 1 + path->len + 2 * 16;
    ...
    (void) memcpy(name, path->name.data, path->name.len);
    ...
    for ( ;; ) {
        if ((&cache->sh->queue == (&cache->sh->queue)->prev)) {
            break;
        }
        ...
        fcn = (ngx_http_file_cache_node_t *) ((u_char *) q - 
# 1743 "src/http/ngx_http_file_cache.c" 3 4
             0
# 1743 "src/http/ngx_http_file_cache.c"
             );
        ...
        p = ngx_hex_dump(key, (u_char *) &fcn->node.key,
                         sizeof(ngx_rbtree_key_t));
        len = 16 - sizeof(ngx_rbtree_key_t);
        ...
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "ignore long locked inactive cache entry %*s, count:%d", (size_t) 2 * 16, key, fcn->count)
...
ngx_http_file_cache_expire(ngx_http_file_cache_t *cache)
{
    ...
    for ( ;; ) {
        ...
        fcn = (ngx_http_file_cache_node_t *) ((u_char *) q - 
# 1838 "src/http/ngx_http_file_cache.c" 3 4
             0
# 1838 "src/http/ngx_http_file_cache.c"
             );
...
next:
        ...
        elapsed = ((((ngx_msec_int_t) (ngx_current_msec - cache->last)) >= 0) ? ((ngx_msec_int_t) (ngx_current_msec - cache->last)) : - ((ngx_msec_int_t) (ngx_current_msec - cache->last)));
    ...
    u_char *name)
{
    ...
    fcn = (ngx_http_file_cache_node_t *) ((u_char *) q - 
# 1915 "src/http/ngx_http_file_cache.c" 3 4
         0
# 1915 "src/http/ngx_http_file_cache.c"
         );
    ...
    if (fcn->exists) {
        ...
        *p = '\0';
        ...
        if (unlink((const char *) name) == -1) {
            if ((ngx_cycle->log)->log_level >= 3) ngx_log_error_core(3, ngx_cycle->log, errno, "unlink()" " \"%s\" failed", name)
                                                                   ;
        }
...
ngx_http_file_cache_loader(void *data)
{
    ...
    if (!__sync_bool_compare_and_swap(&cache->sh->loading, 0, ngx_pid)) {
        return;
    }
    ...
    if (ngx_walk_tree(&tree, &cache->path->name) == -6) {
        cache->sh->loading = 0;
        return;
    }
    ...
    if ((ngx_cycle->log)->log_level >= 6) ngx_log_error_core(6, ngx_cycle->log, 0, "http file cache: %V %.3fM, bsize: %uz", &cache->path->name, ((double) cache->sh->size * cache->bsize) / (1024 * 1024), cache->bsize)
...
ngx_http_file_cache_manage_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    if (ngx_http_file_cache_add_file(ctx, path) != 0) {
    ...
    return (ngx_quit || ngx_terminate) ? -6 : 0;
}
...
{
    if (path->len >= 5
        && strncmp((const char *) path->data + path->len - 5, (const char *) "/temp", 5) == 0)
    {
        return -5;
    }
...
{
    (void) usleep(cache->loader_sleep * 1000);
...
ngx_http_file_cache_add_file(ngx_tree_ctx_t *ctx, ngx_str_t *name)
{
    ...
    if (ctx->size < (off_t) sizeof(ngx_http_file_cache_header_t)) {
        if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, 0, "cache file \"%s\" is too small", name->data)
                                                                   ;
        return -1;
    }

    (void) memset(&c, 0, sizeof(ngx_http_cache_t));
    cache = ctx->data;

    c.length = ctx->size;
    c.fs_size = (ctx->fs_size + cache->bsize - 1) / cache->bsize;

    p = &name->data[name->len - 2 * 16];
...
ngx_http_file_cache_add(ngx_http_file_cache_t *cache, ngx_http_cache_t *c)
{
    ...
    if (fcn == 0) {
        ...
        if (fcn == 0) {
            ...
            if (cache->fail_time != ngx_cached_time->sec) {
                cache->fail_time = ngx_cached_time->sec;
                if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "could not allocate node%s", cache->shpool->log_ctx)
                                                                               ;
            }
...
ngx_http_file_cache_delete_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    if (unlink((const char *) path->data) == -1) {
        if ((ctx->log)->log_level >= 3) ngx_log_error_core(3, ctx->log, errno, "unlink()" " \"%s\" failed", path->data)
                                                                     ;
    }
...
ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    name.len = 0;
    size = 0;
    max_size = 9223372036854775807LL;
    ...
    if (ngx_conf_full_name(cf->cycle, &cache->path->name, 0) != 0) {
        return (void *) -1;
    }

    for (i = 2; i < cf->args->nelts; i++) {

        if (strncmp((const char *) value[i].data, (const char *) "levels=", 7) == 0) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "use_temp_path=", 14) == 0) {

            if (strcmp((const char *) &value[i].data[14], (const char *) "on") == 0) {
                use_temp_path = 1;

            } else if (strcmp((const char *) &value[i].data[14], (const char *) "off") == 0) {
                use_temp_path = 0;
        ...
        if (strncmp((const char *) value[i].data, (const char *) "keys_zone=", 10) == 0) {

            name.data = value[i].data + 10;

            p = (u_char *) strchr((const char *) name.data, (int) ':');
        ...
        if (strncmp((const char *) value[i].data, (const char *) "max_size=", 9) == 0) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "loader_files=", 13) == 0) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "loader_sleep=", 13) == 0) {

            s.len = value[i].len - 13;
            s.data = value[i].data + 13;

            loader_sleep = ngx_parse_time(&s, 0);
            if (loader_sleep == (ngx_msec_t) -1) {
                ngx_conf_log_error(1, cf, 0,
        ...
        if (strncmp((const char *) value[i].data, (const char *) "loader_threshold=", 17) == 0) {

            s.len = value[i].len - 17;
            s.data = value[i].data + 17;

            loader_threshold = ngx_parse_time(&s, 0);
            if (loader_threshold == (ngx_msec_t) -1) {
                ngx_conf_log_error(1, cf, 0,
        ...
        if (strncmp((const char *) value[i].data, (const char *) "manager_files=", 14) == 0) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "manager_sleep=", 14) == 0) {

            s.len = value[i].len - 14;
            s.data = value[i].data + 14;

            manager_sleep = ngx_parse_time(&s, 0);
            if (manager_sleep == (ngx_msec_t) -1) {
                ngx_conf_log_error(1, cf, 0,
        ...
        if (strncmp((const char *) value[i].data, (const char *) "manager_threshold=", 18) == 0) {

            s.len = value[i].len - 18;
            s.data = value[i].data + 18;

            manager_threshold = ngx_parse_time(&s, 0);
            if (manager_threshold == (ngx_msec_t) -1) {
                ngx_conf_log_error(1, cf, 0,
    ...
    if (ngx_add_path(cf, &cache->path) != 0) {
        return (void *) -1;
    }
    ...
    void *conf)
{
    ...
    for (i = 1; i < n; i++) {
        ...
        if (strcmp((const char *) value[i].data, (const char *) "any") == 0) {
