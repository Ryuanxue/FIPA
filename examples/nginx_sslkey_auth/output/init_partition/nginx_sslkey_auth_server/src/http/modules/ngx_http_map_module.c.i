# 1 "src/http/modules/ngx_http_map_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_map_module.c" 2
...
# 10 "src/http/modules/ngx_http_map_module.c" 2
...
# 11 "src/http/modules/ngx_http_map_module.c" 2
...
static ngx_command_t ngx_http_map_commands[] = {
    ...
    { { sizeof("map") - 1, (u_char *) "map" },
      0x02000000|0x00000100|0x00000004,
      ngx_http_map_block,
      
# 54 "src/http/modules/ngx_http_map_module.c" 3 4
     0
# 54 "src/http/modules/ngx_http_map_module.c"
                              ,
      0,
      0 },

    { { sizeof("map_hash_max_size") - 1, (u_char *) "map_hash_max_size" },
      0x02000000|0x00000002,
      ngx_conf_set_num_slot,
      
# 61 "src/http/modules/ngx_http_map_module.c" 3 4
     0
# 61 "src/http/modules/ngx_http_map_module.c"
                              ,
      
# 62 "src/http/modules/ngx_http_map_module.c" 3 4
     0
# 62 "src/http/modules/ngx_http_map_module.c"
                                                 ,
      0 },

    { { sizeof("map_hash_bucket_size") - 1, (u_char *) "map_hash_bucket_size" },
      0x02000000|0x00000002,
      ngx_conf_set_num_slot,
      
# 68 "src/http/modules/ngx_http_map_module.c" 3 4
     0
# 68 "src/http/modules/ngx_http_map_module.c"
                              ,
      
# 69 "src/http/modules/ngx_http_map_module.c" 3 4
     0
# 69 "src/http/modules/ngx_http_map_module.c"
                                                    ,
      0 },
    ...
    uintptr_t data)
{
    ...
    if (ngx_http_complex_value(r, &map->value, &val) != 0) {
        return -1;
    }
    ...
    if (!value->valid) {
        ...
        if (ngx_http_complex_value(r, cv, &str) != 0) {
            return -1;
        }
...
ngx_http_map_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (mcf->hash_max_size == (ngx_uint_t) -1) {
        mcf->hash_max_size = 2048;
    }

    if (mcf->hash_bucket_size == (ngx_uint_t) -1) {
        mcf->hash_bucket_size = ngx_cacheline_size;

    } else {
        mcf->hash_bucket_size = (((mcf->hash_bucket_size) + (ngx_cacheline_size - 1)) & ~(ngx_cacheline_size - 1))
                                                             ;
    }
    ...
    if (ngx_hash_keys_array_init(&ctx.keys, 2) != 0) {
        ngx_destroy_pool(pool);
        return (void *) -1;
    }

    ctx.values_hash = ngx_pcalloc(pool, sizeof(ngx_array_t) * ctx.keys.hsize);
    if (ctx.values_hash == 0) {
        ngx_destroy_pool(pool);
        return (void *) -1;
    }
# 266 "src/http/modules/ngx_http_map_module.c"
    ctx.default_value = 0;
    ctx.cf = &save;
    ctx.hostnames = 0;
    ctx.no_cacheable = 0;
    ...
    *cf = save;
    ...
    if (ctx.keys.dns_wc_head.nelts) {
        ...
        qsort(ctx.keys.dns_wc_head.elts,
    ...
    if (ctx.keys.dns_wc_tail.nelts) {
        ...
        qsort(ctx.keys.dns_wc_tail.elts,
...
# 362 "src/http/modules/ngx_http_map_module.c"
    ngx_destroy_pool(pool);
...
ngx_http_map(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (cf->args->nelts == 1
        && strcmp((const char *) value[0].data, (const char *) "hostnames") == 0)
    {
        ctx->hostnames = 1;
        return 0;
    }
    ...
    if (cf->args->nelts == 1
        && strcmp((const char *) value[0].data, (const char *) "volatile") == 0)
    {
        ctx->no_cacheable = 1;
        return 0;
    }
    ...
    for (i = 0; i < value[1].len; i++) {
        key = ((ngx_uint_t) key * 31 + value[1].data[i]);
    }
    ...
    if (vp) {
        for (i = 0; i < ctx->values_hash[key].nelts; i++) {
            ...
            if (strncmp((const char *) value[1].data, (const char *) data, len) == 0) {
                var = vp[i];
                goto found;
            }
        }
...
# 566 "src/http/modules/ngx_http_map_module.c"
