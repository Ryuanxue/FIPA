# 1 "src/http/modules/ngx_http_autoindex_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_autoindex_module.c" 2
...
# 10 "src/http/modules/ngx_http_autoindex_module.c" 2
...
# 11 "src/http/modules/ngx_http_autoindex_module.c" 2
# 26 "src/http/modules/ngx_http_autoindex_module.c"
typedef struct {
    ngx_str_t name;
    size_t utf_len;
    size_t escape;
...
# 58 "src/http/modules/ngx_http_autoindex_module.c"
...
static ngx_conf_enum_t ngx_http_autoindex_format[] = {
    { { sizeof("html") - 1, (u_char *) "html" }, 0 },
    { { sizeof("json") - 1, (u_char *) "json" }, 1 },
    { { sizeof("jsonp") - 1, (u_char *) "jsonp" }, 2 },
    { { sizeof("xml") - 1, (u_char *) "xml" }, 3 },
    { { 0, 0 }, 0 }
};
...
static ngx_command_t ngx_http_autoindex_commands[] = {
    ...
    { { sizeof("autoindex") - 1, (u_char *) "autoindex" },
      0x02000000|0x04000000|0x08000000|0x00000200,
      ngx_conf_set_flag_slot,
      
# 92 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 92 "src/http/modules/ngx_http_autoindex_module.c"
                             ,
      
# 93 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 93 "src/http/modules/ngx_http_autoindex_module.c"
                                                    ,
      0 },

    { { sizeof("autoindex_format") - 1, (u_char *) "autoindex_format" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_enum_slot,
      
# 99 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 99 "src/http/modules/ngx_http_autoindex_module.c"
                             ,
      
# 100 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 100 "src/http/modules/ngx_http_autoindex_module.c"
                                                    ,
      &ngx_http_autoindex_format },

    { { sizeof("autoindex_localtime") - 1, (u_char *) "autoindex_localtime" },
      0x02000000|0x04000000|0x08000000|0x00000200,
      ngx_conf_set_flag_slot,
      
# 106 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 106 "src/http/modules/ngx_http_autoindex_module.c"
                             ,
      
# 107 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 107 "src/http/modules/ngx_http_autoindex_module.c"
                                                       ,
      0 },

    { { sizeof("autoindex_exact_size") - 1, (u_char *) "autoindex_exact_size" },
      0x02000000|0x04000000|0x08000000|0x00000200,
      ngx_conf_set_flag_slot,
      
# 113 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 113 "src/http/modules/ngx_http_autoindex_module.c"
                             ,
      
# 114 "src/http/modules/ngx_http_autoindex_module.c" 3 4
     0
# 114 "src/http/modules/ngx_http_autoindex_module.c"
                                                        ,
      0 },
...
ngx_http_autoindex_handler(ngx_http_request_t *r)
{
    ...
    if (!(r->method & (0x0002|0x0004))) {
        return -5;
    }

    alcf = (r)->loc_conf[ngx_http_autoindex_module.ctx_index];
    ...
    if (format == 2) {
        if (ngx_http_autoindex_jsonp_callback(r, &callback) != 0) {
            return 400;
        }
    ...
    if (ngx_open_dir(&path, &dir) == -1) {
        ...
        if ((r->connection->log)->log_level >= level) ngx_log_error_core(level, r->connection->log, err, "opendir()" " \"%s\" failed", path.data)
                                                                 ;
    ...
    (void) memset(&entries, 0, sizeof(ngx_array_t));
    ...
    case 1:
        (&r->headers_out.content_type)->len = sizeof("application/json") - 1; (&r->headers_out.content_type)->data = (u_char *) "application/json";
        break;

    case 2:
        (&r->headers_out.content_type)->len = sizeof("application/javascript") - 1; (&r->headers_out.content_type)->data = (u_char *) "application/javascript";
        break;

    case 3:
        (&r->headers_out.content_type)->len = sizeof("text/xml") - 1; (&r->headers_out.content_type)->data = (u_char *) "text/xml";
        (&r->headers_out.charset)->len = sizeof("utf-8") - 1; (&r->headers_out.charset)->data = (u_char *) "utf-8";
        break;

    default:
        (&r->headers_out.content_type)->len = sizeof("text/html") - 1; (&r->headers_out.content_type)->data = (u_char *) "text/html";
        break;
    }
    ...
    if (rc == -1 || rc > 0 || r->header_only) {
        if (closedir((&dir)->dir) == -1) {
            if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, errno, "closedir()" " \"%V\" failed", &path)
                                                                  ;
        }
    ...
    for ( ;; ) {
        ...
        if (ngx_read_dir(&dir) == -1) {
            err = errno;

            if (err != 0) {
                if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, err, "readdir()" " \"%V\" failed", &path)
                                                                     ;
                return ngx_http_autoindex_error(r, &dir, &path);
            }
        ...
        len = strlen((const char *) (&dir)->de->d_name);

        if (((u_char *) (&dir)->de->d_name)[0] == '.') {
            continue;
        }
        ...
        if (!dir.valid_info) {
            ...
            ngx_cpystrn(last, ((u_char *) (&dir)->de->d_name), len + 1);

            if (ngx_de_info(filename, &dir) == -1) {
                err = errno;

                if (err != ENOENT && err != ELOOP) {
                    if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, err, "stat()" " \"%s\" failed", filename)
                                                                           ;
                ...
                if (lstat((const char *) filename, &(&dir)->info) == -1) {
                    if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, errno, "lstat()" " \"%s\" failed", filename)
        ...
        ngx_cpystrn(entry->name.data, ((u_char *) (&dir)->de->d_name), len + 1);

        entry->dir = (((&dir)->type) ? ((&dir)->type == DT_DIR) : (S_ISDIR((&dir)->info.st_mode)));
        entry->file = (((&dir)->type) ? ((&dir)->type == DT_REG) : (S_ISREG((&dir)->info.st_mode)));
        entry->mtime = (&dir)->info.st_mtime;
    ...
    if (entries.nelts > 1) {
        qsort(entries.elts, (size_t) entries.nelts,
    ...
    case 1:
        b = ngx_http_autoindex_json(r, &entries, 0);
        break;
...
ngx_http_autoindex_html(ngx_http_request_t *r, ngx_array_t *entries)
{
    ...
    escape_html = ngx_escape_html(0, r->uri.data, r->uri.len);
    ...
    for (i = 0; i < entries->nelts; i++) {
        entry[i].escape = 2 * ngx_escape_uri(0, entry[i].name.data,
                                             entry[i].name.len,
                                             2);

        entry[i].escape_html = ngx_escape_html(0, entry[i].name.data,
                                               entry[i].name.len);
    ...
    b->last = (((u_char *) memcpy(b->last, title, sizeof(title) - 1)) + (sizeof(title) - 1));

    if (escape_html) {
        b->last = (u_char *) ngx_escape_html(b->last, r->uri.data, r->uri.len);
        b->last = (((u_char *) memcpy(b->last, header, sizeof(header) - 1)) + (sizeof(header) - 1));
        b->last = (u_char *) ngx_escape_html(b->last, r->uri.data, r->uri.len);

    } else {
        b->last = (((u_char *) memcpy(b->last, r->uri.data, r->uri.len)) + (r->uri.len));
        b->last = (((u_char *) memcpy(b->last, header, sizeof(header) - 1)) + (sizeof(header) - 1));
        b->last = (((u_char *) memcpy(b->last, r->uri.data, r->uri.len)) + (r->uri.len));
    }

    b->last = (((u_char *) memcpy(b->last, "</h1>", sizeof("</h1>") - 1)) + (sizeof("</h1>") - 1));

    b->last = (((u_char *) memcpy(b->last, "<hr><pre><a href=\"../\">../</a>" "\r\n", sizeof("<hr><pre><a href=\"../\">../</a>" "\r\n") - 1)) + (sizeof("<hr><pre><a href=\"../\">../</a>" "\r\n") - 1))
                                                                             ;
    ...
    for (i = 0; i < entries->nelts; i++) {
        b->last = (((u_char *) memcpy(b->last, "<a href=\"", sizeof("<a href=\"") - 1)) + (sizeof("<a href=\"") - 1));
        ...
        if (entry[i].escape) {
            ...
            b->last = (((u_char *) memcpy(b->last, entry[i].name.data, entry[i].name.len)) + (entry[i].name.len))
                                                   ;
        }
        ...
        *b->last++ = '>';
        ...
        if (len > 50) {
            b->last = (((u_char *) memcpy(last, "..&gt;</a>", sizeof("..&gt;</a>") - 1)) + (sizeof("..&gt;</a>") - 1));
            ...
            b->last = (((u_char *) memcpy(b->last, "</a>", sizeof("</a>") - 1)) + (sizeof("</a>") - 1));

            if (50 - len > 0) {
                (void) memset(b->last, ' ', 50 - len);
                b->last += 50 - len;
            }
        }
        ...
        *b->last++ = ' ';
        ...
        if (alcf->exact_size) {
            if (entry[i].dir) {
                b->last = (((u_char *) memcpy(b->last, "                  -", sizeof("                  -") - 1)) + (sizeof("                  -") - 1))
                                                                       ;
            } else {
    ...
    b->last = (((u_char *) memcpy(b->last, "</pre><hr>", sizeof("</pre><hr>") - 1)) + (sizeof("</pre><hr>") - 1));

    b->last = (((u_char *) memcpy(b->last, tail, sizeof(tail) - 1)) + (sizeof(tail) - 1));
    ...
    ngx_str_t *callback)
{
    ...
    for (i = 0; i < entries->nelts; i++) {
        entry[i].escape = ngx_escape_json(0, entry[i].name.data,
                                          entry[i].name.len);
        ...
        if (entry[i].file) {
            len += sizeof(", \"size\":") - 1 + (sizeof("-9223372036854775808") - 1);
        }
    }
    ...
    if (callback) {
        b->last = (((u_char *) memcpy(b->last, "/* callback */" "\r\n", sizeof("/* callback */" "\r\n") - 1)) + (sizeof("/* callback */" "\r\n") - 1))
                                                               ;

        b->last = (((u_char *) memcpy(b->last, callback->data, callback->len)) + (callback->len));
    ...
    *b->last++ = '[';
    ...
    for (i = 0; i < entries->nelts; i++) {
        ...
        b->last = (((u_char *) memcpy(b->last, "\", \"type\":\"", sizeof("\", \"type\":\"") - 1)) + (sizeof("\", \"type\":\"") - 1))
                                                           ;

        if (entry[i].dir) {
            b->last = (((u_char *) memcpy(b->last, "directory", sizeof("directory") - 1)) + (sizeof("directory") - 1));

        } else if (entry[i].file) {
            b->last = (((u_char *) memcpy(b->last, "file", sizeof("file") - 1)) + (sizeof("file") - 1));

        } else {
            b->last = (((u_char *) memcpy(b->last, "other", sizeof("other") - 1)) + (sizeof("other") - 1));
        }

        b->last = (((u_char *) memcpy(b->last, "\", \"mtime\":\"", sizeof("\", \"mtime\":\"") - 1)) + (sizeof("\", \"mtime\":\"") - 1))
                                                            ;

        b->last = ngx_http_time(b->last, entry[i].mtime);

        if (entry[i].file) {
            b->last = (((u_char *) memcpy(b->last, "\", \"size\":", sizeof("\", \"size\":") - 1)) + (sizeof("\", \"size\":") - 1))
                                                             ;
...
ngx_http_autoindex_jsonp_callback(ngx_http_request_t *r, ngx_str_t *callback)
{
    ...
    if (ngx_http_arg(r, (u_char *) "callback", 8, callback) != 0) {
        callback->len = 0;
        return 0;
    }

    if (callback->len > 128) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent too long callback name: \"%V\"", callback)
                                                                             ;
        return -5;
    }
    ...
    for (i = 0; i < callback->len; i++) {
        ...
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent invalid callback name: \"%V\"", callback)
                                                                            ;
...
ngx_http_autoindex_xml(ngx_http_request_t *r, ngx_array_t *entries)
{
    ...
    for (i = 0; i < entries->nelts; i++) {
        entry[i].escape = ngx_escape_html(0, entry[i].name.data,
                                          entry[i].name.len);
    ...
    b->last = (((u_char *) memcpy(b->last, head, sizeof(head) - 1)) + (sizeof(head) - 1));

    for (i = 0; i < entries->nelts; i++) {
        *b->last++ = '<';

        if (entry[i].dir) {
            (&type)->len = sizeof("directory") - 1; (&type)->data = (u_char *) "directory";

        } else if (entry[i].file) {
            (&type)->len = sizeof("file") - 1; (&type)->data = (u_char *) "file";

        } else {
            (&type)->len = sizeof("other") - 1; (&type)->data = (u_char *) "other";
        }

        b->last = (((u_char *) memcpy(b->last, type.data, type.len)) + (type.len));
...
ngx_http_autoindex_cmp_entries(const void *one, const void *two)
{
    ...
    return (int) strcmp((const char *) first->name.data, (const char *) second->name.data);
}
# 993 "src/http/modules/ngx_http_autoindex_module.c"
static ngx_int_t
ngx_http_autoindex_error(ngx_http_request_t *r, ngx_dir_t *dir, ngx_str_t *name)
{
    if (closedir((dir)->dir) == -1) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, errno, "closedir()" " \"%V\" failed", name)
                                                             ;
    }

    return r->header_sent ? -1 : 500;
}
...
ngx_http_autoindex_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->format == (ngx_uint_t) -1) { conf->format = (prev->format == (ngx_uint_t) -1) ? 0 : prev->format; }
                                                      ;
    if (conf->localtime == -1) { conf->localtime = (prev->localtime == -1) ? 0 : prev->localtime; };
    if (conf->exact_size == -1) { conf->exact_size = (prev->exact_size == -1) ? 1 : prev->exact_size; };
