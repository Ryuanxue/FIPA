# 1 "src/http/modules/ngx_http_static_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_static_module.c" 2
...
# 10 "src/http/modules/ngx_http_static_module.c" 2
...
# 11 "src/http/modules/ngx_http_static_module.c" 2
...
ngx_http_static_handler(ngx_http_request_t *r)
{
        ...
        != 0)
    {
        ...
        if (rc != 404 || clcf->log_not_found) {
            if ((log)->log_level >= level) ngx_log_error_core(level, log, of.err, "%s \"%s\" failed", of.failed, path.data)
                                                                   ;
        }
    ...
    if (!of.is_file) {
        if ((log)->log_level >= 3) ngx_log_error_core(3, log, 0, "\"%s\" is not a regular file", path.data)
                                                                ;
