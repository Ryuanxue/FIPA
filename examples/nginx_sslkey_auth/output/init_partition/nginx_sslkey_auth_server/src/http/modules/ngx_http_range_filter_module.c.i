# 1 "src/http/modules/ngx_http_range_filter_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_range_filter_module.c" 2
...
# 10 "src/http/modules/ngx_http_range_filter_module.c" 2
...
# 11 "src/http/modules/ngx_http_range_filter_module.c" 2
# 48 "src/http/modules/ngx_http_range_filter_module.c"
typedef struct {
    off_t start;
    off_t end;
    ...
    if (r->headers_in.if_range) {
        ...
        if (if_range->len >= 2 && if_range->data[if_range->len - 1] == '"') {
            ...
            if (if_range->len != etag->len
                || strncmp((const char *) if_range->data, (const char *) etag->data, etag->len) != 0)
            {
                goto next_filter;
            }
...
next_filter:
    ...
    (&r->headers_out.accept_ranges->key)->len = sizeof("Accept-Ranges") - 1; (&r->headers_out.accept_ranges->key)->data = (u_char *) "Accept-Ranges";
    (&r->headers_out.accept_ranges->value)->len = sizeof("bytes") - 1; (&r->headers_out.accept_ranges->value)->data = (u_char *) "bytes";
    ...
    ngx_uint_t ranges)
{
    ...
    if (r != r->main) {
        mctx = (r->main)->ctx[ngx_http_range_body_filter_module.ctx_index]
                                                                         ;
        if (mctx) {
    ...
    cutoff = 9223372036854775807LL / 10;
    cutlim = 9223372036854775807LL % 10;
    ...
    found:
        ...
        if (start < end) {
            ...
            if (size > 9223372036854775807LL - (end - start)) {
                return 416;
            }
    ...
    ngx_http_range_filter_ctx_t *ctx)
{
    ...
    (&content_range->key)->len = sizeof("Content-Range") - 1; (&content_range->key)->data = (u_char *) "Content-Range";

    content_range->value.data = ngx_pnalloc(r->pool,
                                    sizeof("bytes -/") - 1 + 3 * (sizeof("-9223372036854775808") - 1));
    if (content_range->value.data == 0) {
        content_range->hash = 0;
        r->headers_out.content_range = 0;
        return -1;
    }
    ...
    ngx_http_range_filter_ctx_t *ctx)
{
    ...
    size = sizeof("\r\n" "--") - 1 + (sizeof("-9223372036854775808") - 1)
           + sizeof("\r\n" "Content-Type: ") - 1
    ...
    if (ctx->boundary_header.data == 0) {
        return -1;
    }

    boundary = ngx_next_temp_number(0);
# 498 "src/http/modules/ngx_http_range_filter_module.c"
    if (r->headers_out.content_type_len == r->headers_out.content_type.len
        && r->headers_out.charset.len)
    {
    ...
    if (r->headers_out.content_type.data == 0) {
        return -1;
    }
    ...
    len = sizeof("\r\n" "--") - 1 + (sizeof("-9223372036854775808") - 1) + sizeof("--" "\r\n") - 1;
    ...
    for (i = 0; i < ctx->ranges.nelts; i++) {
        ...
        range[i].content_range.data =
                               ngx_pnalloc(r->pool, 3 * (sizeof("-9223372036854775808") - 1) + 2 + 4);

        if (range[i].content_range.data == 0) {
            return -1;
        }
...
ngx_http_range_not_satisfiable(ngx_http_request_t *r)
{
    ...
    content_range->value.data = ngx_pnalloc(r->pool,
                                       sizeof("bytes */") - 1 + (sizeof("-9223372036854775808") - 1));
    if (content_range->value.data == 0) {
        content_range->hash = 0;
        r->headers_out.content_range = 0;
        return -1;
...
ngx_http_range_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx = (r)->ctx[ngx_http_range_body_filter_module.ctx_index];
    ...
    if (ngx_http_range_test_overlapped(r, ctx, in) != 0) {
        return -1;
    }
    ...
    ngx_http_range_filter_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    if (!buf->last_buf) {
        start = ctx->offset;
        last = ctx->offset + ((buf->temporary || buf->memory || buf->mmap) ? (off_t) (buf->last - buf->pos): (buf->file_last - buf->file_pos));
    ...
    ctx->offset = ((buf->temporary || buf->memory || buf->mmap) ? (off_t) (buf->last - buf->pos): (buf->file_last - buf->file_pos));

    return 0;

overlapped:

    if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "range in overlapped buffers")
                                                ;
    ...
    ngx_http_range_filter_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    for (i = 0; i < ctx->ranges.nelts; i++) {
# 808 "src/http/modules/ngx_http_range_filter_module.c"
    ...
    b->pos = ngx_pnalloc(r->pool, sizeof("\r\n" "--") - 1 + (sizeof("-9223372036854775808") - 1)
                                  + sizeof("--" "\r\n") - 1);
    if (b->pos == 0) {
        return -1;
    }

    b->last = (((u_char *) memcpy(b->pos, ctx->boundary_header.data, sizeof("\r\n" "--") - 1 + (sizeof("-9223372036854775808") - 1))) + (sizeof("\r\n" "--") - 1 + (sizeof("-9223372036854775808") - 1)))
                                                                  ;
    *b->last++ = '-'; *b->last++ = '-';
