# 1 "src/http/modules/ngx_http_auth_basic_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_auth_basic_module.c" 2
...
# 10 "src/http/modules/ngx_http_auth_basic_module.c" 2
...
# 11 "src/http/modules/ngx_http_auth_basic_module.c" 2
# 1 "src/core/ngx_crypt.h" 1
# 16 "src/core/ngx_crypt.h"
ngx_int_t ngx_crypt(ngx_pool_t *pool, u_char *key, u_char *salt,
    u_char **encrypted);
# 12 "src/http/modules/ngx_http_auth_basic_module.c" 2
...
static ngx_command_t ngx_http_auth_basic_commands[] = {
    ...
    { { sizeof("auth_basic") - 1, (u_char *) "auth_basic" },
      0x02000000|0x04000000|0x08000000|0x80000000
                        |0x00000002,
      ngx_http_set_complex_value_slot,
      
# 43 "src/http/modules/ngx_http_auth_basic_module.c" 3 4
     0
# 43 "src/http/modules/ngx_http_auth_basic_module.c"
                             ,
      
# 44 "src/http/modules/ngx_http_auth_basic_module.c" 3 4
     0
# 44 "src/http/modules/ngx_http_auth_basic_module.c"
                                                    ,
      0 },

    { { sizeof("auth_basic_user_file") - 1, (u_char *) "auth_basic_user_file" },
      0x02000000|0x04000000|0x08000000|0x80000000
                        |0x00000002,
      ngx_http_auth_basic_user_file,
      
# 51 "src/http/modules/ngx_http_auth_basic_module.c" 3 4
     0
# 51 "src/http/modules/ngx_http_auth_basic_module.c"
                             ,
      
# 52 "src/http/modules/ngx_http_auth_basic_module.c" 3 4
     0
# 52 "src/http/modules/ngx_http_auth_basic_module.c"
                                                        ,
      0 },
...
ngx_http_auth_basic_handler(ngx_http_request_t *r)
{
    ...
    if (rc == -5) {
        ...
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "no user/password was provided for basic authentication")
                                                                               ;
    ...
    if (fd == -1) {
        ...
        if ((r->connection->log)->log_level >= level) ngx_log_error_core(level, r->connection->log, err, "open()" " \"%s\" failed", user_file.data)
                                                                       ;
    ...
    if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, 0, "user \"%V\" was not found in \"%s\"", &r->headers_in.user, user_file.data)
    ...
    ngx_str_t *realm)
{
    ...
    if (strcmp((const char *) encrypted, (const char *) passwd->data) == 0) {
        return 0;
    }
    ...
    if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, 0, "user \"%V\": password mismatch", &r->headers_in.user)
...
ngx_http_auth_basic_set_realm(ngx_http_request_t *r, ngx_str_t *realm)
{
    ...
    if (r->headers_out.www_authenticate == 0) {
        return 500;
    }
    ...
    p = (((u_char *) memcpy(basic, "Basic realm=\"", sizeof("Basic realm=\"") - 1)) + (sizeof("Basic realm=\"") - 1));
    p = (((u_char *) memcpy(p, realm->data, realm->len)) + (realm->len));
    *p = '"';

    r->headers_out.www_authenticate->hash = 1;
    (&r->headers_out.www_authenticate->key)->len = sizeof("WWW-Authenticate") - 1; (&r->headers_out.www_authenticate->key)->data = (u_char *) "WWW-Authenticate";
...
{
    if (close(file->fd) == -1) {
        if ((file->log)->log_level >= 2) ngx_log_error_core(2, file->log, errno, "close()" " \"%s\" failed", file->name.data)
                                                                         ;
    }
}
...
ngx_http_auth_basic_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->user_file.value.data == 0) {
