...
ngx_int_t ngx_http_log_handler_sense_1(int *ngx_http_log_handler_sense_1_ret, u_char *line, u_char *p, size_t *len, size_t *size, ssize_t *n, ngx_str_t val, ngx_uint_t *i, ngx_uint_t *l, ngx_http_log_t *log, ngx_http_log_op_t *op, ngx_http_log_buf_t *buffer, ngx_http_log_loc_conf_t *lcf, ngx_http_request_t *r)
{
  lcf = r->loc_conf[ngx_http_log_module.ctx_index];
  if (lcf->off)
  {
    {
      *ngx_http_log_handler_sense_1_ret = 1;
      return 0;
    }
  }
  log = lcf->logs->elts;
  for (*l = 0; (*l) < lcf->logs->nelts; (*l)++)
  {
    if (log[*l].filter)
    {
      if (ngx_http_complex_value(r, log[*l].filter, &val) != 0)
      {
        {
          *ngx_http_log_handler_sense_1_ret = 1;
          return -1;
    ...
    if (ngx_cached_time->sec == log[*l].disk_full_time)
    {
      continue;
    }
    ...
    *len += 1;
    buffer = (log[*l].file) ? (log[*l].file->data) : (0);
    if (buffer)
    {
    ...
    if (log[*l].syslog_peer)
    {
      ...
      *n = ngx_syslog_send(log[*l].syslog_peer, line, *size);
      if ((*n) < 0)
      {
        if (r->connection->log->log_level >= 5)
          ngx_log_error_core(5, r->connection->log, 0, "send() to syslog failed");
      }
      else
        if (((size_t) (*n)) != (*size))
      {
        if (r->connection->log->log_level >= 5)
          ngx_log_error_core(5, r->connection->log, 0, "send() to syslog has written only %z of %uz", *n, *size);
      }
      continue;
    }
    *(p++) = (u_char) '\n';
...
ngx_int_t ngx_http_log_init_sense_1(int *ngx_http_log_init_sense_1_ret, ngx_conf_t *cf)
{
  ngx_str_t *value;
  ngx_array_t a;
  ...
  lmcf = ((ngx_http_conf_ctx_t *) cf->ctx)->main_conf[ngx_http_log_module.ctx_index];
  if (lmcf->combined_used)
  {
    if (ngx_array_init(&a, cf->pool, 1, sizeof(ngx_str_t)) != 0)
    {
      {
        *ngx_http_log_init_sense_1_ret = 1;
        return -1;
      }
    }
    ...
    *value = ngx_http_combined_fmt;
    fmt = lmcf->formats.elts;
    if (ngx_http_log_compile_format(cf, 0, fmt->ops, &a, 0) != 0)
    {
      {
        *ngx_http_log_init_sense_1_ret = 1;
        return -1;
