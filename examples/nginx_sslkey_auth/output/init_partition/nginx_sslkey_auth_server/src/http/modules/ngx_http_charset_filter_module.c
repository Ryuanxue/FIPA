...
ngx_int_t ngx_http_charset_header_filter_sense_1(int *ngx_http_charset_header_filter_sense_1_ret, ngx_int_t *charset, ngx_int_t *source_charset, ngx_str_t dst, ngx_str_t src, ngx_http_charset_t *charsets, ngx_http_charset_main_conf_t *mcf, ngx_http_request_t *r)
{
  if (r == r->main)
  {
  ...
  {
    {
      *ngx_http_charset_header_filter_sense_1_ret = 1;
      return -1;
    }
  }
  if ((*charset) == (-5))
  ...
  *source_charset = ngx_http_source_charset(r, &src);
  ...
  if ((charsets[*source_charset].tables == 0) || (charsets[*source_charset].tables[*charset] == 0))
  {
    goto no_charset_map;
  }
  r->headers_out.content_type.len = r->headers_out.content_type_len;
  ...
  if (r->connection->log->log_level >= 4)
    ngx_log_error_core(4, r->connection->log, 0, "no \"charset_map\" between the charsets \"%V\" and \"%V\"", &src, &dst);
...
ngx_int_t ngx_http_charset_postconfiguration_sense_1(int *ngx_http_charset_postconfiguration_sense_1_ret, ngx_conf_t *cf)
{
  u_char **src;
  u_char **dst;
  ngx_int_t c;
  ...
  for (i = 0; i < mcf->recodes.nelts; i++)
  {
    ...
    if (cf->log->log_level >= 1)
      ngx_log_error_core(1, cf->log, 0, "no \"charset_map\" between the charsets \"%V\" and \"%V\"", &charset[c].name, &charset[recode[i].dst].name);
    {
      *ngx_http_charset_postconfiguration_sense_1_ret = 1;
      return -1;
    }
    next:
    continue;
