# 1 "src/http/modules/ngx_http_upstream_hash_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_upstream_hash_module.c" 2
...
# 10 "src/http/modules/ngx_http_upstream_hash_module.c" 2
...
# 11 "src/http/modules/ngx_http_upstream_hash_module.c" 2
...
static ngx_command_t ngx_http_upstream_hash_commands[] = {
    ...
    { { sizeof("hash") - 1, (u_char *) "hash" },
      0x10000000|(0x00000002|0x00000004),
      ngx_http_upstream_hash,
      
# 71 "src/http/modules/ngx_http_upstream_hash_module.c" 3 4
     0
# 71 "src/http/modules/ngx_http_upstream_hash_module.c"
                             ,
      0,
      0 },
...
{
    if (ngx_http_upstream_init_round_robin(cf, us) != 0) {
        return -1;
    }
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (ngx_http_upstream_init_round_robin_peer(r, us) != 0) {
        return -1;
    }

    r->upstream->peer.get = ngx_http_upstream_get_hash_peer;

    hcf = us->srv_conf[ngx_http_upstream_hash_module.ctx_index];

    if (ngx_http_complex_value(r, &hcf->key, &hp->key) != 0) {
        return -1;
    }
...
ngx_http_upstream_get_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    time_t now;
    u_char buf[(sizeof("-9223372036854775808") - 1)];
    size_t size;
    uint32_t hash;
    ngx_int_t w;
    uintptr_t m;
    ...
    if (hp->rrp.peers->shpool) { ngx_rwlock_rlock(&hp->rrp.peers->rwlock); };

    if (hp->tries > 20 || hp->rrp.peers->single) {
        if (hp->rrp.peers->shpool) { ngx_rwlock_unlock(&hp->rrp.peers->rwlock); };
    ...
    for ( ;; ) {
        ...
        if (hp->rrp.peers->shpool) { ngx_rwlock_wlock(&peer->lock); };

       
                                                                         ;

        if (peer->down) {
            if (hp->rrp.peers->shpool) { ngx_rwlock_unlock(&peer->lock); };
            goto next;
        }
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (ngx_http_upstream_init_hash_peer(r, us) != 0) {
        return -1;
    }
...
ngx_http_upstream_get_chash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (hp->rrp.peers->shpool) { ngx_rwlock_wlock(&hp->rrp.peers->rwlock); };
    ...
    for ( ;; ) {
             ...
             peer = peer->next, i++)
        {
            ...
            if (peer->server.len != server->len
                || strncmp((const char *) peer->server.data, (const char *) server->data, server->len)
                   != 0)
            {
                continue;
            }
...
ngx_http_upstream_hash(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    } else if (strcmp((const char *) value[2].data, (const char *) "consistent") == 0) {
