# 1 "src/http/modules/ngx_http_upstream_random_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 8 "src/http/modules/ngx_http_upstream_random_module.c" 2
...
# 9 "src/http/modules/ngx_http_upstream_random_module.c" 2
...
# 10 "src/http/modules/ngx_http_upstream_random_module.c" 2
...
static ngx_command_t ngx_http_upstream_random_commands[] = {
    ...
    { { sizeof("random") - 1, (u_char *) "random" },
      0x10000000|0x00000001|(0x00000002|0x00000004),
      ngx_http_upstream_random,
      
# 57 "src/http/modules/ngx_http_upstream_random_module.c" 3 4
     0
# 57 "src/http/modules/ngx_http_upstream_random_module.c"
                             ,
      0,
      0 },
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    rcf = us->srv_conf[ngx_http_upstream_random_module.ctx_index];
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (rp->rrp.peers->shpool) { ngx_rwlock_rlock(&rp->rrp.peers->rwlock); };


    if (rp->rrp.peers->shpool && rcf->ranges == 0) {
        if (ngx_http_upstream_update_random(0, us) != 0) {
            if (rp->rrp.peers->shpool) { ngx_rwlock_unlock(&rp->rrp.peers->rwlock); };
            return -1;
        }
    }
...
ngx_http_upstream_get_random_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    for ( ;; ) {
        ...
        if (peer->down) {
            if (peers->shpool) { ngx_rwlock_unlock(&peer->lock); };
            goto next;
        }
...
ngx_http_upstream_random(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (strcmp((const char *) value[1].data, (const char *) "two") == 0) {
        rcf->two = 1;
    ...
    if (strcmp((const char *) value[2].data, (const char *) "least_conn") != 0) {
        ngx_conf_log_error(1, cf, 0,
