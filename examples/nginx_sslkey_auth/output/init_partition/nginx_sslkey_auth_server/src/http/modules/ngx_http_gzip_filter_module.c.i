# 1 "src/http/modules/ngx_http_gzip_filter_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_gzip_filter_module.c" 2
...
# 10 "src/http/modules/ngx_http_gzip_filter_module.c" 2
...
# 11 "src/http/modules/ngx_http_gzip_filter_module.c" 2

# 1 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/zlib.h" 1



# 1 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/_fake_defines.h" 1
# 5 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/zlib.h" 2
# 1 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/_fake_typedefs.h" 1
# 307 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/_fake_typedefs.h"
typedef struct x509_store_ctx_st X509_STORE_CTX;
typedef struct ssl_st SSL;
...
typedef int ASN1_BOOLEAN;
typedef int ASN1_NULL;
# 6 "/home/raoxue/Desktop/IF-driver-partition/FIPA/scripts/fake_libc_include/zlib.h" 2
...
# 13 "src/http/modules/ngx_http_gzip_filter_module.c" 2
...
# 88 "src/http/modules/ngx_http_gzip_filter_module.c"
...
static ngx_command_t ngx_http_gzip_filter_commands[] = {
    ...
    { { sizeof("gzip_buffers") - 1, (u_char *) "gzip_buffers" },
    ...
    { { sizeof("gzip_types") - 1, (u_char *) "gzip_types" },
    ...
    { { sizeof("gzip_comp_level") - 1, (u_char *) "gzip_comp_level" },
    ...
    { { sizeof("gzip_window") - 1, (u_char *) "gzip_window" },
      0x02000000|0x04000000|0x08000000|0x00000002,
    ...
    { { sizeof("gzip_hash") - 1, (u_char *) "gzip_hash" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_size_slot,
      0,
      0,
    ...
    { { sizeof("postpone_gzipping") - 1, (u_char *) "postpone_gzipping" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_size_slot,
      0,
      0,
    ...
    { { sizeof("gzip_no_buffer") - 1, (u_char *) "gzip_no_buffer" },
    ...
    { { sizeof("gzip_min_length") - 1, (u_char *) "gzip_min_length" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_size_slot,
      0,
      0,
...
static ngx_str_t ngx_http_gzip_ratio = { sizeof("gzip_ratio") - 1, (u_char *) "gzip_ratio" };
...
ngx_http_gzip_header_filter(ngx_http_request_t *r)
{
    ...
    if (!conf->enable
        ...
        || ngx_http_test_content_type(r, &conf->types) == 0
        || r->header_only)
    {
    ...
    r->gzip_vary = 1;
# 277 "src/http/modules/ngx_http_gzip_filter_module.c"
    if (!r->gzip_tested) {
...
ngx_http_gzip_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx = (r)->ctx[ngx_http_gzip_filter_module.ctx_index];

    if (ctx == 0 || ctx->done || r->header_only) {
    ...
    if (ctx->buffering) {
# 346 "src/http/modules/ngx_http_gzip_filter_module.c"
        if (in) {
    ...
    if (ctx->preallocated == 0) {
        if (ngx_http_gzip_filter_deflate_start(r, ctx) != 0) {
            goto failed;
        }
    }
    ...
    if (ctx->nomem) {
        ...
        if (ngx_http_next_body_filter(r, 0) == -1) {
            goto failed;
        }
    ...
    for ( ;; ) {
        ...
        if (!ctx->gzheader) {
            if (ngx_http_gzip_filter_gzheader(r, ctx) != 0) {
                goto failed;
            }
        }
...
# 533 "src/http/modules/ngx_http_gzip_filter_module.c"
    if (!ngx_http_gzip_assume_intel) {
        ctx->allocated = 8192 + (1 << (wbits + 2)) + (1 << (memlevel + 9));

    } else {
# 545 "src/http/modules/ngx_http_gzip_filter_module.c"
        if (conf->level == 1) {
            wbits = ((wbits < 13) ? (13) : (wbits));
        }

        ctx->allocated = 8192 + 16 + (1 << (wbits + 2))
                         + (1 << (((memlevel < 8) ? (8) : (memlevel)) + 8))
                         + (1 << (memlevel + 8));
        ctx->intel = 1;
    }
}
...
ngx_http_gzip_filter_buffer(ngx_http_gzip_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    while (in) {
        ...
        if (ctx->buffering && size) {
            ...
            buf->last = (((u_char *) memcpy(buf->pos, b->pos, size)) + (size));
            b->pos = b->last;
    ...
    ngx_http_gzip_ctx_t *ctx)
{
    ...
    if (rc != Z_OK) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "deflateInit2() failed: %d", rc)
                                                      ;
        return -1;
    }
...
ngx_http_gzip_filter_add_data(ngx_http_request_t *r, ngx_http_gzip_ctx_t *ctx)
{
    ...
    if (ctx->in_buf->tag == (ngx_buf_tag_t) &ngx_http_gzip_filter_module) {
        ...
        cl->next = r->pool->chain; r->pool->chain = cl;
    }
...
ngx_http_gzip_filter_deflate(ngx_http_request_t *r, ngx_http_gzip_ctx_t *ctx)
{
    ...
    if (rc != Z_OK && rc != Z_STREAM_END && rc != Z_BUF_ERROR) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "deflate() failed: %d, %d", ctx->flush, rc)
                                                                 ;
        return -1;
    }
    ...
    if (rc == Z_STREAM_END) {
        ...
        if (ngx_http_gzip_filter_deflate_end(r, ctx) != 0) {
            return -1;
        }
    ...
    ngx_http_gzip_ctx_t *ctx)
{
    ...
    if (rc != Z_OK) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "deflateEnd() failed: %d", rc)
                                                    ;
        return -1;
    }
...
# 1008 "src/http/modules/ngx_http_gzip_filter_module.c"
...
ngx_http_gzip_filter_alloc(void *opaque, u_int items, u_int size)
{
    ...
    if (ctx->intel) {
        if ((ctx->request->connection->log)->log_level >= 2) ngx_log_error_core(2, ctx->request->connection->log, 0, "gzip filter failed to use preallocated memory: " "%ud of %ui", items * size, ctx->allocated)
...
ngx_http_gzip_add_variables(ngx_conf_t *cf)
{
    ...
    var = ngx_http_add_variable(cf, &ngx_http_gzip_ratio, 8);
    if (var == 0) {
        return -1;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, (sizeof("-2147483648") - 1) + 3);
    if (v->data == 0) {
        return -1;
    }
...
# 1170 "src/http/modules/ngx_http_gzip_filter_module.c"
    conf->enable = -1;
    conf->no_buffer = -1;
...
ngx_http_gzip_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->no_buffer == -1) { conf->no_buffer = (prev->no_buffer == -1) ? 0 : prev->no_buffer; };

    if (conf->bufs.num == 0) { if (prev->bufs.num) { conf->bufs.num = prev->bufs.num; conf->bufs.size = prev->bufs.size; } else { conf->bufs.num = (128 * 1024) / ngx_pagesize; conf->bufs.size = ngx_pagesize; } }
                                                                        ;

    if (conf->postpone_gzipping == (size_t) -1) { conf->postpone_gzipping = (prev->postpone_gzipping == (size_t) -1) ? 0 : prev->postpone_gzipping; }
                                ;
    if (conf->level == -1) { conf->level = (prev->level == -1) ? 1 : prev->level; };
    if (conf->wbits == (size_t) -1) { conf->wbits = (prev->wbits == (size_t) -1) ? MAX_WBITS : prev->wbits; };
    if (conf->memlevel == (size_t) -1) { conf->memlevel = (prev->memlevel == (size_t) -1) ? MAX_MEM_LEVEL - 1 : prev->memlevel; }
                                                ;
    if (conf->min_length == -1) { conf->min_length = (prev->min_length == -1) ? 20 : prev->min_length; };
