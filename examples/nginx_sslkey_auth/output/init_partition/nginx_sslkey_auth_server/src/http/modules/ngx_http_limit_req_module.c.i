# 1 "src/http/modules/ngx_http_limit_req_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_limit_req_module.c" 2
...
# 10 "src/http/modules/ngx_http_limit_req_module.c" 2
...
# 11 "src/http/modules/ngx_http_limit_req_module.c" 2
...
static ngx_conf_enum_t ngx_http_limit_req_log_levels[] = {
    { { sizeof("info") - 1, (u_char *) "info" }, 7 },
    { { sizeof("notice") - 1, (u_char *) "notice" }, 6 },
    { { sizeof("warn") - 1, (u_char *) "warn" }, 5 },
    { { sizeof("error") - 1, (u_char *) "error" }, 4 },
    { { 0, 0 }, 0 }
};
...
static ngx_command_t ngx_http_limit_req_commands[] = {
    ...
    { { sizeof("limit_req_zone") - 1, (u_char *) "limit_req_zone" },
    ...
    { { sizeof("limit_req") - 1, (u_char *) "limit_req" },
      0x02000000|0x04000000|0x08000000|(0x00000002|0x00000004|0x00000008),
      ngx_http_limit_req,
      
# 103 "src/http/modules/ngx_http_limit_req_module.c" 3 4
     0
# 103 "src/http/modules/ngx_http_limit_req_module.c"
                             ,
      0,
      0 },

    { { sizeof("limit_req_log_level") - 1, (u_char *) "limit_req_log_level" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_enum_slot,
      
# 110 "src/http/modules/ngx_http_limit_req_module.c" 3 4
     0
# 110 "src/http/modules/ngx_http_limit_req_module.c"
                             ,
      
# 111 "src/http/modules/ngx_http_limit_req_module.c" 3 4
     0
# 111 "src/http/modules/ngx_http_limit_req_module.c"
                                                         ,
      &ngx_http_limit_req_log_levels },

    { { sizeof("limit_req_status") - 1, (u_char *) "limit_req_status" },
      0x02000000|0x04000000|0x08000000|0x00000002,
      ngx_conf_set_num_slot,
      
# 117 "src/http/modules/ngx_http_limit_req_module.c" 3 4
     0
# 117 "src/http/modules/ngx_http_limit_req_module.c"
                             ,
      
# 118 "src/http/modules/ngx_http_limit_req_module.c" 3 4
     0
# 118 "src/http/modules/ngx_http_limit_req_module.c"
                                                     ,
...
ngx_http_limit_req_handler(ngx_http_request_t *r)
{
    ...
    for (n = 0; n < lrcf->limits.nelts; n++) {
        ...
        if (key.len > 65535) {
            if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, 0, "the value of the \"%V\" key " "is more than 65535 bytes: \"%V\"", &ctx->key.value, &key)
    ...
    if (rc == -3 || rc == -1) {
        ...
        if (rc == -3) {
            if ((r->connection->log)->log_level >= lrcf->limit_log_level) ngx_log_error_core(lrcf->limit_log_level, r->connection->log, 0, "limiting requests, excess: %ui.%03ui by zone \"%V\"", excess / 1000, excess % 1000, &limit->shm_zone->shm.name)
    ...
    if ((r->connection->log)->log_level >= lrcf->delay_log_level) ngx_log_error_core(lrcf->delay_log_level, r->connection->log, 0, "delaying request, excess: %ui.%03ui, by zone \"%V\"", excess / 1000, excess % 1000, &limit->shm_zone->shm.name)
    ...
    ngx_str_t *key, ngx_uint_t *ep, ngx_uint_t account)
{
    ...
    while (node != sentinel) {
        ...
        if (rc == 0) {
            (&lr->queue)->next->prev = (&lr->queue)->prev; (&lr->queue)->prev->next = (&lr->queue)->next;
            (&lr->queue)->next = (&ctx->sh->queue)->next; (&lr->queue)->next->prev = &lr->queue; (&lr->queue)->prev = &ctx->sh->queue; (&ctx->sh->queue)->next = &lr->queue;
    ...
    *ep = 0;
    ...
    size = 
# 443 "src/http/modules/ngx_http_limit_req_module.c" 3 4
          0
           
# 444 "src/http/modules/ngx_http_limit_req_module.c"
          + 
# 444 "src/http/modules/ngx_http_limit_req_module.c" 3 4
            0
           
# 445 "src/http/modules/ngx_http_limit_req_module.c"
          + key->len;
    ...
    if (node == 0) {
        ...
        node = ngx_slab_alloc_locked(ctx->shpool, size);
        if (node == 0) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "could not allocate node%s", ctx->shpool->log_ctx)
                                                                            ;
            return -1;
        }
    }
    ...
    (void) memcpy(lr->data, key->data, key->len);
...
ngx_http_limit_req_expire(ngx_http_limit_req_ctx_t *ctx, ngx_uint_t n)
{
    ...
    while (n < 3) {
        ...
        if ((&ctx->sh->queue == (&ctx->sh->queue)->prev)) {
            return;
        }

        q = (&ctx->sh->queue)->prev;

        lr = (ngx_http_limit_req_node_t *) ((u_char *) q - 
# 590 "src/http/modules/ngx_http_limit_req_module.c" 3 4
            0
# 590 "src/http/modules/ngx_http_limit_req_module.c"
            );
        ...
        node = (ngx_rbtree_node_t *)
                   ((u_char *) lr - 
# 621 "src/http/modules/ngx_http_limit_req_module.c" 3 4
                                   0
# 621 "src/http/modules/ngx_http_limit_req_module.c"
                                                                     );
...
ngx_http_limit_req_init_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (octx) {
        if (ctx->key.value.len != octx->key.value.len
            || strncmp((const char *) ctx->key.value.data, (const char *) octx->key.value.data, ctx->key.value.len)

               != 0)
        {
            if ((shm_zone->shm.log)->log_level >= 1) ngx_log_error_core(1, shm_zone->shm.log, 0, "limit_req \"%V\" uses the \"%V\" key " "while previously it used the \"%V\" key", &shm_zone->shm.name, &ctx->key.value, &octx->key.value)
    ...
    ((&ctx->sh->sentinel)->color = 0); (&ctx->sh->rbtree)->root = &ctx->sh->sentinel; (&ctx->sh->rbtree)->sentinel = &ctx->sh->sentinel; (&ctx->sh->rbtree)->insert = ngx_http_limit_req_rbtree_insert_value
                                                           ;

    (&ctx->sh->queue)->prev = &ctx->sh->queue; (&ctx->sh->queue)->next = &ctx->sh->queue;

    len = sizeof(" in limit_req zone \"\"") + shm_zone->shm.name.len;

    ctx->shpool->log_ctx = ngx_slab_alloc(ctx->shpool, len);
    if (ctx->shpool->log_ctx == 0) {
        return -1;
    }
...
ngx_http_limit_req_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->limits.elts == 0) {
        conf->limits = prev->limits;
    }

    if (conf->limit_log_level == (ngx_uint_t) -1) { conf->limit_log_level = (prev->limit_log_level == (ngx_uint_t) -1) ? 4 : prev->limit_log_level; }
                                          ;

    conf->delay_log_level = (conf->limit_log_level == 7) ?
                                7 : conf->limit_log_level + 1;

    if (conf->status_code == (ngx_uint_t) -1) { conf->status_code = (prev->status_code == (ngx_uint_t) -1) ? 503 : prev->status_code; }
                                                           ;
...
ngx_http_limit_req_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "zone=", 5) == 0) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "rate=", 5) == 0) {

            len = value[i].len;
            p = value[i].data + len - 3;

            if (strncmp((const char *) p, (const char *) "r/s", 3) == 0) {
                scale = 1;
                len -= 3;

            } else if (strncmp((const char *) p, (const char *) "r/m", 3) == 0) {
                scale = 60;
                len -= 3;
            }
...
ngx_http_limit_req(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (strncmp((const char *) value[i].data, (const char *) "burst=", 6) == 0) {
        ...
        if (strcmp((const char *) value[i].data, (const char *) "nodelay") == 0) {
            nodelay = 1;
            continue;
        }
