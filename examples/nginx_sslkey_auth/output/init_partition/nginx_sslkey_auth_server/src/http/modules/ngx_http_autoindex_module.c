...
ngx_int_t ngx_http_autoindex_handler_sense_1(int *ngx_http_autoindex_handler_sense_1_ret, u_char *last, u_char *filename, size_t *len, size_t *allocated, size_t root, ngx_err_t *err, ngx_buf_t *b, ngx_int_t *rc, ngx_str_t path, ngx_str_t callback, ngx_dir_t dir, ngx_uint_t *level, ngx_uint_t *format, ngx_pool_t *pool, ngx_chain_t out, ngx_array_t entries, ngx_http_autoindex_entry_t *entry, ngx_http_autoindex_loc_conf_t *alcf, ngx_http_request_t *r)
{
  ...
  *rc = ngx_http_discard_request_body(r);
  ...
  last = ngx_http_map_uri_to_path(r, &path, &root, 50);
  if (last == 0)
  {
    {
      *ngx_http_autoindex_handler_sense_1_ret = 1;
  ...
  *format = alcf->*format;
  ...
  if (ngx_open_dir(&path, &dir) == (-1))
  {
    *err = errno;
    ...
    if (r->connection->log->log_level >= (*level))
      ngx_log_error_core(*level, r->connection->log, *err, "opendir() \"%s\" failed", path.data);
    {
      *ngx_http_autoindex_handler_sense_1_ret = 1;
      return *rc;
    }
  ...
  pool = r->pool;
  if (ngx_array_init(&entries, pool, 40, sizeof(ngx_http_autoindex_entry_t)) != 0)
  {
    {
      *ngx_http_autoindex_handler_sense_1_ret = 1;
  ...
  *rc = ngx_http_send_header(r);
  if ((((*rc) == (-1)) || ((*rc) > 0)) || r->header_only)
  {
    if (closedir((&dir)->dir) == (-1))
    {
      if (r->connection->log->log_level >= 2)
        ngx_log_error_core(2, r->connection->log, errno, "closedir() \"%V\" failed", &path);
    }
    {
      *ngx_http_autoindex_handler_sense_1_ret = 1;
      return *rc;
      ...
      *err = errno;
      if ((*err) != 0)
      {
        if (r->connection->log->log_level >= 3)
          ngx_log_error_core(3, r->connection->log, *err, "readdir() \"%V\" failed", &path);
        {
          *ngx_http_autoindex_handler_sense_1_ret = 1;
          return ngx_http_autoindex_error(r, &dir, &path);
        }
    ...
    if (!dir.valid_info)
    {
      ...
      if (ngx_de_info(filename, &dir) == (-1))
      {
        *err = errno;
        ...
        {
          if (r->connection->log->log_level >= 3)
            ngx_log_error_core(3, r->connection->log, *err, "stat() \"%s\" failed", filename);
          if ((*err) == EACCES)
          {
            continue;
          }
        ...
        {
          if (r->connection->log->log_level >= 3)
            ngx_log_error_core(3, r->connection->log, errno, "lstat() \"%s\" failed", filename);
          {
            *ngx_http_autoindex_handler_sense_1_ret = 1;
            return ngx_http_autoindex_error(r, &dir, &path);
          }
  ...
  if (entries.nelts > 1)
  {
    qsort(entries.elts, (size_t) entries.nelts, sizeof(ngx_http_autoindex_entry_t), ngx_http_autoindex_cmp_entries);
  }
  switch (*format)
  {
    case 1:
