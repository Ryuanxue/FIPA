# 1 "src/http/modules/ngx_http_geo_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_geo_module.c" 2
...
# 10 "src/http/modules/ngx_http_geo_module.c" 2
...
# 11 "src/http/modules/ngx_http_geo_module.c" 2
...
static ngx_command_t ngx_http_geo_commands[] = {
    ...
    { { sizeof("geo") - 1, (u_char *) "geo" },
      0x02000000|0x00000100|(0x00000002|0x00000004),
      ngx_http_geo_block,
      
# 118 "src/http/modules/ngx_http_geo_module.c" 3 4
     0
# 118 "src/http/modules/ngx_http_geo_module.c"
                              ,
      0,
      0 },
    ...
    uintptr_t data)
{
    ...
    if (ngx_http_geo_addr(r, ctx, &addr) != 0) {
        vv = (ngx_http_variable_value_t *)
                  ngx_radix32tree_find(ctx->u.trees.tree, ((unsigned int) -1));
        goto done;
    }
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_http_geo_real_addr(r, ctx, addr) != 0) {
        return -1;
    }

    xfwd = &r->headers_in.x_forwarded_for;

    if (xfwd->nelts > 0 && ctx->proxies != 0) {
        (void) ngx_http_get_forwarded_addr(r, addr, xfwd, 0,
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_parse_addr(r->pool, addr, v->data, v->len) == 0) {
        return 0;
    }
...
ngx_http_geo_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    pool = ngx_create_pool((16 * 1024), cf->log);
    if (pool == 0) {
        return (void *) -1;
    }

    (void) memset(&ctx, 0, sizeof(ngx_http_geo_conf_ctx_t));

    ctx.temp_pool = ngx_create_pool((16 * 1024), cf->log);
    if (ctx.temp_pool == 0) {
        ngx_destroy_pool(pool);
        return (void *) -1;
    }

    ((&ctx.sentinel)->color = 0); (&ctx.rbtree)->root = &ctx.sentinel; (&ctx.rbtree)->sentinel = &ctx.sentinel; (&ctx.rbtree)->insert = ngx_str_rbtree_insert_value;
    ...
    if (ctx.ranges) {
        ...
        if (ctx.high.low && !ctx.binary_include) {
            for (i = 0; i < 0x10000; i++) {
                ...
                (void) memcpy(ctx.high.low[i], a->elts, len);
                ctx.high.low[i][a->nelts].value = 0;
...
ngx_http_geo(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (cf->args->nelts == 1) {
        ...
        if (strcmp((const char *) value[0].data, (const char *) "ranges") == 0) {
        ...
        else if (strcmp((const char *) value[0].data, (const char *) "proxy_recursive") == 0) {
    ...
    } else if (strcmp((const char *) value[0].data, (const char *) "proxy") == 0) {

        if (ngx_http_geo_cidr_value(cf, &value[1], &cidr) != 0) {
            goto failed;
        }
    ...
    ngx_str_t *value)
{
    ...
    if (strcmp((const char *) value[0].data, (const char *) "default") == 0) {
    ...
    if (strcmp((const char *) value[0].data, (const char *) "delete") == 0) {
        net = &value[1];
        del = 1;
...
invalid:
    ...
    ngx_conf_log_error(1, cf, 0, "invalid range \"%V\"", net);
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        while (i) {
            ...
            if (s > (ngx_uint_t) range[i].end) {
                ...
                (void) memmove(&range[i + 2], &range[i + 1], (a->nelts - 2 - i) * sizeof(ngx_http_geo_range_t))
                                                                              ;
                ...
                && e < (ngx_uint_t) range[i].end)
            {
                ...
                (void) memmove(&range[i + 3], &range[i + 1], (a->nelts - 3 - i) * sizeof(ngx_http_geo_range_t))
                                                                              ;
                ...
                && e < (ngx_uint_t) range[i].end)
            {
                ...
                (void) memmove(&range[i + 1], &range[i], (a->nelts - 1 - i) * sizeof(ngx_http_geo_range_t))
                                                                              ;
        ...
        (void) memmove(&range[1], &range[0], (a->nelts - 1) * sizeof(ngx_http_geo_range_t))
                                                                  ;
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        for (i = 0; i < a->nelts; i++) {
            ...
            if (s == (ngx_uint_t) range[i].start
                && e == (ngx_uint_t) range[i].end)
            {
                (void) memmove(&range[i], &range[i + 1], (a->nelts - 1 - i) * sizeof(ngx_http_geo_range_t))
                                                                              ;
    ...
    ngx_str_t *value)
{
    ...
    if (strcmp((const char *) value[0].data, (const char *) "default") == 0) {
        ...
        (void) memset(&cidr.u.in6, 0, sizeof(ngx_in6_cidr_t));
    ...
    if (ngx_http_geo_cidr_value(cf, net, &cidr) != 0) {
        return (void *) -1;
    }
    ...
    ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net)
{
    ...
    case AF_INET6:
        ...
        if (rc == -1) {
            ngx_conf_log_error(1, cf, 0, "invalid radix tree");
            return (void *) -1;
        }
    ...
    ngx_str_t *value)
{
    ...
    ctx->data_size += (((sizeof(ngx_http_variable_value_t) + value->len) + (sizeof(void *) - 1)) & ~(sizeof(void *) - 1))
                                               ;
...
ngx_http_geo_cidr_value(ngx_conf_t *cf, ngx_str_t *net, ngx_cidr_t *cidr)
{
    ...
    if (strcmp((const char *) net->data, (const char *) "255.255.255.255") == 0) {
        cidr->family = AF_INET;
    ...
    if (rc == -1) {
        ngx_conf_log_error(1, cf, 0, "invalid network \"%V\"", net);
        return -1;
    }
    ...
    ngx_str_t *name)
{
    ...
    file.fd = open((const char *) name->data, O_RDONLY|0, 0);

    if (file.fd == -1) {
        err = errno;
        if (err != ENOENT) {
            ngx_conf_log_error(3, cf, err,
                               "open()" " \"%s\" failed", name->data);
        }
        return -5;
    }
    ...
    if (fstat(file.fd, &fi) == -1) {
        ngx_conf_log_error(3, cf, errno,
                           "fstat()" " \"%s\" failed", name->data);
        goto failed;
    }
    ...
    if (stat((const char *) name->data, &fi) == -1) {
        ngx_conf_log_error(3, cf, errno,
                           "stat()" " \"%s\" failed", name->data);
        goto failed;
    }
    ...
    if (n == -1) {
        ngx_conf_log_error(3, cf, errno,
                           "pread()" " \"%s\" failed", name->data);
        goto failed;
    }

    if ((size_t) n != size) {
        ngx_conf_log_error(3, cf, 0,
    ...
    if (size < 16 || memcmp((const char *) &ngx_http_geo_header, (const char *) header, 12) != 0) {
        ngx_conf_log_error(5, cf, 0,
    ...
    while (vv->data) {
        len = (((sizeof(ngx_http_variable_value_t) + vv->len) + (sizeof(void *) - 1)) & ~(sizeof(void *) - 1))
                                       ;
...
failed:
    ...
    if (close(file.fd) == -1) {
        if ((cf->log)->log_level >= 2) ngx_log_error_core(2, cf->log, errno, "close()" " \"%s\" failed", name->data)
                                                                    ;
    }
...
ngx_http_geo_create_binary_base(ngx_http_geo_conf_ctx_t *ctx)
{
    ...
    if ((fm.log)->log_level >= 6) ngx_log_error_core(6, fm.log, 0, "creating binary geo range base \"%s\"", fm.name)
                                                                   ;

    if (ngx_create_file_mapping(&fm) != 0) {
        return;
    }

    p = (((u_char *) memcpy(fm.addr, &ngx_http_geo_header, sizeof(ngx_http_geo_header_t))) + (sizeof(ngx_http_geo_header_t)))
                                                 ;
    ...
    ngx_rbtree_node_t *sentinel)
{
    ...
    *vv = *gvvn->value;
    ...
    p = (((u_char *) memcpy(p, gvvn->sn.str.data, gvvn->sn.str.len)) + (gvvn->sn.str.len));

    p = (u_char *) (((uintptr_t) (p) + ((uintptr_t) sizeof(void *) - 1)) & ~((uintptr_t) sizeof(void *) - 1));
