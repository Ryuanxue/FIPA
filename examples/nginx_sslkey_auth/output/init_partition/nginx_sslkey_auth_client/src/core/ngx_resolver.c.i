...
# 11 "src/core/ngx_resolver.c" 2
# 19 "src/core/ngx_resolver.c"
typedef struct {
    u_char ident_hi;
    u_char ident_lo;
    u_char flags_hi;
...
ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names, ngx_uint_t n)
{
    ...
    ((&r->name_sentinel)->color = 0); (&r->name_rbtree)->root = &r->name_sentinel; (&r->name_rbtree)->sentinel = &r->name_sentinel; (&r->name_rbtree)->insert = ngx_resolver_rbtree_insert_value
                                                     ;

    ((&r->srv_sentinel)->color = 0); (&r->srv_rbtree)->root = &r->srv_sentinel; (&r->srv_rbtree)->sentinel = &r->srv_sentinel; (&r->srv_rbtree)->insert = ngx_resolver_rbtree_insert_value
                                                     ;

    ((&r->addr_sentinel)->color = 0); (&r->addr_rbtree)->root = &r->addr_sentinel; (&r->addr_rbtree)->sentinel = &r->addr_sentinel; (&r->addr_rbtree)->insert = ngx_rbtree_insert_value
                                            ;

    (&r->name_resend_queue)->prev = &r->name_resend_queue; (&r->name_resend_queue)->next = &r->name_resend_queue;
    (&r->srv_resend_queue)->prev = &r->srv_resend_queue; (&r->srv_resend_queue)->next = &r->srv_resend_queue;
    (&r->addr_resend_queue)->prev = &r->addr_resend_queue; (&r->addr_resend_queue)->next = &r->addr_resend_queue;

    (&r->name_expire_queue)->prev = &r->name_expire_queue; (&r->name_expire_queue)->next = &r->name_expire_queue;
    (&r->srv_expire_queue)->prev = &r->srv_expire_queue; (&r->srv_expire_queue)->next = &r->srv_expire_queue;
    (&r->addr_expire_queue)->prev = &r->addr_expire_queue; (&r->addr_expire_queue)->next = &r->addr_expire_queue;


    r->ipv6 = 1;

    ((&r->addr6_sentinel)->color = 0); (&r->addr6_rbtree)->root = &r->addr6_sentinel; (&r->addr6_rbtree)->sentinel = &r->addr6_sentinel; (&r->addr6_rbtree)->insert = ngx_resolver_rbtree_insert_addr6_value
                                                           ;

    (&r->addr6_resend_queue)->prev = &r->addr6_resend_queue; (&r->addr6_resend_queue)->next = &r->addr6_resend_queue;

    (&r->addr6_expire_queue)->prev = &r->addr6_expire_queue; (&r->addr6_expire_queue)->next = &r->addr6_expire_queue;
    ...
    for (i = 0; i < n; i++) {
        if (strncmp((const char *) names[i].data, (const char *) "valid=", 6) == 0) {
            s.len = names[i].len - 6;
        ...
        if (strncmp((const char *) names[i].data, (const char *) "ipv6=", 5) == 0) {

            if (strcmp((const char *) &names[i].data[5], (const char *) "on") == 0) {
                r->ipv6 = 1;

            } else if (strcmp((const char *) &names[i].data[5], (const char *) "off") == 0) {
                r->ipv6 = 0;
        ...
        (void) memset(rec, 0, u.naddrs * sizeof(ngx_resolver_connection_t));
    ...
    if (n && r->connections.nelts == 0) {
        ngx_conf_log_error(1, cf, 0, "no name servers defined");
        return 0;
    }
...
ngx_resolver_cleanup_tree(ngx_resolver_t *r, ngx_rbtree_t *tree)
{
    ...
    while (tree->root != tree->sentinel) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) (ngx_rbtree_min(tree->root, tree->sentinel)) - 
# 343 "src/core/ngx_resolver.c" 3 4
            0
# 343 "src/core/ngx_resolver.c"
            );

        (&rn->queue)->next->prev = (&rn->queue)->prev; (&rn->queue)->prev->next = (&rn->queue)->next;
...
ngx_resolve_start(ngx_resolver_t *r, ngx_resolver_ctx_t *temp)
{
    ...
    if (temp) {
        ...
        if (addr != ((unsigned int) -1)) {
            ...
            (void) memset(&temp->sin, 0, sizeof(struct sockaddr_in));
...
ngx_resolve_name_done(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state == -2 || ctx->state == ETIMEDOUT) {
        ...
        if (rn) {
            ...
            if ((r->log)->log_level >= 2) ngx_log_error_core(2, r->log, 0, "could not cancel %V resolving", &ctx->name)
                                                                      ;
        }
    }
    ...
    ngx_str_t *name)
{
    ...
    if (rn) {
        ...
        if (rn->valid >= ngx_cached_time->sec) {

            ;

            (&rn->queue)->next->prev = (&rn->queue)->prev; (&rn->queue)->prev->next = (&rn->queue)->next;

            rn->expire = ngx_cached_time->sec + r->expire;

            (&rn->queue)->next = (expire_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = expire_queue; (expire_queue)->next = &rn->queue;
            ...
            if (naddrs) {
                ...
                do {
                    ...
                    if (addrs == 0) {
                        ...
                        (void) memset(&ctx->sin, 0, sizeof(struct sockaddr_in));
            ...
            do {
                ctx->state = 3;
                ctx->valid = ngx_cached_time->sec + (r->valid ? r->valid : 10);
                next = ctx->next;
        ...
        if (rn->waiting) {
            if (ngx_resolver_set_timeout(r, ctx) != 0) {
                return -1;
            }
    ...
    if (ngx_resolver_send_query(r, rn) != 0) {
    ...
    rn->expire = ngx_cached_time->sec + r->resend_timeout;

    (&rn->queue)->next = (resend_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = resend_queue; (resend_queue)->next = &rn->queue;

    rn->code = 0;
    rn->cnlen = 0;
    rn->valid = 0;
    rn->ttl = (uint32_t) 0xffffffff;
    rn->waiting = ctx;
...
ngx_resolve_addr(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ngx_resolver_create_addr_query(r, rn, &ctx->addr) != 0) {
        goto failed;
    }
...
ngx_resolve_addr_done(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state == -2 || ctx->state == ETIMEDOUT) {
        ...
        {
            u_char text[(sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 1180 "src/core/ngx_resolver.c" 3 4
                           0
# 1180 "src/core/ngx_resolver.c"
                           )];
            ngx_str_t addrtext;

            addrtext.data = text;
            addrtext.len = ngx_sock_ntop(ctx->addr.sockaddr, ctx->addr.socklen,
                                         text, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 1185 "src/core/ngx_resolver.c" 3 4
                                              0
# 1185 "src/core/ngx_resolver.c"
                                              ), 0);

            if ((r->log)->log_level >= 2) ngx_log_error_core(2, r->log, 0, "could not cancel %V resolving", &addrtext)
                                                                     ;
        }
    }
...
ngx_resolver_expire(ngx_resolver_t *r, ngx_rbtree_t *tree, ngx_queue_t *queue)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) q - 
# 1233 "src/core/ngx_resolver.c" 3 4
            0
# 1233 "src/core/ngx_resolver.c"
            );
    ...
    u_char *query, u_short qlen)
{
    ...
    if ((size_t) n != (size_t) qlen) {
        if ((&rec->log)->log_level >= 3) ngx_log_error_core(3, &rec->log, 0, "send() incomplete");
        goto failed;
    }
    ...
    u_char *query, u_short qlen)
{
    ...
    if (b->end - b->last < 2 + qlen) {
        if ((&rec->log)->log_level >= 3) ngx_log_error_core(3, &rec->log, 0, "buffer overflow");
        return -1;
    }

    *b->last++ = (u_char) (qlen >> 8);
    *b->last++ = (u_char) qlen;
    b->last = (((u_char *) memcpy(b->last, query, qlen)) + (qlen));
...
ngx_resolver_resend_handler(ngx_event_t *ev)
{
    ...
    if (timer == 0) {
        ...
        timer = ((timer > atimer) ? (atimer) : (timer));
    }
    ...
    if (timer == 0) {
        ...
        timer = ((timer > stimer) ? (stimer) : (timer));
    }
    ...
    if (timer == 0) {
        ...
        timer = ((timer > a6timer) ? (a6timer) : (timer));
    }
...
ngx_resolver_resend(ngx_resolver_t *r, ngx_rbtree_t *tree, ngx_queue_t *queue)
{
    ...
    for ( ;; ) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) q - 
# 1505 "src/core/ngx_resolver.c" 3 4
            0
# 1505 "src/core/ngx_resolver.c"
            );
        ...
        if (rn->waiting) {
            ...
            (q)->next = (queue)->next; (q)->next->prev = q; (q)->prev = queue; (queue)->next = q;
...
{
    return (&r->name_resend_queue == (&r->name_resend_queue)->prev)
           && (&r->srv_resend_queue == (&r->srv_resend_queue)->prev)

           && (&r->addr6_resend_queue == (&r->addr6_resend_queue)->prev)

           && (&r->addr_resend_queue == (&r->addr_resend_queue)->prev);
}
...
ngx_resolver_tcp_write(ngx_event_t *wev)
{
    ...
    if (b->pos != b->start) {
        b->last = (((u_char *) memmove(b->start, b->pos, b->last - b->pos)) + (b->last - b->pos));
        b->pos = b->start;
    }
    ...
    if (ngx_handle_write_event(wev, 0) != 0) {
        goto failed;
    }
    ...
    ngx_uint_t tcp)
{
    ...
    if ((flags & 0xf870) != 0x8000 || (trunc && tcp)) {
        if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "invalid %s DNS response %ui fl:%04Xi", tcp ? "TCP" : "UDP", ident, flags)
    ...
    if (code == 1) {
        ...
        for (q = (&r->name_resend_queue)->next;
             q != (&r->name_resend_queue) && times++ < 100;
             q = (q)->next)
        {
            rn = (ngx_resolver_node_t *) ((u_char *) q - 
# 1752 "src/core/ngx_resolver.c" 3 4
                0
# 1752 "src/core/ngx_resolver.c"
                );
...
found:
    ...
    if (qclass != 1) {
        if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unknown query class %ui in DNS response", qclass)
                                                                        ;
        return;
    }
    ...
    default:
        if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unknown query type %ui in DNS response", qtype)
                                                                      ;
        return;
    }
...
short_response:
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, err);

    return;

dns_error_name:

    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "DNS error (%ui: %s), query id:%ui, name:\"%*s\"", code, ngx_resolver_strerror(code), ident, (size_t) rn->nlen, rn->name)
...
dns_error:
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "DNS error (%ui: %s), query id:%ui", code, ngx_resolver_strerror(code), ident)
    ...
    ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    if (rn == 0) {
        if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unexpected response for %V", &name)
                                                          ;
        ngx_resolver_free(r, name.data);
        goto failed;
    }
    ...
    case 28:
        ...
        if (rn->query6 == 0 || rn->naddrs6 != (u_short) -1) {
            if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unexpected response for %V", &name)
                                                              ;
            ngx_resolver_free(r, name.data);
            goto failed;
    ...
    default:
        ...
        if (rn->query == 0 || rn->naddrs != (u_short) -1) {
            if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unexpected response for %V", &name)
                                                              ;
            ngx_resolver_free(r, name.data);
            goto failed;
    ...
    if (ident != qident) {
        if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "wrong ident %ui response for %V, expect %ui", ident, &name, qident)
    ...
    if (trunc) {
        ...
        (&rn->queue)->next = (&r->name_resend_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = &r->name_resend_queue; (&r->name_resend_queue)->next = &rn->queue;
    ...
    found:
        ...
        if (class != 1) {
            if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unexpected RR class %ui", class)
                                                           ;
            goto failed;
        }
        ...
        rn->ttl = ((rn->ttl > (uint32_t) ttl) ? ((uint32_t) ttl) : (rn->ttl));
        ...
        default:
            ...
            if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "unexpected RR type %ui", type)
                                                         ;
        }
    ...
    if (naddrs) {
        ...
        for (a = 0; a < nan; a++) {
            ...
            if (type == 1) {
                ...
                (void) memcpy(addr6[j].s6_addr, &buf[i], 16);
    ...
    export:
        ...
        rn->valid = ngx_cached_time->sec + (r->valid ? r->valid : (time_t) rn->ttl);
        rn->expire = ngx_cached_time->sec + r->expire;

        (&rn->queue)->next = (&r->name_expire_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = &r->name_expire_queue; (&r->name_expire_queue)->next = &rn->queue;
    ...
    if (cname) {
        ...
        if (ngx_resolver_copy(r, &name, buf, cname, buf + n) != 0) {
            goto failed;
        }
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "no A or CNAME types in DNS response")
                                                        ;
    return;
    ...
    ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    if (trunc) {
        ...
        (&rn->queue)->next = (&r->srv_resend_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = &r->srv_resend_queue; (&r->srv_resend_queue)->next = &rn->queue;
    ...
    found:
        ...
        case 33:
            ...
            if (ngx_resolver_copy(r, 0, buf, &buf[i + 6], buf + n)
                != 0)
            {
                goto failed;
            }
    ...
    if (nsrvs) {
        ...
        (&rn->queue)->next = (&r->srv_expire_queue)->next; (&rn->queue)->next->prev = &rn->queue; (&rn->queue)->prev = &r->srv_expire_queue; (&r->srv_expire_queue)->next = &rn->queue;
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "no SRV type in DNS response");
...
ngx_resolver_resolve_srv_names(ngx_resolver_ctx_t *ctx, ngx_resolver_node_t *rn)
{
    ...
    for (i = 0; i < (ngx_uint_t) rn->nsrvs; i++) {
        ...
        if (srvs[i].name.data == 0) {
            goto failed;
        }

        srvs[i].name.len = rn->u.srvs[i].name.len;
        (void) memcpy(srvs[i].name.data, rn->u.srvs[i].name.data, srvs[i].name.len)
                                    ;

        cctx = ngx_resolve_start(r, 0);
        if (cctx == 0) {
            goto failed;
        }
...
ngx_resolver_srv_names_handler(ngx_resolver_ctx_t *cctx)
{
    ...
    if (cctx->naddrs) {
        ...
        ctx->valid = ((ctx->valid > cctx->valid) ? (cctx->valid) : (ctx->valid));
        ...
        for (i = 0; i < cctx->naddrs; i++) {
            ...
            (void) memcpy(&sockaddr[i], cctx->addrs[i].sockaddr, addrs[i].socklen)
                                        ;
...
invalid_ip6_arpa:
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "invalid in-addr.arpa or ip6.arpa name in DNS response")
                                                                          ;
    ngx_resolver_free(r, name.data);
    return;
...
valid:
    ...
    if ((r->log)->log_level >= r->log_level) ngx_log_error_core(r->log_level, r->log, 0, "no PTR type in DNS response")
                                                ;
    return;

ptr:

    if (ngx_resolver_copy(r, &name, buf, buf + i, buf + n) != 0) {
        goto failed;
    }
    ...
    if (name.len != (size_t) rn->nlen
        || strncmp((const char *) name.data, (const char *) rn->name, name.len) != 0)
    {
        if (rn->nlen) {
            ngx_resolver_free(r, rn->name);
        }
    ...
    rn->valid = ngx_cached_time->sec + (r->valid ? r->valid : ttl);
    rn->expire = ngx_cached_time->sec + r->expire;
...
ngx_resolver_lookup_name(ngx_resolver_t *r, ngx_str_t *name, uint32_t hash)
{
    ...
    while (node != sentinel) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3391 "src/core/ngx_resolver.c" 3 4
            0
# 3391 "src/core/ngx_resolver.c"
            );
...
ngx_resolver_lookup_srv(ngx_resolver_t *r, ngx_str_t *name, uint32_t hash)
{
    ...
    while (node != sentinel) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3432 "src/core/ngx_resolver.c" 3 4
            0
# 3432 "src/core/ngx_resolver.c"
            );
...
ngx_resolver_lookup_addr(ngx_resolver_t *r, in_addr_t addr)
{
    ...
    while (node != sentinel) {
        ...
        return (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3471 "src/core/ngx_resolver.c" 3 4
              0
# 3471 "src/core/ngx_resolver.c"
              );
    }
    ...
    uint32_t hash)
{
    ...
    while (node != sentinel) {
        ...
        rn = (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3507 "src/core/ngx_resolver.c" 3 4
            0
# 3507 "src/core/ngx_resolver.c"
            );

        rc = memcmp((const char *) addr, (const char *) &rn->addr6, 16);
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            rn = (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3545 "src/core/ngx_resolver.c" 3 4
                0
# 3545 "src/core/ngx_resolver.c"
                );
            rn_temp = (ngx_resolver_node_t *) ((u_char *) (temp) - 
# 3546 "src/core/ngx_resolver.c" 3 4
                     0
# 3546 "src/core/ngx_resolver.c"
                     );
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            rn = (ngx_resolver_node_t *) ((u_char *) (node) - 
# 3588 "src/core/ngx_resolver.c" 3 4
                0
# 3588 "src/core/ngx_resolver.c"
                );
            rn_temp = (ngx_resolver_node_t *) ((u_char *) (temp) - 
# 3589 "src/core/ngx_resolver.c" 3 4
                     0
# 3589 "src/core/ngx_resolver.c"
                     );

            p = (memcmp((const char *) &rn->addr6, (const char *) &rn_temp->addr6, 16)
                 < 0) ? &temp->left : &temp->right;
        }
    ...
    ngx_str_t *name)
{
    ...
    *p = (u_char) len;
    ...
    (void) memcpy(p, rn->query, rn->qlen);
    ...
    ngx_resolver_addr_t *addr)
{
    ...
    case AF_INET6:
        ...
        p = (((u_char *) memcpy(p, "\3ip6\4arpa\0", 10)) + (10));
    ...
    default:
        ...
        p = (((u_char *) memcpy(p, "\7in-addr\4arpa\0", 14)) + (14));
    }


    p = (((u_char *) memcpy(p, "\0\14\0\1", 4)) + (4));
    ...
    ngx_uint_t rotate)
{
    ...
    if (rn->naddrs6) {
        ...
        do {
            ...
            (void) memcpy(sin6->sin6_addr.s6_addr, addr6[j++].s6_addr, 16);
...
ngx_udp_connect(ngx_resolver_connection_t *rec)
{
    ...
    s = socket(rec->sockaddr->sa_family, SOCK_DGRAM, 0);

    ;

    if (s == (ngx_socket_t) -1) {
        if ((&rec->log)->log_level >= 2) ngx_log_error_core(2, &rec->log, errno, "socket()" " failed")
                                             ;
        return -1;
    }
    ...
    if (c == 0) {
        if (close(s) == -1) {
            if ((&rec->log)->log_level >= 2) ngx_log_error_core(2, &rec->log, errno, "close() socket" " failed")
                                                       ;
        }
    ...
    if (ngx_nonblocking(s) == -1) {
        if ((&rec->log)->log_level >= 2) ngx_log_error_core(2, &rec->log, errno, "ioctl(FIONBIO)" " failed")
                                                  ;
    ...
    if (rc == -1) {
        if ((&rec->log)->log_level >= 3) ngx_log_error_core(3, &rec->log, errno, "connect() failed")
                                         ;
    ...
    event = (ngx_event_flags & 0x00000004) ?
                                                    EPOLLET:
                                                    0;
...
ngx_tcp_connect(ngx_resolver_connection_t *rec)
{
    ...
    s = socket(rec->sockaddr->sa_family, SOCK_STREAM, 0);
    ...
    if (rc == -1) {
            ...
            )
        {
            ...
            if ((&rec->log)->log_level >= level) ngx_log_error_core(level, &rec->log, err, "connect() to %V failed", &rec->server)
                                       ;
