...
# 106 "src/core/ngx_log.c"
{
    ...
    *p++ = (u_char) '\n';;
    ...
    while (log) {
        ...
        if (ngx_cached_time->sec == log->disk_full_time) {
        ...
        if (n == -1 && errno == ENOSPC) {
            log->disk_full_time = ngx_cached_time->sec;
        }
...
ngx_log_stderr(ngx_err_t err, const char *fmt, ...)
{
    ...
    p = (((u_char *) memcpy(errstr, "nginx: ", 7)) + (7));
    ...
    *p++ = (u_char) '\n';;
    ...
    (void) ngx_write_fd(STDERR_FILENO, errstr, p - errstr);
}
...
ngx_log_init(u_char *prefix)
{
    ...
    if (name[0] != '/') {
        ...
        if (prefix) {
            ...
            prefix = (u_char *) "/home/raoxue/Desktop/IF-driver-partition/FIPA/examples/nginx/output/temp/nginx_install_64/";
            plen = strlen((const char *) prefix);
    ...
    ngx_log_file.fd = open((const char *) name, (O_WRONLY|O_APPEND)|O_CREAT, 0644)
...
{
    ngx_log_t *log;
    static ngx_str_t error_log = { sizeof("logs/error.log") - 1, (u_char *) "logs/error.log" };

    if (ngx_log_get_file_log(&cycle->new_log) != 0) {
        return 0;
    }
...
ngx_log_redirect_stderr(ngx_cycle_t *cycle)
{
    ...
    if (fd != STDERR_FILENO) {
        if (dup2(fd, STDERR_FILENO) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "dup2(STDERR_FILENO)" " failed")
                                                     ;
...
ngx_log_set_log(ngx_conf_t *cf, ngx_log_t **head)
{
    ...
    } else if (strncmp((const char *) value[1].data, (const char *) "memory:", 7) == 0) {
# 642 "src/core/ngx_log.c"
        ngx_conf_log_error(1, cf, 0,
                           "nginx was built without debug support");
        return (void *) -1;


    } else if (strncmp((const char *) value[1].data, (const char *) "syslog:", 7) == 0) {
        peer = ngx_pcalloc(cf->pool, sizeof(ngx_syslog_peer_t));
        if (peer == 0) {
            return (void *) -1;
        }

        if (ngx_syslog_process_conf(cf, peer) != 0) {
            return (void *) -1;
        }
    ...
    if (ngx_log_set_levels(cf, new_log) != 0) {
        return (void *) -1;
    }
