...
ngx_create_pool(size_t size, ngx_log_t *log)
{
    ...
    p->max = (size < (ngx_pagesize - 1)) ? size : (ngx_pagesize - 1);
...
ngx_palloc_small(ngx_pool_t *pool, size_t size, ngx_uint_t align)
{
    ...
    do {
        ...
        if (align) {
            m = (u_char *) (((uintptr_t) (m) + ((uintptr_t) sizeof(unsigned long) - 1)) & ~((uintptr_t) sizeof(unsigned long) - 1));
        }
...
ngx_pool_cleanup_file(void *data)
{
    ...
    if (close(c->fd) == -1) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, errno, "close()" " \"%s\" failed", c->name)
                                                                 ;
    }
}
...
ngx_pool_delete_file(void *data)
{
    ...
    if (unlink((const char *) c->name) == -1) {
        ...
        if (err != ENOENT) {
            if ((c->log)->log_level >= 3) ngx_log_error_core(3, c->log, err, "unlink()" " \"%s\" failed", c->name)
                                                                      ;
        }
    }
