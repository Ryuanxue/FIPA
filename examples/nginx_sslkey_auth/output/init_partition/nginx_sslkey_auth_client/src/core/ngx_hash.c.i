...
ngx_hash_find(ngx_hash_t *hash, ngx_uint_t key, u_char *name, size_t len)
{
    ...
    next:
        ...
        elt = (ngx_hash_elt_t *) (u_char *) (((uintptr_t) (&elt->name[0] + elt->len) + ((uintptr_t) sizeof(void *) - 1)) & ~((uintptr_t) sizeof(void *) - 1))
                                                              ;
        continue;
    }
...
ngx_hash_init(ngx_hash_init_t *hinit, ngx_hash_key_t *names, ngx_uint_t nelts)
{
    ...
    if (hinit->max_size == 0) {
        if ((hinit->pool->log)->log_level >= 1) ngx_log_error_core(1, hinit->pool->log, 0, "could not build %s, you should " "increase %s_max_size: %i", hinit->name, hinit->name, hinit->max_size)
    ...
    for (n = 0; n < nelts; n++) {
        if (hinit->bucket_size < (sizeof(void *) + ((((&names[n])->key.len + 2) + (sizeof(void *) - 1)) & ~(sizeof(void *) - 1))) + sizeof(void *))
        {
            if ((hinit->pool->log)->log_level >= 1) ngx_log_error_core(1, hinit->pool->log, 0, "could not build %s, you should " "increase %s_bucket_size: %i", hinit->name, hinit->name, hinit->bucket_size)
    ...
    for (size = start; size <= hinit->max_size; size++) {
        ...
        (void) memset(test, 0, size * sizeof(u_short));

        for (n = 0; n < nelts; n++) {
            if (names[n].key.data == 0) {
                continue;
            }

            key = names[n].key_hash % size;
            test[key] = (u_short) (test[key] + (sizeof(void *) + ((((&names[n])->key.len + 2) + (sizeof(void *) - 1)) & ~(sizeof(void *) - 1))));
    ...
    if ((hinit->pool->log)->log_level >= 5) ngx_log_error_core(5, hinit->pool->log, 0, "could not build optimal %s, you should increase " "either %s_max_size: %i or %s_bucket_size: %i; " "ignoring %s_bucket_size", hinit->name, hinit->name, hinit->max_size, hinit->name, hinit->bucket_size, hinit->name)
...
found:
    ...
    for (i = 0; i < size; i++) {
        ...
        test[i] = (u_short) ((((test[i]) + (ngx_cacheline_size - 1)) & ~(ngx_cacheline_size - 1)));
    ...
    elts = (u_char *) (((uintptr_t) (elts) + ((uintptr_t) ngx_cacheline_size - 1)) & ~((uintptr_t) ngx_cacheline_size - 1));
    ...
    ngx_uint_t nelts)
{
    ...
    for (n = 0; n < nelts; n = i) {
        ...
        for (i = n + 1; i < nelts; i++) {
            if (strncmp((const char *) names[n].key.data, (const char *) names[i].key.data, len) != 0) {
                break;
            }
...
ngx_hash_key_lc(u_char *data, size_t len)
{
    ...
    for (i = 0; i < len; i++) {
        key = ((ngx_uint_t) key * 31 + (u_char) ((data[i] >= 'A' && data[i] <= 'Z') ? (data[i] | 0x20) : data[i]));
    }
...
ngx_hash_strlow(u_char *dst, u_char *src, size_t n)
{
    ...
    while (n--) {
        *dst = (u_char) ((*src >= 'A' && *src <= 'Z') ? (*src | 0x20) : *src);
        key = ((ngx_uint_t) key * 31 + *dst);
        dst++;
        src++;
    }
...
ngx_hash_keys_array_init(ngx_hash_keys_arrays_t *ha, ngx_uint_t type)
{
    ...
    if (ha->dns_wc_head_hash == 0) {
        return -1;
    }

    ha->dns_wc_tail_hash = ngx_pcalloc(ha->temp_pool,
                                       sizeof(ngx_array_t) * ha->hsize);
    if (ha->dns_wc_tail_hash == 0) {
        return -1;
    }
    ...
    ngx_uint_t flags)
{
    ...
    for (i = 0; i < last; i++) {
        if (!(flags & 2)) {
            key->data[i] = (u_char) ((key->data[i] >= 'A' && key->data[i] <= 'Z') ? (key->data[i] | 0x20) : key->data[i]);
        }
        k = ((ngx_uint_t) k * 31 + key->data[i]);
    }
    ...
    if (name) {
        for (i = 0; i < ha->keys_hash[k].nelts; i++) {
            ...
            if (strncmp((const char *) key->data, (const char *) name[i].data, last) == 0) {
                return -3;
            }
        }
...
wildcard:
    ...
    if (skip == 1) {
        ...
        if (name) {
            ...
            for (i = 0; i < ha->keys_hash[k].nelts; i++) {
                ...
                if (strncmp((const char *) &key->data[1], (const char *) name[i].data, len) == 0) {
                    return -3;
                }
            }
        ...
        (void) memcpy(name->data, &key->data[1], name->len);
    }
    ...
    if (skip) {
        ...
        for (i = last - 1; i; i--) {
            if (key->data[i] == '.') {
                (void) memcpy(&p[n], &key->data[i + 1], len);
                n += len;
                p[n++] = '.';
                len = 0;
                continue;
        ...
        if (len) {
            (void) memcpy(&p[n], &key->data[1], len);
            n += len;
        }
    ...
    if (name) {
        ...
        for (i = 0; i < keys->nelts; i++) {
            ...
            if (strncmp((const char *) key->data + skip, (const char *) name[i].data, len) == 0) {
                return -3;
            }
        }

    } else {
        if (ngx_array_init(keys, ha->temp_pool, 4, sizeof(ngx_str_t)) != 0)
        {
            return -1;
        }
    }
    ...
    (void) memcpy(name->data, key->data + skip, name->len);
