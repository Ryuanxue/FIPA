    ...
    ngx_chain_t **out, ngx_buf_tag_t tag)
{
    ...
    while (*busy) {
        ...
        if (((cl->buf->temporary || cl->buf->memory || cl->buf->mmap) ? (off_t) (cl->buf->last - cl->buf->pos): (cl->buf->file_last - cl->buf->file_pos)) != 0) {
            break;
        }
