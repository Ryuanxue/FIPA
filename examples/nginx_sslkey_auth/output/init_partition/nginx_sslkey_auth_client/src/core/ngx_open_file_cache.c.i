# 1 "src/core/ngx_open_file_cache.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/core/ngx_open_file_cache.c" 2
...
# 10 "src/core/ngx_open_file_cache.c" 2
...
# 11 "src/core/ngx_open_file_cache.c" 2
# 24 "src/core/ngx_open_file_cache.c"
...
ngx_open_file_cache_init(ngx_pool_t *pool, ngx_uint_t max, time_t inactive)
{
    ...
    ((&cache->sentinel)->color = 0); (&cache->rbtree)->root = &cache->sentinel; (&cache->rbtree)->sentinel = &cache->sentinel; (&cache->rbtree)->insert = ngx_open_file_cache_rbtree_insert_value
                                                            ;
...
ngx_open_file_cache_cleanup(void *data)
{
    ...
    for ( ;; ) {
        ...
        file = (ngx_cached_open_file_t *) ((u_char *) q - 
# 107 "src/core/ngx_open_file_cache.c" 3 4
              0
# 107 "src/core/ngx_open_file_cache.c"
              );
    ...
    if (cache->current) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "%ui items still left in open file cache", cache->current)

                                     ;
    }

    if (cache->rbtree.root != cache->rbtree.sentinel) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "rbtree still is not empty in open file cache")
                                                                     ;
    ...
    ngx_open_file_info_t *of, ngx_pool_t *pool)
{
    ...
    if (cache == 0) {
        ...
        if (of->test_only) {
            ...
            of->uniq = (&fi)->st_ino;
            of->mtime = (&fi)->st_mtime;
            of->size = (&fi)->st_size;
            of->fs_size = (((&fi)->st_size < (&fi)->st_blocks * 512) ? ((&fi)->st_blocks * 512) : ((&fi)->st_size));
            of->is_dir = (S_ISDIR((&fi)->st_mode));
            of->is_file = (S_ISREG((&fi)->st_mode));
            of->is_link = (S_ISLNK((&fi)->st_mode));
            of->is_exec = (((&fi)->st_mode & S_IXUSR) == S_IXUSR);
    ...
    if (file) {
        ...
        (&file->queue)->next->prev = (&file->queue)->prev; (&file->queue)->prev->next = (&file->queue)->next;

        if (file->fd == -1 && file->err == 0 && !file->is_dir) {



            rc = ngx_open_and_stat_file(name, of, pool->log);

            if (rc != 0 && (of->err == 0 || !of->errors)) {
                goto failed;
            }
            ...
            ))
        {
            if (file->err == 0) {
                ...
                of->failed = file->disable_symlinks ? "openat()"
                                                    : "open()";
        ...
        if (file->count == 0) {
            ...
            if (close(file->fd) == -1) {
                if ((pool->log)->log_level >= 2) ngx_log_error_core(2, pool->log, errno, "close()" " \"%V\" failed", name)
                                                                      ;
            }
...
found:
    ...
    (&file->queue)->next = (&cache->expire_queue)->next; (&file->queue)->next->prev = &file->queue; (&file->queue)->prev = &cache->expire_queue; (&cache->expire_queue)->next = &file->queue;
...
failed:
    ...
    if (file) {
        ...
        if (file->count == 0) {
            ...
            if (file->fd != -1) {
                if (close(file->fd) == -1) {
                    if ((pool->log)->log_level >= 2) ngx_log_error_core(2, pool->log, errno, "close()" " \"%s\" failed", file->name)
...
# 511 "src/core/ngx_open_file_cache.c"
    fd = openat(at_fd, (const char *) name, mode|create, access);

    if (fd == -1) {
        return -1;
    }

    if (fstatat(at_fd, (const char *) name, &atfi, AT_SYMLINK_NOFOLLOW)
        == -1)
    {
        err = errno;
        goto failed;
    ...
    if (ngx_file_o_path_info(fd, &fi, log) == -1) {
        err = errno;
        goto failed;
    }
...
failed:
    ...
    if (close(fd) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close()" " \"%s\" failed", name)
                                                              ;
    }
...
# 585 "src/core/ngx_open_file_cache.c"
    if (use_fstat) {
        if (fstat(fd, fi) != -1) {
            return 0;
        }
        ...
        if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "fstat(O_PATH) failed with EBADF, " "switching to fstatat(AT_EMPTY_PATH)")
    ...
    if (fstatat(fd, (const char *) "", fi, AT_EMPTY_PATH) != -1) {
        return 0;
    }
...
{
    ngx_fd_t fd;
# 633 "src/core/ngx_open_file_cache.c"
    u_char *p, *cp, *end;
    ngx_fd_t at_fd;
    ngx_str_t at_name;

    if (of->disable_symlinks == 0) {
        fd = open((const char *) name->data, mode|create, access);
    ...
    if (of->disable_symlinks_from) {
        ...
        *cp = '\0';
        ...
        at_fd = open((const char *) p, (O_PATH|O_RDONLY|0)|O_NONBLOCK|0, 0)
                                               ;
        ...
        *cp = '/';
        ...
        at_fd = open((const char *) "/", (O_PATH|O_RDONLY|0)|O_NONBLOCK|0, 0)
    ...
    for ( ;; ) {
        ...
        *cp = '\0';
        ...
        if (of->disable_symlinks == 2) {
            fd = ngx_openat_file_owner(at_fd, p,
                                       (O_PATH|O_RDONLY|0)|O_NONBLOCK,
                                       0, 0, log);

        } else {
            fd = openat(at_fd, (const char *) p, (O_PATH|O_RDONLY|0)|O_NONBLOCK|O_NOFOLLOW|0, 0)
        ...
        *cp = '/';
        ...
        if (at_fd != (ngx_fd_t) AT_FDCWD && close(at_fd) == -1) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close()" " \"%V\" failed", &at_name)
                                                                      ;
        }
    ...
    if (p == end) {
# 748 "src/core/ngx_open_file_cache.c"
        fd = openat(at_fd, (const char *) ".", mode|create, access);
        goto done;
    }

    if (of->disable_symlinks == 2
        && !(create & (O_CREAT|(O_CREAT|O_TRUNC))))
    {
        fd = ngx_openat_file_owner(at_fd, p, mode, create, access, log);

    } else {
        fd = openat(at_fd, (const char *) p, mode|O_NOFOLLOW|create, access);
    }
...
{
    ngx_int_t rc;
# 800 "src/core/ngx_open_file_cache.c"
    ngx_fd_t fd;

    if (of->disable_symlinks == 0) {

        rc = stat((const char *) name->data, fi);
    ...
    fd = ngx_open_file_wrapper(name, of, O_RDONLY|O_NONBLOCK,
                               0, 0, log);
    ...
    if (close(fd) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close()" " \"%V\" failed", name)
                                                              ;
    }
    ...
    ngx_log_t *log)
{
    ...
    if (of->fd != -1) {
        ...
        if (ngx_file_info_wrapper(name, of, &fi, log) == -1) {
            of->fd = -1;
            return -1;
        }
    ...
    if (!of->log) {
        ...
        fd = ngx_open_file_wrapper(name, of, (O_WRONLY|O_APPEND),
                                   O_CREAT,
                                   0644, log);
    }
    ...
    if (fstat(fd, &fi) == -1) {
        if ((log)->log_level >= 3) ngx_log_error_core(3, log, errno, "fstat()" " \"%V\" failed", name)
                                                           ;
    ...
    if ((S_ISDIR((&fi)->st_mode))) {
        ...
        if (of->read_ahead && (&fi)->st_size > (128 * 1024)) {
            if (ngx_read_ahead(fd, of->read_ahead) == -1) {
                if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "posix_fadvise(POSIX_FADV_SEQUENTIAL)" " \"%V\" failed", name)
                                                                      ;
            }
        }
        ...
        if (of->directio <= (&fi)->st_size) {
            if (ngx_directio_on(fd) == -1) {
                if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "fcntl(O_DIRECT)" " \"%V\" failed", name)
                                                                       ;
    ...
    ngx_cached_open_file_t *file, ngx_open_file_info_t *of, ngx_log_t *log)
{
    ...
    if (ngx_event_actions.add(file->event, 0, 0x70000000)
        != 0)
    {
...
# 1013 "src/core/ngx_open_file_cache.c"
    return;
}
    ...
    ngx_cached_open_file_t *file, ngx_uint_t min_uses, ngx_log_t *log)
{
    ...
    if (!file->close) {
        ...
        file->accessed = ngx_cached_time->sec;
    ...
    if (file->fd != -1) {
        ...
        if (close(file->fd) == -1) {
            if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "close()" " \"%s\" failed", file->name)
                                                                        ;
        }
...
ngx_open_file_del_event(ngx_cached_open_file_t *file)
{
    ...
    (void) ngx_event_actions.del(file->event, 0,
                         file->count ? 4 : 1);
    ...
    ngx_log_t *log)
{
    ...
    while (n < 3) {
        ...
        file = (ngx_cached_open_file_t *) ((u_char *) q - 
# 1118 "src/core/ngx_open_file_cache.c" 3 4
              0
# 1118 "src/core/ngx_open_file_cache.c"
              );
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            p = (strcmp((const char *) file->name, (const char *) file_temp->name) < 0)
    ...
    uint32_t hash)
{
    ...
    while (node != sentinel) {
        ...
        rc = strcmp((const char *) name->data, (const char *) file->name);
