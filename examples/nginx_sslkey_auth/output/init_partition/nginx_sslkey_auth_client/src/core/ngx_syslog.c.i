...
# 10 "src/core/ngx_syslog.c" 2
# 18 "src/core/ngx_syslog.c"
...
static char *severities[] = {
    "emerg", "alert", "crit", "error", "warn", "notice", "info", "debug", 0
};
...
ngx_syslog_process_conf(ngx_conf_t *cf, ngx_syslog_peer_t *peer)
{
    ...
    if (ngx_syslog_parse_args(cf, peer) != 0) {
        return (void *) -1;
    }

    if (peer->server.sockaddr == 0) {
        ngx_conf_log_error(1, cf, 0,
                           "no syslog server specified");
        return (void *) -1;
    }

    if (peer->facility == (ngx_uint_t) -1) {
        peer->facility = 23;
    }

    if (peer->severity == (ngx_uint_t) -1) {
        peer->severity = 6;
    }

    if (peer->tag.data == 0) {
        (&peer->tag)->len = sizeof("nginx") - 1; (&peer->tag)->data = (u_char *) "nginx";
    }
...
ngx_syslog_parse_args(ngx_conf_t *cf, ngx_syslog_peer_t *peer)
{
    ...
    for ( ;; ) {
        comma = (u_char *) strchr((const char *) p, (int) ',');
        ...
        if (strncmp((const char *) p, (const char *) "server=", 7) == 0) {
        ...
        } else if (strncmp((const char *) p, (const char *) "facility=", 9) == 0) {
            ...
            for (i = 0; facilities[i] != 0; i++) {
                ...
                if (strcmp((const char *) p + 9, (const char *) facilities[i]) == 0) {
                    peer->facility = i;
                    goto next;
                }
            }
        ...
        } else if (strncmp((const char *) p, (const char *) "severity=", 9) == 0) {
            ...
            for (i = 0; severities[i] != 0; i++) {

                if (strcmp((const char *) p + 9, (const char *) severities[i]) == 0) {
                    peer->severity = i;
                    goto next;
                }
            }
        ...
        } else if (strncmp((const char *) p, (const char *) "tag=", 4) == 0) {
            ...
            for (i = 4; i < len; i++) {
                c = (u_char) ((p[i] >= 'A' && p[i] <= 'Z') ? (p[i] | 0x20) : p[i]);
        ...
        } else if (len == 10 && strncmp((const char *) p, (const char *) "nohostname", 10) == 0) {
            peer->nohostname = 1;
...
ngx_syslog_writer(ngx_log_t *log, ngx_uint_t level, u_char *buf,
    size_t len)
{
    u_char *p, msg[2048 + sizeof("<255>Jan 01 00:00:00 ") - 1 + (256 - 1) + 1 + 32 + 2];
    ngx_uint_t head_len;
    ...
    if (len > 2048 + sizeof("<255>Jan 01 00:00:00 ") - 1 + (256 - 1) + 1 + 32 + 2 - head_len) {
        len = 2048 + sizeof("<255>Jan 01 00:00:00 ") - 1 + (256 - 1) + 1 + 32 + 2 - head_len;
    }
...
ngx_syslog_send(ngx_syslog_peer_t *peer, u_char *buf, size_t len)
{
    ...
    if (n == -1) {
        ...
        if (close(peer->conn.fd) == -1) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "close() socket" " failed")
                                                       ;
        }
...
ngx_syslog_init_peer(ngx_syslog_peer_t *peer)
{
    ...
    if (fd == (ngx_socket_t) -1) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "socket()" " failed")
                                             ;
        return -1;
    }

    if (ngx_nonblocking(fd) == -1) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "ioctl(FIONBIO)" " failed")
                                                  ;
        goto failed;
    }

    if (connect(fd, peer->server.sockaddr, peer->server.socklen) == -1) {
        if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, errno, "connect() failed")
                                         ;
        goto failed;
    }
