...
# 242 "src/core/ngx_inet.c" 3 4
                                  0
# 242 "src/core/ngx_inet.c"
                                                                        ) {
            ...
            n = ngx_strnlen((u_char *) saun->sun_path,
                            socklen - 
# 247 "src/core/ngx_inet.c" 3 4
                                     0
# 247 "src/core/ngx_inet.c"
                                                                           );
...
ngx_inet6_ntop(u_char *p, u_char *text, size_t len)
{
    ...
    if (len < (sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255") - 1)) {
        return 0;
    }
...
ngx_ptocidr(ngx_str_t *text, ngx_cidr_t *cidr)
{
    ...
    if (cidr->u.in.addr != ((unsigned int) -1)) {
        cidr->family = AF_INET;
    ...
    } else if (ngx_inet6_addr(addr, len, cidr->u.in6.addr.s6_addr) == 0) {
        cidr->family = AF_INET6;

        if (mask == 0) {
            (void) memset(cidr->u.in6.mask.s6_addr, 0xff, 16);
            return 0;
        }
...
ngx_parse_addr(ngx_pool_t *pool, ngx_addr_t *addr, u_char *text, size_t len)
{
    ...
    (void) memset(&inaddr6, 0, sizeof(struct in6_addr));
    ...
    } else if (ngx_inet6_addr(text, len, inaddr6.s6_addr) == 0) {
        family = AF_INET6;
    ...
    case AF_INET6:
        ...
        (void) memcpy(sin6->sin6_addr.s6_addr, inaddr6.s6_addr, 16);
        break;
...
ngx_parse_inet_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (sin->sin_addr.s_addr != ((unsigned int) -1)) {
        ...
        (void) memcpy(sin, &u->sockaddr, sizeof(struct sockaddr_in));
    ...
    if (ngx_inet_resolve_host(pool, u) != 0) {
        return -1;
    }

    u->family = u->addrs[0].sockaddr->sa_family;
    u->socklen = u->addrs[0].socklen;
    (void) memcpy(&u->sockaddr, u->addrs[0].sockaddr, u->addrs[0].socklen);
...
ngx_parse_inet6_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (ngx_inet6_addr(host, len, sin6->sin6_addr.s6_addr) != 0) {
    ...
    (void) memcpy(sin6, &u->sockaddr, sizeof(struct sockaddr_in6));
...
ngx_inet_resolve_host(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (getaddrinfo((char *) host, 0, &hints, &res) != 0) {
    ...
    for (i = 0, rp = res; rp != 0; rp = rp->ai_next) {
    ...
    for (rp = res; rp != 0; rp = rp->ai_next) {
        ...
        (void) memcpy(sin, rp->ai_addr, rp->ai_addrlen);
    ...
    for (rp = res; rp != 0; rp = rp->ai_next) {
        ...
        (void) memcpy(sin6, rp->ai_addr, rp->ai_addrlen);
    ...
    struct sockaddr *sa2, socklen_t slen2, ngx_uint_t cmp_port)
{
    ...
    case AF_INET6:
        ...
        if (memcmp((const char *) &sin61->sin6_addr, (const char *) &sin62->sin6_addr, 16) != 0) {
            return -5;
        }
    ...
    case AF_UNIX:
        ...
        if (slen1 < slen2) {
            len = slen1 - 
# 1405 "src/core/ngx_inet.c" 3 4
                         0
# 1405 "src/core/ngx_inet.c"
                                                               ;

        } else {
            len = slen2 - 
# 1408 "src/core/ngx_inet.c" 3 4
                         0
# 1408 "src/core/ngx_inet.c"
                                                               ;
        }
        ...
        if (memcmp((const char *) &saun1->sun_path, (const char *) &saun2->sun_path, len) != 0) {
            return -5;
        }
