...
# 9 "src/core/ngx_slab.c" 2
# 72 "src/core/ngx_slab.c"
...
ngx_slab_init(ngx_slab_pool_t *pool)
{
    ...
    pool->start = (u_char *) (((uintptr_t) (p + pages * sizeof(ngx_slab_page_t)) + ((uintptr_t) ngx_pagesize - 1)) & ~((uintptr_t) ngx_pagesize - 1))
                                             ;
    ...
    ngx_uint_t pages)
{
    ...
    if (pages) {
        (void) memset(&page[1], 0, pages * sizeof(ngx_slab_page_t));
    }
...
{
    if ((ngx_cycle->log)->log_level >= level) ngx_log_error_core(level, ngx_cycle->log, 0, "%s%s", text, pool->log_ctx);
}
