...
ngx_pstrdup(ngx_pool_t *pool, ngx_str_t *src)
{
    ...
    (void) memcpy(dst, src->data, src->len);

    return dst;
}
# 120 "src/core/ngx_string.c"
u_char *
ngx_sprintf(u_char *buf, const char *fmt, ...)
{
# 490 "src/core/ngx_string.c"
}
...
{
    u_char *p, temp[(sizeof("-9223372036854775808") - 1) + 1];
    ...
    p = temp + (sizeof("-9223372036854775808") - 1);

    if (hexadecimal == 0) {

        if (ui64 <= (uint64_t) (uint32_t) 0xffffffff) {
# 528 "src/core/ngx_string.c"
            ui32 = (uint32_t) ui64;
    ...
    len = (temp + (sizeof("-9223372036854775808") - 1)) - p;
...
ngx_strnstr(u_char *s1, char *s2, size_t len)
{
    ...
    } while (strncmp((const char *) s1, (const char *) (u_char *) s2, n) != 0);

    return --s1;
}
# 681 "src/core/ngx_string.c"
u_char *
...
ngx_atoi(u_char *line, size_t n)
{
    ...
    cutoff = 9223372036854775807 / 10;
    cutlim = 9223372036854775807 % 10;
    ...
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (node->key != temp->key) {
            ...
            p = (memcmp((const char *) n->str.data, (const char *) t->str.data, n->str.len) < 0)
...
ngx_str_rbtree_lookup(ngx_rbtree_t *rbtree, ngx_str_t *val, uint32_t hash)
{
    ...
    while (node != sentinel) {
        ...
        rc = memcmp((const char *) val->data, (const char *) n->str.data, val->len);
