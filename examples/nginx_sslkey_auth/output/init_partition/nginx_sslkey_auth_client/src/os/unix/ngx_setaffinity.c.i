# 1 "src/os/unix/ngx_setaffinity.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 8 "src/os/unix/ngx_setaffinity.c" 2
...
# 9 "src/os/unix/ngx_setaffinity.c" 2
# 35 "src/os/unix/ngx_setaffinity.c"
void
ngx_setaffinity(ngx_cpuset_t *cpu_affinity, ngx_log_t *log)
{
    ...
    for (i = 0; i < CPU_SETSIZE; i++) {
        if (CPU_ISSET(i, cpu_affinity)) {
            if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "sched_setaffinity(): using cpu #%ui", i)
                                                                   ;
        }
    }

    if (sched_setaffinity(0, sizeof(cpu_set_t), cpu_affinity) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "sched_setaffinity() failed")
                                                   ;
    }
}
