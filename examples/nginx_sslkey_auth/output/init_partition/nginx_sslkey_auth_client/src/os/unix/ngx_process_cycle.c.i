...
# 9 "src/os/unix/ngx_process_cycle.c" 2
...
# 10 "src/os/unix/ngx_process_cycle.c" 2
...
# 11 "src/os/unix/ngx_process_cycle.c" 2
...
# 12 "src/os/unix/ngx_process_cycle.c" 2
...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    if (sigprocmask(SIG_BLOCK, &set, 0) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "sigprocmask() failed")
                                             ;
    }
    ...
    for ( ;; ) {
        if (delay) {
            ...
            if (setitimer(ITIMER_REAL, &itv, 0) == -1) {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setitimer() failed")
                                                   ;
            }
        }
        ...
        if (ngx_quit) {
            ...
            for (n = 0; n < cycle->listening.nelts; n++) {
                if (close(ls[n].fd) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "close() socket" " %V failed", &ls[n].addr_text)
        ...
        if (ngx_reconfigure) {
            ...
            if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "reconfiguring");
        ...
        if (ngx_reopen) {
            ngx_reopen = 0;
            if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "reopening logs");
        ...
        if (ngx_change_binary) {
            ngx_change_binary = 0;
            if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "changing binary");
...
ngx_start_worker_processes(ngx_cycle_t *cycle, ngx_int_t n, ngx_int_t type)
{
    ...
    if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "start worker processes");
...
ngx_signal_worker_processes(ngx_cycle_t *cycle, int signo)
{
    ...
    for (i = 0; i < ngx_last_process; i++) {
        ...
        if (kill(ngx_processes[i].pid, signo) == -1) {
            err = errno;
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, err, "kill(%P, %d) failed", ngx_processes[i].pid, signo)
                                                                             ;
        ...
        if (ngx_processes[i].exited) {
                ...
                && !ngx_quit)
            {
                if (ngx_spawn_process(cycle, ngx_processes[i].proc,
                    ...
                    == -1)
                {
                    if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "could not respawn %s", ngx_processes[i].name)
            ...
            if (ngx_processes[i].pid == ngx_new_binary) {
                ...
                if (rename((const char *) (char *) ccf->oldpid.data, (const char *) (char *) ccf->pid.data)

                    == -1)
                {
                    if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "rename()" " %s back to %s failed " "after the new binary process \"%s\" exited", ccf->oldpid.data, ccf->pid.data, ngx_argv[0])
...
ngx_master_process_exit(ngx_cycle_t *cycle)
{
    ...
    if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "exit");
...
# 709 "src/os/unix/ngx_process_cycle.c"
    ...
    for ( ;; ) {
        ...
        if (ngx_exiting) {
            if (ngx_event_no_timers_left() == 0) {
                if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "exiting");
        ...
        if (ngx_quit) {
            ngx_quit = 0;
            if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "gracefully shutting down")
                                                     ;
...
ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker)
{
    ...
    if (worker >= 0 && ccf->priority != 0) {
        if (setpriority(PRIO_PROCESS, 0, ccf->priority) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setpriority(%d) failed", ccf->priority)
                                                                  ;
        }
    }
    ...
    if (ccf->rlimit_nofile != -1) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setrlimit(RLIMIT_NOFILE, %i) failed", ccf->rlimit_nofile)
    ...
    if (ccf->rlimit_core != -1) {
        ...
        if (setrlimit(RLIMIT_CORE, &rlmt) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "setrlimit(RLIMIT_CORE, %O) failed", ccf->rlimit_core)
    ...
    if (geteuid() == 0) {
        if (setgid(ccf->group) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "setgid(%d) failed", ccf->group)
                                                          ;
        ...
        if (initgroups(ccf->username, ccf->group) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "initgroups(%s, %d) failed", ccf->username, ccf->group)
        ...
        if (ccf->transparent && ccf->user) {
            if (prctl(PR_SET_KEEPCAPS, 1, 0, 0, 0) == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "prctl(PR_SET_KEEPCAPS, 1) failed")
                                                                 ;
        ...
        if (setuid(ccf->user) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "setuid(%d) failed", ccf->user)
                                                         ;
        ...
        if (ccf->transparent && ccf->user) {
            ...
            header.version = 
# 868 "src/os/unix/ngx_process_cycle.c" 3 4
                            0x19980330
# 868 "src/os/unix/ngx_process_cycle.c"
                                                       ;
            data.effective = 
# 869 "src/os/unix/ngx_process_cycle.c" 3 4
                            (1U << ((13) & 31))
# 869 "src/os/unix/ngx_process_cycle.c"
                                                    ;
            data.permitted = data.effective;

            if (syscall(
# 872 "src/os/unix/ngx_process_cycle.c" 3 4
                       126
# 872 "src/os/unix/ngx_process_cycle.c"
                                 , &header, &data) == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, errno, "capset() failed")
                                                ;
    ...
    if (prctl(PR_SET_DUMPABLE, 1, 0, 0, 0) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "prctl(PR_SET_DUMPABLE) failed")
                                                      ;
    }
    ...
    if (ccf->working_directory.len) {
        if (chdir((char *) ccf->working_directory.data) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "chdir(\"%s\") failed", ccf->working_directory.data)
                                                                              ;
    ...
    for (n = 0; n < ngx_last_process; n++) {
        ...
        if (close(ngx_processes[n].channel[1]) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "close() channel failed")
                                                   ;
        }
    }
    ...
    if (ngx_add_channel_event(cycle, ngx_channel, (EPOLLIN|EPOLLRDHUP),
                              ngx_channel_handler)
        == -1)
    {
    ...
    if (ngx_exiting) {
        ...
        for (i = 0; i < cycle->connection_n; i++) {
            if (c[i].fd != -1
                ...
                && !c[i].read->resolver)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "*%uA open socket #%d left in connection %ui", c[i].number, c[i].fd, i)
        ...
        if (ngx_debug_quit) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "aborting");
            ngx_debug_point();
        }
    }
# 1018 "src/os/unix/ngx_process_cycle.c"
    ngx_exit_log = *ngx_log_get_file_log(ngx_cycle->log);
    ...
    if ((ngx_cycle->log)->log_level >= 6) ngx_log_error_core(6, ngx_cycle->log, 0, "exit");
    ...
    for ( ;; ) {
        ...
        case 2:
            ...
            if (close(ngx_processes[ch.slot].channel[0]) == -1) {
                if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, errno, "close() channel failed")
                                                       ;
            }
