...
# 33 "src/os/unix/ngx_send.c"
    for ( ;; ) {
        ...
        if (n == 0) {
            if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, err, "send() returned zero");
            wev->ready = 0;
            return n;
        }
