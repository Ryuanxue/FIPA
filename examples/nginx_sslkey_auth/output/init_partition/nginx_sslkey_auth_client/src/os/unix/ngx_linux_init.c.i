# 1 "src/os/unix/ngx_linux_init.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/os/unix/ngx_linux_init.c" 2
...
# 10 "src/os/unix/ngx_linux_init.c" 2
...
ngx_os_specific_init(ngx_log_t *log)
{
    ...
    if (uname(&u) == -1) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, errno, "uname() failed");
        return -1;
    }
...
{
    if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "OS: %s %s", ngx_linux_kern_ostype, ngx_linux_kern_osrelease)
                                                                  ;
}
