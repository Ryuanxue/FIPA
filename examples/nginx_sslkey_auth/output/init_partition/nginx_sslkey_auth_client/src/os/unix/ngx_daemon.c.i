...
ngx_daemon(ngx_log_t *log)
{
    ...
    switch (fork()) {
    case -1:
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "fork() failed");
        return -1;
    ...
    if (setsid() == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "setsid() failed");
        return -1;
    }
    ...
    if (fd == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "open(\"/dev/null\") failed")
                                                   ;
        return -1;
    }

    if (dup2(fd, STDIN_FILENO) == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "dup2(STDIN) failed");
        return -1;
    }

    if (dup2(fd, STDOUT_FILENO) == -1) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "dup2(STDOUT) failed");
        return -1;
    }
# 63 "src/os/unix/ngx_daemon.c"
    if (fd > STDERR_FILENO) {
        if (close(fd) == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, errno, "close() failed");
            return -1;
        }
    }
