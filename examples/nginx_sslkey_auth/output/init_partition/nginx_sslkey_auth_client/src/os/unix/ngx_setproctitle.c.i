# 1 "src/os/unix/ngx_setproctitle.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/os/unix/ngx_setproctitle.c" 2
...
# 10 "src/os/unix/ngx_setproctitle.c" 2
# 30 "src/os/unix/ngx_setproctitle.c"
extern char **environ;
...
ngx_init_setproctitle(ngx_log_t *log)
{
    ...
    for (i = 0; environ[i]; i++) {
        size += strlen((const char *) environ[i]) + 1;
    }
    ...
    for (i = 0; ngx_os_argv[i]; i++) {
        if (ngx_os_argv_last == ngx_os_argv[i]) {
            ngx_os_argv_last = ngx_os_argv[i] + strlen((const char *) ngx_os_argv[i]) + 1;
        }
    }
...
{
    u_char *p;
# 90 "src/os/unix/ngx_setproctitle.c"
    ngx_os_argv[1] = 0;
...
# 127 "src/os/unix/ngx_setproctitle.c"
    if (ngx_os_argv_last - (char *) p) {
        (void) memset(p, '\0', ngx_os_argv_last - (char *) p);
    }
