...
# 140 "src/event/ngx_event.c" 3 4
     0
# 140 "src/event/ngx_event.c"
                                             ,
      0 },
...
# 147 "src/event/ngx_event.c" 3 4
     0
# 147 "src/event/ngx_event.c"
                                             ,
      0 },
...
# 154 "src/event/ngx_event.c" 3 4
     0
# 154 "src/event/ngx_event.c"
                                                   ,
      0 },
...
ngx_handle_read_event(ngx_event_t *rev, ngx_uint_t flags)
{
    if (ngx_event_flags & 0x00000004) {
        ...
        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, (EPOLLIN|EPOLLRDHUP), EPOLLET)
                == -1)
            {
                return -1;
            }
        ...
        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, (EPOLLIN|EPOLLRDHUP), 0)
                == -1)
            {
                return -1;
            }
        ...
        if (rev->active && (rev->ready || (flags & 1))) {
            if (ngx_event_actions.del(rev, (EPOLLIN|EPOLLRDHUP), 0 | flags)
                == -1)
            {
                return -1;
            }
        ...
        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, (EPOLLIN|EPOLLRDHUP), 0) == -1) {
                return -1;
            }
        ...
        if (rev->oneshot && !rev->ready) {
            if (ngx_event_actions.del(rev, (EPOLLIN|EPOLLRDHUP), 0) == -1) {
                return -1;
            }
...
ngx_event_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    if (cycle->conf_ctx[ngx_events_module.index] == 0) {
    ...
    if (cycle->connection_n < cycle->listening.nelts + 1) {
        ...
        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "%ui worker_connections are not enough " "for %ui listening sockets", cycle->connection_n, cycle->listening.nelts)
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_clone_listening(cycle, &ls[i]) != 0) {
            return (void *) -1;
        }
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "getrlimit(RLIMIT_NOFILE) failed, ignored")
                                                                 ;
    ...
    size = cl
           + cl
           + cl;
# 542 "src/event/ngx_event.c"
    shm.size = size;
    (&shm.name)->len = sizeof("nginx_shared_zone") - 1; (&shm.name)->data = (u_char *) "nginx_shared_zone";
    shm.log = cycle->log;
...
# 641 "src/event/ngx_event.c"
    (&ngx_posted_accept_events)->prev = &ngx_posted_accept_events; (&ngx_posted_accept_events)->next = &ngx_posted_accept_events;
    (&ngx_posted_events)->prev = &ngx_posted_events; (&ngx_posted_events)->next = &ngx_posted_events;
    ...
    for (m = 0; cycle->modules[m]; m++) {
        if (cycle->modules[m]->type != 0x544E5645) {
            continue;
        }
    ...
    if (ngx_timer_resolution && !(ngx_event_flags & 0x00000800)) {
        ...
        if (sigaction(SIGALRM, &sa, 0) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "sigaction(SIGALRM) failed")
                                                      ;
            return -1;
        }
    ...
    if (ngx_event_flags & 0x00000400) {
        ...
        if (getrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, errno, "getrlimit(RLIMIT_NOFILE) failed")
                                                            ;
            return -1;
        }
...
# 723 "src/event/ngx_event.c"
    cycle->connections =
        ngx_alloc(sizeof(ngx_connection_t) * cycle->connection_n, cycle->log);
    if (cycle->connections == 0) {
        return -1;
    }
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (!(ngx_event_flags & 0x00000200)) {
            if (ls[i].previous) {
                ...
                if (ngx_event_actions.del(old->read, (EPOLLIN|EPOLLRDHUP), 1)
                    == -1)
                {
                    return -1;
                }
...
# 861 "src/event/ngx_event.c"
        ...
        {
            if (ngx_event_actions.add(rev, (EPOLLIN|EPOLLRDHUP), EPOLLEXCLUSIVE)
                == -1)
            {
                return -1;
            }
...
ngx_events_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_event_max_module = ngx_count_modules(cf->cycle, 0x544E5645);
    ...
    *(void **) conf = ctx;
    ...
    for (i = 0; cf->cycle->modules[i]; i++) {
        if (cf->cycle->modules[i]->type != 0x544E5645) {
            continue;
        }
        ...
        if (m->create_conf) {
            ...
            if ((*ctx)[cf->cycle->modules[i]->ctx_index] == 0) {
                return (void *) -1;
            }
        }
    }
...
ngx_event_use(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        if (cf->cycle->modules[m]->type != 0x544E5645) {
            continue;
        }
...
# 1293 "src/event/ngx_event.c"
    if (module == 0) {
        for (i = 0; cycle->modules[i]; i++) {

            if (cycle->modules[i]->type != 0x544E5645) {
                continue;
            }
    ...
    if (ecf->accept_mutex == -1) { ecf->accept_mutex = 0; };
