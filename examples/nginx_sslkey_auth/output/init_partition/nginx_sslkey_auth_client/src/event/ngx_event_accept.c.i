...
# 9 "src/event/ngx_event_accept.c" 2
...
# 10 "src/event/ngx_event_accept.c" 2
...
# 11 "src/event/ngx_event_accept.c" 2
...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    if (ev->timedout) {
        if (ngx_enable_accept_events((ngx_cycle_t *) ngx_cycle) != 0) {
            return;
        }
    ...
    ecf = (*(ngx_cycle->conf_ctx[ngx_events_module.index])) [ngx_event_core_module.ctx_index];;

    if (!(ngx_event_flags & 0x00000008)) {
    ...
    do {
        ...
        if (s == (ngx_socket_t) -1) {
            ...
            if ((ev->log)->log_level >= level) ngx_log_error_core(level, ev->log, err, use_accept4 ? "accept4() failed" : "accept() failed")
                                                                               ;
        ...
        if (c == 0) {
            if (close(s) == -1) {
                if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, errno, "close() socket" " failed")
                                                           ;
            }
        ...
        (void) memcpy(c->sockaddr, &sa, socklen);
        ...
        if (ngx_inherited_nonblocking) {
            if (ngx_event_flags & 0x00000200) {
                if (ngx_blocking(s) == -1) {
                    if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, errno, "ioctl(!FIONBIO)" " failed")
                                                           ;
                    ngx_close_accepted_connection(c);
                    return;
                }
        ...
        *log = ls->log;
        ...
        rev->log = log;
        wev->log = log;
# 257 "src/event/ngx_event_accept.c"
...
# 298 "src/event/ngx_event_accept.c"
...
ngx_trylock_accept_mutex(ngx_cycle_t *cycle)
{
    if (ngx_shmtx_trylock(&ngx_accept_mutex)) {
        ...
        if (ngx_enable_accept_events(cycle) == -1) {
            ngx_shmtx_unlock(&ngx_accept_mutex);
            return -1;
        }
    ...
    if (ngx_accept_mutex_held) {
        if (ngx_disable_accept_events(cycle, 0) == -1) {
            return -1;
        }
...
ngx_enable_accept_events(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (c == 0 || c->read->active) {
            continue;
        }

        if (ngx_event_actions.add(c->read, (EPOLLIN|EPOLLRDHUP), 0) == -1) {
            return -1;
        }
    }
...
# 404 "src/event/ngx_event_accept.c"
        ...
        if (ngx_event_actions.del(c->read, (EPOLLIN|EPOLLRDHUP), 2)
            == -1)
        {
            return -1;
        }
...
ngx_close_accepted_connection(ngx_connection_t *c)
{
    ...
    if (close(fd) == -1) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, errno, "close() socket" " failed")
                                                   ;
    }
