# 1 "src/event/ngx_event_posted.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/event/ngx_event_posted.c" 2
...
# 10 "src/event/ngx_event_posted.c" 2
...
# 11 "src/event/ngx_event_posted.c" 2
...
ngx_event_process_posted(ngx_cycle_t *cycle, ngx_queue_t *posted)
{
    ...
    while (!(posted == (posted)->prev)) {

        q = (posted)->next;
        ev = (ngx_event_t *) ((u_char *) q - 
# 26 "src/event/ngx_event_posted.c" 3 4
            0
# 26 "src/event/ngx_event_posted.c"
            );

       
                                            ;

        (ev)->posted = 0; (&(ev)->queue)->next->prev = (&(ev)->queue)->prev; (&(ev)->queue)->prev->next = (&(ev)->queue)->next; ;;
