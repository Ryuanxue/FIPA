...
# 10 "src/event/ngx_event_udp.c" 2
...
# 11 "src/event/ngx_event_udp.c" 2
...
ngx_event_recvmsg(ngx_event_t *ev)
{
    ...
    do {
        ...
        if (n == -1) {
            ...
            if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, err, "recvmsg() failed");
        ...
        if (msg.msg_flags & (MSG_TRUNC|MSG_CTRUNC)) {
            if ((ev->log)->log_level >= 2) ngx_log_error_core(2, ev->log, 0, "recvmsg() truncated data")
                                                     ;
            continue;
        }
...
ngx_udp_shared_recv(ngx_connection_t *c, u_char *buf, size_t size)
{
    ...
    if (c->udp == 0 || c->udp->buffer == 0) {
        return -2;
    }

    b = c->udp->buffer;

    n = ((b->last - b->pos > (ssize_t) size) ? ((ssize_t) size) : (b->last - b->pos));

    (void) memcpy(buf, b->pos, n);
...
# 582 "src/event/ngx_event_udp.c" 3 4
                                  0
            
# 583 "src/event/ngx_event_udp.c"
           || saun->sun_path[0] == '\0')
        {
