...
# 9 "src/http/ngx_http_script.c" 2
...
# 10 "src/http/ngx_http_script.c" 2
...
# 11 "src/http/ngx_http_script.c" 2
...
static uintptr_t ngx_http_script_exit_code = (uintptr_t) 0;
...
ngx_http_compile_complex_value(ngx_http_compile_complex_value_t *ccv)
{
    ...
    {
        if (ngx_conf_full_name(ccv->cf->cycle, v, ccv->conf_prefix) != 0) {
            return -1;
        }
    ...
    ccv->complex_value->flushes = 0;
    ccv->complex_value->lengths = 0;
    ccv->complex_value->values = 0;
    ...
    if (ngx_array_init(&lengths, ccv->cf->pool, n, 1) != 0) {
        return -1;
    }
    ...
    if (ngx_array_init(&values, ccv->cf->pool, n, 1) != 0) {
        return -1;
    }
...
ngx_http_test_predicates(ngx_http_request_t *r, ngx_array_t *predicates)
{
    ...
    for (i = 0; i < predicates->nelts; i++) {
        if (ngx_http_complex_value(r, &cv[i], &val) != 0) {
            return -1;
        }
...
ngx_http_script_compile(ngx_http_script_compile_t *sc)
{
    ...
    if (ngx_http_script_init_arrays(sc) != 0) {
        return -1;
    }
...
# 373 "src/http/ngx_http_script.c"
                ngx_conf_log_error(1, sc->cf, 0,
            ...
            if (ngx_http_script_add_var_code(sc, &name) != 0) {
                return -1;
            }
        ...
        if (sc->source->data[i] == '?' && sc->compile_args) {
            ...
            if (ngx_http_script_add_args_code(sc) != 0) {
                return -1;
            }
...
invalid_variable:
    ...
    ngx_conf_log_error(1, sc->cf, 0, "invalid variable name");
...
ngx_http_script_done(ngx_http_script_compile_t *sc)
{
    ...
    if (sc->zero) {
        ...
        if (ngx_http_script_add_copy_code(sc, &zero, 0) != 0) {
            return -1;
        }
    }

    if (sc->conf_prefix || sc->root_prefix) {
        if (ngx_http_script_add_full_name_code(sc) != 0) {
            return -1;
        }
    }

    if (sc->complete_lengths) {
        code = ngx_http_script_add_code(*sc->lengths, sizeof(uintptr_t), 0);
        if (code == 0) {
            return -1;
        }
    ...
    ngx_uint_t last)
{
    ...
    code = ngx_http_script_add_code(*sc->lengths,
                                    sizeof(ngx_http_script_copy_code_t), 0);
    if (code == 0) {
        return -1;
    }
    ...
    p = (((u_char *) memcpy((u_char *) code + sizeof(ngx_http_script_copy_code_t), value->data, value->len)) + (value->len))
                                           ;
...
ngx_http_script_copy_code(ngx_http_script_engine_t *e)
{
    ...
    if (!e->skip) {
        e->pos = (((u_char *) memcpy(p, e->ip + sizeof(ngx_http_script_copy_code_t), code->len)) + (code->len))
                                    ;
    }
...
ngx_http_script_add_var_code(ngx_http_script_compile_t *sc, ngx_str_t *name)
{
    ...
    code = ngx_http_script_add_code(*sc->lengths,
                                    sizeof(ngx_http_script_var_code_t), 0);
    if (code == 0) {
        return -1;
    }
...
ngx_http_script_copy_var_code(ngx_http_script_engine_t *e)
{
    ...
    if (!e->skip) {
        ...
        if (value && !value->not_found) {
            p = e->pos;
            e->pos = (((u_char *) memcpy(p, value->data, value->len)) + (value->len));
...
# 1286 "src/http/ngx_http_script.c"
static ngx_int_t
ngx_http_script_add_full_name_code(ngx_http_script_compile_t *sc)
{
...
ngx_http_script_return_code(ngx_http_script_engine_t *e)
{
    ...
    if (code->status < 400
        ...
        || code->text.lengths)
    {
        e->status = ngx_http_send_response(e->request, code->status, 0,
                                           &code->text);
    } else {
...
ngx_http_script_equal_code(ngx_http_script_engine_t *e)
{
    ...
    if (val->len == res->len
        && strncmp((const char *) val->data, (const char *) res->data, res->len) == 0)
    {
...
ngx_http_script_file_code(ngx_http_script_engine_t *e)
{
        ...
        != 0)
    {
        ...
        if (of.err != ENOENT
            ...
            && of.err != ENAMETOOLONG)
        {
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, of.err, "%s \"%s\" failed", of.failed, value->data)
                                                                     ;
        }
...
# 1710 "src/http/ngx_http_script.c"
}
