# 1 "src/http/ngx_http_postpone_filter_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/ngx_http_postpone_filter_module.c" 2
...
# 10 "src/http/ngx_http_postpone_filter_module.c" 2
...
# 11 "src/http/ngx_http_postpone_filter_module.c" 2
...
ngx_http_postpone_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (r != c->data) {
        ...
        if (in) {
            if (ngx_http_postpone_filter_add(r, in) != 0) {
                return -1;
            }
    ...
    do {
        ...
        if (pr->request) {
            ...
            return ngx_http_post_request(pr->request, 0);
        }

        if (pr->out == 0) {
            if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "http postpone filter NULL output")
                                                             ;
            ...
            if (ngx_http_next_body_filter(r->main, pr->out) == -1) {
                return -1;
            }
        }
...
found:
    ...
    if (ngx_chain_add_copy(r->pool, &pr->out, in) == 0) {
        return 0;
    }
...
ngx_http_postpone_filter_in_memory(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (r->out == 0) {
        ...
        if (r->headers_out.content_length_n != -1) {
            ...
            if (len > clcf->subrequest_output_buffer_size) {
                if ((c->log)->log_level >= 4) ngx_log_error_core(4, c->log, 0, "too big subrequest response: %uz", len)
                                                                      ;
                return -1;
            }
    ...
    for ( ; in; in = in->next) {
        ...
        if (len > (size_t) (b->end - b->last)) {
            if ((c->log)->log_level >= 4) ngx_log_error_core(4, c->log, 0, "too big subrequest response")
                                                        ;
            return -1;
        }
        ...
        b->last = (((u_char *) memcpy(b->last, in->buf->pos, len)) + (len));
