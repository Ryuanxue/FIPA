# 1 "src/http/ngx_http_request_body.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/ngx_http_request_body.c" 2
...
# 10 "src/http/ngx_http_request_body.c" 2
...
# 11 "src/http/ngx_http_request_body.c" 2
...
# 88 "src/http/ngx_http_request_body.c"
    ...
    if (preread) {
        ...
        if (ngx_http_request_body_filter(r, 0) != 0) {
            rc = 500;
            goto done;
        }
    }
    ...
    if (rb->rest < 0) {
        if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "negative request body rest")
                                                   ;
        rc = 500;
        goto done;
    }
...
{
    ngx_int_t rc;
# 229 "src/http/ngx_http_request_body.c"
        ...
        for ( ;; ) {
            if (rb->buf->last == rb->buf->end) {
                ...
                if (rb->buf->pos != rb->buf->last) {
                    ...
                    rc = ngx_http_request_body_filter(r, 0);
...
# 528 "src/http/ngx_http_request_body.c"
    if (ngx_http_test_expect(r) != 0) {
        return 500;
    }
...
ngx_http_read_discarded_request_body(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        ...
        size = (size_t) ((r->headers_in.content_length_n > 4096) ? (4096) : (r->headers_in.content_length_n))
                                                             ;
...
ngx_http_discard_request_body_filter(ngx_http_request_t *r, ngx_buf_t *b)
{
    ...
    if (r->headers_in.chunked) {
        ...
        for ( ;; ) {
            ...
            if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, 0, "client sent invalid chunked body")
                                                             ;
    ...
    for (cl = in; cl; cl = cl->next) {
        ...
        for ( ;; ) {
            ...
            if (rc == 0) {
                ...
                {
                    if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, 0, "client intended to send too large chunked " "body: %O+%O bytes", r->headers_in.content_length_n, rb->chunked->size)
...
ngx_http_request_body_save_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    rb = r->request_body;
# 1122 "src/http/ngx_http_request_body.c"
    if (ngx_chain_add_copy(r->pool, &rb->bufs, in) != 0) {
        return 500;
    }
    ...
    if (rb->rest > 0) {
        ...
        if (rb->buf && rb->buf->last == rb->buf->end
            && ngx_http_write_request_body(r) != 0)
        {
            return 500;
        }
    ...
    if (rb->temp_file || r->request_body_in_file_only) {
        ...
        if (ngx_http_write_request_body(r) != 0) {
            return 500;
        }
