# 1 "src/http/modules/ngx_http_upstream_keepalive_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_upstream_keepalive_module.c" 2
...
# 10 "src/http/modules/ngx_http_upstream_keepalive_module.c" 2
...
# 11 "src/http/modules/ngx_http_upstream_keepalive_module.c" 2
...
# 75 "src/http/modules/ngx_http_upstream_keepalive_module.c"
...
static ngx_command_t ngx_http_upstream_keepalive_commands[] = {
    ...
    { { sizeof("keepalive") - 1, (u_char *) "keepalive" },
      0x10000000|0x00000002,
      ngx_http_upstream_keepalive,
      
# 85 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
     0
# 85 "src/http/modules/ngx_http_upstream_keepalive_module.c"
                             ,
      0,
      0 },
...
# 92 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
     0
# 92 "src/http/modules/ngx_http_upstream_keepalive_module.c"
                             ,
      
# 93 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
     0
# 93 "src/http/modules/ngx_http_upstream_keepalive_module.c"
                                                              ,
      0 },
...
# 99 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
     0
# 99 "src/http/modules/ngx_http_upstream_keepalive_module.c"
                             ,
      
# 100 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
     0
# 100 "src/http/modules/ngx_http_upstream_keepalive_module.c"
                                                               ,
      0 },
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    kcf = us->srv_conf[ngx_http_upstream_keepalive_module.ctx_index]
                                                                             ;

    if (kcf->timeout == (ngx_msec_t) -1) { kcf->timeout = 60000; };
    if (kcf->requests == (ngx_uint_t) -1) { kcf->requests = 100; };

    if (kcf->original_init_upstream(cf, us) != 0) {
        return -1;
    }
    ...
    (&kcf->cache)->prev = &kcf->cache; (&kcf->cache)->next = &kcf->cache;
    (&kcf->free)->prev = &kcf->free; (&kcf->free)->next = &kcf->free;

    for (i = 0; i < kcf->max_cached; i++) {
        (&cached[i].queue)->next = (&kcf->free)->next; (&cached[i].queue)->next->prev = &cached[i].queue; (&cached[i].queue)->prev = &kcf->free; (&kcf->free)->next = &cached[i].queue;
        cached[i].conf = kcf;
    }
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (kcf->original_init_peer(r, us) != 0) {
        return -1;
    }
...
# 222 "src/http/modules/ngx_http_upstream_keepalive_module.c"
    return 0;
}
...
ngx_http_upstream_get_keepalive_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    for (q = (cache)->next;
         ...
         q = (q)->next)
    {
        item = (ngx_http_upstream_keepalive_cache_t *) ((u_char *) q - 
# 255 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
              0
# 255 "src/http/modules/ngx_http_upstream_keepalive_module.c"
              );
        c = item->connection;
            ...
            == 0)
        {
            ...
            (q)->next = (&kp->conf->free)->next; (q)->next->prev = q; (q)->prev = &kp->conf->free; (&kp->conf->free)->next = q;
    ...
    ngx_uint_t state)
{
    ...
    if ((&kp->conf->free == (&kp->conf->free)->prev)) {

        q = (&kp->conf->cache)->prev;
        (q)->next->prev = (q)->prev; (q)->prev->next = (q)->next;

        item = (ngx_http_upstream_keepalive_cache_t *) ((u_char *) q - 
# 352 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
              0
# 352 "src/http/modules/ngx_http_upstream_keepalive_module.c"
        ...
        item = (ngx_http_upstream_keepalive_cache_t *) ((u_char *) q - 
# 360 "src/http/modules/ngx_http_upstream_keepalive_module.c" 3 4
              0
# 360 "src/http/modules/ngx_http_upstream_keepalive_module.c"
              );
    }

    (q)->next = (&kp->conf->cache)->next; (q)->next->prev = q; (q)->prev = &kp->conf->cache; (&kp->conf->cache)->next = q;
    ...
    item->socklen = pc->socklen;
    (void) memcpy(&item->sockaddr, pc->sockaddr, pc->socklen);
...
close:
    ...
    (&item->queue)->next->prev = (&item->queue)->prev; (&item->queue)->prev->next = (&item->queue)->next;
    (&item->queue)->next = (&conf->free)->next; (&item->queue)->next->prev = &item->queue; (&item->queue)->prev = &conf->free; (&conf->free)->next = &item->queue;
}
...
{
# 468 "src/http/modules/ngx_http_upstream_keepalive_module.c"
    ngx_destroy_pool(c->pool);
    ngx_close_connection(c);
}
# 496 "src/http/modules/ngx_http_upstream_keepalive_module.c"
static void *
...
# 515 "src/http/modules/ngx_http_upstream_keepalive_module.c"
    conf->timeout = (ngx_msec_t) -1;
    conf->requests = (ngx_uint_t) -1;
...
ngx_http_upstream_keepalive(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    uscf = ((ngx_http_conf_ctx_t *) cf->ctx)->srv_conf[ngx_http_upstream_module.ctx_index];
