# 1 "src/http/modules/ngx_http_upstream_zone_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_upstream_zone_module.c" 2
...
# 10 "src/http/modules/ngx_http_upstream_zone_module.c" 2
...
# 11 "src/http/modules/ngx_http_upstream_zone_module.c" 2
    ...
    ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    (void) memcpy(peers, uscf->peer.data, sizeof(ngx_http_upstream_rr_peers_t));
    ...
    (void) memcpy(name->data, peers->name->data, peers->name->len);
    ...
    (void) memcpy(backup, peers->next, sizeof(ngx_http_upstream_rr_peers_t));
    ...
    ngx_http_upstream_rr_peer_t *src)
{
    ...
    if (src) {
        (void) memcpy(dst, src, sizeof(ngx_http_upstream_rr_peer_t));
        dst->sockaddr = 0;
        dst->name.data = 0;
        dst->server.data = 0;
    }
    ...
    dst->name.data = ngx_slab_calloc_locked(pool, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 289 "src/http/modules/ngx_http_upstream_zone_module.c" 3 4
                                                 0
# 289 "src/http/modules/ngx_http_upstream_zone_module.c"
                                                 ));
    if (dst->name.data == 0) {
        goto failed;
    }
    ...
    if (src) {
        (void) memcpy(dst->sockaddr, src->sockaddr, src->socklen);
        (void) memcpy(dst->name.data, src->name.data, src->name.len);

        dst->server.data = ngx_slab_alloc_locked(pool, src->server.len);
        if (dst->server.data == 0) {
            goto failed;
        }

        (void) memcpy(dst->server.data, src->server.data, src->server.len);
    }
