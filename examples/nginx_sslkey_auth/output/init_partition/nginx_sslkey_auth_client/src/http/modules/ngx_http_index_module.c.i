# 1 "src/http/modules/ngx_http_index_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_index_module.c" 2
...
# 10 "src/http/modules/ngx_http_index_module.c" 2
...
# 11 "src/http/modules/ngx_http_index_module.c" 2
...
static ngx_command_t ngx_http_index_commands[] = {
    ...
    { { sizeof("index") - 1, (u_char *) "index" },
      0x02000000|0x04000000|0x08000000|0x00000800,
      ngx_http_index_set_index,
      
# 47 "src/http/modules/ngx_http_index_module.c" 3 4
     0
# 47 "src/http/modules/ngx_http_index_module.c"
                             ,
      0,
      0 },
...
# 96 "src/http/modules/ngx_http_index_module.c"
static ngx_int_t
ngx_http_index_handler(ngx_http_request_t *r)
{
    ...
    for (i = 0; i < ilcf->indices->nelts; i++) {
            ...
            != 0)
        {
            ...
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, of.err, "%s \"%s\" failed", of.failed, path.data)
                                                                   ;
    ...
    u_char *path, u_char *last)
{
    ...
    *last = '\0';
    ...
    if (ngx_http_set_disable_symlinks(r, clcf, &dir, &of) != 0) {
        return 500;
    }
        ...
        != 0)
    {
        if (of.err) {
            ...
            if (of.err == ENOENT) {
                *last = c;
                return ngx_http_index_error(r, clcf, dir.data, ENOENT);
            }
            ...
            if ((r->connection->log)->log_level >= 3) ngx_log_error_core(3, r->connection->log, of.err, "%s \"%s\" failed", of.failed, dir.data)
                                                                  ;
        }
    ...
    *last = c;
    ...
    if ((r->connection->log)->log_level >= 2) ngx_log_error_core(2, r->connection->log, 0, "\"%s\" is not a directory", dir.data)
                                                        ;
...
{
    if (err == EACCES) {
        if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, err, "\"%s\" is forbidden", file)
                                                  ;
    ...
    if (clcf->log_not_found) {
        if ((r->connection->log)->log_level >= 4) ngx_log_error_core(4, r->connection->log, err, "\"%s\" is not found", file)
                                                  ;
    }
...
ngx_http_index_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->indices == 0) {
        ...
        index->name.len = sizeof("index.html");
        index->name.data = (u_char *) "index.html";
        index->lengths = 0;
        index->values = 0;
