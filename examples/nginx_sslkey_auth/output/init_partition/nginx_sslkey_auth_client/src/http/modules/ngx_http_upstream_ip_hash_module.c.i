# 1 "src/http/modules/ngx_http_upstream_ip_hash_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_upstream_ip_hash_module.c" 2
...
# 10 "src/http/modules/ngx_http_upstream_ip_hash_module.c" 2
...
# 11 "src/http/modules/ngx_http_upstream_ip_hash_module.c" 2
...
static ngx_command_t ngx_http_upstream_ip_hash_commands[] = {
    ...
    { { sizeof("ip_hash") - 1, (u_char *) "ip_hash" },
...
ngx_module_t ngx_http_upstream_ip_hash_module = {
    ...
    &ngx_http_upstream_ip_hash_module_ctx,
    ngx_http_upstream_ip_hash_commands,
    0x50545448,
    0,
    0,
    0,
...
ngx_http_upstream_get_ip_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (iphp->rrp.peers->shpool) { ngx_rwlock_rlock(&iphp->rrp.peers->rwlock); };

    if (iphp->tries > 20 || iphp->rrp.peers->single) {
        if (iphp->rrp.peers->shpool) { ngx_rwlock_unlock(&iphp->rrp.peers->rwlock); };
    ...
    for ( ;; ) {
        ...
        if (iphp->rrp.peers->shpool) { ngx_rwlock_wlock(&peer->lock); };

        if (peer->down) {
            if (iphp->rrp.peers->shpool) { ngx_rwlock_unlock(&peer->lock); };
            goto next;
        }
