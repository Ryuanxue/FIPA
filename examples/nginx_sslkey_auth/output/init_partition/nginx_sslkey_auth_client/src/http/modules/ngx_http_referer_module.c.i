# 1 "src/http/modules/ngx_http_referer_module.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/modules/ngx_http_referer_module.c" 2
...
# 10 "src/http/modules/ngx_http_referer_module.c" 2
...
# 11 "src/http/modules/ngx_http_referer_module.c" 2
...
static ngx_command_t ngx_http_referer_commands[] = {
    ...
    { { sizeof("valid_referers") - 1, (u_char *) "valid_referers" },
      0x04000000|0x08000000|0x00000800,
      ngx_http_valid_referers,
      
# 58 "src/http/modules/ngx_http_referer_module.c" 3 4
     0
# 58 "src/http/modules/ngx_http_referer_module.c"
                             ,
      0,
      0 },

    { { sizeof("referer_hash_max_size") - 1, (u_char *) "referer_hash_max_size" },
      0x04000000|0x08000000|0x00000002,
      ngx_conf_set_num_slot,
      
# 65 "src/http/modules/ngx_http_referer_module.c" 3 4
     0
# 65 "src/http/modules/ngx_http_referer_module.c"
                             ,
      
# 66 "src/http/modules/ngx_http_referer_module.c" 3 4
     0
# 66 "src/http/modules/ngx_http_referer_module.c"
                                                             ,
      0 },

    { { sizeof("referer_hash_bucket_size") - 1, (u_char *) "referer_hash_bucket_size" },
      0x04000000|0x08000000|0x00000002,
      ngx_conf_set_num_slot,
      
# 72 "src/http/modules/ngx_http_referer_module.c" 3 4
     0
# 72 "src/http/modules/ngx_http_referer_module.c"
                             ,
      
# 73 "src/http/modules/ngx_http_referer_module.c" 3 4
     0
# 73 "src/http/modules/ngx_http_referer_module.c"
                                                                ,
      0 },
...
static ngx_str_t ngx_http_invalid_referer_name = { sizeof("invalid_referer") - 1, (u_char *) "invalid_referer" };
    ...
    uintptr_t data)
{
    ...
    rlcf = (r)->loc_conf[ngx_http_referer_module.ctx_index];

    if (rlcf->hash.hash.buckets == 0
        && rlcf->hash.wc_head == 0
        && rlcf->hash.wc_tail == 0
...
valid_scheme:
    ...
    for (p = ref; p < last; p++) {
        ...
        buf[i] = (u_char) ((*p >= 'A' && *p <= 'Z') ? (*p | 0x20) : *p);
        key = ((ngx_uint_t) key * 31 + buf[i++]);
    }
...
# 238 "src/http/modules/ngx_http_referer_module.c"
invalid:
...
uri:
    ...
    if (len < uri->len || strncmp((const char *) uri->data, (const char *) p, uri->len) != 0) {
        goto invalid;
    }
...
# 310 "src/http/modules/ngx_http_referer_module.c"
    conf->no_referer = -1;
    conf->blocked_referer = -1;
...
ngx_http_referer_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->keys == 0) {
        ...
        if (conf->no_referer == -1) { conf->no_referer = (prev->no_referer == -1) ? 0 : prev->no_referer; };
        if (conf->blocked_referer == -1) { conf->blocked_referer = (prev->blocked_referer == -1) ? 0 : prev->blocked_referer; };
        if (conf->referer_hash_max_size == (ngx_uint_t) -1) { conf->referer_hash_max_size = (prev->referer_hash_max_size == (ngx_uint_t) -1) ? 2048 : prev->referer_hash_max_size; }
                                                                    ;
        if (conf->referer_hash_bucket_size == (ngx_uint_t) -1) { conf->referer_hash_bucket_size = (prev->referer_hash_bucket_size == (ngx_uint_t) -1) ? 64 : prev->referer_hash_bucket_size; }
                                                                     ;
    ...
    if (conf->server_names == 1) {
        cscf = ((ngx_http_conf_ctx_t *) cf->ctx)->srv_conf[ngx_http_core_module.ctx_index];

        sn = cscf->server_names.elts;
        for (n = 0; n < cscf->server_names.nelts; n++) {
# 367 "src/http/modules/ngx_http_referer_module.c"
            if (ngx_http_add_referer(cf, conf->keys, &sn[n].name, 0)
                != 0)
            {
                return (void *) -1;
            }
    ...
    {
        if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "the \"none\" or \"blocked\" referers are specified " "in the \"valid_referers\" directive " "without any valid referer")
    ...
    if (conf->referer_hash_bucket_size == (ngx_uint_t) -1) { conf->referer_hash_bucket_size = (prev->referer_hash_bucket_size == (ngx_uint_t) -1) ? 64 : prev->referer_hash_bucket_size; }
                                                                 ;
    conf->referer_hash_bucket_size = (((conf->referer_hash_bucket_size) + (ngx_cacheline_size - 1)) & ~(ngx_cacheline_size - 1))
                                                                  ;
    ...
    if (conf->keys->dns_wc_head.nelts) {
        ...
        qsort(conf->keys->dns_wc_head.elts,
    ...
    if (conf->keys->dns_wc_tail.nelts) {
        ...
        qsort(conf->keys->dns_wc_tail.elts,
    ...
    if (conf->no_referer == -1) {
        conf->no_referer = 0;
    }

    if (conf->blocked_referer == -1) {
...
ngx_http_valid_referers(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (rlcf->keys == 0) {
        ...
        if (ngx_hash_keys_array_init(rlcf->keys, 1) != 0) {
            return (void *) -1;
        }
    }
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (strcmp((const char *) value[i].data, (const char *) "blocked") == 0) {
            rlcf->blocked_referer = 1;
            continue;
        }

        if (strcmp((const char *) value[i].data, (const char *) "server_names") == 0) {
            rlcf->server_names = 1;
            continue;
        }

        if (value[i].data[0] == '~') {
            if (ngx_http_add_regex_referer(cf, rlcf, &value[i]) != 0) {
                return (void *) -1;
            }
        ...
        (&uri)->len = 0; (&uri)->data = 0;

        p = (u_char *) strchr((const char *) value[i].data, (int) '/');
        ...
        if (ngx_http_add_referer(cf, rlcf->keys, &value[i], &uri) != 0) {
            return (void *) -1;
        }
    }
    ...
    ngx_str_t *value, ngx_str_t *uri)
{
    ...
    rc = ngx_hash_add_key(keys, value, u, 1);
...
ngx_http_add_regex_referer(ngx_conf_t *cf, ngx_http_referer_conf_t *rlcf,
    ngx_str_t *name)
{
# 633 "src/http/modules/ngx_http_referer_module.c"
    ngx_conf_log_error(1, cf, 0,
...
# 673 "src/http/modules/ngx_http_referer_module.c"
static int
