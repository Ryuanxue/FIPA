# 1 "src/http/ngx_http_upstream_round_robin.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 9 "src/http/ngx_http_upstream_round_robin.c" 2
...
# 10 "src/http/ngx_http_upstream_round_robin.c" 2
...
# 11 "src/http/ngx_http_upstream_round_robin.c" 2
...
# 30 "src/http/ngx_http_upstream_round_robin.c"
ngx_int_t
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (us->servers) {
        ...
        if (n == 0) {
            if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "no servers in upstream \"%V\" in %s:%ui", &us->host, us->file_name, us->line)
    ...
    if (us->port == 0) {
        if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "no port in upstream \"%V\" in %s:%ui", &us->host, us->file_name, us->line)
    ...
    if (ngx_inet_resolve_host(cf->pool, &u) != 0) {
        if (u.err) {
            if ((cf->log)->log_level >= 1) ngx_log_error_core(1, cf->log, 0, "%s in upstream \"%V\" in %s:%ui", u.err, &us->host, us->file_name, us->line)
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    r->upstream->peer.tries = ((rrp->peers)->number + ((rrp->peers)->next ? (rrp->peers)->next->number : 0));
    ...
    ngx_http_upstream_resolved_t *ur)
{
    ...
    if (ur->sockaddr) {
        ...
        for (i = 0; i < ur->naddrs; i++) {
            ...
            (void) memcpy(sockaddr, ur->addrs[i].sockaddr, socklen);
            ngx_inet_set_port(sockaddr, ur->port);

            p = ngx_pnalloc(r->pool, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 364 "src/http/ngx_http_upstream_round_robin.c" 3 4
                                    0
# 364 "src/http/ngx_http_upstream_round_robin.c"
                                    ));
            if (p == 0) {
                return -1;
            }

            len = ngx_sock_ntop(sockaddr, socklen, p, (sizeof("unix:") - 1 + sizeof(struct sockaddr_un) - 
# 369 "src/http/ngx_http_upstream_round_robin.c" 3 4
                                                     0
# 369 "src/http/ngx_http_upstream_round_robin.c"
                                                     ), 1);
...
ngx_http_upstream_get_round_robin_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (peers->shpool) { ngx_rwlock_unlock(&peers->rwlock); };
    ...
    ngx_uint_t state)
{
    ...
    if (rrp->peers->shpool) { ngx_rwlock_rlock(&rrp->peers->rwlock); };
    if (rrp->peers->shpool) { ngx_rwlock_wlock(&peer->lock); };

    if (rrp->peers->single) {

        peer->conns--;

        if (rrp->peers->shpool) { ngx_rwlock_unlock(&peer->lock); };
        if (rrp->peers->shpool) { ngx_rwlock_unlock(&rrp->peers->rwlock); };
    ...
    if (state & 4) {
        ...
        if (peer->max_fails) {
            ...
            if (peer->fails >= peer->max_fails) {
                if ((pc->log)->log_level >= 5) ngx_log_error_core(5, pc->log, 0, "upstream server temporarily disabled")
                                                                     ;
            }
        }
