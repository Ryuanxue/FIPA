...
static ngx_int_t ngx_http_auth_basic_handler(ngx_http_request_t *r)
{
    clnt = clnt_create("localhost", RPCNGINX, RPCNGINXTVERS, "udp");
    if (clnt == NULL) {
        fprintf(stderr, "Error: could not create RPC client\n");
        exit(1);
    }
  ngx_int_t ngx_http_auth_basic_handler_sense_1_return;
    int ngx_http_auth_basic_handler_sense_1_ret = 0;
     ngx_http_auth_basic_handler_sense_1_return = ngx_http_auth_basic_handler_sense_1_wrapper(&ngx_http_auth_basic_handler_sense_1_ret, r);
    if (ngx_http_auth_basic_handler_sense_1_ret)
      return ngx_http_auth_basic_handler_sense_1_return;
return ngx_http_auth_basic_handler_sense_1_return;
