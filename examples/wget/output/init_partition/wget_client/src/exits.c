...
void inform_exit_status(uerr_t err)
{
  ...
  if ((new_status != WGET_EXIT_SUCCESS) && ((get_final_exit_status_wrapper() == WGET_EXIT_SUCCESS) || (new_status < get_final_exit_status_wrapper())))
  {
    set_final_exit_status_wrapper(new_status);
  }
}
...
{
  return (get_final_exit_status_wrapper() == WGET_EXIT_UNKNOWN) ? (1) : (get_final_exit_status_wrapper());
}
