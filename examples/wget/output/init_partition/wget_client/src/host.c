...
{
  struct address_list *al;
  if (!get_host_name_addresses_map_wrapper())
    return 0;
  al = hash_table_get(get_host_name_addresses_map_wrapper(), host);
  if (al)
  {
    do
    {
...
{
  if (!get_host_name_addresses_map_wrapper())
    set_host_name_addresses_map_wrapper(make_nocase_string_hash_table(0));
  ++al->refcount;
  hash_table_put(get_host_name_addresses_map_wrapper(), xstrdup_lower(host), al);
  if (__builtin_expect(get_opt_debug_wrapper(), 0))
  {
    int i;
...
static void cache_remove(const char *host)
{
  ...
  if (al)
  {
    ...
    hash_table_remove(get_host_name_addresses_map_wrapper(), host);
  }
}
...
struct address_list *lookup_host(const char *host, int flags)
{
  ...
  bool use_cache;
  bool numeric_address = 0;
  double timeout = get_opt_dns_timeout_wrapper();
  {
    const char *end = host + strlen(host);
    if (is_valid_ipv4_address(host, end) || is_valid_ipv6_address(host, end))
      numeric_address = 1;
  }
  use_cache = get_opt_dns_cache_wrapper();
  ...
  if ((!silent) && (!numeric_address))
  {
    ...
    logprintf(LOG_VERBOSE, gettext("Resolving %s... "), quotearg_style(escape_quoting_style, (str) ? (str) : (host)));
    do
    {
      free((void *) str);
      str = 0;
  ...
  }
  {
    ...
    if (get_opt_ipv4_only_wrapper())
      hints.ai_family = AF_INET;
    else
      if (get_opt_ipv6_only_wrapper())
      hints.ai_family = AF_INET6;
    else
      hints.ai_family = AF_UNSPEC;
    if (flags & LH_BIND)
      hints.ai_flags |= AI_PASSIVE;
    err = getaddrinfo_with_timeout(host, 0, &hints, &res, timeout);
    if ((err != 0) || (res == 0))
    {
      if (!silent)
        logprintf(LOG_VERBOSE, gettext("failed: %s.\n"), (err != EAI_SYSTEM) ? (gai_strerror(err)) : (strerror(errno)));
      return 0;
    }
  ...
  if (!al)
  {
    logprintf(LOG_VERBOSE, gettext("failed: No IPv4/IPv6 addresses for host.\n"));
    return 0;
  }
  if ((al->count > 1) && (get_opt_prefer_family_wrapper() != prefer_none))
    stable_sort(al->addresses, al->count, sizeof(ip_address), (get_opt_prefer_family_wrapper() == prefer_ipv4) ? (cmp_prefer_ipv4) : (cmp_prefer_ipv6));
  if ((!silent) && (!numeric_address))
  {
    int i;
    int printmax = al->count;
    if ((!get_opt_show_all_dns_entries_wrapper()) && (printmax > 3))
      printmax = 3;
...
{
  assert(u->host != 0);
  if (get_opt_domains_wrapper())
  {
    if (!sufmatch((const char **) get_opt_domains_wrapper(), u->host))
      return 0;
  }
  if (get_opt_exclude_domains_wrapper())
  {
    if (sufmatch((const char **) get_opt_exclude_domains_wrapper(), u->host))
      return 0;
  }
  return 1;
}
...
void host_cleanup(void)
{
  if (get_host_name_addresses_map_wrapper())
  {
    ...
    for (hash_table_iterate(get_host_name_addresses_map_wrapper(), &iter); hash_table_iter_next(&iter);)
    {
      char *host = iter.key;
    ...
    hash_table_destroy(get_host_name_addresses_map_wrapper());
    set_host_name_addresses_map_wrapper(0);
  }
}
