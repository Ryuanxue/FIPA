...
{
  if (get_nonexisting_urls_set_wrapper())
    string_set_free(get_nonexisting_urls_set_wrapper());
}
...
void nonexisting_url(const char *url)
{
  if (is_robots_txt_url(url))
    return;
  if (!get_nonexisting_urls_set_wrapper())
    set_nonexisting_urls_set_wrapper(make_string_hash_table(0));
  string_set_add(get_nonexisting_urls_set_wrapper(), url);
}
...
void print_broken_links(void)
{
  ...
  num_elems = hash_table_count(get_nonexisting_urls_set_wrapper());
  assert(num_elems > 0);
  logprintf(LOG_NOTQUIET, ngettext("Found %d broken link.\n\n", "Found %d broken links.\n\n", num_elems), num_elems);
  for (hash_table_iterate(get_nonexisting_urls_set_wrapper(), &iter); hash_table_iter_next(&iter);)
  {
