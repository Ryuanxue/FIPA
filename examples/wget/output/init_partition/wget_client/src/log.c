...
static void saved_append_1(const char *start, const char *end)
{
  ...
  if (!len)
    return;
  if (!get_trailing_line_wrapper())
  {
    struct log_ln *ln;
    if (get_log_line_current_wrapper() == (-1))
      set_log_line_current_wrapper(0);
    else
      free_log_line(get_log_line_current_wrapper());
    ln = log_lines + get_log_line_current_wrapper();
    if (len > 128)
    {
  ...
  {
    struct log_ln *ln = log_lines + get_log_line_current_wrapper();
    if (ln->malloced_line)
    {
  ...
  set_trailing_line_wrapper(!(end[-1] == '\n'));
  if (!get_trailing_line_wrapper())
    do
  {
    if (gsetter_log_line_current_prefix_wrapper() >= 24)
      set_log_line_current_wrapper(0);
  }
  while (0);
}
...
{
  if (get_inhibit_logging_wrapper())
    return 0;
  if (get_logfp_wrapper())
    return get_logfp_wrapper();
  return stderr;
}
...
{
  if (get_opt_show_progress_wrapper() == 1)
    return stderr;
  return get_log_fp();
}
...
static FILE *get_warc_log_fp(void)
{
  if (get_inhibit_logging_wrapper())
    return 0;
  if (get_warclogfp_wrapper())
    return get_warclogfp_wrapper();
  return 0;
}
...
void logputs(enum log_options o, const char *s)
{
  ...
  if (warcfp != 0)
    fputs(s, warcfp);
  if (get_save_context_p_wrapper())
    saved_append(s);
  if (get_flush_log_p_wrapper())
    logflush();
  else
    set_needs_flushing_wrapper(1);
  errno = errno_save;
}
...
static bool log_vprintf_internal(struct logvprintf_state *state, const char *fmt, va_list args)
{
  ...
  if ((!get_save_context_p_wrapper()) && (warcfp == 0))
  {
...
{
  if (flush == get_flush_log_p_wrapper())
    return;
  if (flush == 0)
  {
    set_flush_log_p_wrapper(0);
  }
  else
  {
    if (get_needs_flushing_wrapper())
      logflush();
    set_flush_log_p_wrapper(1);
  }
}
...
{
  bool old = get_save_context_p_wrapper();
  set_save_context_p_wrapper(savep);
  return old;
}
...
void log_init(const char *file, bool appendp)
{
  if (file)
  {
    if (((*file) == '-') && (!(*(file + 1))))
    {
      set_logfp_wrapper(stdout);
    }
    else
    {
      set_logfp_wrapper(fopen(file, (appendp) ? ("a") : ("w")));
      if (!get_logfp_wrapper())
      {
  ...
  {
    set_logfp_wrapper(stderr);
    if (1 && isatty(fileno(get_logfp_wrapper())))
    {
      set_save_context_p_wrapper(1);
    }
  }
}
...
{
  int i;
  if (get_logfp_wrapper() && (get_logfp_wrapper() != stderr))
    fclose(get_logfp_wrapper());
  set_logfp_wrapper(0);
  set_inhibit_logging_wrapper(1);
  set_save_context_p_wrapper(0);
  for (i = 0; i < 24; i++)
    free_log_line(i);

  set_log_line_current_wrapper(-1);
  set_trailing_line_wrapper(0);
}
...
{
  int num = get_log_line_current_wrapper();
  FILE *fp = get_log_fp();
  FILE *warcfp = get_warc_log_fp();
  if (!fp)
    return;
  ...
  while (num != get_log_line_current_wrapper());
  if (get_trailing_line_wrapper())
    if (log_lines[get_log_line_current_wrapper()].content)
  {
    fputs(log_lines[get_log_line_current_wrapper()].content, fp);
    if (warcfp != 0)
      fputs(log_lines[get_log_line_current_wrapper()].content, warcfp);
  }
  fflush(fp);
  fflush(warcfp);
}
...
{
  char *logfile;
  set_logfp_wrapper(unique_create("wget-log", 0, &logfile));
  if (get_logfp_wrapper())
  {
    fprintf(stderr, gettext("\n%s received, redirecting output to %s.\n"), get_redirect_request_signal_name_wrapper(), quote(logfile));
    do
    {
      free((void *) logfile);
      logfile = 0;
  ...
  {
    fprintf(stderr, gettext("\n%s received.\n"), get_redirect_request_signal_name_wrapper());
    fprintf(stderr, gettext("%s: %s; disabling logging.\n"), (logfile) ? (logfile) : ("wget-log"), strerror(errno));
    set_inhibit_logging_wrapper(1);
  }
  set_save_context_p_wrapper(0);
}
...
static void check_redirect_output(void)
{
  if (get_redirect_request_wrapper() == RR_REQUESTED)
  {
    set_redirect_request_wrapper(RR_DONE);
    redirect_output();
  }
}
...
{
  if ((get_redirect_request_wrapper() == RR_NONE) && get_save_context_p_wrapper())
    set_redirect_request_wrapper(RR_REQUESTED);
  set_redirect_request_signal_name_wrapper(signal_name);
}
