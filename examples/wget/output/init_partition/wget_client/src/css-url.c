...
void get_urls_css(struct map_context *ctx, int offset, int buf_length)
{
  ...
  while ((token = yylex()) != CSSEOF)
  {
    if (token == IMPORT_SYM)
    {
      do
      {
        buffer_pos += get_yyleng_wrapper();
      }
      while ((token = yylex()) == S);
      if ((token == STRING) || (token == URI))
      {
        pos = buffer_pos + offset;
        length = get_yyleng_wrapper();
        if (token == URI)
        {
        ...
        else
        {
          ...
          memcpy(uri, get_yytext_wrapper() + 1, length);
          uri[length] = '\0';
        }
        if (uri)
        {
          ...
          do
          {
            ...
            {
              debug_logprintf("Found @import: [%s] at %d [%s]\n", get_yytext_wrapper(), buffer_pos, uri);
            }
          }
          while (0);
          if (up)
      ...
      if (token == URI)
    {
      ...
      if (uri)
      {
        ...
        do
        {
          if (__builtin_expect(get_opt_debug_wrapper(), 0))
          {
            debug_logprintf("Found URI: [%s] at %d [%s]\n", get_yytext_wrapper(), buffer_pos, uri);
          }
        }
        while (0);
        if (up)
