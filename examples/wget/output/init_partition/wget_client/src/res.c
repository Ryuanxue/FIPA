...
struct robot_specs *res_parse(const char *source, int length)
{
  ...
  bool user_agent_applies = 0;
  bool user_agent_exact = 0;
  bool found_exact = 0;
  int record_count = 0;
  struct robot_specs *specs = xcalloc(1, sizeof(struct robot_specs));
  while (1)
  {
    const char *lineend;
    const char *lineend_real;
    const char *field_b;
    const char *field_e;
    const char *value_b;
    const char *value_e;
    ...
    while (0);
    if (((field_b == field_e) || (p >= lineend)) || ((*p) != ':'))
    {
      do
      {
        if (__builtin_expect(get_opt_debug_wrapper(), 0))
        {
          debug_logprintf("Ignoring malformed line %d\n", line_count);
        }
      }
      while (0);
      goto next;
    ...
    value_e = p;
    if (((field_e - field_b) == ((sizeof("user-agent")) - 1)) && (!c_strncasecmp(field_b, "user-agent", (sizeof("user-agent")) - 1)))
    {
      ...
      if (((field_e - field_b) == ((sizeof("allow")) - 1)) && (!c_strncasecmp(field_b, "allow", (sizeof("allow")) - 1)))
    {
      ...
      if (((field_e - field_b) == ((sizeof("disallow")) - 1)) && (!c_strncasecmp(field_b, "disallow", (sizeof("disallow")) - 1)))
    {
      if (user_agent_applies)
      {
        bool allowed = 0;
    ...
    else
    {
      do
      {
        if (__builtin_expect(get_opt_debug_wrapper(), 0))
        {
          debug_logprintf("Ignoring unknown field at line %d\n", line_count);
        }
      }
      while (0);
      goto next;
...
void res_register_specs(const char *host, int port, struct robot_specs *specs)
{
  ...
  {
    int HP_len = strlen(host);
    hp = alloca(((HP_len + 1) + numdigit(port)) + 1);
    memcpy(hp, host, HP_len);
    hp[HP_len] = ':';
    number_to_string((hp + HP_len) + 1, port);
  }
  while (0);
  if (!get_registered_specs_wrapper())
    set_registered_specs_wrapper(make_nocase_string_hash_table(0));
  if (hash_table_get_pair(get_registered_specs_wrapper(), hp, &hp_old, &old))
  {
    if (old)
      free_specs(old);
    hash_table_put(get_registered_specs_wrapper(), hp_old, specs);
  }
  else
  {
    hash_table_put(get_registered_specs_wrapper(), xstrdup(hp), specs);
  }
}
...
struct robot_specs *res_get_specs(const char *host, int port)
{
  ...
  if (!get_registered_specs_wrapper())
    return 0;
  return hash_table_get(get_registered_specs_wrapper(), hp);
}
...
bool res_retrieve_file(const char *url, char **file, struct iri *iri)
{
  ...
  uerr_t err;
  char *robots_url = uri_merge(url, "/robots.txt");
  int saved_ts_val = get_opt_timestamping_wrapper();
  int saved_sp_val = get_opt_spider_wrapper();
  int url_err;
  struct url *url_parsed;
  ;
  i->utf8_encode = 0;
  logputs(LOG_VERBOSE, gettext("Loading robots.txt; please ignore errors.\n"));
  *file = 0;
  set_opt_timestamping_wrapper(0);
  set_opt_spider_wrapper(0);
  url_parsed = url_parse(robots_url, &url_err, i, 1);
  if (!url_parsed)
  {
  ...
  {
    err = retrieve_url(url_parsed, robots_url, file, 0, 0, 0, 0, i, 0);
    url_free(url_parsed);
  }
  set_opt_timestamping_wrapper(saved_ts_val);
  set_opt_spider_wrapper(saved_sp_val);
  do
  {
...
void res_cleanup(void)
{
  if (get_registered_specs_wrapper())
  {
    ...
    for (hash_table_iterate(get_registered_specs_wrapper(), &iter); hash_table_iter_next(&iter);)
    {
      do
      {
        free((void *) iter.key);
    ...
    hash_table_destroy(get_registered_specs_wrapper());
    set_registered_specs_wrapper(0);
  }
}
