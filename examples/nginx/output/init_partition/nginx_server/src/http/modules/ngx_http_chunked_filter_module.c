...
ngx_int_t ngx_http_chunked_body_filter_sense_1(int *ngx_http_chunked_body_filter_sense_1_ret, ngx_http_request_t *r, ngx_chain_t *in)
{
  u_char *chunk;
  off_t size;
  ngx_int_t rc;
  ...
  {
    {
      *ngx_http_chunked_body_filter_sense_1_ret = 1;
  ...
  for (;;)
  {
    ...
    if (((cl->buf->flush || cl->buf->sync) || ((cl->buf->temporary || cl->buf->memory) || cl->buf->mmap)) || cl->buf->in_file)
    {
