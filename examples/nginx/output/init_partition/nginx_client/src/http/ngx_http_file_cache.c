...
static ngx_int_t ngx_http_file_cache_lock(ngx_http_request_t *r, ngx_http_cache_t *c)
{
  ...
  now = get_ngx_current_msec_wrapper();
  cache = c->file_cache;
...
static void ngx_http_file_cache_lock_wait(ngx_http_request_t *r, ngx_http_cache_t *c)
{
  ...
  if (((ngx_msec_int_t) timer) <= 0)
  {
    if (r->connection->log->log_level >= 7)
      ngx_log_error_core(7, r->connection->log, 0, "cache lock timeout");
    c->lock_timeout = 0;
    goto wakeup;
  }
  cache = c->file_cache;
...
static ngx_int_t ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c)
{
  ...
  if (((size_t) n) < c->header_start)
  {
    if (r->connection->log->log_level >= 3)
      ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" is too small", c->file.name.data);
    return -5;
  }
  ...
  {
    if (r->connection->log->log_level >= 7)
      ngx_log_error_core(7, r->connection->log, 0, "cache file \"%s\" version mismatch", c->file.name.data);
    return -5;
  }
  if ((h->crc32 != c->crc32) || (((size_t) h->header_start) != c->header_start))
  {
    if (r->connection->log->log_level >= 3)
      ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has md5 collision", c->file.name.data);
    return -5;
  }
  ...
  {
    if (r->connection->log->log_level >= 3)
      ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has too long header", c->file.name.data);
    return -5;
  }
  if (h->vary_len > 128)
  {
    if (r->connection->log->log_level >= 3)
      ngx_log_error_core(3, r->connection->log, 0, "cache file \"%s\" has incorrect vary length", c->file.name.data);
    return -5;
  }
  if (h->vary_len)
  {
...
static ngx_int_t ngx_http_file_cache_exists(ngx_http_file_cache_t *cache, ngx_http_cache_t *c)
{
  ...
  if (fcn)
  {
    ...
    if (fcn->error)
    {
      if (fcn->valid_sec < get_ngx_cached_time_sec_wrapper())
      {
        goto renew;
      }
      rc = 0;
  ...
  if (fcn == 0)
  {
    ...
    if (fcn == 0)
    {
      if (get_ngx_cycle_wrapper()->log->log_level >= 2)
        ngx_log_error_core(2, get_ngx_cycle_wrapper()->log, 0, "could not allocate node%s", cache->shpool->log_ctx);
      rc = -1;
      goto failed;
    }
  }
  ...
  fcn->expire = get_ngx_cached_time_sec_wrapper() + cache->inactive;
...
static time_t ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache)
{
  ...
  name = ngx_alloc(len + 1, get_ngx_cycle_wrapper()->log);
  if (name == 0)
  {
    return 10;
  }
  ...
  for (;;)
  {
    ...
    fcn = (ngx_http_file_cache_node_t *) (((u_char *) q) - offsetof(ngx_http_file_cache_node_t, queue));
    ;
    if (fcn->count == 0)
    {
    ...
    (&cache->sh->queue)->next = &fcn->queue;
    if (get_ngx_cycle_wrapper()->log->log_level >= 2)
      ngx_log_error_core(2, get_ngx_cycle_wrapper()->log, 0, "ignore long locked inactive cache entry %*s, count:%d", ((size_t) 2) * 16, key, fcn->count);
    if (sentinel == 0)
    {
      sentinel = q;
    }
...
static time_t ngx_http_file_cache_expire(ngx_http_file_cache_t *cache)
{
  ...
  for (;;)
  {
    if (ngx_quit || get_ngx_terminate_wrapper())
    {
      wait = 1;
      break;
    }
    ...
    ngx_time_update();
    elapsed = (((ngx_msec_int_t) (get_ngx_current_msec_wrapper() - cache->last)) >= 0) ? ((ngx_msec_int_t) (get_ngx_current_msec_wrapper() - cache->last)) : (-((ngx_msec_int_t) (get_ngx_current_msec_wrapper() - cache->last)));
...
static void ngx_http_file_cache_delete(ngx_http_file_cache_t *cache, ngx_queue_t *q, u_char *name)
{
  ...
  if (fcn->exists)
  {
    ...
    *p = '\0';
    ...
    {
      if (get_ngx_cycle_wrapper()->log->log_level >= 3)
        ngx_log_error_core(3, get_ngx_cycle_wrapper()->log, errno, "unlink() \"%s\" failed", name);
    }
    ngx_shmtx_lock(&cache->shpool->mutex);
    fcn->count--;
    fcn->deleting = 0;
...
static ngx_msec_t ngx_http_file_cache_manager(void *data)
{
  ...
  cache->last = get_ngx_current_msec_wrapper();
  cache->files = 0;
...
static void ngx_http_file_cache_loader(void *data)
{
  ...
  if (!__sync_bool_compare_and_swap(&cache->sh->loading, 0, get_ngx_pid_wrapper()))
  {
    return;
  }
  ;
  ...
  tree.data = cache;
  tree.alloc = 0;
  tree.log = get_ngx_cycle_wrapper()->log;
  cache->last = get_ngx_current_msec_wrapper();
  cache->files = 0;
  ...
  if (get_ngx_cycle_wrapper()->log->log_level >= 6)
    ngx_log_error_core(6, get_ngx_cycle_wrapper()->log, 0, "http file cache: %V %.3fM, bsize: %uz", &cache->path->name, (((double) cache->sh->size) * cache->bsize) / (1024 * 1024), cache->bsize);
}
...
static ngx_int_t ngx_http_file_cache_manage_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
  ...
  return (ngx_quit || get_ngx_terminate_wrapper()) ? (-6) : (0);
}
...
static ngx_int_t ngx_http_file_cache_add(ngx_http_file_cache_t *cache, ngx_http_cache_t *c)
{
  ...
  if (fcn == 0)
  {
    ...
    {
      ngx_http_file_cache_set_watermark(cache);
      if (cache->fail_time != get_ngx_cached_time_sec_wrapper())
      {
        cache->fail_time = get_ngx_cached_time_sec_wrapper();
        if (get_ngx_cycle_wrapper()->log->log_level >= 2)
          ngx_log_error_core(2, get_ngx_cycle_wrapper()->log, 0, "could not allocate node%s", cache->shpool->log_ctx);
      }
      ngx_shmtx_unlock(&cache->shpool->mutex);
