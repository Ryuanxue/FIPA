...
static char *ngx_http_upstream_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
  ...
  if (!value[1].len)
  {
    ngx_conf_log_error(1, cf, 0, "invalid zone name \"%V\"", &value[1]);
    return (void *) (-1);
  }
  if (cf->args->nelts == 3)
  {
    ...
    if (size == (-1))
    {
      ngx_conf_log_error(1, cf, 0, "invalid zone size \"%V\"", &value[2]);
      return (void *) (-1);
    }
    if (size < ((ssize_t) (8 * get_ngx_pagesize_wrapper())))
    {
      ngx_conf_log_error(1, cf, 0, "zone \"%V\" is too small", &value[1]);
      return (void *) (-1);
    }
  }
  else
