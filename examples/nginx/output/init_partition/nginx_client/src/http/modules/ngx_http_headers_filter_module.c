...
{
  {
    int ngx_http_headers_filter_sense_1_ret = 0;
    ngx_int_t ngx_http_headers_filter_sense_1_return = ngx_http_headers_filter_sense_1_wrapper(&ngx_http_headers_filter_sense_1_ret, r);
    if (ngx_http_headers_filter_sense_1_ret)
      return ngx_http_headers_filter_sense_1_return;
  }
}
...
{
  {
    int ngx_http_trailers_filter_sense_1_ret = 0;
    ngx_int_t ngx_http_trailers_filter_sense_1_return = ngx_http_trailers_filter_sense_1_wrapper(&ngx_http_trailers_filter_sense_1_ret, r, in);
    if (ngx_http_trailers_filter_sense_1_ret)
      return ngx_http_trailers_filter_sense_1_return;
  }
}
...
static ngx_int_t ngx_http_set_expires(ngx_http_request_t *r, ngx_http_headers_conf_t *conf)
{
  ...
  if (ccp == 0)
  {
    if (ngx_array_init(&r->headers_out.cache_control, r->pool, 1, sizeof(ngx_table_elt_t *)) != 0)
    {
      return -1;
    }
  ...
  cc->value.data = ngx_pnalloc(r->pool, ((sizeof("max-age=")) + ((sizeof("-9223372036854775808")) - 1)) + 1);
  if (cc->value.data == 0)
  {
    return -1;
  }
...
static ngx_int_t ngx_http_headers_filter_init(ngx_conf_t *cf)
{
  ...
  set_ngx_http_top_header_filter_wrapper(ngx_http_headers_filter);
  set_ngx_http_next_body_filter_wrapper(ngx_http_top_body_filter);
  set_ngx_http_top_body_filter_wrapper(ngx_http_trailers_filter);
  return 0;
}
