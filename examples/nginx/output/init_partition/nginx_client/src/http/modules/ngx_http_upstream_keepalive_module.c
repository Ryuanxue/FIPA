...
static void ngx_http_upstream_free_keepalive_peer(ngx_peer_connection_t *pc, void *data, ngx_uint_t state)
{
  ...
  u = kp->upstream;
  c = pc->connection;
  if (((((((state & 4) || (c == 0)) || c->read->eof) || c->read->error) || c->read->timedout) || c->write->error) || c->write->timedout)
  {
    goto invalid;
  }
  ...
  if (get_ngx_terminate_wrapper() || get_ngx_exiting_wrapper())
  {
    goto invalid;
  }
  ...
  if ((&kp->conf->free) == (&kp->conf->free)->prev)
  {
    ...
    q->next->prev = q->prev;
    q->prev->next = q->next;
    item = (ngx_http_upstream_keepalive_cache_t *) (((u_char *) q) - offsetof(ngx_http_upstream_keepalive_cache_t, queue));
  ...
  c->data = item;
  c->idle = 1;
  c->log = get_ngx_cycle_wrapper()->log;
  c->read->log = get_ngx_cycle_wrapper()->log;
  c->write->log = get_ngx_cycle_wrapper()->log;
  c->pool->log = get_ngx_cycle_wrapper()->log;
  item->socklen = pc->socklen;
