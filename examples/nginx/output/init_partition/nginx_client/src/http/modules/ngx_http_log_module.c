...
static ngx_int_t ngx_http_log_handler(ngx_http_request_t *r)
{
  ...
  ;
  {
    int ngx_http_log_handler_sense_1_ret = 0;
    ngx_int_t ngx_http_log_handler_sense_1_return = ngx_http_log_handler_sense_1_wrapper(&ngx_http_log_handler_sense_1_ret, line, p, &len, &size, &n, val, &i, &l, log, op, buffer, lcf, r);
    if (ngx_http_log_handler_sense_1_ret)
      return ngx_http_log_handler_sense_1_return;
  }
}
...
static void ngx_http_log_write(ngx_http_request_t *r, ngx_http_log_t *log, u_char *buf, size_t len)
{
  ...
  if (n == (-1))
  {
    ...
    if ((now - log->error_log_time) > 59)
    {
      if (r->connection->log->log_level >= 2)
        ngx_log_error_core(2, r->connection->log, err, "write() to \"%s\" failed", name);
  ...
  {
    if (r->connection->log->log_level >= 2)
      ngx_log_error_core(2, r->connection->log, 0, "write() to \"%s\" was incomplete: %z of %uz", name, n, len);
    log->error_log_time = now;
  }
}
...
{
  {
    int ngx_http_log_body_bytes_sent_sense_1_ret = 0;
    u_char *ngx_http_log_body_bytes_sent_sense_1_return = ngx_http_log_body_bytes_sent_sense_1_wrapper(&ngx_http_log_body_bytes_sent_sense_1_ret, r, buf);
    if (ngx_http_log_body_bytes_sent_sense_1_ret)
      return ngx_http_log_body_bytes_sent_sense_1_return;
  }
}
