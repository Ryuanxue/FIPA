...
void ngx_http_upstream_init(ngx_http_request_t *r)
{
  ...
  if (get_ngx_event_flags_wrapper() & 0x00000004)
  {
    if (!c->write->active)
    {
      if (ngx_event_actions.add(c->write, EPOLLOUT, EPOLLET) == (-1))
      {
...
static void ngx_http_upstream_init_request(ngx_http_request_t *r)
{
  ...
  else
  {
    ...
    if (u->resolved->sockaddr)
    {
      if ((u->resolved->port == 0) && (u->resolved->sockaddr->sa_family != AF_UNIX))
      {
        if (r->connection->log->log_level >= 4)
          ngx_log_error_core(4, r->connection->log, 0, "no port in upstream \"%V\"", host);
        ngx_http_upstream_finalize_request(r, u, 500);
        return;
      }
      if (ngx_http_upstream_create_round_robin_peer(r, u->resolved) != 0)
      {
        ngx_http_upstream_finalize_request(r, u, 500);
        return;
      }
    ...
    {
      if (r->connection->log->log_level >= 4)
        ngx_log_error_core(4, r->connection->log, 0, "no resolver defined to resolve %V", host);
  ...
  if (uscf == 0)
  {
    if (r->connection->log->log_level >= 2)
      ngx_log_error_core(2, r->connection->log, 0, "no upstream configuration");
    ngx_http_upstream_finalize_request(r, u, 500);
    return;
  }
  ...
  u->peer.start_time = get_ngx_current_msec_wrapper();
...
static void ngx_http_upstream_resolve_handler(ngx_resolver_ctx_t *ctx)
{
  ...
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "%V could not be resolved (%i: %s)", &ctx->name, ctx->state, ngx_resolver_strerror(ctx->state));
    ngx_http_upstream_finalize_request(r, u, 502);
    goto failed;
  }
...
static void ngx_http_upstream_check_broken_connection(ngx_http_request_t *r, ngx_event_t *ev)
{
  ...
  if (c->error)
  {
    if ((get_ngx_event_flags_wrapper() & 0x00000001) && ev->active)
    {
    ...
    if (!u->cacheable)
    {
      ngx_http_upstream_finalize_request(r, u, 499);
    }
    return;
  }
  if ((get_ngx_event_flags_wrapper() & 0x00000040) && ngx_use_epoll_rdhup)
  {
    ...
    {
      if (ev->log->log_level >= 7)
        ngx_log_error_core(7, ev->log, err, "epoll_wait() reported that client prematurely closed connection, so upstream connection is closed too");
      ngx_http_upstream_finalize_request(r, u, 499);
      return;
    }
    if (ev->log->log_level >= 7)
      ngx_log_error_core(7, ev->log, err, "epoll_wait() reported that client prematurely closed connection");
  ...
  {
    if (ev->log->log_level >= 7)
      ngx_log_error_core(7, ev->log, err, "client prematurely closed connection, so upstream connection is closed too");
    ngx_http_upstream_finalize_request(r, u, 499);
    return;
  }
  if (ev->log->log_level >= 7)
    ngx_log_error_core(7, ev->log, err, "client prematurely closed connection");
  if (u->peer.connection == 0)
  {
    ngx_http_upstream_finalize_request(r, u, 499);
  }
...
static void ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  {
    u->state->response_time = get_ngx_current_msec_wrapper() - u->state->response_time;
  }
  u->state = ngx_array_push(r->upstream_states);
  if (u->state == 0)
  {
  ...
  (void) memset(u->state, 0, sizeof(ngx_http_upstream_state_t));
  u->state->response_time = get_ngx_current_msec_wrapper();
  ...
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "no live upstreams");
...
static void ngx_http_upstream_send_request(ngx_http_request_t *r, ngx_http_upstream_t *u, ngx_uint_t do_write)
{
  ...
  {
    u->state->connect_time = get_ngx_current_msec_wrapper() - u->state->response_time;
  }
  ...
  if (rc == (-2))
  {
    ...
    if (c->write->ready && (c->tcp_nopush == NGX_TCP_NOPUSH_SET))
    {
      ...
      {
        if (c->log->log_level >= 3)
          ngx_log_error_core(3, c->log, errno, "setsockopt(!TCP_CORK) failed");
        ngx_http_upstream_finalize_request(r, u, 500);
        return;
      }
...
static void ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  if (u->buffer.start == 0)
  {
    ...
    if (ngx_list_init(&u->headers_in.headers, r->pool, 8, sizeof(ngx_table_elt_t)) != 0)
    {
      ngx_http_upstream_finalize_request(r, u, 500);
      return;
    }
    if (ngx_list_init(&u->headers_in.trailers, r->pool, 2, sizeof(ngx_table_elt_t)) != 0)
    {
      ngx_http_upstream_finalize_request(r, u, 500);
      return;
    }
  ...
  for (;;)
  {
    ...
    if (n == 0)
    {
      if (c->log->log_level >= 4)
        ngx_log_error_core(4, c->log, 0, "upstream prematurely closed connection");
    }
    if ((n == (-1)) || (n == 0))
    {
      ngx_http_upstream_next(r, u, 0x00000002);
    ...
    if (rc == (-2))
    {
      ...
      {
        if (c->log->log_level >= 4)
          ngx_log_error_core(4, c->log, 0, "upstream sent too big header");
  ...
  u->state->header_time = get_ngx_current_msec_wrapper() - u->state->response_time;
...
static ngx_int_t ngx_http_upstream_test_next(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  for (un = ngx_http_upstream_next_errors; un->status; un++)
  {
    ...
    if (u->request_sent && (r->method & ((0x0008 | 0x1000) | 0x4000)))
    {
    ...
    if ((((u->peer.tries > 1) && ((u->conf->next_upstream & mask) == mask)) && (!(u->request_sent && r->request_body_no_buffering))) && (!(timeout && ((get_ngx_current_msec_wrapper() - u->peer.start_time) >= timeout))))
    {
      ngx_http_upstream_next(r, u, un->mask);
      return 0;
    }
    if ((u->cache_status == 3) && ((u->conf->cache_use_stale & un->mask) || r->cache->stale_error))
    {
      ngx_int_t rc;
  ...
  if (((status == 304) && (u->cache_status == 3)) && u->conf->cache_revalidate)
  {
    time_t now;
    time_t valid;
    time_t updating;
...
static ngx_int_t ngx_http_upstream_intercept_errors(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  for (i = 0; i < clcf->error_pages->nelts; i++)
  {
    ...
    {
      if ((status == 401) && u->headers_in.www_authenticate)
      {
      ...
      if (r->cache)
      {
        if (u->cacheable)
        {
          ...
          if (valid == 0)
          {
            ...
            if (valid)
            {
              r->cache->valid_sec = get_ngx_cached_time_sec_wrapper() + valid;
            }
          }
          if (valid)
          {
...
static ngx_int_t ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  ngx_http_upstream_main_conf_t *umcf;
  umcf = r->main_conf[ngx_http_upstream_module.ctx_index];
  if (u->headers_in.x_accel_redirect && (!(u->conf->ignore_headers & 0x00000002)))
  {
    ...
    else
    {
      ...
      {
        r->method = 0x0002;
        r->method_name = get_ngx_http_core_get_method_wrapper();
      }
...
static void ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  if (u->cacheable)
  {
    ...
    if (valid)
    {
      ...
      if ((u->headers_in.status_n == 200) || (u->headers_in.status_n == 206))
      {
  ...
  p->start_sec = get_ngx_cached_time_sec_wrapper();
  ...
  p->length = -1;
  if (u->input_filter_init && (u->input_filter_init(p->input_ctx) != 0))
  {
    ngx_http_upstream_finalize_request(r, u, -1);
    return;
  }
...
static void ngx_http_upstream_upgrade(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
  ...
  {
    if (c->log->log_level >= 4)
      ngx_log_error_core(4, c->log, 0, "connection upgrade in subrequest");
    ngx_http_upstream_finalize_request(r, u, -1);
    return;
  }
  r->keepalive = 0;
  ...
  if (u->peer.connection->read->ready || (u->buffer.pos != u->buffer.last))
  {
    ...
    {
      c->read->posted = 1;
      (&c->read->queue)->prev = (&get_ngx_posted_events_wrapper())->prev;
      (&c->read->queue)->prev->next = &c->read->queue;
      (&c->read->queue)->next = &get_ngx_posted_events_wrapper();
      (&get_ngx_posted_events_wrapper())->prev = &c->read->queue;
      ;
    }
    else
    {
...
static void ngx_http_upstream_next(ngx_http_request_t *r, ngx_http_upstream_t *u, ngx_uint_t ft_type)
{
  ...
  if (u->peer.sockaddr)
  {
    if ((ft_type == 0x00000100) || (ft_type == 0x00000200))
    {
      state = 2;
    }
    else
  ...
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, ETIMEDOUT, "upstream timed out");
  }
  ...
  if ((((u->peer.tries == 0) || ((u->conf->next_upstream & ft_type) != ft_type)) || (u->request_sent && r->request_body_no_buffering)) || (timeout && ((get_ngx_current_msec_wrapper() - u->peer.start_time) >= timeout)))
  {
    if ((u->cache_status == 3) && ((u->conf->cache_use_stale & ft_type) || r->cache->stale_error))
    {
      ngx_int_t rc;
      rc = u->reinit_request(r);
      if (rc != 0)
...
static void ngx_http_upstream_finalize_request(ngx_http_request_t *r, ngx_http_upstream_t *u, ngx_int_t rc)
{
  ...
  *u->cleanup = 0;
  ...
  if (((u->store && u->pipe) && u->pipe->temp_file) && (u->pipe->temp_file->file.fd != (-1)))
  {
    if (unlink((const char *) u->pipe->temp_file->file.name.data) == (-1))
    {
      if (r->connection->log->log_level >= 3)
        ngx_log_error_core(3, r->connection->log, errno, "unlink() \"%s\" failed", u->pipe->temp_file->file.name.data);
    }
  }
  if (r->cache)
  {
  ...
  if (((!u->header_sent) || (rc == 408)) || (rc == 499))
  {
    ngx_http_finalize_request(r, rc);
    return;
  }
  ...
  *ph = h;
  {
    ...
    if (((ngx_strlcasestrn(start, last, (u_char *) "no-cache", 8 - 1) != 0) || (ngx_strlcasestrn(start, last, (u_char *) "no-store", 8 - 1) != 0)) || (ngx_strlcasestrn(start, last, (u_char *) "private", 7 - 1) != 0))
    {
      u->cacheable = 0;
      return 0;
    }
    ...
    if (p)
    {
      ...
      r->cache->valid_sec = get_ngx_cached_time_sec_wrapper() + n;
    }
...
static ngx_int_t ngx_http_upstream_process_expires(ngx_http_request_t *r, ngx_table_elt_t *h, ngx_uint_t offset)
{
  ...
  u->headers_in.expires = h;
  {
    ...
    if ((expires == (-1)) || (expires < get_ngx_cached_time_sec_wrapper()))
    {
      u->cacheable = 0;
      return 0;
    }
...
static char *ngx_http_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
  ...
  u.no_resolve = 1;
  u.no_port = 1;
  uscf = ngx_http_upstream_add(cf, &u, (((((0x0001 | 0x0002) | 0x0100) | 0x0004) | 0x0008) | 0x0010) | 0x0020);
  if (uscf == 0)
  {
    return (void *) (-1);
  }
  ...
  *cf = pcf;
  ...
  {
    ngx_conf_log_error(1, cf, 0, "no servers are inside upstream");
    return (void *) (-1);
  }
  return rv;
}
