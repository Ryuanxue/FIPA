...
ngx_int_t ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
  ...
  ;
  {
    int ngx_http_core_generic_phase_sense_1_ret = 0;
    ngx_int_t ngx_http_core_generic_phase_sense_1_return = ngx_http_core_generic_phase_sense_1_wrapper(&ngx_http_core_generic_phase_sense_1_ret, &rc, r, ph);
    if (ngx_http_core_generic_phase_sense_1_ret)
      return ngx_http_core_generic_phase_sense_1_return;
  }
}
...
ngx_int_t ngx_http_core_find_config_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
  ...
  {
    int ngx_http_core_find_config_phase_sense_1_ret = 0;
    ngx_int_t ngx_http_core_find_config_phase_sense_1_return = ngx_http_core_find_config_phase_sense_1_wrapper(&ngx_http_core_find_config_phase_sense_1_ret, p, &len, &rc, clcf, r);
    if (ngx_http_core_find_config_phase_sense_1_ret)
      return ngx_http_core_find_config_phase_sense_1_return;
  }
  ;
  ;
}
...
ngx_int_t ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
  ...
  ngx_http_core_loc_conf_t *clcf;
  {
    int ngx_http_core_access_phase_sense_1_ret = 0;
    ngx_int_t ngx_http_core_access_phase_sense_1_return = ngx_http_core_access_phase_sense_1_wrapper(&ngx_http_core_access_phase_sense_1_ret, &rc, clcf, r, ph);
    if (ngx_http_core_access_phase_sense_1_ret)
      return ngx_http_core_access_phase_sense_1_return;
  }
  ;
}
...
ngx_int_t ngx_http_core_post_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
  ...
  ;
  {
    int ngx_http_core_post_access_phase_sense_1_ret = 0;
    ngx_int_t ngx_http_core_post_access_phase_sense_1_return = ngx_http_core_post_access_phase_sense_1_wrapper(&ngx_http_core_post_access_phase_sense_1_ret, &access_code, r);
    if (ngx_http_core_post_access_phase_sense_1_ret)
      return ngx_http_core_post_access_phase_sense_1_return;
  }
}
...
void ngx_http_update_location_config(ngx_http_request_t *r)
{
  ...
  if ((get_ngx_io_flags_wrapper() & 1) && clcf->sendfile)
  {
  ...
  if (clcf->client_body_in_file_only)
  {
    ...
    r->request_body_in_clean_file = clcf->client_body_in_file_only == 2;
  ...
  if (r->keepalive)
  {
      ...
      if (r->connection->requests >= clcf->keepalive_requests)
    {
      ...
      if ((r->headers_in.msie6 && (r->method == 0x0008)) && (clcf->keepalive_disable & 0x0004))
    {
      r->keepalive = 0;
    }
    else
      if (r->headers_in.safari && (clcf->keepalive_disable & 0x0008))
    {
      r->keepalive = 0;
    }
  }
...
ngx_int_t ngx_http_send_header(ngx_http_request_t *r)
{
  ...
  if (r->header_sent)
  {
    if (r->connection->log->log_level >= 2)
      ngx_log_error_core(2, r->connection->log, 0, "header already sent");
    return -1;
  }
  if (r->err_status)
  {
  ...
  return get_ngx_http_top_header_filter_wrapper()(r);
}
...
ngx_int_t ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
  ...
  c = r->connection;
  ;
  rc = get_ngx_http_top_body_filter_wrapper()(r, in);
  if (rc == (-1))
  {
    c->error = 1;
  }
...
u_char *ngx_http_map_uri_to_path(ngx_http_request_t *r, ngx_str_t *path, size_t *root_length, size_t reserved)
{
  ...
  {
    if (r->connection->log->log_level >= 2)
      ngx_log_error_core(2, r->connection->log, 0, "\"alias\" cannot be used in location \"%V\" where URI was rewritten", &clcf->name);
    return 0;
  }
  ...
  else
  {
    ...
    if (ngx_http_script_run(r, path, clcf->root_lengths->elts, reserved, clcf->root_values->elts) == 0)
    {
      return 0;
    }
    if (ngx_get_full_name(r->pool, (ngx_str_t *) (&get_ngx_cycle_wrapper()->prefix), path) != 0)
    {
      return 0;
    }
...
ngx_int_t ngx_http_subrequest(ngx_http_request_t *r, ngx_str_t *uri, ngx_str_t *args, ngx_http_request_t **psr, ngx_http_post_subrequest_t *ps, ngx_uint_t flags)
{
  ...
  if (r->subrequests == 0)
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "subrequests cycle while processing \"%V\"", uri);
    return -1;
  }
  if (r->main->count >= (65535 - 1000))
  {
    if (r->connection->log->log_level >= 3)
      ngx_log_error_core(3, r->connection->log, 0, "request reference counter overflow while processing \"%V\"", uri);
    return -1;
  }
  if (r->subrequest_in_memory)
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "nested in-memory subrequest \"%V\"", uri);
    return -1;
  }
  ...
  c = r->connection;
  sr->connection = c;
  sr->ctx = ngx_pcalloc(r->pool, (sizeof(void *)) * get_ngx_http_max_module_wrapper());
  if (sr->ctx == 0)
  {
    return -1;
  }
  if (ngx_list_init(&sr->headers_out.headers, r->pool, 20, sizeof(ngx_table_elt_t)) != 0)
  {
    return -1;
  }
  if (ngx_list_init(&sr->headers_out.trailers, r->pool, 4, sizeof(ngx_table_elt_t)) != 0)
  {
    return -1;
  }
  ...
  {
    sr->headers_out.content_length->hash = 0;
    sr->headers_out.content_length = 0;
  }
  ;
  sr->allow_ranges = 0;
  if (sr->headers_out.accept_ranges)
  {
    sr->headers_out.accept_ranges->hash = 0;
    sr->headers_out.accept_ranges = 0;
  }
  ;
  sr->headers_out.last_modified_time = -1;
  if (sr->headers_out.last_modified)
  {
    sr->headers_out.last_modified->hash = 0;
    sr->headers_out.last_modified = 0;
  }
  ;
  ...
  sr->method_name = get_ngx_http_core_get_method_wrapper();
...
ngx_int_t ngx_http_internal_redirect(ngx_http_request_t *r, ngx_str_t *uri, ngx_str_t *args)
{
  ...
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "rewrite or internal redirection cycle while internally redirecting to \"%V\"", uri);
    r->main->count++;
  ...
  (void) memset(r->ctx, 0, (sizeof(void *)) * get_ngx_http_max_module_wrapper());
  cscf = r->srv_conf[ngx_http_core_module.ctx_index];
  r->loc_conf = cscf->ctx->loc_conf;
...
ngx_int_t ngx_http_named_location(ngx_http_request_t *r, ngx_str_t *name)
{
  ...
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "rewrite or internal redirection cycle while redirect to named location \"%V\"", name);
    ngx_http_finalize_request(r, 500);
    return -4;
  }
  if (r->uri.len == 0)
  {
    if (r->connection->log->log_level >= 4)
      ngx_log_error_core(4, r->connection->log, 0, "empty URI in redirect to named location \"%V\"", name);
    ngx_http_finalize_request(r, 500);
    return -4;
  }
  cscf = r->srv_conf[ngx_http_core_module.ctx_index];
  if (cscf->named_locations)
  {
    ...
    {
      ;
      if ((name->len != (*clcfp)->name.len) || (strncmp((const char *) name->data, (const char *) (*clcfp)->name.data, name->len) != 0))
      {
        continue;
      }
      ;
  ...
  if (r->connection->log->log_level >= 4)
    ngx_log_error_core(4, r->connection->log, 0, "could not find named location \"%V\"", name);
  ngx_http_finalize_request(r, 500);
  return -4;
}
...
static char *ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
  ...
  ctx->srv_conf = ngx_pcalloc(cf->pool, (sizeof(void *)) * get_ngx_http_max_module_wrapper());
  if (ctx->srv_conf == 0)
  {
    return (void *) (-1);
  }
  ctx->loc_conf = ngx_pcalloc(cf->pool, (sizeof(void *)) * get_ngx_http_max_module_wrapper());
  if (ctx->loc_conf == 0)
  {
    return (void *) (-1);
  }
  ...
  *cf = pcf;
  if ((rv == 0) && (!cscf->listen))
  {
    ...
    lsopt.fastopen = -1;
    lsopt.wildcard = 1;
    (void) ngx_sock_ntop(&lsopt.sockaddr.sockaddr, lsopt.socklen, lsopt.addr, (((sizeof("unix:")) - 1) + (sizeof(struct sockaddr_un))) - offsetof(struct sockaddr_un, sun_path), 1);
...
static char *ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
  ...
  if (cf->args->nelts == 3)
  {
    ...
    {
      ngx_conf_log_error(1, cf, 0, "invalid location modifier \"%V\"", &value[1]);
      return (void *) (-1);
    }
  }
  else
  ...
  if (cf->cmd_type == 0x08000000)
  {
    ...
    {
      ngx_conf_log_error(1, cf, 0, "location \"%V\" cannot be inside the exact location \"%V\"", &clcf->name, &pclcf->name);
      return (void *) (-1);
    }
    if (pclcf->named)
    {
      ngx_conf_log_error(1, cf, 0, "location \"%V\" cannot be inside the named location \"%V\"", &clcf->name, &pclcf->name);
      return (void *) (-1);
    }
    if (clcf->named)
    {
      ngx_conf_log_error(1, cf, 0, "named location \"%V\" can be on the server level only", &clcf->name);
      return (void *) (-1);
    }
    len = pclcf->name.len;
    if (ngx_filename_cmp(clcf->name.data, pclcf->name.data, len) != 0)
    {
      ngx_conf_log_error(1, cf, 0, "location \"%V\" is outside location \"%V\"", &clcf->name, &pclcf->name);
      return (void *) (-1);
    }
  }
...
static ngx_int_t ngx_http_core_postconfiguration(ngx_conf_t *cf)
{
  set_ngx_http_top_request_body_filter_wrapper(ngx_http_request_body_save_filter);
  return 0;
}
...
static char *ngx_http_core_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
  ...
  if (conf->types_hash_max_size == ((ngx_uint_t) (-1)))
  {
    conf->types_hash_max_size = (prev->types_hash_max_size == ((ngx_uint_t) (-1))) ? (1024) : (prev->types_hash_max_size);
  }
  ;
  if (conf->types_hash_bucket_size == ((ngx_uint_t) (-1)))
  {
    conf->types_hash_bucket_size = (prev->types_hash_bucket_size == ((ngx_uint_t) (-1))) ? (64) : (prev->types_hash_bucket_size);
  }
  ;
  ...
  if (prev->types && (prev->types_hash.buckets == 0))
  {
    ...
    if (ngx_hash_init(&types_hash, prev->types->elts, prev->types->nelts) != 0)
    {
      return (void *) (-1);
    }
  }
  ...
  if (conf->types_hash.buckets == 0)
  {
    ...
    types_hash.pool = cf->pool;
    types_hash.temp_pool = 0;
    if (ngx_hash_init(&types_hash, conf->types->elts, conf->types->nelts) != 0)
    {
      return (void *) (-1);
    }
  }
  ...
  if (conf->default_type.data == 0)
  {
    ...
    {
      conf->default_type.len = (sizeof("text/plain")) - 1;
      conf->default_type.data = (u_char *) "text/plain";
    }
  }
  ;
  if (conf->client_max_body_size == (-1))
  {
    conf->client_max_body_size = (prev->client_max_body_size == (-1)) ? ((1 * 1024) * 1024) : (prev->client_max_body_size);
  }
  ;
  if (conf->client_body_buffer_size == ((size_t) (-1)))
  {
    conf->client_body_buffer_size = (prev->client_body_buffer_size == ((size_t) (-1))) ? (((size_t) 2) * get_ngx_pagesize_wrapper()) : (prev->client_body_buffer_size);
  }
  ;
  if (conf->client_body_timeout == ((ngx_msec_t) (-1)))
  {
    conf->client_body_timeout = (prev->client_body_timeout == ((ngx_msec_t) (-1))) ? (60000) : (prev->client_body_timeout);
  }
  ;
  if (conf->keepalive_disable == 0)
  {
    conf->keepalive_disable = (prev->keepalive_disable == 0) ? (1 | 0x0004) : (prev->keepalive_disable);
  }
  ;
  if (conf->satisfy == ((ngx_uint_t) (-1)))
  {
    conf->satisfy = (prev->satisfy == ((ngx_uint_t) (-1))) ? (0) : (prev->satisfy);
  }
  ;
  if (conf->if_modified_since == ((ngx_uint_t) (-1)))
  {
    conf->if_modified_since = (prev->if_modified_since == ((ngx_uint_t) (-1))) ? (1) : (prev->if_modified_since);
  }
  ;
  if (conf->max_ranges == ((ngx_uint_t) (-1)))
  {
    conf->max_ranges = (prev->max_ranges == ((ngx_uint_t) (-1))) ? ((uint32_t) 0x7fffffff) : (prev->max_ranges);
  }
  ;
  if (conf->client_body_in_file_only == ((ngx_uint_t) (-1)))
  {
    conf->client_body_in_file_only = (prev->client_body_in_file_only == ((ngx_uint_t) (-1))) ? (0) : (prev->client_body_in_file_only);
  }
  ;
  if (conf->client_body_in_single_buffer == (-1))
  {
    conf->client_body_in_single_buffer = (prev->client_body_in_single_buffer == (-1)) ? (0) : (prev->client_body_in_single_buffer);
  }
  ;
  if (conf->internal == (-1))
  {
    conf->internal = (prev->internal == (-1)) ? (0) : (prev->internal);
  }
  ;
  if (conf->sendfile == (-1))
  {
    conf->sendfile = (prev->sendfile == (-1)) ? (0) : (prev->sendfile);
  }
  ;
  if (conf->sendfile_max_chunk == ((size_t) (-1)))
  {
    conf->sendfile_max_chunk = (prev->sendfile_max_chunk == ((size_t) (-1))) ? (0) : (prev->sendfile_max_chunk);
  }
  ;
  if (conf->subrequest_output_buffer_size == ((size_t) (-1)))
  {
    conf->subrequest_output_buffer_size = (prev->subrequest_output_buffer_size == ((size_t) (-1))) ? ((size_t) get_ngx_pagesize_wrapper()) : (prev->subrequest_output_buffer_size);
  }
  ;
  if (conf->aio == (-1))
  {
    conf->aio = (prev->aio == (-1)) ? (0) : (prev->aio);
  }
  ;
  if (conf->aio_write == (-1))
  {
    conf->aio_write = (prev->aio_write == (-1)) ? (0) : (prev->aio_write);
  }
  ;
  if (conf->read_ahead == ((size_t) (-1)))
  {
    conf->read_ahead = (prev->read_ahead == ((size_t) (-1))) ? (0) : (prev->read_ahead);
  }
  ;
  if (conf->directio == (-1))
  {
    conf->directio = (prev->directio == (-1)) ? (9223372036854775807LL) : (prev->directio);
  }
  ;
  if (conf->directio_alignment == (-1))
  {
    conf->directio_alignment = (prev->directio_alignment == (-1)) ? (512) : (prev->directio_alignment);
  }
  ;
  if (conf->tcp_nopush == (-1))
  {
    conf->tcp_nopush = (prev->tcp_nopush == (-1)) ? (0) : (prev->tcp_nopush);
  }
  ;
  if (conf->tcp_nodelay == (-1))
  {
    conf->tcp_nodelay = (prev->tcp_nodelay == (-1)) ? (1) : (prev->tcp_nodelay);
  }
  ;
  if (conf->send_timeout == ((ngx_msec_t) (-1)))
  {
    conf->send_timeout = (prev->send_timeout == ((ngx_msec_t) (-1))) ? (60000) : (prev->send_timeout);
  }
  ;
  if (conf->send_lowat == ((size_t) (-1)))
  {
    conf->send_lowat = (prev->send_lowat == ((size_t) (-1))) ? (0) : (prev->send_lowat);
  }
  ;
  if (conf->postpone_output == ((size_t) (-1)))
  {
    conf->postpone_output = (prev->postpone_output == ((size_t) (-1))) ? (1460) : (prev->postpone_output);
  }
  ;
  if (conf->limit_rate == ((size_t) (-1)))
  {
    conf->limit_rate = (prev->limit_rate == ((size_t) (-1))) ? (0) : (prev->limit_rate);
  }
  ;
  if (conf->limit_rate_after == ((size_t) (-1)))
  {
    conf->limit_rate_after = (prev->limit_rate_after == ((size_t) (-1))) ? (0) : (prev->limit_rate_after);
  }
  ;
  if (conf->keepalive_timeout == ((ngx_msec_t) (-1)))
  {
    conf->keepalive_timeout = (prev->keepalive_timeout == ((ngx_msec_t) (-1))) ? (75000) : (prev->keepalive_timeout);
  }
  ;
  if (conf->keepalive_header == (-1))
  {
    conf->keepalive_header = (prev->keepalive_header == (-1)) ? (0) : (prev->keepalive_header);
  }
  ;
  if (conf->keepalive_requests == ((ngx_uint_t) (-1)))
  {
    conf->keepalive_requests = (prev->keepalive_requests == ((ngx_uint_t) (-1))) ? (100) : (prev->keepalive_requests);
  }
  ;
  if (conf->lingering_close == ((ngx_uint_t) (-1)))
  {
    conf->lingering_close = (prev->lingering_close == ((ngx_uint_t) (-1))) ? (1) : (prev->lingering_close);
  }
  ;
  if (conf->lingering_time == ((ngx_msec_t) (-1)))
  {
    conf->lingering_time = (prev->lingering_time == ((ngx_msec_t) (-1))) ? (30000) : (prev->lingering_time);
  }
  ;
  if (conf->lingering_timeout == ((ngx_msec_t) (-1)))
  {
    conf->lingering_timeout = (prev->lingering_timeout == ((ngx_msec_t) (-1))) ? (5000) : (prev->lingering_timeout);
  }
  ;
  if (conf->resolver_timeout == ((ngx_msec_t) (-1)))
  {
    conf->resolver_timeout = (prev->resolver_timeout == ((ngx_msec_t) (-1))) ? (30000) : (prev->resolver_timeout);
  }
  ;
  if (conf->resolver == 0)
  {
  ...
  if (ngx_conf_merge_path_value(cf, &conf->client_body_temp_path, prev->client_body_temp_path, &ngx_http_client_temp_path) != 0)
  {
    return (void *) (-1);
  }
  if (conf->reset_timedout_connection == (-1))
  {
    conf->reset_timedout_connection = (prev->reset_timedout_connection == (-1)) ? (0) : (prev->reset_timedout_connection);
  }
  ;
  if (conf->absolute_redirect == (-1))
  {
    conf->absolute_redirect = (prev->absolute_redirect == (-1)) ? (1) : (prev->absolute_redirect);
  }
  ;
  if (conf->server_name_in_redirect == (-1))
  {
    conf->server_name_in_redirect = (prev->server_name_in_redirect == (-1)) ? (0) : (prev->server_name_in_redirect);
  }
  ;
  if (conf->port_in_redirect == (-1))
  {
    conf->port_in_redirect = (prev->port_in_redirect == (-1)) ? (1) : (prev->port_in_redirect);
  }
  ;
  if (conf->msie_padding == (-1))
  {
    conf->msie_padding = (prev->msie_padding == (-1)) ? (1) : (prev->msie_padding);
  }
  ;
  if (conf->msie_refresh == (-1))
  {
    conf->msie_refresh = (prev->msie_refresh == (-1)) ? (0) : (prev->msie_refresh);
  }
  ;
  if (conf->log_not_found == (-1))
  {
    conf->log_not_found = (prev->log_not_found == (-1)) ? (1) : (prev->log_not_found);
  }
  ;
  if (conf->log_subrequest == (-1))
  {
    conf->log_subrequest = (prev->log_subrequest == (-1)) ? (0) : (prev->log_subrequest);
  }
  ;
  if (conf->recursive_error_pages == (-1))
  {
    conf->recursive_error_pages = (prev->recursive_error_pages == (-1)) ? (0) : (prev->recursive_error_pages);
  }
  ;
  if (conf->chunked_transfer_encoding == (-1))
  {
    conf->chunked_transfer_encoding = (prev->chunked_transfer_encoding == (-1)) ? (1) : (prev->chunked_transfer_encoding);
  }
  ;
  if (conf->etag == (-1))
  {
    conf->etag = (prev->etag == (-1)) ? (1) : (prev->etag);
  }
  ;
  if (conf->server_tokens == ((ngx_uint_t) (-1)))
  {
    conf->server_tokens = (prev->server_tokens == ((ngx_uint_t) (-1))) ? (1) : (prev->server_tokens);
  }
  ;
  if (conf->open_file_cache == ((void *) (-1)))
  {
    conf->open_file_cache = (prev->open_file_cache == ((void *) (-1))) ? (0) : (prev->open_file_cache);
  }
  ;
  if (conf->open_file_cache_valid == (-1))
  {
    conf->open_file_cache_valid = (prev->open_file_cache_valid == (-1)) ? (60) : (prev->open_file_cache_valid);
  }
  ;
  if (conf->open_file_cache_min_uses == ((ngx_uint_t) (-1)))
  {
    conf->open_file_cache_min_uses = (prev->open_file_cache_min_uses == ((ngx_uint_t) (-1))) ? (1) : (prev->open_file_cache_min_uses);
  }
  ;
  if (conf->open_file_cache_errors == (-1))
  {
    conf->open_file_cache_errors = (prev->open_file_cache_errors == (-1)) ? (0) : (prev->open_file_cache_errors);
  }
  ;
  if (conf->open_file_cache_events == (-1))
  {
    conf->open_file_cache_events = (prev->open_file_cache_events == (-1)) ? (0) : (prev->open_file_cache_events);
  }
  ;
  if (conf->disable_symlinks == ((ngx_uint_t) (-1)))
  {
    conf->disable_symlinks = (prev->disable_symlinks == ((ngx_uint_t) (-1))) ? (0) : (prev->disable_symlinks);
  }
  ;
  if (conf->disable_symlinks_from == ((void *) (-1)))
  {
    conf->disable_symlinks_from = (prev->disable_symlinks_from == ((void *) (-1))) ? (0) : (prev->disable_symlinks_from);
  }
  ;
  return 0;
}
...
static char *ngx_http_core_limit_except(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
  ...
  for (i = 1; i < cf->args->nelts; i++)
  {
    ...
    ngx_conf_log_error(1, cf, 0, "invalid method \"%V\"", &value[i]);
    return (void *) (-1);
    next:
    continue;
