  ...
  for (;;)
  {
    ...
    else
    {
      if (p->limit_rate)
      {
        ...
        limit = (((off_t) p->limit_rate) * ((get_ngx_cached_time_sec_wrapper() - p->start_sec) + 1)) - p->read_length;
        if (limit <= 0)
        {
        ...
        if (p->allocated < p->bufs.num)
      {
        ...
        if ((((!p->cacheable) && (p->downstream->data == p->output_ctx)) && p->downstream->write->ready) && (!p->downstream->write->delayed))
      {
