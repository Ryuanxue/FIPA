...
static ngx_int_t ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
  ...
  if (get_ep_wrapper() == (-1))
  {
    set_ep_wrapper(epoll_create(cycle->connection_n / 2));
    if (get_ep_wrapper() == (-1))
    {
      if (cycle->log->log_level >= 1)
        ngx_log_error_core(1, cycle->log, errno, "epoll_create() failed");
      return -1;
    }
  ...
  nevents = epcf->events;
  set_ngx_io_wrapper(ngx_os_io);
  ngx_event_actions = ngx_epoll_module_ctx.actions;
  set_ngx_event_flags_wrapper((0x00000004 | 0x00000020) | 0x00000040);
  return 0;
}
...
static void ngx_epoll_done(ngx_cycle_t *cycle)
{
  if (close(get_ep_wrapper()) == (-1))
  {
    if (cycle->log->log_level >= 2)
      ngx_log_error_core(2, cycle->log, errno, "epoll close() failed");
  }
  set_ep_wrapper(-1);
  if (close(notify_fd) == (-1))
  {
    if (cycle->log->log_level >= 2)
      ngx_log_error_core(2, cycle->log, errno, "eventfd close() failed");
  }
  notify_fd = -1;
  free(event_list);
  event_list = 0;
...
static ngx_int_t ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
  ...
  ;
  if (epoll_ctl(get_ep_wrapper(), op, c->fd, &ee) == (-1))
  {
    if (ev->log->log_level >= 2)
      ngx_log_error_core(2, ev->log, errno, "epoll_ctl(%d, %d) failed", op, c->fd);
    return -1;
  }
  ev->active = 1;
  return 0;
...
static ngx_int_t ngx_epoll_add_connection(ngx_connection_t *c)
{
  ...
  ;
  if (epoll_ctl(get_ep_wrapper(), EPOLL_CTL_ADD, c->fd, &ee) == (-1))
  {
    if (c->log->log_level >= 2)
      ngx_log_error_core(2, c->log, errno, "epoll_ctl(EPOLL_CTL_ADD, %d) failed", c->fd);
    return -1;
  }
  c->read->active = 1;
  c->write->active = 1;
...
static ngx_int_t ngx_epoll_del_connection(ngx_connection_t *c, ngx_uint_t flags)
{
  ...
  {
    if (c->log->log_level >= 2)
      ngx_log_error_core(2, c->log, errno, "epoll_ctl(%d, %d) failed", op, c->fd);
    return -1;
  }
  c->read->active = 0;
  c->write->active = 0;
...
static ngx_int_t ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
  ...
  ngx_connection_t *c;
  ;
  events = epoll_wait(get_ep_wrapper(), event_list, (int) nevents, timer);
  ...
  if (events == 0)
  {
    ...
    if (cycle->log->log_level >= 2)
      ngx_log_error_core(2, cycle->log, 0, "epoll_wait() returned no events without timeout");
    return -1;
  }
  for (i = 0; i < events; i++)
  {
    ...
    if ((revents & EPOLLIN) && rev->active)
    {
      ...
      if (flags & 2)
      {
        queue = (rev->accept) ? (&ngx_posted_accept_events) : (&get_ngx_posted_events_wrapper());
        if (!rev->posted)
        {
          rev->posted = 1;
    ...
    if ((revents & EPOLLOUT) && wev->active)
    {
      ...
      if (flags & 2)
      {
        ...
        {
          wev->posted = 1;
          (&wev->queue)->prev = (&get_ngx_posted_events_wrapper())->prev;
          (&wev->queue)->prev->next = &wev->queue;
          (&wev->queue)->next = &get_ngx_posted_events_wrapper();
          (&get_ngx_posted_events_wrapper())->prev = &wev->queue;
          ;
        }
        else
        {
