...
static void ngx_open_file_cache_cleanup(void *data)
{
  ...
  for (;;)
  {
    ...
    file = (ngx_cached_open_file_t *) (((u_char *) q) - offsetof(ngx_cached_open_file_t, queue));
    q->next->prev = q->prev;
    q->prev->next = q->next;
    ...
    if ((!file->err) && (!file->is_dir))
    {
      file->close = 1;
      file->count = 0;
      ngx_close_cached_file(cache, file, 0, get_ngx_cycle_wrapper()->log);
    }
    else
    {
      free(file->name);
  ...
  if (cache->current)
  {
    if (get_ngx_cycle_wrapper()->log->log_level >= 2)
      ngx_log_error_core(2, get_ngx_cycle_wrapper()->log, 0, "%ui items still left in open file cache", cache->current);
  }
  if (cache->rbtree.root != cache->rbtree.sentinel)
  {
    if (get_ngx_cycle_wrapper()->log->log_level >= 2)
      ngx_log_error_core(2, get_ngx_cycle_wrapper()->log, 0, "rbtree still is not empty in open file cache");
  }
}
...
ngx_int_t ngx_open_cached_file(ngx_open_file_cache_t *cache, ngx_str_t *name, ngx_open_file_info_t *of, ngx_pool_t *pool)
{
  ...
  if (cache == 0)
  {
    if (of->test_only)
    {
      if (ngx_file_info_wrapper(name, of, &fi, pool->log) == (-1))
      {
        return -1;
      }
      of->uniq = (&fi)->st_ino;
  ...
  if (file)
  {
    ...
    if (file->use_event || (((((file->event == 0) && ((of->uniq == 0) || (of->uniq == file->uniq))) && ((now - file->created) < of->valid)) && (of->disable_symlinks == file->disable_symlinks)) && (of->disable_symlinks_from == file->disable_symlinks_from)))
    {
      if (file->err == 0)
      {
        of->fd = file->fd;
      ...
      {
        of->err = file->err;
        of->failed = (file->disable_symlinks) ? ("openat()") : ("open()");
      }
      goto found;
    }
    ;
    ...
    if (file->count == 0)
    {
      ...
      if (close(file->fd) == (-1))
      {
        if (pool->log->log_level >= 2)
          ngx_log_error_core(2, pool->log, errno, "close() \"%V\" failed", name);
      }
      goto add_event;
    }
    ngx_rbtree_delete(&cache->rbtree, &file->node);
  ...
  if (file)
  {
    ...
    if (file->count == 0)
    {
      if (file->fd != (-1))
      {
        ...
        {
          if (pool->log->log_level >= 2)
            ngx_log_error_core(2, pool->log, errno, "close() \"%s\" failed", file->name);
        }
      }
      free(file->name);
      free(file);
...
static void ngx_open_file_add_event(ngx_open_file_cache_t *cache, ngx_cached_open_file_t *file, ngx_open_file_info_t *of, ngx_log_t *log)
{
  ...
  if (((((!(get_ngx_event_flags_wrapper() & 0x00002000)) || (!of->events)) || file->event) || (of->fd == (-1))) || (file->uses < of->min_uses))
  {
    return;
  }
  file->use_event = 0;
  ...
  file->event->data = fev;
  file->event->log = get_ngx_cycle_wrapper()->log;
  if (ngx_event_actions.add(file->event, 0, 0x70000000) != 0)
  {
...
static void ngx_close_cached_file(ngx_open_file_cache_t *cache, ngx_cached_open_file_t *file, ngx_uint_t min_uses, ngx_log_t *log)
{
  ...
  if (!file->close)
  {
    file->accessed = get_ngx_cached_time_sec_wrapper();
    (&file->queue)->next->prev = (&file->queue)->prev;
    (&file->queue)->prev->next = (&file->queue)->next;
    (&file->queue)->next = (&cache->expire_queue)->next;
    (&file->queue)->next->prev = &file->queue;
  ...
  if (file->fd != (-1))
  {
    ...
    {
      if (log->log_level >= 2)
        ngx_log_error_core(2, log, errno, "close() \"%s\" failed", file->name);
    }
    file->fd = -1;
  }
  if (!file->close)
