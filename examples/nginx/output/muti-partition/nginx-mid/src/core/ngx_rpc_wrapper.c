...
ngx_int_t auth_basic_handler_wrapper(ngx_http_request_t *r)
{
    char *host = "localhost";
	clnt = clnt_create (host, RPCAUTH, RPCAUTHTVERS, "udp");
	if (clnt == NULL) {
	...
	long  *result_1;
printf("=== FUNCTION START: auth_basic_handler_wrapper called ===\n");
fflush(stdout);
...
result_1=ngx_http_auth_basic_handler_rpc_1(r_rpc_ptr,clnt);
