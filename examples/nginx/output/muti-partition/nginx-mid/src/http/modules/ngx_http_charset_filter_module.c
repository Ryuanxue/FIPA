...
ngx_int_t ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in);
typedef struct {
...
ngx_int_t ngx_http_ssi_header_filter(ngx_http_request_t *r);
...
ngx_http_charset_header_filter(ngx_http_request_t *r)
{
    ...
    if (charset == NGX_DECLINED) {ngx_http_next_header_filter=ngx_http_ssi_header_filter;
...
ngx_http_charset_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ctx == NULL || ctx->table == NULL) {ngx_http_next_body_filter=ngx_http_ssi_body_filter; 
