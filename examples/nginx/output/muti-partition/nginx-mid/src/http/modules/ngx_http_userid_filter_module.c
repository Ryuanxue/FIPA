...
ngx_int_t ngx_http_charset_header_filter(ngx_http_request_t *r);
...
ngx_http_userid_filter(ngx_http_request_t *r)
{
    ...
    if (conf->enable < NGX_HTTP_USERID_V1) { ngx_http_next_header_filter= ngx_http_charset_header_filter;
        return ngx_http_next_header_filter(r);
    }
