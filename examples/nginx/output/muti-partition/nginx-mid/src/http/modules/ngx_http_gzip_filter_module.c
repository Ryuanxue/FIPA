...
#endif
ngx_int_t ngx_http_chunked_body_filter(ngx_http_request_t *r, ngx_chain_t *in);
ngx_int_t ngx_http_range_header_filter(ngx_http_request_t *r);
...
ngx_http_gzip_header_filter(ngx_http_request_t *r)
{
    ...
    if (!conf->enable
        ...
        || r->header_only)
    { ngx_http_next_header_filter = ngx_http_range_header_filter;
...
ngx_http_gzip_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ctx == NULL || ctx->done || r->header_only) {ngx_http_next_body_filter=ngx_http_chunked_body_filter;    
