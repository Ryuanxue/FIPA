...
ngx_int_t ngx_http_userid_filter(ngx_http_request_t *r);
...
ngx_http_headers_filter(ngx_http_request_t *r)
{
    ...
    if (conf->expires == NGX_HTTP_EXPIRES_OFF
        && conf->headers == NULL
        && conf->trailers == NULL)
    { ngx_http_next_header_filter=ngx_http_userid_filter;
        return ngx_http_next_header_filter(r);
    }
...
ngx_http_trailers_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (in == NULL
        ...
        || !r->expect_trailers
        || r->header_only)
    {ngx_http_next_body_filter=ngx_http_charset_body_filter;    
